...
- A lock taken through `Mailbox.getMailboxLock()` on the primary connection cancels the return-to-main-mailbox timer, and the `release()` it returns re-arms it (`onTaskCompleted()`), so no holder reports the task itself - a holder that skipped the report used to leave the connection parked on whatever folder it opened until the next resync pass. `Mailbox.getMailboxStatus()` throws `MailboxNotSelected` when the connection has another folder open, so the sync operations read their snapshots only under the lock and `onOpen()` decides from the mailbox object the open event carries. A folder excluded from syncing (`syncDisabled`) that an API command opens gets its counters refreshed by `onOpen()` and nothing else
- `getCurrentListing()` processes only half of the diff it computes: it clears deleted folders itself, but reports the folders that appeared through a one-shot `isNew` flag on the entries it returns, and persists the new listing in the same call. A caller that reads the listing and drops the result therefore swallows that flag, and the folder is registered by a later resync with its `mailboxNew` never sent. Read-only callers hand the listing to `processNewListingEntries()`; `test/imap-listing-consumers-guardrail-test.js` asserts every call site does so, or names the method and why it need not. `processListing()` itself skips the work when there is no primary connection (the provider flags it filters on are only decided by `connect()`, which registers the whole listing anyway) or when every entry is already tracked. A pass that cannot register does not record what it found either: `getCurrentListing()` leaves the `isNew` entries out of the listing it writes back when there is no primary connection, because the stored listing is what the next pass diffs against, so a folder created while the primary was down and seen over a pooled secondary would otherwise look known forever and never be announced. Deletions are not deferred that way - they are complete on their own
- The stored listing hash is the record that a folder was ever listed, so removing an entry from it is `Mailbox.clear()`'s claim to announce the deletion: the HDEL result gates the `mailboxDeleted` notification and exactly one caller wins. Dropping the folder from the connection's own `mailboxes` map is the other half of that claim, and just as one-shot, because a folder can be tracked there while its listing entry is already gone - `processListing()` registers a Mailbox without writing the hash, and a pass that had to defer the entry (no primary connection) or purged a corrupt value writes the hash back without it. Gating on the HDEL alone tore such a folder down in silence, and no later pass could announce it either: the deletions a pass reports come from the very hash the folder is missing from. Announcements are otherwise made off a listing snapshot, and every holder of one announces - `deleteMailbox()` clears the folder while the stored listing still holds it, so the pass that follows announced it a second time, and two `getCurrentListing()` passes that overlap (only `refreshFolderList()` is latched; `listMailboxes()`, `processSubConnections()`, `runPostListing()` and the upload recovery are not) announced it once each. The HDEL rides in the same MULTI as the index teardown (the `lib/lists.js` `deleteList()` idiom) so the claim cannot outlive what it guards: an entry removed while its index survived would leave a folder nothing enumerates again, neither cleared nor announced by any later pass. A residual case is accepted: a pass whose LIST predates the deletion legitimately re-adds the folder when it writes the listing back
- IMAP makes the name INBOX case-insensitive, so a server is free to answer "Inbox" where it listed "INBOX". The stored listing and the mailbox map are keyed through `normalizePath()` for that reason, but a live LIST result keeps the server's spelling and so does stored account configuration (`path`, `subconnections` are persisted exactly as the API received them - normalizing on write is not possible, the hierarchy delimiter is not known until the account has connected). Compare folder paths with `samePath()` from `lib/utils/mailbox-path.js`, or `matchesListingEntry()` / `configuredPathIndex()` from `lib/email-client/imap/listing-diff.js` when matching configuration against a listing; `test/imap-path-comparison-guardrail-test.js` asserts the IMAP client carries no bare `===` between paths. A raw comparison is the bug: an account configured `path: ["INBOX"]` against a server answering "Inbox" matched nothing and flagged every folder `syncDisabled`, so it indexed nothing at all; a subconnection configured `INBOX` was marked `mailboxMissing` forever while `hasListedMailbox()` (which does normalize) kept finding the folder and rerunning the reconciler every pass; and `this.path !== 'INBOX'` suppressed inbox backfill after a reconnect and turned off ARF and DSN detection. Special-use tokens (`\Sent`) are matched literally, never normalized - a fixed vocabulary, not a server spelling. `normalizePath` and `samePath` live under `lib/utils/` rather than in `lib/tools.js` so pure modules can compare folder names without inheriting its Redis connections. Two gaps are known and accepted: only the top-level inbox name folds (a configured `Inbox/Archive` still has to match the server's spelling, the same gap the map keys have), and per-folder Redis keys are safe by construction because `mailboxPathHash()` normalizes internally
...
- A failed IMAP login is `authenticationError` only when `isRefusedImapLogin()` (`lib/email-client/imap/login-failure.js`, dependency-free so the pure subconnection tests do not open Redis) says the server refused the credential. ImapFlow flags every failed AUTHENTICATE as `authenticationFailed`, but an RFC 5530 `[UNAVAILABLE]` / `[SERVERBUG]` / `[INUSE]` / `[LIMIT]` answer and Exchange Online's codeless `User is authenticated but not connected.` (its front end failing to reach the mailbox behind an accepted token; also what it says permanently when IMAP is disabled for the mailbox) take the `connectError` path: one webhook carrying the text, the reconnect loop keeps going, and no `authenticationSuccess` follows the recovery. Never branch a login-failure path on the bare flag - `test/imap-login-failure-test.js` scans both IMAP clients for it. `setErrorState()` compares the stored JSON form of the payload, because a key left `undefined` (a response with no bracketed code) otherwise read as a new error on every retry and restarted the auth-failure park clock each time
