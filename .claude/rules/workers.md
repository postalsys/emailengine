...
- The stored listing is keyed through `normalizePath()`, which case-folds INBOX; a live LIST result keeps the server's own spelling. Anything matching the two has to normalize (`Account.getMailboxListing()` merges the live STATUS counters that way), and per-folder Redis keys are safe by construction because `mailboxPathHash()` normalizes internally
