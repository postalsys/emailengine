...
class OAuth2AppsHandler {
                ...
                try {
                    ...
                    if (metaEntry) {
                        try {
                            ...
                            logger.error({ msg: 'Failed to decode app meta', entry: metaEntry?.toString('base64') || '<null>' });
                        }
                    }
                    response.apps.push(data);
                } catch (err) {
                    logger.error({ msg: 'Failed to process app', entry: dataEntry?.toString('base64') || '<null>' });
                }
            }
        }
            ...
            switch (err?.oauthRequest?.response?.error?.code) {
                ...
                default:
                    if (TRANSIENT_NETWORK_CODES.has(err.code)) {
                        logger.warn({
                            msg: `Transient network error deleting Pub/Sub ${resourceType}, retrying in 2s`,
                            app: appData.id,
                            [resourceType]: resourceName,
                            code: err.code
                        });
    ...
    async ensurePubsub(appData) {
        ...
        let ensureLock;
        try {
            ensureLock = await lock.waitAcquireLock(lockKey, 2 * 60 * 1000, 1 * 60 * 1000);
        ...
        try {
            ...
            */
                ...
                if (desiredExpirationPolicy !== null) {
                    ...
                    let desiredTtl = desiredExpirationPolicy.ttl || null;
                    if (currentTtl !== desiredTtl) {
                        try {
                            await client.request(accessToken, subscriptionUrl, 'PATCH', {
                                subscription: { expirationPolicy: desiredExpirationPolicy },
                            ...
                            subscriptionData.expirationPolicy = desiredExpirationPolicy;
                        } catch (patchErr) {
                            logger.warn({
