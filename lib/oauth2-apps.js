...
class OAuth2AppsHandler {
        ...
        if (keys.length) {
            ...
            for (let i = 0; i < list.length; i++) {
                ...
                try {
                    ...
                    logger.error({ msg: 'Failed to process app', entry: entry?.toString('base64') || '<null>' });
                    continue;
                }
            }
        }
    ...
    async _deletePubSubResource(client, accessToken, appData, resourceType, resourceName) {
        ...
        try {
            await client.request(accessToken, url, 'DELETE', Buffer.alloc(0), { returnText: true });
        } catch (err) {
            switch (err?.oauthRequest?.response?.error?.code) {
                ...
                default:
                    if (TRANSIENT_NETWORK_CODES.has(err.code)) {
                        logger.warn({
                            msg: `Transient network error deleting Pub/Sub ${resourceType}, resources may need manual cleanup`,
                            app: appData.id,
                            [resourceType]: resourceName,
                            code: err.code
                        });
