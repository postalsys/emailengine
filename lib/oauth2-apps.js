    ...
    async ensurePubsub(appData) {
        ...
        try {
            ...
            */
            ...
            // Patch existing subscriptions to remove expiration policy (default 31-day TTL)
            if (subscriptionData?.expirationPolicy?.ttl) {
                try {
                    await client.request(accessToken, subscriptionUrl, 'PATCH', {
                        subscription: { expirationPolicy: {} },
                        updateMask: 'expirationPolicy'
                    });
                    logger.info({ msg: 'Removed expiration policy from subscription', app: appData.id, subscription: subscriptionName });
                } catch (patchErr) {
                    logger.warn({
                        msg: 'Failed to remove expiration policy from subscription',
                        app: appData.id,
