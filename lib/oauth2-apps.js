        ...
        case 'outlook':
            {
                ...
                if (provider === 'outlookService') {
                    providerData.tutorialUrl = 'https://emailengine.app/outlook-client-credentials';
                }
                providerData.imap = {
                    host: imapHost,
                    port: 993,
...
class OAuth2AppsHandler {
    ...
    async list(page, pageSize, opts) {
        ...
        response.apps.forEach(app => {
            ...
            if (['gmailService', 'outlookService'].includes(app.provider)) {
    ...
    async get(id) {
        if (LEGACY_KEYS.includes(id)) {
            ...
            if (['gmailService', 'outlookService'].includes(data.provider)) {
