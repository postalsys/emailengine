...
class OAuth2AppsHandler {
    ...
    async create(data) {
        ...
        try {
            ...
            if (appData.baseScopes === 'pubsub') {
                await this.ensurePubsub(appData);
            }
        } catch (err) {
    ...
    async deleteTopic(appData) {
        let project = appData.serviceProjectId;
        let topic = `ee5-${appData.id}`;

        let topicName = appData.pubSubTopic || `projects/${project}/topics/${topic}`;
    ...
    async ensurePubsub(appData) {
        ...
        // Step 3. Grant access to Gmail publisher
    ...
    async getClient(id, extraOpts) {
        ...
        switch (appData.provider) {
            ...
            case 'gmailService': {
                ...
                let serviceProjectId = appData.serviceProjectId;
    ...
    async getServiceAccessToken(appData, client) {
        ...
        if (accessToken && accessTokenExpires && accessTokenExpires > new Date(now + 30 * 1000)) {
            ...
            // create new
            let { access_token: accessToken, expires_in: expiresIn } = await client.refreshToken({ isPrincipal: true });
