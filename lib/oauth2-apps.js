...
class OAuth2AppsHandler {
    ...
    async backfillPubSubApps() {
        let subscriberIds = await this.redis.smembers(this.getSubscribersKey());

        if (this._pubSubBackfillDone) {
            return subscriberIds;
        }

        let allIds = await this.redis.smembers(this.getIndexKey());
        ...
        for (let i = 0; i < entries.length; i++) {
            ...
            try {
                ...
                if (data.baseScopes === 'pubsub') {
                    await this.redis.sadd(this.getSubscribersKey(), missingIds[i]);
                    subscriberIds.push(missingIds[i]);
    ...
    async create(data) {
        ...
        if (data.pubSubSubscription || data.baseScopes === 'pubsub') {
