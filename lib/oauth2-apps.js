...
class OAuth2AppsHandler {
    ...
    async list(page, pageSize) {
        ...
        response.apps.forEach(app => {
            app.includeListing = app.enabled;
            if (['gmailService'].includes(app.provider)) {
                // service account are always enabled
                app.enabled = true;
                app.includeListing = false;
            }
        });

        console.log('APPS LIST', response.apps);
    ...
    async setMeta(id, meta) {
        ...
        console.log('META UPDATE', this.getDataKey(), updates);
        ...
        switch (appData.provider) {
            ...
            case 'gmailService': {
                ...
                return new GmailOauth({
                    ...
                    scopes,
                    setFlag: async flag => {
                        console.log('FLAG UPDATE', flag, appData.legacy);
                        try {
                            if (appData.legacy) {
                                await settings.set('gmailServiceAuthFlag', flag);
                            } else {
                                console.log('SET META', id, { authFlag: flag });
                                await this.setMeta(id, { authFlag: flag });
                            }
                        } catch (err) {
                            // ignore
                            console.error('SET META ERROR', err);
                        }
                    }
                });
            }
