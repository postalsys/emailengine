...
async function readRecord(field) {
    let value = await redis.hget(TLS_KEY, field);
    if (!value) {
        return false;
    }
 ...
 *
 * Written with HSETNX so that two workers reaching this at the same moment agree on one
 * certificate: whoever loses the race reads back the winner's rather than serving a second one that
 * nobody pinned.
 *
...
async function getSelfSignedCertificate(hostnames, logger) {
    ...
    const existing = await readRecord(SELF_SIGNED_FIELD);
    if (existing && existing.cert && existing.privateKey && !selfSignedIsStale(existing, names)) {
        return asMaterial(existing);
    }

    if (existing) {
        // Replacing rather than keeping: whatever is there covers the wrong names or is about to
        // expire, and both make a handshake fail.
        await redis.hdel(TLS_KEY, SELF_SIGNED_FIELD);
    }
    ...
    const stored = await redis.hsetnx(TLS_KEY, SELF_SIGNED_FIELD, await serializeRecord(record));
