...
const { hostnamesFrom, acmeEligibleHostnames, getManualCertificate, peekSelfSignedCertificate, parseCertificate, coversHostname } = require('./store');
...
function certificateLabel(certificate, status) {
    ...
    const daysRemaining = certificate.validTo ? Math.floor((certificate.validTo.getTime() - now) / (24 * 3600 * 1000)) : null;
...
async function buildCertificateStatus(opts) {
    ...
    const [values, manual, selfSigned, provisioning] = await Promise.all([
    ...
    // it is per listener and per process, and the listeners report it themselves.
    ...
    const certificates = hostnames.map((hostname, index) => {
        const certificate = resolved[index] || (coversHostname(selfSignedX509, hostname) ? selfSigned : false);
        ...
        return {
            ...
            status,
            // How the recorded message is rendered, which is not always how the certificate is: a
            // failed order sits under a certificate that is still perfectly valid.
            statusVariant: status && (status.state === 'failed' || status.state === 'skipped') ? 'error' : 'info'
        };
    });
...
module.exports = { buildCertificateStatus, certificateLabel, publicView, SOURCE_LABELS, EXPIRY_WARNING_DAYS };
