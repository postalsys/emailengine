...
const { hostnamesFrom, acmeEligibleHostnames, LISTENER_TLS_SETTINGS, listenerTlsFrom } = require('./store');
...
/**
 * The stored record for a name, without ever asking the certificate authority for one.
 *
 * @param {Object} certs @postalsys/certs handler
 * @param {string} hostname Name to look up
 * @returns {Promise<Object|false>} The record, or false when there is none
 */
async function readStoredRecord(certs, hostname) {
    try {
        return (await certs.getCertificate(hostname, true)) || false;
    } catch (err) {
        return false;
    }
}
 ...
 * @param {Object|false} [opts.before] The stored record, when the caller has already read it
...
async function provisionHostname(opts) {
    ...
    // Only the fingerprint of it is used, to tell an installed certificate from an unchanged one,
    // and the reconciler has already read the record to decide whether to order at all.
    const before = opts.before !== undefined ? opts.before : await readStoredRecord(certs, hostname);
...
/**
 * Whether any listener would present a certificate to a client right now.
 *
 * A first order is gated on this. A certificate nothing serves is not needed, and ordering one
 * anyway is not harmless: most instances sit behind a reverse proxy that terminates TLS itself,
 * with a public https Service URL and port 80 routed somewhere else, and every one of them would
 * otherwise start an http-01 order on the timer the moment it upgraded, fail it, and go on failing
 * it every retry interval for good - against the CA's failed-validation limit, with an error in
 * the log each time, for a certificate the operator never asked for. The old code had the same
 * gate by construction: the one thing that ordered a first certificate was a listener's TLS
 * checkbox. Renewals are not gated - a certificate that exists is kept valid, as it always was -
 * and neither is the button, which is an explicit request.
 *
 * @param {Object} values Settings, at least LISTENER_TLS_SETTINGS (the API listener is read from
 *                        the environment, which every process shares)
 * @returns {boolean} True when some listener has TLS switched on
 */
function listenersServeTls(values) {
    return !!apiTlsConfig() || Object.values(listenerTlsFrom(values)).some(listener => listener.enabled && listener.tlsEnabled);
}

/**
 * Whether a stored record holds a certificate a listener could serve.
 *
 * @param {Object|false} record The stored record
 * @returns {boolean}
 */
function hasValidCertificate(record) {
    return !!(record && record.cert && record.status === 'valid');
}
 ...
 * @param {Object|false} [opts.record] The stored record, when the caller has already read it
...
async function needsProvisioning(opts) {
    ...
    const record = opts.record !== undefined ? opts.record : await readStoredRecord(certs, hostname);
...
async function requestProvisioning(opts) {
    ...
    const accepted = [];
    const declined = [];
    // What this call put in flight, which is not the same list as what it reports as accepted: a
    // name another click is already ordering is accepted too, and scheduling it a second time
    // starts a second order that overwrites the running one's state and reports its own failure
    // when the certificate library's per-domain lock times out under it.
    const scheduled = [];
    ...
    for (const hostname of hostnames) {
        ...
        if (mode !== 'acme') {
            ...
            reason = `Let's Encrypt cannot validate ${hostname}. Automatic certificates need a public domain name.`;
    ...
    if (scheduled.length) {
        reconcileCertificates({ certs, logger, call, hostnames: scheduled, force: true })
            .catch(err => logger.error({ msg: 'Certificate provisioning failed', err }))
            .finally(() => {
                // Only the names this call added, so a second click does not release the first
                // click's order and let a third one stack on top of it.
                for (const hostname of scheduled) {
...
async function reconcileCertificates(opts) {
    ...
    // Everything the pass decides from, in one round of reads: the settings together (the
    // hostname list and the listener switches derive from them), and the recorded state alongside
    // - it serves both the cleanup below and the per-hostname pacing further down.
    const [values, status] = await Promise.all([
        settings.getMulti('tlsProvisioning', 'serviceUrl', 'tlsHostnames', ...LISTENER_TLS_SETTINGS),
        getProvisioningStatus()
    ]);

    const mode = values.tlsProvisioning || 'acme';

    const configured = hostnamesFrom(values);
    const targets = opts.hostnames || configured;
    const hostnames = acmeEligibleHostnames(targets);

    // Whether a name with no certificate gets a first order on this pass; a forced pass is an
    // explicit request and is not asked. See listenersServeTls().
    const wanted = force || listenersServeTls(values);
    ...
    for (const hostname of hostnames) {
        // Read once per name: the renewal decision and the fingerprint comparison that decides
        // whether the listeners have to be reloaded both want the stored record.
        const before = await readStoredRecord(certs, hostname);

        // A name with no usable certificate is only ordered for while a listener would serve it
        if (!force && !wanted && !hasValidCertificate(before)) {
            continue;
        }

        if (!force && !(await needsProvisioning({ certs, hostname, status: status[hostname], record: before }))) {
            continue;
        }

        const result = await provisionHostname({ certs, logger, hostname, before });
        results.push(result);
