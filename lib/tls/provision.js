...
function asDate(value) {
    ...
    // new Date() clones a Date and parses the ISO string the record carries once it has been
    // through JSON, which are the two shapes this sees
    const date = new Date(value);
    return isNaN(date.getTime()) ? null : date;
}

/**
 * The failure attached to a certificate record, when it is the record's latest word.
 *
 * A successful order clears the field, so a failure that is present is normally the last thing that
 * happened. Normally is not always: acquireCert() can answer from the stored record without
 * attempting anything (the domain is blocked after an earlier failure, the lock is held elsewhere,
 * the CA's renewal advice moved the window out), and it then hands back whatever the record still
 * carries. A failure older than the certificate being served was settled by the order that issued
 * that certificate, whatever cleared or did not clear the field afterwards.
 *
 * The certificate's own validity date is the comparison, not `lastCheck`: the reconciler stamps
 * that field after every pass, failed ones included, so it says when the certificate was last
 * looked at and not when it was obtained.
 *
 * @param {Object} record Certificate record from the certificate store
 * @returns {Object|false} The record's `lastError`, or false when there is no live failure
 */
function renewalFailure(record) {
    if (!record || !record.lastError || !record.lastError.err) {
        return false;
    }

    const failedAt = asDate(record.lastError.time);
    const issuedAt = asDate(record.validFrom);

    if (failedAt && issuedAt && failedAt.getTime() < issuedAt.getTime()) {
        return false;
    }
...
async function provisionHostname(opts) {
    ...
    if (record && record.status === 'valid' && record.cert) {
        ...
        const renewError = renewalFailure(record);
        if (renewError) {
