...
async function createTlsContext(opts) {
    ...
    const refresh = async () => {
        ...
        // listener that has been switched on has to start. Decided before the self-signed entry
        // joins the list, so it stays the last resort for the default context too.
        ...
        if (!fallback) {
            fallback = entries[0] || null;
        }

        // The fourth source. The self-signed certificate covers every configured name, and it is
        // the entry for the ones no stored source answers for - not whichever certificate came
        // first, which fails the client's name check just the same and is not what the page says
        // the name has. Generated only when some name needs it: once every name has a certificate
        // of its own there is nothing for it to answer for, and generating it anyway would put a
        // certificate on the page that no listener serves. Material from the environment answers
        // for those names itself, as the default context above: the operator pinned it on purpose.
        const uncovered = env ? [] : hostnames.filter(hostname => !entries.some(entry => coversHostname(entry.x509, hostname)));

        if (uncovered.length || !fallback) {
            const selfSigned = buildEntry(uncovered[0] || hostnames[0] || null, await getSelfSignedCertificate(hostnames, logger));
            if (selfSigned) {
                entries.push(selfSigned);
                fallback = fallback || selfSigned;
            }
        }
...
module.exports = { createTlsContext, applyTlsContext, resolveHostname, sourcesFor, apiTlsConfig };
