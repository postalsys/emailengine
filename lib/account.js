...
class Account {
    ...
    async update(accountData, { reauthorized = false } = {}) {
        ...
        // auth-failure park before any reconnect is considered. Gated only on `reauthorized`
        // (not on the narrower conditions below) because the park has no operator-facing control
        // to untick, unlike `imap.disabled` - re-authorizing must always be enough to recover.
