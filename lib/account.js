...
class Account {
        ...
        try {
            ...
            if (updateLock?.success) {
                try {
                    ...
                    this.logger.error({ msg: 'Failed to release account update lock', account: accountData.account, lockKey, err });
                }
            }
        }
    ...
    async delete(opts) {
        ...
        let pipeline = this.redis
            ...
            .unlink(`${REDIS_PREFIX}iat:${this.account}`) // access tokens
    ...
    async getMailboxListing(query) {
        ...
        for (let i = 0; i < paths.length; i++) {
            ...
            try {
                ...
                if (!decoded) {
                    this.logger.error({ msg: 'Skipping corrupt stored mailbox listing entry', path, account: this.account });
                    continue;
                }
...
module.exports = { Account, accountExists };
