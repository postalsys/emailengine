    ...
    async update(accountData) {
        ...
        if (addProvider && !LEGACY_KEYS.includes(addProvider)) {
            ...
            if (accountData.oauth2?.auth?.user) {
                ...
                let existingAppBinding = await this.redis.hget(`${REDIS_PREFIX}oapp:a:${accountData.oauth2.provider}`, accountData.oauth2.auth?.user);
                ...
                pipeline = pipeline.hset(`${REDIS_PREFIX}oapp:h:${addProvider}`, accountData.oauth2?.auth?.user);
            }
        }
