...
class Account {
    ...
    async listAccounts(state, query, page, limit) {
        ...
        let result = await this.redis.sListAccounts(`${REDIS_PREFIX}ia:accounts`, filterState, skip, limit, `${REDIS_PREFIX}`, query);
    ...
    // `reauthorized` marks an update that carries credentials supplied by an external caller,
    // i.e. possible operator re-authorization. Only the public account-update API route sets it;
    // every unattended writer (token renewal, client initialize(), invalidateAccessToken) leaves
    // it false and can therefore never trigger the reconnect gate below.
    async update(accountData, { reauthorized = false } = {}) {
        ...
        // manual "Reconnect".
        //
        // Requires explicit `reauthorized` intent from the caller. Account state cannot be used
        // to infer it: unattended token renewal persists through update() as well, and it runs
        // while the stored state is still the error state, so a state-only gate re-fired on every
        // renewal and dispatched cmd:'reconnect'. That rebuilt the client, discarding its backoff,
        // and invalidateAccessToken() forced the next cycle to renew again - latching a reconnect
        // loop that pinned the CPU at ~100%.
        // Did this write actually replace a credential? PUT /v1/account/{account} passes
        // `reauthorized: true` on every call regardless of payload, so the flag alone only means
        // "an interactive caller sent this" - it is the credential comparison that means "the
        // reason this account was failing may be gone". Covers all three transports because the
        // auth-failure park is transport-neutral.
        const oauth2CredentialsChanged = !!(
        ...
        const credentialsChanged =
            oauth2CredentialsChanged ||
            !!(accountData.imap?.auth?.pass && accountData.imap.auth.pass !== oldAccountData.imap?.auth?.pass) ||
            !!(accountData.smtp?.auth?.pass && accountData.smtp.auth.pass !== oldAccountData.smtp?.auth?.pass);

        // Fresh credentials mean the reason the account was parked may be gone, so lift the
        // auth-failure park before any reconnect is considered. The park has no operator-facing
        // control to untick, unlike `imap.disabled`, so re-authorizing has to be enough to recover.
        //
        // Deliberately NOT gated on the state test below: a parked account has already been closed
        // and its next connect attempt sets state 'unset', which is not in ERROR_STATES - gating on
        // that would make the park permanent. It IS gated on a real credential change, because
        // otherwise any unrelated PUT (`{name: "x"}`) would lift the park, and since the park write
        // also clears the error counters, re-parking then needs a fresh three-day window - so a
        // periodic PUT would keep a broken account retrying forever.
        //
        // Idempotent and dispatches nothing, so it cannot contribute to the reconnect loop above.
        if (reauthorized && credentialsChanged) {
            await clearAuthFailurePark(this.redis, this.account);
        }

        if (reauthorized && !reconnectRequested && oauth2CredentialsChanged && ERROR_STATES.includes(oldAccountData.state)) {
            this.logger.info({
    ...
    async requestReconnect(data) {
        ...
        if (data.reconnect) {
            // An explicit reconnect is the operator saying "try again". Un-park first, otherwise
            // the connect gates reject the attempt and the account can never recover - the park
            // blocks the very success path that would otherwise clear it.
            await clearAuthFailurePark(this.redis, this.account);
