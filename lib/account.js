...
class Account {
    ...
    async listAccounts(state, page, limit) {
        ...
        console.log('LISTING', 'ia:accounts', state || '*', skip, limit, page);
    ...
    async delete() {
        ...
        let result = await this.redis
            ...
            .del(`iat:${this.account}`) // access tokens
            .exec();
    ...
    async renewAccessToken() {
        ...
        switch (accountData.oauth2.provider) {
            case 'gmail': {
                ...
                if (!r || !r.credentials || !r.credentials.access_token) {
                ...
                if (r.credentials.refresh_token) {
                    updates.refreshToken = r.credentials.refresh_token;
                }
            ...
            case 'outlook': {
                let r = await oAuth2Client.refreshToken({ refreshToken: accountData.oauth2.refreshToken });
                if (!r.access_token) {
