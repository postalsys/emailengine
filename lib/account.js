...
class Account {
    ...
    // Change notifications waiting for a retry, scored by the time they are due
    getDeferredQueueKey() {
    ...
    async delete(opts) {
        ...
        let pipeline = this.redis
            ...
            .unlink(this.getDeferredQueueKey()) // external events waiting for a retry
    ...
    async pushQueueEvent(event) {
        try {
            let res = await this.redis.lpush(this.getExternalQueueKey(), encodeQueueEvent(event));
    ...
    /**
     * Parks a queue event until `dueTime`, when promoteDueQueueEvents() puts it back on the queue.
     * Kept in Redis rather than a timer so a restart in between does not lose the event
     * @param {Object} event - The queue event, as pullQueueEvent() returned it
     * @param {number} dueTime - Epoch milliseconds
     */
    async deferQueueEvent(event, dueTime) {
        await this.redis.zadd(this.getDeferredQueueKey(), dueTime, JSON.stringify(event));
    }

    /**
     * Moves the deferred events that are due to the end of the queue, a bounded batch per call
     * @returns {Promise<number|null>} When the next deferred event is due (epoch ms), or null when
     *   none is left
     */
    async promoteDueQueueEvents() {
        const deferredKey = this.getDeferredQueueKey();
        const now = Date.now();
        const entries = await this.redis.zrange(deferredKey, 0, DEFERRED_QUEUE_BATCH - 1, 'WITHSCORES');

        const due = [];
        let next = null;
        for (let i = 0; i < entries.length; i += 2) {
            if (Number(entries[i + 1]) > now) {
                next = Number(entries[i + 1]);
                break;
            }
            due.push(entries[i]);
        }
        ...
        if (due.length) {
            let pipeline = this.redis.multi().zrem(deferredKey, ...due);
            for (const entry of due) {
                pipeline = pipeline.lpush(this.getExternalQueueKey(), encodeQueueEvent(JSON.parse(entry)));
            }
            await pipeline.exec();

            if (next === null && due.length === DEFERRED_QUEUE_BATCH) {
...
const DEFERRED_QUEUE_BATCH = 100;

// The envelope pullQueueEvent() decodes
function encodeQueueEvent(event) {
    return msgpack.encode({ t: new Date(), e: event });
}
