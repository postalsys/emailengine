...
const { isTransientCredentialError, credentialErrorStatus } = require('./email-client/credential-errors');
...
class Account {
    ...
    // Mailbox changes waiting for a retry (Gmail and MS Graph), scored by the time they are due
    getDeferredQueueKey() {
        return `${REDIS_PREFIX}iap:${this.account}:r`;
    }
    ...
    async delete(opts) {
        ...
        let pipeline = this.redis
            ...
            .unlink(`${REDIS_PREFIX}iar:b:${this.account}`) // retired bounce store, an older version sharing this Redis may still write it
            ...
            .unlink(this.getDeferredQueueKey()) // mailbox changes waiting for a retry
    ...
    async getMailboxListing(query) {
        ...
        if (mailboxListing) {
            ...
            for (let entry of mailboxListing) {
                ...
                // Keyed the way the stored listing is keyed, because that is what the lookup
                // below uses. A server that spells the inbox "Inbox" reports it that way in
                // every LIST response while the stored hash holds the case-folded "INBOX", so
                // an unnormalized key silently dropped the live counters for the one folder
                // every account has.
                mailboxListingMap.set(normalizePath(entry.path), entry);
            }
        }
    ...
    async getMessage(message, options) {
        ...
        if (options.documentStore && (await isDocumentStoreEnabled())) {
            ...
            const reqOpts = {
                index,
                id: `${this.account}:${message}`,
                _source_excludes: 'preview,seemsLikeNew,account,created,updateTime,bounces'
            };
    ...
    async listMessages(query) {
        if (query.documentStore && (await isDocumentStoreEnabled())) {
            ...
            let searchResult = await client.search({
                ...
                query: searchQuery,
                sort: { uid: 'desc' },
                _source_excludes: 'headers,text.plain,text.html,text._generatedHtml,seemsLikeNew,attachments.content,summary,riskAssessment,updateTime,bounces'
            });
        ...
        try {
            ...
            try {
                let pipeline = this.redis
                    ...
                    .del(`${REDIS_PREFIX}iar:b:${this.account}`) // retired bounce store, an older version sharing this Redis may still write it
    ...
    async invalidateAccessToken() {
        ...
        if (accountData.oauth2) {
            // Rewinding the expiry is the whole encoding of "invalidated", and isAccessTokenInvalidated()
            // below is its decoder: change one and change the other
    ...
    async getActiveAccessTokenData() {
        ...
        if (await useAuthServerForOAuth2(accountData.oauth2, this.logger, { account: this.account, target: 'api' })) {
            ...
            try {
                ...
                // The authentication server's own HTTP status must not become EmailEngine's, which
                // is why resolveCredentials() keeps it in an `authRequest` record rather than on
                // err.statusCode. It is translated into a status of EmailEngine's own here:
                // returning it verbatim told an API client holding a perfectly valid EmailEngine
                // token that its token was rejected (401) or that the account does not exist (404).
                let transient = isTransientCredentialError(err);
                ...
                let error = Boom.boomify(err, { statusCode: transient ? 503 : 403 });
                error.output.payload.code = transient ? 'AuthServerUnavailable' : 'AuthServerError';
                if (!transient) {
    ...
    /**
     * Queues the events of one Graph notification delivery, encoded as the envelope
     * pullQueueEvent() decodes. One write, so it is all or nothing, and not caught: the webhook
     * route has to answer with an error for Graph to deliver the notification again - a 202 after
     * a failed write lost it for good, and a partial write would repeat the events already queued
     * @param {Object[]} events - Queue events
     */
    async pushQueueEvents(events) {
        const t = new Date();
        let res = await this.redis.lpush(this.getExternalQueueKey(), ...events.map(e => msgpack.encode({ t, e })));
    ...
    /**
     * Parks a mailbox change until `dueTime`. Kept in Redis rather than a timer so a restart in
     * between does not lose it
     * @param {Object} event - Any JSON-serializable description of the change
     * @param {number} dueTime - Epoch milliseconds
     */
    async deferChangeEvent(event, dueTime) {
        await this.redis.zadd(this.getDeferredQueueKey(), dueTime, JSON.stringify(event));
    }

    /**
     * Lists the deferred changes that are due, a bounded batch per call. They stay stored until
     * completeChangeEvent() is called for each, so a worker that dies mid-batch loses none of them
     * @returns {Promise<{due: {event: Object, member: string}[], next: number|null}>} The due
     *   changes, and when the next one is due (epoch ms, null when none is left)
     */
    async listDueChangeEvents() {
        const deferredKey = this.getDeferredQueueKey();
        const now = Date.now();
        const [[, members], [, nextDue]] = await this.redis
            .multi()
            .zrangebyscore(deferredKey, '-inf', now, 'LIMIT', 0, DEFERRED_CHANGE_BATCH)
            .zrangebyscore(deferredKey, `(${now}`, '+inf', 'WITHSCORES', 'LIMIT', 0, 1)
            .exec();

        const due = members.map(member => ({ event: JSON.parse(member), member }));

        let next = nextDue.length ? Number(nextDue[1]) : null;
        if (due.length === DEFERRED_CHANGE_BATCH) {
            // a full batch was due, there may be more behind it
            next = now;
        }
    ...
    /**
     * Drops a deferred change once it has been processed, re-deferred or given up on
     * @param {string} member - As listDueChangeEvents() returned it
     */
    async completeChangeEvent(member) {
        await this.redis.zrem(this.getDeferredQueueKey(), member);
    }
}

const DEFERRED_CHANGE_BATCH = 100;
...
/**
 * Tells whether an account's access token was invalidated by Account#invalidateAccessToken().
 *
 * The invalidation rewinds `expires` by a day and leaves `generated` alone, so the token reads
 * as a validity window that ends before it starts. A live token can never have that shape (a
 * renewal writes both fields from the same clock), which makes the inversion the marker.
 *
 * @param {Object} oauth2 - The parsed oauth2 record of an account (dates as Date objects)
 * @returns {boolean}
 */
function isAccessTokenInvalidated(oauth2) {
    return !!(oauth2 && oauth2.expires && oauth2.generated && oauth2.expires < oauth2.generated);
}
...
module.exports = {
    ...
    accountsExist,
    accountSummary,
    authFailureDisabledAt,
    isAuthFailureDisabled,
    isOperatorDisabled,
