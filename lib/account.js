    ...
    async listMessages(query) {
        if (query.documentStore && (await settings.get('documentStoreEnabled'))) {
            ...
            let searchResult = await client.search({
                ...
                query: searchQuery,
                sort: { uid: 'desc' },
                _source_excludes: 'headers,text.plain,text.html,text._generatedHtml,seemsLikeNew,attachments.content'
            });
    ...
    async searchMessages(query) {
        if (query.documentStore && (await settings.get('documentStoreEnabled'))) {
            ...
            let searchResult = await client.search({
                ...
                query: searchQuery,
                sort: { uid: 'desc' },
                _source_excludes: 'headers,text.plain,text.html,seemsLikeNew,attachments.content'
            });
    ...
    async renewAccessToken() {
        ...
        try {
            ...
            if (
                ...
                this.logger.trace({ msg: 'Using cached OAuth2 access token' });
                return accountData;
            }

            const oAuth2Client = await oauth2Apps.getClient(accountData.oauth2.provider);
