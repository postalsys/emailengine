...
class Account {
    ...
    checkAccountScopes(provider, scopes) {
        ...
        // Add support for Outlook in the future
        if (provider === 'outlook') {
            // TODO: Implement Outlook scope checking
            // const hasSendScope = scopes.some(s => s.includes('Mail.Send'));
            // const hasReadScope = scopes.some(s => s.includes('Mail.Read') || s.includes('Mail.ReadWrite'));
            // return { hasSendScope, hasReadScope };
        }
    ...
    async listAccounts(state, query, page, limit) {
        ...
        for (let accountData of accounts) {
            if (accountData.oauth2 && accountData.oauth2.provider) {
                ...
                // Check if this is a send-only Gmail account (regardless of baseScopes setting)
