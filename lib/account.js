...
const { extractMultiValues } = require('./redis-operations');
...
const { REDIS_PREFIX, ACCOUNT_DELETED_NOTIFY } = require('./consts');
...
class Account {
    ...
    async update(accountData, { reauthorized = false } = {}) {
        ...
        await this.dispatchPostUpdateCommands(accountData, oldAccountData, reauthorized);
     ...
     */
    async dispatchPostUpdateCommands(accountData, oldAccountData, reauthorized) {
        ...
        if (
            ...
            // A reconnect cannot resume an account that the auth-failure safety net has switched
            // off, so lift that first. For a password account the operator clears the flag by
            // saving new IMAP settings, but an OAuth2 account has no IMAP settings to save - the
            // re-authorization IS the fix, and without this the account would stay off until
            // somebody hand-edited it through the API.
            await this.clearAuthFailureDisable();
     ...
     * failures. Called when fresh credentials arrive for an account in an error state.
     *
     * Reads the stored value rather than trusting the copy loaded before the update, because the
    ...
    async clearAuthFailureDisable() {
        ...
        try {
            ...
            this.logger.error({ msg: 'Failed parsing IMAP data', account: this.account, err });
            return false;
        }
        ...
        await this.redis.hset(this.getAccountKey(), 'imap', JSON.stringify(imapData));

        this.logger.info({ msg: 'Re-enabled IMAP for account after re-authorization', account: this.account });
...
module.exports = { Account, accountExists, accountsExist, accountSummary };
