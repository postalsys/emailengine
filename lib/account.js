...
class Account {
    ...
    unserializeAccountData(accountData) {
        ...
        Object.keys(accountData).forEach(key => {
            ...
            // Secret-bearing connection structures are parsed separately below, by static
            // property name, so their (sensitive) contents never pass through the generic
            // dynamic-key copy in this loop. This keeps identifier fields (e.g. `account`,
            // `state`) from being tainted with credentials by static analysis - the source
            // of recurring CodeQL js/clear-text-logging false positives on log lines.
            if (key === 'imap' || key === 'smtp' || key === 'oauth2') {
                return;
            }
        ...
        // Parse secret-bearing connection structures by static property name (see the guard
        // in the loop above). Runs before the decrypt pass below, which expects
        // result.imap/smtp/oauth2 to already be populated.
        for (const key of ['imap', 'smtp', 'oauth2']) {
            if (!(key in accountData)) {
                continue;
            }
            try {
                let value = this.parseStoredStructure(accountData[key]);
    ...
    // Parse a single stored imap/smtp/oauth2 JSON blob and normalize its date subkeys.
    // Kept separate from unserializeAccountData's generic key loop so the credential
    // contents are read/written through static property names only. Returns undefined when
    // the stored value is JSON null (the caller then leaves the field unset).
    parseStoredStructure(raw) {
        ...
        for (let subKey of ['created', 'expires', 'generated']) {
            if (value[subKey]) {
                let dateVal = /^[0-9]+$/.test(value[subKey]) ? Number(value[subKey]) : value[subKey];
                let date = new Date(dateVal);
                if (date.toString() !== 'Invalid Date') {
                    value[subKey] = date;
                }
    ...
    serializeAccountData(accountData) {
        ...
        Object.keys(accountData).forEach(key => {
            ...
            // Secret-bearing connection structures are serialized separately below, by
            // static property name, so their (sensitive) contents never pass through the
            // generic dynamic-key copy in this loop. Keeping credentials out of the
            // dynamic path stops static analysis from tainting unrelated identifier
            // fields (e.g. `account`) with them - the source of recurring CodeQL
            // js/clear-text-logging false positives on log lines that include the id.
            if (key === 'imap' || key === 'smtp' || key === 'oauth2') {
                return;
            }
        ...
        // Serialize secret-bearing connection structures by static property name (see the
        // guard in the loop above). Handling these outside the dynamic-key copy keeps the
        // credential values from being conflated with identifier fields by static analysis.
        for (const key of ['imap', 'smtp', 'oauth2']) {
            if (!(key in accountData)) {
                continue;
            }
            try {
                result[key] = this.serializeConnectionData(accountData[key]);
            } catch (err) {
                this.logger.error({ msg: 'Failed to stringify input for Redis', key, err });
            }
        }
    ...
    // Encrypt (when a secret is configured) and JSON-stringify a single imap/smtp/oauth2
    // connection object for storage. Kept separate from serializeAccountData's generic key
    // loop so the credential contents are read/written through static property names only.
    serializeConnectionData(rawValue) {
        // make a deep copy for manipulation
        let connectData = JSON.parse(JSON.stringify(rawValue));

        // if possible encrypt passwords
        if (this.secret && connectData.auth) {
            for (let field of ['pass', 'accessToken', 'refreshToken']) {
                if (field in connectData.auth) {
                    try {
                        connectData.auth[field] = encrypt(connectData.auth[field], this.secret);
                    } catch (err) {
                        this.logger.error({ msg: 'Failed to encrypt value', err });
                    }
                } else if (field in connectData) {
                    try {
                        connectData[field] = encrypt(connectData[field], this.secret);
                    } catch (err) {
                        this.logger.error({ msg: 'Failed to encrypt value', err });
                    }
                }
        ...
        for (let subKey of ['created', 'expires', 'generated']) {
            if (rawValue[subKey] && typeof rawValue[subKey] === 'object' && rawValue[subKey].toString() !== 'Invalid Date') {
                connectData[subKey] = rawValue[subKey].toISOString();
            }
        }
    ...
    async update(accountData) {
        ...
        // When OAuth2 credentials are re-authorized for an account that is currently in a
        // non-operational error state, trigger a full reconnect so syncing resumes without a
        // manual "Reconnect". Gated on the error state and on the credentials actually
        // changing, so routine token renewal - which runs while the account is connected and
        // re-saves oauth2 hourly - never triggers a reconnect. During an actual reconnect the
        // state is set to "connecting" before tokens are fetched, so this cannot loop.
        if (
            !reconnectRequested &&
            'oauth2' in accountData &&
            accountData.oauth2?.accessToken &&
            (accountData.oauth2.accessToken !== oldAccountData.oauth2?.accessToken ||
                accountData.oauth2.refreshToken !== oldAccountData.oauth2?.refreshToken) &&
            ERROR_STATES.includes(oldAccountData.state)
        ) {
            this.logger.info({
                msg: 'OAuth2 credentials re-authorized for account in error state, requesting reconnect',
                account: this.account,
    ...
    async create(accountData) {
        ...
        if (result[0][1] && result[0][1].account) {
            ...
            if (ERROR_STATES.includes(oldState)) {
                // Re-registration of an account that was in a non-operational error state (the
                // interactive OAuth re-auth redirect and the IMAP re-credentials form both land here).
                ...
                // Gated on the error state alone (unlike update(), which also requires the oauth2
                // token to have changed): create() runs only on explicit re-registration, never on
                // routine token renewal, so there is no healthy-account churn to suppress here.
                this.logger.info({
                    msg: 'Account re-created while in error state, requesting full reconnect',
                    account: this.account,
                    state: oldState
                });
                try {
