...
const { REDIS_PREFIX, ACCOUNT_DELETED_NOTIFY, AUTH_FAILURE_DISABLED_FIELD } = require('./consts');
...
/**
 * Whether an account was switched off by the auth-failure safety net, as opposed to by the operator.
 *
 * Both halves matter: the marker can outlive the flag (an operator re-enables the account by hand,
 * and the stale marker is only retired on the next recovery pass), and re-authorizing a healthy
 ...
 * @returns {Boolean} True when the account was parked
 */
function wasAuthFailureDisabled(accountData) {
    return !!(accountData && accountData._authFailureDisabledAt && isImapDisabled(accountData));
}
...
class Account {
    ...
    async dispatchPostUpdateCommands(accountData, oldAccountData, reauthorized, disabledIntent) {
        ...
        // settle that first. For a password account the operator clears the flag by saving new IMAP
        // settings, but an OAuth2 account has none to save - the re-authorization IS the fix, and
        // without this the account stays off until somebody hand-edits it through the API.
        //
        ...
        await this.resolveAuthFailureDisable(disabledIntent, oldAccountData, oauth2Reauthorized);
        ...
        const wasParked = wasAuthFailureDisabled(oldAccountData);

        if (oauth2Reauthorized && !reconnectRequested && (ERROR_STATES.includes(oldAccountData.state) || wasParked)) {
            this.logger.info({
    ...
    async create(accountData) {
        ...
        if (result[0][1] && result[0][1].account) {
            ...
            if (ERROR_STATES.includes(oldState) || wasParked) {
