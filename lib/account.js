            ...
            switch (key) {
                ...
                case 'path':
                    if (accountData[key] && typeof accountData[key] === 'string') {
                        if (accountData[key].charAt(0) !== '[') {
                            if (['false', 'true'].includes(accountData[key])) {
                            ...
                            try {
                                result[key] = [].concat(JSON.parse(accountData[key]) || []);
                            } catch (err) {
    ...
    serializeAccountData(accountData) {
        ...
        Object.keys(accountData).forEach(key => {
            switch (key) {
                ...
                case 'path':
                    result[key] = JSON.stringify(typeof accountData[key] === 'boolean' ? accountData[key] : [].concat(accountData[key] || []));
                    break;
    ...
    async update(accountData) {
        ...
        if ('imap' in accountData) {
            ...
            try {
                ...
                strictEqual(oldAccountData.path || false, accountData.path || false);
            } catch (err) {
                // changes detected!
