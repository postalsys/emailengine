...
const { checkRateLimit, peekRateLimit } = require('./rate-limit');
...
const REASON_MESSAGES = {
    ...
    limited: 'Too many failed authentication attempts, try again later'
};

// Failed logins a client address may accumulate against one username before the surface stops
// answering that pair for the rest of the window. The guard exists for the shared server
// password, which is operator-chosen with no entropy requirement and was guessable at the full
// speed of a Redis lookup; the 64-hex API tokens are not guessable at any rate this allows.
//
// Keyed on address AND username on purpose. The submission surfaces are typically driven by one
// application host sending for many accounts, and a single stale credential in that host's
// configuration must not lock its other accounts out. The username buys an attacker nothing:
// a guess needs a real account id either way, and each id they know only adds one more budget.
//
// Only failures count. A busy sender authenticating on every connection would exhaust an
// attempt-based budget by itself.
const AUTH_FAILURE_LIMIT = 20;
const AUTH_FAILURE_WINDOW = 5 * 60; // seconds

const authFailureKey = (remoteAddress, account) => `auth:fail:${remoteAddress || 'unknown'}:${account || ''}`;

/**
 * Runs a credential check under the client's failed-login budget.
 *
 * Looks before the attempt (a peek, so an accepted login spends nothing) and records only a
 * refusal. Everything `attempt` throws counts as one: the caller decides what belongs inside
 * the budget by what it puts in the function, so a refusal that is about the account rather
 * than the credential (an API-only account on the IMAP proxy) is left outside it.
 *
 * @param {Object} client
 * @param {String} client.remoteAddress - client IP
 * @param {String} client.account - username the client is authenticating as
 * @param {Function} attempt - async () => result; the credential check
 * @param {Function} limitedError - () => Error; the protocol-specific refusal for a spent budget
 * @returns {Promise<*>} whatever `attempt` resolves with
 */
async function withAuthFailureBudget({ remoteAddress, account }, attempt, limitedError) {
    const key = authFailureKey(remoteAddress, account);

    let limit = await peekRateLimit(key, AUTH_FAILURE_LIMIT, AUTH_FAILURE_WINDOW);
    if (!limit.success) {
        logger.warn({
            msg: 'Refusing authentication attempt, failure limit reached',
            remoteAddress,
            account,
            count: limit.count,
            allowed: limit.allowed,
            ttlReset: limit.ttlReset
        });
        throw limitedError();
    }
    ...
    try {
        ...
        // Never masks the refusal: a counter that could not be written is logged, and the
        // client still gets the error the attempt raised
        try {
            await checkRateLimit(key, 1, AUTH_FAILURE_LIMIT, AUTH_FAILURE_WINDOW);
        } catch (recordErr) {
            logger.error({ msg: 'Failed to record authentication failure', remoteAddress, account, err: recordErr });
        }
        throw err;
    }
}
