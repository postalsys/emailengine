 ...
 * attempt? finishedOn and discarded cover jobs failed via UnrecoverableError or discard(), which
 * end the job regardless of remaining attempts.
 *
...
function isFinalFailedAttempt(job) {
    return !!(job.finishedOn || job.discarded) || job.attemptsMade >= (job.opts.attempts || 1);
}
