...
// BullMQ increments job.attemptsMade when an attempt FAILS, not when it starts. The two helpers
// below encode that phase difference once, because getting it wrong by one is a recurring trap:
// inside a processor the ongoing attempt is attemptsMade + 1, while inside a worker 'failed' event
// handler the failed attempt is already counted. Used by the queue workers to decide whether a
// failure log is the final, operator-actionable record or a routine retryable attempt.

/**
 * Inside a job processor: is the CURRENTLY RUNNING attempt the job's last one?
 *
 * @param {Object} job - BullMQ job (mid-processing, attemptsMade not yet incremented)
 * @returns {Boolean} True when a failure of this attempt will not be retried
 */
function willBeFinalAttempt(job) {
    return job.attemptsMade + 1 >= (job.opts.attempts || 1);
}

/**
 * Inside a worker 'failed' event handler: was the failure that just happened the job's last
 * attempt? finishedOn covers a job ended by UnrecoverableError regardless of remaining attempts.
 * (It used to also check `discarded`, for jobs ended by Job#discard(); BullMQ 6 removed both the
 * method and the property, and UnrecoverableError is the replacement.)
 *
 * @param {Object} job - BullMQ job (attemptsMade already incremented for the failed attempt)
 * @returns {Boolean} True when nothing will be retried and the job is finished for good
 */
function isFinalFailedAttempt(job) {
    return !!job.finishedOn || job.attemptsMade >= (job.opts.attempts || 1);
}

module.exports = { NON_RETRYABLE_CODES, isPermanentDeliveryError, shouldDiscardJob, willBeFinalAttempt, isFinalFailedAttempt };
