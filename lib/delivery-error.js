...
function isPermanentDeliveryError(err) {
    ...
    // An OAuth2 token refresh failure is not a delivery verdict. It happens before any SMTP
    // conversation, and its statusCode is the *token endpoint's* HTTP status - so a provider
    // returning 502 while renewing a token used to be read as a permanent 5xx SMTP rejection and
    // silently discarded a queued message that nothing was wrong with. Let it retry: the queue's
    // own attempt budget bounds it, and the provider outage is by nature temporary.
