...
const { buildPagingView } = require('./route-helpers');
...
function init(args) {
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            let uniqueAccounts = Array.from(new Set(data.addresses.map(entry => entry.account).filter(Boolean)));
            let knownAccounts = new Map(await Promise.all(uniqueAccounts.map(async account => [account, await accountExists(redis, account)])));
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            try {
                ...
                return h.redirect(`/admin/suppression-lists/list/${listId}`);
            } catch (err) {
