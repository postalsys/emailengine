...
// Where a request that named a list goes back to: the list's own page when the ID is one
// the routes would accept, the index otherwise (the ID might be the invalid part)
function listPagePath(listId) {
    return blocklistListIdSchema.validate(listId).error ? '/admin/suppression-lists' : adminUrl`/admin/suppression-lists/list/${listId}`;
}
    ...
    server.route({
        ...
        options: {
            validate: {
                ...
                async failAction(request, h /*, err*/) {
                    // A bad page argument on a real list goes back to that list, not to the index
                    return h.redirect(listPagePath(request.params.listId)).takeover();
                },
    ...
    server.route({
        ...
        options: {
            validate: {
                ...
                async failAction(request, h, err) {
                    // The add form is a modal that closes on submit, so the field error cannot be
                    // shown inline: flash what joi objected to and land back where the modal was
                    // opened, which is the list page whenever the list ID itself is valid
                    let reasons = (err.details || []).map(detail => detail.message).join('; ');
                    await request.flash({ type: 'danger', message: `Couldn't add the address${reasons ? `: ${reasons}` : ''}` });
                    request.logger.debug({ msg: 'Failed to add suppression list entry', err });

                    return h.redirect(listPagePath(request.payload && request.payload.listId)).takeover();
                },
