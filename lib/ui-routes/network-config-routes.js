...
const { listenerCertificateSummary, reportedListenerTls } = require('../tls/status');
const { getCertificateHostnames } = require('../tls/store');
const { getProvisioningStatus } = require('../tls/provision');
...
function init(args) {
    ...
    const serverPageContext = async (h, serverType) => {
        ...
        const [serverState, hostnames] = await Promise.all([getServerStatus(serverType), getCertificateHostnames()]);
        ...
        const reported = reportedListenerTls(serverState);

        // The provisioning state comes along with the certificate: it is what turns the badge into
        // "Requesting", "Failed" or "Renewal failed". Without it this page could only ever read
        // "Valid", including under a renewal that had been failing for weeks. Read only for a
        // listener that reported one; the stored model below carries its own copy of it.
        const provisioning = reported ? await getProvisioningStatus() : {};

        let tlsSummary = listenerCertificateSummary({
            reported,
            hostname: primaryHostname,
            status: provisioning[primaryHostname] || null
        });

        if (!tlsSummary.certificate) {
