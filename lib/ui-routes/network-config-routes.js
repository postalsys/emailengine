...
const { failAction, getServiceHostname, reloadTlsServers } = require('../tools');
...
function init(args) {
    ...
    server.route({
        ...
        async handler(request) {
            try {
                ...
                let addresses = await listPublicInterfaces(localAddresses);
                ...
                return {
                    ...
                    html: compiledFragments.addressList({ addresses })
                };
            } catch (err) {
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                // What is installed before provisioning, so the reload below fires only when a new
                // certificate actually arrived and repeated clicks do not keep restarting both
                // servers.
                let existingCertificateData = await h.getCertificate();

                let certificateData = await h.getCertificate(true);
                if (!certificateData) {
                    throw new Error(`Failed to provision a certificate`);
                }

                if (certificateData.fingerprint && certificateData.fingerprint !== (existingCertificateData && existingCertificateData.fingerprint)) {
                    await reloadTlsServers(call, request.logger, { domain: certificateData.domain });
                }
                ...
                request.logger.error({ msg: 'Failed to provision a certificate', err });
                return {
                    success: false,
                    error: err.message
                };
