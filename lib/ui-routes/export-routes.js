...
const { throwAsBoom, isClientError } = require('./route-helpers');
...
function init(args) {
    ...
    server.route({
        ...
        options: {
            // Crumb validates the crumb field of POST payloads only, so a DELETE sent by the page
            // script passed with no CSRF check at all. Restful mode checks the X-CSRF-Token header
            // instead, which is what the account page sends with this request
