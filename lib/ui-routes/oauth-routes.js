...
function init({ server, call }) {
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            if (provider === 'outlook' || provider === 'outlookService') {
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            let serviceUrl = await settings.get('serviceUrl');
            let defaultRedirectUrl = `${serviceUrl}/oauth`;
            if (providerData.provider === 'outlook' || providerData.provider === 'outlookService') {
                defaultRedirectUrl = defaultRedirectUrl.replace(/^http:\/\/127\.0\.0\.1\b/i, 'http://localhost');
            }
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            try {
                ...
                let serviceUrl = await settings.get('serviceUrl');
                let defaultRedirectUrl = `${serviceUrl}/oauth`;
                if (appData.provider === 'outlook' || appData.provider === 'outlookService') {
                    defaultRedirectUrl = defaultRedirectUrl.replace(/^http:\/\/127\.0\.0\.1\b/i, 'http://localhost');
                }
