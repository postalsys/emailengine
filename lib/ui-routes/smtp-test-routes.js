...
const { compiledFragments } = require('./route-helpers');
...
<p>${now}</p>`,
    ...
    server.route({
        ...
        async handler(request) {
            ...
            try {
                request.logger.debug({ msg: 'Request SMTP test response', user });
                ...
                if (testResponse) {
                    ...
                    if (testResponse.status === 'success') {
                        // The page shows the results through this fragment rather than
                        // compiling the template in the browser, which the admin CSP forbids
                        testResponse.html = compiledFragments.testSend(testResponse);
                    }
                }
