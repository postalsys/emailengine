...
const { accountStateLabel } = require('./route-helpers');

function init(args) {
    const { server, call } = args;

    // Time windows selectable for the Activity counters. Counter buckets are kept for
    // MAX_DAYS_STATS (7) days, so 7d is the longest window that has full data.
    ...
    // Account states surfaced as filter chips under the Accounts cards; connected is
    // covered by its own card. Labels and badge variants derive from the shared
    // accountStateLabel descriptor so the chips cannot drift from the state badges.
    const CHIP_STATES = ['authenticationError', 'connectError', 'disconnected', 'init', 'connecting', 'syncing', 'paused', 'unset'].map(state => {
        let { type: variant, name: label } = accountStateLabel(state);
        if (state === 'authenticationError') {
            // accountStateLabel merges both error states into one "Connection failed"
            // badge; the chips link to distinct ?state= filters, so keep them apart here
            label = 'Authentication failed';
        }
        return { state, variant, label };
    });
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            let attentionCount = (stats.connections.authenticationError || 0) + (stats.connections.connectError || 0);
