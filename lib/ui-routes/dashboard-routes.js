...
// /admin dashboard (stats counters), the legal page, and the upgrade page. Extracted
// verbatim from lib/routes-ui.js. The API reference lives in reference-routes.js.
...
function init(args) {
    ...
    // The same dependency inventory /sbom.json serves, on the admin surface.
    //
    // /sbom.json is deliberately behind the api-token strategy - the file names every package and
    // version the instance runs - and that strategy has no session-cookie fallback, so a link to
    // it from a rendered admin page answered a signed-in admin with a 401. Rather than widening
    // the API gate for a browser, the download an operator clicks is served where their session
    // already authenticates them, inside the EENGINE_ADMIN_ACCESS_ADDRESSES perimeter.
    server.route({
        method: 'GET',
        path: '/admin/legal/sbom.json',
        handler: {
            file: {
                path: pathlib.join(__dirname, '..', '..', 'sbom.json'),
                confine: false,
                // Named for the operator saving it, not for the route it came from
                filename: 'sbom.json',
                mode: 'attachment'
            }
        }
