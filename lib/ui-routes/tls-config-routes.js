...
async function tlsPageContext(h) {
    ...
    const listenerReads = Promise.all([getServerStatus('smtp'), getServerStatus('imapProxy'), settings.getMulti(...LISTENER_TLS_SETTINGS)]);

    // What each listener says it is serving, in the order the listener list below is built in.
    const reports = listenerReads.then(([smtp, imapProxy]) => [apiTls.active || null, reportedListenerTls(smtp), reportedListenerTls(imapProxy)]);
    ...
    // The one statement of "enabled, and TLS on" - the reconciler reads the same derivation
    const switches = listenerTlsFrom(serverSettings);

    // The name clients connect to, and the order state recorded for it. A listener serves every
    // configured name, but the one it reports on is the default context, which is the first name.
    const primary = status.certificates[0] || null;
    const primaryHostname = primary ? primary.hostname : null;
    ...
    const serving = report => listenerCertificateSummary({ reported: report, hostname: primaryHostname, status: primary ? primary.status : null });

    // A server listener that reports no certificate gets one sentence saying why. The template
    // adds the link to the listener's own page when switching something on there is the next step.
    const serverListener = (listener, report) =>
        Object.assign(listener, {
            serving: serving(report),
            hint: !listener.enabled
                ? 'The server is disabled.'
                : listener.tlsEnabled
                  ? 'TLS is on, but the listener has not reported a certificate yet.'
                  : 'TLS is off.',
            needsSetup: !listener.enabled || !listener.tlsEnabled
        });
        ...
        serverListener(
            {
                ...
                enabled: switches.smtp.enabled,
                tlsEnabled: switches.smtp.tlsEnabled,
        ...
        serverListener(
            {
                ...
                enabled: switches.imapProxy.enabled,
                tlsEnabled: switches.imapProxy.tlsEnabled,
