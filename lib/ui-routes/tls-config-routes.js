...
const { buildCertificateStatus } = require('../tls/status');
...
async function tlsPageContext(h) {
    // Independent reads, so the page costs one round of latency rather than four.
    const [status, smtpState, imapProxyState, serverSettings] = await Promise.all([
        buildCertificateStatus({ certs: h.certs }),
    ...
    const listeners = [
        ...
        },
        {
            ...
            active: (smtpState.payload && smtpState.payload.tls) || null
        },
        {
            key: 'imapProxy',
            name: 'IMAP proxy',
            ...
            active: (imapProxyState.payload && imapProxyState.payload.tls) || null
        }
    ];
