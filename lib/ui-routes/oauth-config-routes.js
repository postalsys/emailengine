...
const { throwAsBoom, adminUrl, submittedValues } = require('./route-helpers');
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                ...
                return h.redirect(adminUrl`/admin/config/oauth/app/${request.payload.app}`);
            }
        },
        options: {
            validate: {
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            try {
                ...
                return h.redirect(adminUrl`/admin/accounts/${result.account}`);
            } catch (err) {
                ...
                return h.redirect(adminUrl`/admin/config/oauth/app/${appId}`);
            }
        },
        options: {
            validate: {
                ...
                async failAction(request, h, err) {
                    request.logger.debug({ msg: 'Failed to register service account', err, app: request.params.app });
                    await request.flash({ type: 'danger', message: `Failed to add account. Provide a valid email address.` });
                    return h.redirect(adminUrl`/admin/config/oauth/app/${request.params.app}`).takeover();
                },
    ...
    server.route({
        ...
        options: {
            validate: {
                ...
                async failAction(request, h, err) {
                    request.logger.debug({ msg: 'Invalid verify request', err, app: request.params.app });
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                ...
                return h.redirect(adminUrl`/admin/config/oauth/app/${oauth2App.id}`);
            } catch (err) {
                ...
                return h.view(
                    'config/oauth/new',
                    {
                        ...
                        // The submitted fields, not the stored app: without them the new-app form
                        // loses its provider on a validation error and the next submit is bounced
                        // to /admin
                        values: submittedValues(request, 'clientSecret', 'serviceKey'),
        ...
        options: {
            validate: {
                ...
                async failAction(request, h, err) {
                    ...
                    await request.flash({ type: 'danger', message: `Couldn't register OAuth2 app. Try again.` });
                    request.logger.debug({ msg: 'Failed to register OAuth2 app', err });
            ...
            validate: {
                ...
                async failAction(request, h, err) {
                    ...
                    if (!appData) {
                        ...
                        request.logger.warn({ msg: 'OAuth2 application not found', app: request.payload.app });
                        return h.redirect('/admin').takeover();
                    }

                    await request.flash({ type: 'danger', message: `Couldn't save OAuth2 app. Try again.` });
                    request.logger.debug({ msg: 'Failed to update OAuth2 app', err });
