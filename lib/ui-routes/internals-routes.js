...
const { severityRank } = require('../utils/severity');
        ...
        async handler(request, h) {
            ...
            let processedThreads = threads.map(threadInfo => {
                ...
                if (
                    ...
                    severityRank(threadInfo.statusVariant) < severityRank('warning')
                ) {
                    threadInfo.statusVariant = 'warning';
            ...
            let groups = orderedTypes.map(type => {
                ...
                let rollup = list.reduce(
                    (worst, threadInfo) =>
                        severityRank(threadInfo.statusVariant) > severityRank(worst.variant)
        ...
        async handler(request, h) {
            try {
                ...
                if (killed) {
                    ...
                    await request.flash({ type: 'danger', message: `Requested worker thread was not found` });
                }
