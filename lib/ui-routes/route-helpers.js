...
//
// Every symbol below was moved verbatim from lib/routes-ui.js, apart from compiledFragments,
// which replaced the raw template sources the monolith handed to the browser.
...
const { Account, accountsExist, accountSummary, isAuthFailureDisabled, isAccessTokenInvalidated } = require('../account');
...
// Partials a page renders again after it has loaded: the delivery-test results and the address
// list after an IP rescan. They used to be shipped to the browser as source and compiled there,
// which runs `new Function` and would need 'unsafe-eval' in the Content-Security-Policy the
// admin surface sends. Compiled once here instead, outside vision but on the same handlebars
// instance it renders with, so the helpers and partials registered on it resolve at render
// time; the JSON endpoints return the rendered HTML. Not going through vision is deliberate: its
// per-request view context consumes the pending flash messages. What a fragment therefore lacks
// is that context - no `cspNonce`, so a fragment can never carry an inline <script>.
const FRAGMENT_COMPILE_OPTIONS = { preventIndent: true }; // matches the vision compileOptions in workers/api.js

const compiledFragments = {
    addressList: handlebars.compile(
        fs.readFileSync(pathlib.join(__dirname, '..', '..', 'views', 'partials', 'address_list.hbs'), 'utf-8'),
        FRAGMENT_COMPILE_OPTIONS
    ),
    testSend: handlebars.compile(fs.readFileSync(pathlib.join(__dirname, '..', '..', 'views', 'partials', 'test_send.hbs'), 'utf-8'), FRAGMENT_COMPILE_OPTIONS)
};
...
function formatAccountData(account, gt) {
    ...
    if (account.oauth2) {
        ...
        // the view names the state instead of printing the inverted range the invalidation leaves
        account.oauth2.accessTokenInvalidated = isAccessTokenInvalidated(account.oauth2);
