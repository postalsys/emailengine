...
const { Account, accountsExist } = require('../account');
...
/**
 * Account id/label suggestions for a datalist, shared by the MCP config page and the MCP OAuth
 * consent page so the policy (first 100, label precedence, failure renders without suggestions)
 * lives once. The field these feed accepts any typed id; this is a convenience for small
 * instances, which is why a listing failure is only logged.
 *
 * @param {Function} call - RPC helper for talking to the main thread
 * @param {Object} [logger] - request logger for the failure branch
 * @returns {Promise<Array<{account: String, label: String}>>}
 */
async function accountSuggestions(call, logger) {
    try {
        const listing = await new Account({ redis, call }).listAccounts(false, false, 0, 100);
        return (listing.accounts || []).map(entry => ({
            account: entry.account,
            label: entry.name || entry.email || entry.account
        }));
    } catch (err) {
        if (logger) {
