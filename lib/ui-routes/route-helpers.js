...
const { Account, accountsExist, accountSummary, authFailureDisabledAt } = require('../account');
...
function formatAccountData(account, gt) {
    ...
    //
    // Keyed on the marker the safety net writes, not on `imap.disabled` plus a leftover error:
    // that flag is also the operator's send-only switch, so the old guess badged a deliberately
    // send-only account "Connection failed" if it had ever failed to connect, and stayed silent
    // about a parked account whose lastErrorState had been overwritten since.
    if (authFailureDisabledAt(account)) {
        account.stateLabel = {
            type: 'error',
