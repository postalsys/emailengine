...
function formatAccountData(account, gt) {
    ...
    // Exposed for the views, which cannot call a predicate. Covers the operator-owned
    // imap.disabled and the system-owned auth-failure park, so a template asking "is this account
    // live" does not have to know which flags exist.
    account.connectionBlocked = isConnectionBlocked(account);

    // Show the underlying error rather than a stale label when the account will not connect. The
    // auth-failure park is transport-neutral, so auto-disabled Gmail/Outlook accounts land here too.
    if (account.connectionBlocked && account.lastErrorState) {
        account.stateLabel = {
            type: 'error',
            name: 'Connection failed',
