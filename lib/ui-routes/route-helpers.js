 ...
 * account rather than per row, and each is a single HEXISTS - no account load, nothing decrypted.
 *
...
async function resolveKnownAccounts(entries) {
    let accounts = Array.from(new Set(entries.map(entry => entry.account).filter(Boolean)));
    return new Map(await Promise.all(accounts.map(async account => [account, await accountExists(redis, account)])));
}
