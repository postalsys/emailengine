...
// admin surface sends. Compiled once here instead, on the same handlebars instance vision
// renders with, so the helpers workers/api.js registers on it (statusColor, statusIcon, equals,
// inc) resolve at render time; the JSON endpoints return the rendered HTML.
const compiledFragments = {
    addressList: handlebars.compile(fs.readFileSync(pathlib.join(__dirname, '..', '..', 'views', 'partials', 'address_list.hbs'), 'utf-8')),
    testSend: handlebars.compile(fs.readFileSync(pathlib.join(__dirname, '..', '..', 'views', 'partials', 'test_send.hbs'), 'utf-8'))
};
