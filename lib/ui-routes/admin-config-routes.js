...
const { getByteSize, formatByteSize, getDuration, failAction, readEnvValue, readEnvList, httpAgent } = require('../tools');
...
const { getOpenAiModels, OPEN_AI_MODELS, getExampleDocumentsPayloads, accountSuggestions, mcpToolGrants } = require('./route-helpers');
...
function init(args) {
    ...
    registerSettingsPage(server, {
        ...
        async viewContext(request) {
            const [{ serviceUrl, mcpOAuthEnabled }, accountOptions] = await Promise.all([
                settings.getMulti('serviceUrl', 'mcpOAuthEnabled'),
                accountSuggestions(call, request.logger)
            ]);
