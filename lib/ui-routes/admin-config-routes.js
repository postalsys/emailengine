    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                ...
                return h.redirect('/admin/config/security');
            } catch (err) {
                await request.flash({ type: 'danger', message: `Couldn't save settings. Try again.` });
                request.logger.error({ msg: 'Failed to update configuration', err });
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            const values = {
                ...
                gmailExportBatchSize: storedValues.gmailExportBatchSize || 10,
                outlookExportBatchSize: storedValues.outlookExportBatchSize || 20
            };
            ...
            return h.view(
                'config/email-processing',
                {
                    pageTitle: 'Email Processing',
                    menuConfig: true,
                    menuConfigEmailProcessing: true,
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                ...
                return h.redirect('/admin/config/email-processing');
            } catch (err) {
                await request.flash({ type: 'danger', message: `Couldn't save settings. Try again.` });
                request.logger.error({ msg: 'Failed to update configuration', err });
        ...
        options: {
            validate: {
                ...
                payload: Joi.object({
                    ...
                    gmailExportBatchSize: settingsSchema.gmailExportBatchSize.empty('').default(10),
                    outlookExportBatchSize: settingsSchema.outlookExportBatchSize.empty('').default(20)
                })
            }
        }
    });
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                ...
                return h.redirect('/admin/config/branding');
            } catch (err) {
                await request.flash({ type: 'danger', message: `Couldn't save settings. Try again.` });
                request.logger.error({ msg: 'Failed to update configuration', err });
