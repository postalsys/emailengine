    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                ...
                // Only overwrite the service secret when a non-empty value is provided, so clearing
                // the field cannot blank it (an empty secret would disable HMAC signing/verification).
