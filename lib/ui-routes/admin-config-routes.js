...
const { sendWebhookRequest } = require('../webhook-request');
...
const { mcpResource } = require('../mcp/oauth');
const { getToolRegistry } = require('../mcp/tools');
const { serverUrl } = require('../openapi');
const { MCP_SECTIONS } = require('../token-permission-view');
const { ACTION } = require('../api-routes/permission-map');

// How the MCP config page badges a tool: by the action it dispatches under, read off the grant
// rather than the annotation hints - openWorldHint does not imply sending, the lookups and
// connection tests declare it too
const TOOL_BADGES = {
    [ACTION.SEND]: { label: 'sends email', variant: 'warning' },
    [ACTION.DESTRUCTIVE]: { label: 'destructive', variant: 'error' },
    [ACTION.READ]: { label: 'read-only', variant: 'success' },
    [ACTION.WRITE]: { label: 'write', variant: 'info' }
};
const { locales } = require('../translations');
const { settingsSchema } = require('../schemas');
const { getOpenAiModels, OPEN_AI_MODELS, getExampleDocumentsPayloads, mcpToolGrants } = require('./route-helpers');
...
function init(args) {
    ...
    registerSettingsPage(server, {
        ...
        async applySettings(request) {
            ...
            // Written when emptied too: the schema accepts a blank field, and skipping the
            // write would leave the old URL in place after a save meant to clear it
            data.serviceUrl = request.payload.serviceUrl ? new URL(request.payload.serviceUrl).origin : null;

            // Compared before the write, and only for the reload: this form submits the service URL
            // on every save, so reloading on the write alone would hand three listeners a new
            // certificate every time somebody changed the timezone.
            const serviceUrlChanged = data.serviceUrl !== (await settings.get('serviceUrl'));

            await settings.setMulti(data);

            // The service URL names the first hostname EmailEngine serves TLS for, so changing it
            // here changes what the listeners have to offer exactly as the TLS page's own hostname
            // list does. Without this the listeners kept answering with a certificate for the
            // previous name until something restarted them.
            await maybeReloadTlsCertificates(call, request.logger, serviceUrlChanged ? ['serviceUrl'] : [], { action: 'settings' });
        }
    });
    ...
    registerSettingsPage(server, {
        ...
        schema: {
            ...
            tokenAuditLog: settingsSchema.tokenAuditLog.default(false),
        ...
        async loadValues() {
            const storedValues = await settings.getMulti('serviceSecret', 'disableTokens', 'enableOAuthTokensApi', 'ignoreMailCertErrors', 'tokenAuditLog');
            ...
            return {
                ...
                tokenAuditLog: storedValues.tokenAuditLog || false
            };
        },
        ...
        async applySettings(request) {
            let data = {
                ...
                tokenAuditLog: request.payload.tokenAuditLog
            };
    ...
    registerSettingsPage(server, {
        ...
        view: 'config/mcp',
        pageTitle: 'MCP',
        menuKey: 'menuConfigMcp',
        schema: {
            mcpEnabled: settingsSchema.mcpEnabled.default(false),
            mcpOAuthEnabled: settingsSchema.mcpOAuthEnabled.default(false)
        },

        async loadValues() {
            const storedValues = await settings.getMulti('mcpEnabled', 'mcpOAuthEnabled');

            return {
                mcpEnabled: !!storedValues.mcpEnabled,
                mcpOAuthEnabled: !!storedValues.mcpOAuthEnabled
            };
        },

        async viewContext(request) {
            const { serviceUrl, mcpOAuthEnabled } = await settings.getMulti('serviceUrl', 'mcpOAuthEnabled');

            // The configured public address when a Service URL is set (the same derivation the
            // OAuth metadata publishes as the resource identifier), otherwise the address this
            // request arrived on - the same request-derived answer /swagger.json publishes
            let configuredOrigin = null;
            try {
                configuredOrigin = serviceUrl ? new URL(serviceUrl).origin : null;
            } catch (err) {
                // an unparseable serviceUrl setting falls back to the request address
            }

            const mcpUrl = mcpResource(configuredOrigin || serverUrl(request));

            // The catalog and the endpoint read one registry (lib/mcp/tools.js), so this list is
            // exactly what an agent receives from tools/list - the operator approves what they
            // can see, not a prose summary of it. The badge collapses the annotation hints back
            // into the one action they were derived from.
            const mcpTools = mcpToolGrants(server, request.logger);

            // The catalog, split the way the scopes split it: one bucket per access section in
            // section order, each tool filed under the first section whose scope covers it (so
            // the account reads both scopes share sit under management, marked as shared), and
            // anything no scope covers in a bucket of its own - a guardrail keeps that one empty,
            // and the page says so rather than filing such a tool under a scope it did not check.
            const toolCatalog = { sections: [], unreached: [] };
            try {
                const grantsByName = new Map(mcpTools.map(entry => [entry.name, entry]));
                const buckets = new Map(Object.values(MCP_SECTIONS).map(section => [section.scope, { label: section.label, scope: section.scope, tools: [] }]));
                for (const tool of getToolRegistry(server).tools) {
                    const grant = grantsByName.get(tool.name);
                    const surfaces = (grant && grant.surfaces) || [];
                    const entry = {
                        name: tool.name,
                        description: tool.description,
                        badge: (grant && TOOL_BADGES[grant.action]) || TOOL_BADGES[ACTION.WRITE],
                        shared: surfaces.length > 1
                    };
                    const bucket = [...buckets.keys()].find(scope => surfaces.includes(scope));
                    (bucket ? buckets.get(bucket).tools : toolCatalog.unreached).push(entry);
                }
                toolCatalog.sections = [...buckets.values()];
            } catch (err) {
                request.logger.error({ msg: 'Failed to derive the MCP tool catalog', err });
            }
            toolCatalog.total = toolCatalog.sections.reduce((sum, section) => sum + section.tools.length, toolCatalog.unreached.length);

            const serviceUrlConfigured = !!configuredOrigin;

            // Which precondition blocks the OAuth connection method right now, or null when it
            // is ready: the stored setting first (the page-local action), then a usable Service
            // URL. This is what oauthOrigin() checks minus the mcpEnabled setting - deliberately,
            // because the Connect tab's own "endpoint not enabled" alert owns that state and the
            // badge must not double-report it; null with the deployment gate off for the same
            // reason. The one value drives both the method tab's badge and the panel's warning,
            // so the two cannot disagree. Stored state on purpose, not the posted form value:
            // it reports what a connector would actually get today. The blocked method's tab
            // stays clickable - a disabled tab cannot explain how to enable itself - and carries
            // a "not enabled" badge instead.
            let oauthMethodBlocker = null;
            if (mcpFeatureEnabled) {
                if (!mcpOAuthEnabled) {
                    oauthMethodBlocker = 'setting';
                } else if (!serviceUrlConfigured) {
                    oauthMethodBlocker = 'serviceUrl';
                }
            }
            ...
            return {
                ...
                oauthMethodBlocker,
                toolCatalog,
                // The same catalog reduced to what the tool count needs: one entry per tool with
                // the operation it dispatches under, the scopes that reach it and whether it can
                // be bound to an account
                mcpTools,
                // The two access sections with their levels and what each mints: the generator's
                // radio groups render through the shared mcp_access_levels partial and its script
                // mints from the same table the consent prompt does, so a level added there is
                // offered here without a second list to update
                mcpSections: MCP_SECTIONS,
                // The same gate as the token pages: minting needs an authenticated admin
                // session on this request, not just a password existing in Redis
        ...
        async applySettings(request) {
            await settings.setMulti({
                mcpEnabled: request.payload.mcpEnabled,
                mcpOAuthEnabled: request.payload.mcpOAuthEnabled
            });
        }
    });
    ...
    server.route({
        ...
        async handler(request) {
            try {
                ...
                request.logger.warn({ msg: 'Failed to reload OpenAI models', err });
                return {
                    success: false,
                    error: err.message
                };
        ...
        options: {
            validate: {
                ...
                async failAction(request, h, err) {
                    request.logger.debug({ msg: 'Failed to process preview', err });
    ...
    registerSettingsPage(server, {
        ...
        async viewContext(request, values) {
            ...
            return {
                ...
                // reporting was turned on by trial activation, not by the operator; disclosed
                // on the page because saving the form converts it into an explicit choice
                sentryTrialManaged: !sentryEnvManaged && !!(await settings.get('sentryAutoEnabled'))
            };
        },
    ...
    server.route({
        ...
        async handler(request) {
            ...
            try {
                ...
                // The same call the notify worker makes, rather than a lookalike: a probe that
                // reimplements delivery can only prove that the reimplementation works. It also
                // inherits the delivery timeout and the response drain, both of which this route
                // used to do without while running on the same keep-alive retry dispatcher.
                try {
                    await sendWebhookRequest(fetchCmd, parsed.toString(), {
                        method: 'post',
                        body:
                    ...
                    duration = Date.now() - start;
                    // undici wraps a connection failure in a generic "fetch failed"; the cause is
                    // the part worth reporting back to the operator
                    throw err.cause || err;
                }
                ...
                // log the parsed URL, not the raw payload value, so embedded basic-auth credentials
                // stay out of the log
                request.logger.warn({ msg: 'Failed to post test webhook', webhooks: parsed.toString(), event: 'test', err });
                return {
                    success: false,
                    target: webhooks,
                    duration,
                    error: err.message,
                    code: err.code,
                    // Same field the stored webhookErrorFlag carries, so the probe and the error
                    // block on this page describe a failed delivery the same way
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                ...
                request.logger.warn({ msg: 'Failed to register license key', err });
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                ...
                request.logger.error({ msg: 'Failed to unregister license key', err, remoteAddress: request.app.ip });
                return h.redirect('/admin/config/license');
            }
        },
        options: {
