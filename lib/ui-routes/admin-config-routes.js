...
const configWebhooksSchema = {
    ...
    customHeaders: Joi.string().allow('').trim().max(10 * 1024)
};
        ...
        options: {
            validate: {
                ...
                payload: Joi.object({
                    ...
                    locale: settingsSchema.locale.empty('').valid(...locales.map(locale => locale.locale)).default('en'),
    ...
    server.route({
        ...
        options: {
            ...
            validate: {
                ...
                failAction,
                payload: Joi.object({
                    webhooks: Joi.string().uri({ scheme: ['http', 'https'], allowRelative: false }).allow(''),
                    customHeaders: Joi.string().allow('').trim().max(10 * 1024).default(''),
                    payload: Joi.string().max(1024 * 1024).empty('').trim()
                })
            }
        }
    });
        ...
        options: {
            validate: {
                ...
                payload: Joi.object({
                    ...
                    contentFnJson: Joi.string().max(1024 * 1024).default('').allow('').trim(),
        ...
        options: {
            validate: {
                ...
                payload: Joi.object({
                    ...
                    language: Joi.string().empty('').lowercase().regex(/^[a-z0-9]{1,5}([-_][a-z0-9]{1,15})?$/).allow(false),
