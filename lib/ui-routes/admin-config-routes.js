...
function init(args) {
    ...
    registerSettingsPage(server, {
        ...
        async viewContext(request) {
            ...
            // The catalog, split the way the scopes split it: the management tools (which
            // include the account reads both scopes share, marked as such) and the mail tools.
            // The badge names the action the tool dispatches under, read off the grant rather
            // than the annotation hints - openWorldHint no longer implies sending now that the
            // lookups and connection tests declare it too.
            const BADGES = {
            ...
            try {
                ...
                for (const tool of getToolRegistry(server).tools) {
                    ...
                    const entry = {
                        ...
                        badge: (grant && BADGES[grant.action]) || BADGES[ACTION.WRITE],
                        shared: !!grant && grant.surfaces.includes('mcp') && grant.surfaces.includes('mcp-manage')
                    };
                    toolCatalog[grant && grant.surfaces.includes('mcp-manage') ? 'manage' : 'mail'].push(entry);
                }
            } catch (err) {
                request.logger.error({ msg: 'Failed to derive the MCP tool catalog', err });
            }
            toolCatalog.total = toolCatalog.manage.length + toolCatalog.mail.length;
