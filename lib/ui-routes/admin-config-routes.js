...
const { MCP_ACCESS_LEVELS, MCP_ACCESS_LEVEL_OPTIONS } = require('../token-permission-view');
...
function init(args) {
    ...
    registerSettingsPage(server, {
        ...
        async viewContext(request) {
            ...
            let toolCatalog = [];
            try {
                toolCatalog = getToolRegistry(server).tools.map(tool => ({
                    name: tool.name,
                    description: tool.description,
                    badge: tool.annotations.openWorldHint
                        ? { label: 'sends email', variant: 'warning' }
                        : tool.annotations.destructiveHint
                          ? { label: 'destructive', variant: 'error' }
                          : tool.annotations.readOnlyHint
                            ? { label: 'read-only', variant: 'success' }
                            : { label: 'write', variant: 'info' }
                }));
            } catch (err) {
            ...
            return {
                ...
                // the operation it dispatches under and whether it can be bound to an account
                mcpTools,
                // The whole access-level table, `full: null` included: the page script keys on
                // the same map the consent route mints from, so a level added there is offered
                // here without a second list to update
                mcpAccessPresets: MCP_ACCESS_LEVELS,
                // The generator's radio group renders through the shared mcp_access_levels
                // partial, from the same table the consent prompt offers
                accessOptions: MCP_ACCESS_LEVEL_OPTIONS,
