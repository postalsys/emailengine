...
function init(args) {
    ...
    registerSettingsPage(server, {
        ...
        async viewContext() {
            ...
            // The same derivation the OAuth metadata publishes as the resource identifier, so
            // the URL the operator copies from this page and the one the instance advertises
            // cannot disagree
            let mcpUrl = false;
            try {
                mcpUrl = serviceUrl ? mcpResource(new URL(serviceUrl).origin) : false;
            } catch (err) {
                // an unparseable serviceUrl setting leaves the snippet on its placeholder
            }
