...
// Must match the policy the webhooks worker enforces on real deliveries
    ...
    server.route({
        ...
        async handler(request) {
            ...
            try {
                ...
                // Same egress policy as real delivery (workers/webhooks.js). This route posts to
                // the same operator-supplied URL with the same custom headers and reports the
                // response status back to the caller, so leaving it unguarded would keep a
                // better-instrumented version of the hole the delivery path now closes.
                await assertAllowedUrl(parsed.toString(), { policy: WEBHOOK_EGRESS_POLICY });
                ...
                try {
                    res = await fetchCmd(parsed.toString(), {
                        ...
                        // Matches the delivery path: a target that passed the egress check could
                        // otherwise redirect us to one it rejects, and this route reports the
                        // result back to the caller, which makes it the better probe of the two.
                        redirect: WEBHOOK_EGRESS_POLICY === POLICY_OFF ? 'follow' : 'manual'
                    });
                ...
                if (WEBHOOK_EGRESS_POLICY !== POLICY_OFF && res.status >= 300 && res.status < 400) {
