...
function init(args) {
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            const nonceKey = data.n ? `${REDIS_PREFIX}account:form:${data.n}` : null;
            if (nonceKey) {
                const claimed = await redis.set(nonceKey, (data.t || '0').toString(), 'EX', Math.floor(MAX_FORM_TTL / 1000), 'NX').catch(err => {
            ...
            try {
                ...
                // Release the nonce so the user can fix the form and retry.
                if (nonceKey) {
                    await redis.del(nonceKey).catch(() => false);
                }
                throw err;
            }
            ...
            return h.view(
                'accounts/account',
                {
                    ...
                    accountForm: await getSignedFormData({
                        ...
                        // Include a nonce + timestamp so this re-auth link inherits the same 24h TTL as
                        // API-issued links; a fresh link is minted on each page render.
                        n: crypto.randomBytes(NONCE_BYTES).toString('base64url'),
                        t: Date.now()
                    }),
