...
function init(args) {
    ...
    server.route({
        ...
        options: {
            // Rendered email HTML keeps its remote images, which no other admin page loads. The
            // stylesheet the widget appends is accepted without a nonce until the vendored
            // @postalsys/ee-client carries the styleNonce option the page already passes (1.8.0):
            // removing style-src-elem falls back to style-src, whose 'unsafe-inline' the sheet
            // needs - listing 'unsafe-inline' next to the nonce the directive would get appended
            // would disable it instead. Drop the null entry with that upgrade
            plugins: { securityHeaders: { directives: { 'img-src': "'self' data: https:", 'style-src-elem': null } } },
            validate: {
                options: {
                    stripUnknown: true,
                    abortEarly: false,
