...
const { OAUTH_REDIRECT_FORM_DIRECTIVES } = require('../security-headers');
...
function init(args) {
    ...
    // The names above, in declaration order, for the checking page's hidden inputs - so the field set
    // it posts is the one this schema declares rather than a second copy written out in the template.
    const connectionFieldNames = Object.keys(connectionFields);

    // Values validated against the fields above, ignoring everything else the form values carry
    // (the signed blob, the address, the typed password, the app-password hint).
    const connectionFieldsSchema = Joi.object(connectionFields).unknown();

    // Whether autodiscovery gave the form enough to configure an account without asking the visitor
    // for anything more. Two decisions read it: the settings page auto-runs its connection check on
    // load, and a setup link issued with skipServerSettings skips that page altogether. One answer,
    // because the skip turns a wrong one into an account created from settings nobody looked at.
    //
    // Asked of the schema rather than of a hand-kept list of required names - the shape that has to
    // be kept in sync by hand, and the reason rebuildFormValues above rebuilds from the schema too.
    // So "complete" means exactly "the create endpoint would accept this": a field added or made
    // required there moves both decisions with it, and a discovered value the schema rejects (an
    // address literal where a hostname is required) falls back to the form rather than skipping
    // ahead to a submit that would 400.
    //
    // `_source` is the part no schema can answer. The usernames and passwords are filled in from
    // what the visitor typed whatever autodiscovery found, so without it a domain that resolved to
    // nothing would look complete on the strength of the password alone.
    const hasCompleteServerSettings = values => !!(values && values._source) && !connectionFieldsSchema.validate(values, { convert: true }).error;
    ...
    // Re-derive the app-password hint for a settings form that is being rendered from posted values
    // rather than from a fresh autodiscovery result, and merge it into those values.
    //
    // serverFormSchema does not carry the appPassword_* fields (they feed unescaped template sinks),
    // so stripUnknown drops them and they cannot be carried in a hidden input - echoing a
    // client-controlled hint into the page was an XSS vector. The hint is therefore rebuilt here from
    // the (trusted) email, out of a static table, never from the submitted payload.
    //
    // Shared by the failAction re-render and by the checking page's hand-off to the settings form, so
    // a form reached either way shows the same "Application Password" label and instructions.
    //
    // Try the static domain table first: it is synchronous, needs no network, and covers the
    // canonical provider domains, so a common-domain render costs no Redis round trips, no DNS, and
    // no probe budget. Never throws - the hint is cosmetic and a failAction must not throw.
    const applyAppPasswordHint = async (values, request, parsedBlob) => {
        if (!values.email) {
            return;
        }
        ...
        // Custom-domain fallback (e.g. Google Workspace / M365 on their own domain) needs an MX
        // lookup. failAction runs BEFORE the handler's signature gate, and the lookup drives outbound
        // DNS, so gate it with the SAME machinery as the handler siblings, not a bare signature
        // check: authorizeSignedProbe rejects a signature-only blob (e.g. a leaked tracking/
        // unsubscribe blob) and one past its TTL, and bounds how many lookups one signed link can
        // drive. Without it, any holder of any validly-signed blob could turn this render into an
        // unbounded DNS oracle. On a normal validation-error re-render the nonce is not yet consumed
        // (the create handler claims it only on success), so the hint still shows. values.data /
        // values.sig are the schema-validated copies - rebuildFormValues drops them when malformed
        // (e.g. duplicated fields).
        if (!appPassword && values.data && values.sig) {
            // Consult the per-domain memo BEFORE the probe/DNS work: a repeat re-submit of the same
            // email must not re-run the MX lookup or draw the probe budget again.
            ...
            if (cached) {
                ...
                try {
                    if (parsedBlob) {
                        // Already parsed and signature-checked by the caller, so only the cap is left
                        // to apply - re-parsing would mean a second serviceSecret read and HMAC check
                        await enforceFormProbeLimit(parsedBlob.n, request.app.gt, request.logger);
                    } else {
                    ...
                    // Cache the outcome (including a falsy "no hint" result) so subsequent renders for
                    // this domain are free.
                    setCachedAppPasswordHint(domain, appPassword);
                } catch (err) {
                    // A rejected/expired blob, an exhausted probe cap, or a lookup failure must never
                    // break the render. Do NOT cache on failure - an exhausted probe cap or transient
                    // DNS error should not poison the hint for the whole TTL.
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            let serverSettings;
            try {
                // The password the user just typed is passed along because hosted Exchange refuses
                // to answer autodiscovery anonymously; it only ever reaches the autodiscovery host
                // named by the address domain's own DNS, and only after that host has asked for it
            ...
            // Autodiscovery has already answered the app-password question for this domain, so record
            // it for the renders that cannot compute it from a static table: the failAction re-render
            // and the checking page's hand-off both reach applyAppPasswordHint with nothing but the
            // address, and would otherwise repeat the MX lookup this result came with. Only a positive
            // answer is recorded - a miss here is not authoritative, since the MX-success branch
            // resolves the hint without knowing whether the domain is an Exchange one.
            if (serverSettings && serverSettings.appPassword) {
                setCachedAppPasswordHint(request.payload.email.split('@').pop().trim().toLowerCase(), serverSettings.appPassword);
            }

            // Nothing is left for the user to fill in: the settings page would auto-verify what is
            // already there and wait for a click. hasCompleteServerSettings() (above, beside
            // connectionFields) is what both that auto-verify and the skip below agree on.
            const settingsComplete = hasCompleteServerSettings(values);

            if (settingsComplete && data.skipServerSettings) {
                // The setup link asked for the server settings step to be skipped where possible.
                // The page checks the connection and, when it holds, submits the same value set the
                // settings form would have submitted; a failed check hands the user that form after
                // all, so the skip can never end the setup on its own.
                return h.view(
                    'accounts/register/validating',
                    {
                        pageTitleFull: request.app.gt.gettext('Email Account Setup'),
                        values,
    ...
    // Where the checking page sends the user when the connection check did not pass: the ordinary
    // server settings form, prefilled with everything that was discovered and typed, so the one wrong
    // field can be corrected. Signature-gated like its siblings because it renders submitted values
    // back into a page. Rendering it drives no outbound connection of its own, so the gate here is
    // the signature and the nonce rather than authorizeSignedProbe; the one thing that can still
    // reach the network is the app-password hint's MX lookup, which draws on the per-nonce probe
    // budget itself and normally answers from the memo page 1 seeded.
    server.route({
        method: 'POST',
        path: '/accounts/new/imap/settings',

        async handler(request, h) {
            const data = await parseSignedFormData(redis, request.payload, request.app.gt, { requireNonce: true });

            // Rebuilt from the same schema that validated the submission, exactly as the failAction
            // re-renders do: coerced tri-state checkboxes, no unknown or prototype keys. The two
            // passwords are kept rather than dropped - they are what the user typed one step ago and
            // the form has to come up filled in, the same as it would have been had this page been
            // rendered directly.
            const values = rebuildFormValues(serverFormSchema, request.payload);
            await applyAppPasswordHint(values, request, data);
            ...
            return h.view(
                'accounts/register/imap-server',
                {
                    pageTitleFull: request.app.gt.gettext('Email Account Setup'),
                    values,
                    // The checking page says a connection failed, so the form opens by saying why it
                    // is being asked for rather than looking like a step that appeared from nowhere.
                    // The detail stays on the page the visitor read it on; this is the reminder. The
                    // no-script Continue and the manual escape from a slow check arrive without it,
                    // because no check ran for them and neither must claim one failed.
                    checkFailed: request.query.checkFailed,
                    // The check just failed, so do not repeat it on load: the page opens on the fields
                    // to correct rather than on a spinner about to give the same answer.
                    autoTest: false
                },
                {
                    layout: 'public'
        ...
        options: {
            ...
            validate: {
                ...
                // Cosmetic and client-set: it chooses which sentence the page opens with and nothing
                // else, so it rides the action URL rather than widening a payload schema that carries
                // two plaintext passwords.
                query: Joi.object({
                    checkFailed: Joi.boolean().truthy('1').falsy('', '0').default(false)
                }),
        ...
        options: {
            ...
            validate: {
                ...
                async failAction(request, h, err) {
                    ...
                    // The hint fields are not carried by the schema, so rebuild them from the email
                    // (see applyAppPasswordHint) - otherwise a Gmail or iCloud re-render loses the
                    // "Application Password" label and the instructions link.
                    await applyAppPasswordHint(values, request);
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            // State comes from the same derivation the API reports, so the page and the response
            // cannot disagree about whether push is working. The reason and the failed request stay
            // here and out of the API: both are text the provider wrote, and this page is behind an
            // admin session. NB! a watch that is still armed but whose last renewal failed now
            // reads Failed rather than Active - it used to show the expiry alone and hide the
            // failure in a tooltip, which said nothing was wrong right up to the day it lapsed
            let watchState = formatGmailWatch(accountData);
            let gmailWatch = watchState
                ? {
                      lastCheckStr: watchState.lastCheck,
                      expiresStr: watchState.expires || false,
                      active: watchState.state === 'active',
                      stateLabel: { active: 'Active', error: 'Failed', expired: 'Expired' }[watchState.state] || 'Unknown',
                      // The reason and the failed request are read straight off the record here and
                      // deliberately absent from the shared derivation: both carry text the
                      // provider wrote, including the operator's own Pub/Sub topic path, and this
                      // page is behind an admin session while the API field is not
                      error: accountData.watchFailure ? accountData.watchFailure.err || 'Failed to set up Gmail pubsub watch' : false
                  }
                : false;

            // Not "watchFailure truthy means gmailWatch is": the derivation reports nothing for an
            // account whose application has no Pub/Sub app, whatever record a previous one left
            if (gmailWatch && accountData.watchFailure && accountData.watchFailure.req) {
                gmailWatch.request = {
        ...
        options: {
            // The "Re-authenticate" modal posts to /accounts/new, which answers with a redirect to the
            // provider's authorization page. Without this the admin preset's `form-action 'self'` blocks
            // the submission, even though the form posts to this instance
            plugins: { securityHeaders: { directives: OAUTH_REDIRECT_FORM_DIRECTIVES } },
            validate: {
                options: {
                    stripUnknown: true,
                    abortEarly: false,
    ...
    server.route({
        ...
        options: {
            // Rendered email HTML keeps its remote images, which no other admin page loads. Its
            // stylesheet needs no exception: the widget stamps the nonce the page passes it
            // (styleNonce in views/accounts/browse.hbs) onto the element it appends
            plugins: { securityHeaders: { directives: { 'img-src': "'self' data: https:" } } },
            validate: {
                options: {
                    stripUnknown: true,
                    abortEarly: false,
