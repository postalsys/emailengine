...
function init(args) {
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            const canReadMail = !isConnectionBlocked(accountData) && !DISABLE_MESSAGE_BROWSER;
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                ...
                // An operator saving this form is the interactive, human re-authorization the flag
                // exists for - the same position PUT /v1/account/{account} occupies. Without it a
                // parked account could not be revived by fixing its credentials in the UI, which is
                // the route most operators will reach for. update() still requires an actual
                // credential change before it lifts the park or dispatches a reconnect, so passing
                // the flag here cannot re-arm the unattended-renewal loop the 2.73.1 hotfix closed.
                await accountObject.update(updates, { reauthorized: true });
