...
// is enabled, so the `&&` short-circuit must guard the call.
async function ssoProviders(h) {
    let okta = USE_OKTA_AUTH && (await h.validateOktaConfig());
    let oidc = sso.USE_OIDC_AUTH && (await h.validateOidcConfig()) ? { name: sso.OIDC_PROVIDER_NAME } : false;
...
function init(args) {
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            if (sso.OIDC_FORCED && sso.USE_OIDC_AUTH && (await h.validateOidcConfig())) {
