...
function init(args) {
            ...
            try {
                ...
                if (authData && authData.password) {
                    ...
                    try {
                        ...
                        // verify() only throws on a malformed stored hash, not on a wrong password
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            let [registeredPasskeys, serviceUrl] = await Promise.all([listPasskeysForView(username), settings.get('serviceUrl')]);
        ...
        async handler(request, h) {
            ...
            try {
                ...
                if (hasExistingPassword) {
                    // The current-password field is a password check like the login form's, and
                    // unthrottled it let a hijacked session guess the password at full speed - the
                    // one thing a session alone is not supposed to reveal. Keyed on the account
                    // rather than the address because the caller is already authenticated as it,
                    // and the same budget as the login form's per-username limiter.
                    ...
                    let valid;
                    try {
                        valid = await pbkdf2.verify(authData.password, request.payload.password0);
                    } catch (E) {
                        // verify() only throws on a malformed stored hash, not on a wrong password
                        request.logger.error({ msg: 'Failed to verify password hash', err: E });
                        valid = false;
