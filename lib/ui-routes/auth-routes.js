    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            if (forceLogin && !request.query.sso_denied && !request.query.loggedout) {
                // Rendered rather than redirected, so that a form submission whose session expired
                // is not killed by the admin form-action policy - see the view for the whole story.
                // NB! rendering drains the flash queue, so a message queued just before the login
                // bounce is shown for the frame this page is up rather than on the login form.
                return h.view(
                    'account/login-redirect',
                    {
                        ...
                        menuLogin: true,
                        providers,
                        ssoRedirectUrl: '/admin/login/oidc'
                    },
                    {
                        layout: 'login'
                    }
