        ...
        async handler(request) {
            try {
                ...
                if (request.payload.permissions) {
                    // Both axes have to name something. An empty allowlist allows nothing, so a form
                    // submitted with a box unticked everywhere would mint a token that can do nothing
                    // at all - refused here rather than issued as a puzzle for whoever uses it.
                    for (const axis of ['actions', 'groups']) {
                        if (!request.payload.permissions[axis] || !request.payload.permissions[axis].length) {
                            throw Boom.badRequest(`Select at least one ${axis === 'actions' ? 'action' : 'section'} for a restricted token`);
                        }
                    }
