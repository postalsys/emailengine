...
const { authAwareLevel, resolveKnownAccounts, adminUrl } = require('./route-helpers');
...
return payload;`)
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            let mcpTools = [];
            try {
                mcpTools = toolGrants(server);
            } catch (err) {
                request.logger.error({ msg: 'Failed to derive the MCP tool catalog', err });
            }
            ...
            return h.view(
                'tokens/new',
                {
                    ...
                    mcpAccessPresetsJson: JSON.stringify(tokenPermissionView.MCP_ACCESS_LEVELS),
