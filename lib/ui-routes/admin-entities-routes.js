...
return payload;`)
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            const knownAccounts = await resolveKnownAccounts(data.tokens);
        ...
        options: {
            validate: {
                ...
                query: Joi.object({
                    ...
                    page: Joi.number().integer().min(1).max(1000000).default(1),
                    pageSize: Joi.number().integer().min(1).max(250).default(DEFAULT_PAGE_SIZE),
                    query: Joi.string().empty('').example('prometheus').description('Filter by search term')
                })
            }
        }
    });
