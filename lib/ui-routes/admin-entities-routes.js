...
return payload;`)
    ...
    server.route({
        ...
        async handler(request) {
            try {
                // This route is only protected by `server.auth.default('session')`, which
                // workers/api.js applies conditionally on authData - so while no admin password is
                // set it is reachable with no credential at all and would mint a token that
                // outlives the unprotected window (nothing invalidates tokens when the password is
                // later set).
                //
                // POST /v1/token carries its own version of this check rather than sharing one in
                // tokens.provision(): there, a caller presenting a real API token is trusted, so the
                // check narrows to the `preauth` (disableTokens) bypass. Here there is no credential
                // to distinguish, so the gate is unconditional.
