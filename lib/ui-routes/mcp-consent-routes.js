...
function renderConsent(h, { client, redirectUri, canProvision, values, errors }) {
    // What approving actually grants, derived from the same tables the enforcement reads
    // (SURFACE_GRANTS.mcp through the token form's view model) - hand-written prose here would
    // drift the first time the tool set changes
    ...
    return h.view(
        'mcp/authorize',
        {
            ...
            // Mirrors the admin token mint: the form warns instead of letting the operator fill
            // everything in and lose it to a 403
            canProvision,
