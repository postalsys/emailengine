...
const { accountSuggestions, mcpToolGrants } = require('./route-helpers');
...
async function init(args) {
    ...
    async function renderConsent(h, { client, redirectUri, canProvision, values, errors }) {
        // Only fetched for a render that can actually approve: without an admin session the
        // account field is inert, and the page is reachable unauthenticated, so an anonymous
        // visitor should not cost an account-listing IPC per render
        const accountOptions = canProvision ? await accountSuggestions(call) : [];
