...
// (wrong response_type, a code_challenge_method missing or other than S256, a resource naming another
...
const { accountPickerSelection, mcpToolGrants } = require('./route-helpers');
const { failAction } = require('../tools');
const { accountIdSchema } = require('../schemas');
const { MCP_SECTIONS, mcpGrantsFor } = require('../token-permission-view');
const { mcpFeatureEnabled } = require('../mcp');
const { oauthOrigin, getClient, touchClient, createAuthorizationCode, isAcceptableResource, normalizeScopes } = require('../mcp/oauth');

// Both consent routes answer with a redirect to the OAuth client's registered redirect_uri,
// often http://localhost for a native client, and Chrome checks the redirect target of a form
// submission against form-action - so neither carries that directive
const CONSENT_SECURITY_HEADERS = { securityHeaders: { directives: { 'form-action': null } } };
...
// Both render paths use the chrome-free `prompt` layout: the operator lands here from an MCP
// client mid-flow, so the page reads as a consent dialog, not as a place inside the admin app.
// It is still an admin-surface page (admin session gate, admin IP perimeter, FlyonUI) - only
// the navigation chrome is gone.
...
async function init(args) {
    ...
    async function renderConsent(h, { client, redirectUri, canProvision, values, errors }) {
        // Only resolved for a render that can actually approve, and only when the form carries an
        // account at all: the picker fetches its own suggestions, so a render costs an account
        // lookup exactly when there is a choice on the form to show back
        const selectedAccount = canProvision && values && values.account ? await accountPickerSelection(values.account, server.logger) : null;

        // Management at Observe and no mail access unless the request or the operator said
        // otherwise; a re-render after a validation error keeps whatever they chose. The template
        // compares the levels against each option itself (the `equals` helper), so a level is one
        // value rather than a boolean per option.
        const manageLevel = values && Object.hasOwn(MCP_SECTIONS.manage.levels, values.manage) ? values.manage : MCP_SECTIONS.manage.defaultLevel;
        const mailEnabled = !!(values && values.mailEnabled);
        const mailLevel =
            values && values.mailLevel !== 'none' && Object.hasOwn(MCP_SECTIONS.mail.levels, values.mailLevel)
                ? values.mailLevel
                : MCP_SECTIONS.mail.defaultLevel;

        // From the registry the endpoint dispatches through, so the count under the radios cannot
        // describe a tool set that has since changed
        const mcpTools = mcpToolGrants(server, server.logger);
        ...
        return h.view(
            'mcp/authorize',
            {
                ...
                // The two sections and their levels, with what approving each one actually grants
                // in plain words, derived once at load from the same tables the enforcement reads -
                // hand-written prose here would drift the first time the tool set changes. The
                // radio groups render through the shared mcp_access_levels partial.
                mcpSections: MCP_SECTIONS,
                manageLevel,
                mailEnabled,
                mailLevel,

                // The tool count under the sections, through the shared mcp_tool_count partial. The
                // grant sentences above it say what a level allows in the permission vocabulary;
                // this says which tools the client ends up holding, which is the thing the person
                // approving is actually being asked about.
                mcpTools,
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            // An absent method is not S256: RFC 7636 defaults it to "plain", which the token
            // endpoint refuses - so a client that omits it would clear consent and only fail at
            // the exchange, where nobody is looking. Required explicitly, the reason renders here.
            ...
            // The `scope` parameter is a hint for the starting position of the form, never a
            // grant: a client asking for `mcp` is asking for mail access, so the mail section
            // starts switched on; one asking only for `mcp` starts with no management access,
            // since it did not ask for any. A request naming no scope starts where the page
            // starts anyway, management at Observe and mail off. The person still decides.
            const requested = normalizeScopes(request.query.scope);
            ...
            return renderConsent(h, {
                ...
                values: {
                    ...
                    manage: requested.length && !requested.includes(MCP_SECTIONS.manage.scope) ? 'none' : MCP_SECTIONS.manage.defaultLevel,
                    mailEnabled: requested.includes(MCP_SECTIONS.mail.scope)
                }
            });
        },

        options: {
            plugins: CONSENT_SECURITY_HEADERS,
            validate: {
                options: {
                    stripUnknown: true,
                    abortEarly: false,
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            // Re-checked here and not only on the GET, because the POST is where the value is
            // written onto the code record and the GET is not what supplied it - a hand-crafted
            // form can name any resource it likes. Redemption refuses a code whose resource does
            // not name this server, so without this the mint succeeds and the exchange fails,
            // which reads to the operator as an approval that did nothing.
            if (!isAcceptableResource(request.payload.resource, origin)) {
                return renderError(h, 'This authorization request names a different server as its resource, so it was not meant for this instance.');
            }
            ...
            // Management at Observe and no mail access are the defaults here exactly as they are
            // on the admin generator, and for a stronger reason: this flow issues credentials to
            // the least controllable clients on the least controllable devices. The controls
            // always post a value, so the schema defaults only ever fire for a stale or
            // hand-crafted POST - and they mint the narrowest useful token, not the widest. The
            // mail level counts only while the mail checkbox was posted with it, so a level left
            // in a hidden radio grants nothing.
            const choice = {
                manage: request.payload.manage,
                mail: request.payload.mailEnabled ? request.payload.mailLevel : 'none'
            };
            const { scopes, permissions } = mcpGrantsFor(choice);

            if (!scopes.length) {
                // Every section declined is not a narrow token, it is no token: re-rendered with
                // the reason rather than minting a credential that can do nothing
                return renderConsent(h, {
                    client,
                    redirectUri: request.payload.redirect_uri,
                    canProvision: true,
                    values: request.payload,
                    errors: { access: 'Choose at least one kind of access, or deny the request.' }
                });
            }
            ...
            validate: {
                ...
                payload: Joi.object(
                    Object.assign({}, oauthParamsSchema, {
                        ...
                        // The level keys of MCP_SECTIONS, defaulting to the narrowest useful
                        // choice: a payload that names no level (a stale form, a hand-crafted
                        // POST) mints a management token that can only observe, and no mail
                        // access unless the checkbox was posted
                        manage: Joi.string()
                            .valid(...Object.keys(MCP_SECTIONS.manage.levels))
                            .default(MCP_SECTIONS.manage.defaultLevel),
                        mailEnabled: Joi.boolean().truthy('on', 'true', '1').falsy('', 'off', 'false', '0').default(false),
                        mailLevel: Joi.string()
                            .valid(...Object.keys(MCP_SECTIONS.mail.levels).filter(level => level !== 'none'))
                            .default(MCP_SECTIONS.mail.defaultLevel)
                    })
                )
            }
        }
