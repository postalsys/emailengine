...
const { MCP_SECTIONS, mcpGrantsFor, mcpLevelNames } = require('../token-permission-view');
...
async function init(args) {
    ...
    async function renderConsent(h, { client, redirectUri, canProvision, values, errors }) {
        ...
        // otherwise; a re-render after a validation error keeps whatever they chose. One value per
        // section, which the template compares against each level itself (the `equals` helper).
        const selectedLevels = {};
        for (const [key, section] of Object.entries(MCP_SECTIONS)) {
            const posted = values && values[key];
            selectedLevels[key] = mcpLevelNames(section).includes(posted) ? posted : section.defaultLevel;
        }
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            // starts at its narrowest level; one asking only for `mcp` starts with no management
            // access, since it did not ask for any. A request naming no scope starts where the
            // page starts anyway, management at Observe and mail declined. The person still
            // decides.
            const requested = normalizeScopes(request.query.scope);
            const hinted = {};
            for (const [key, section] of Object.entries(MCP_SECTIONS)) {
                if (requested.includes(section.scope)) {
                    hinted[key] = section.levels[0].value;
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            // the least controllable clients on the least controllable devices. The radios always
            // post a value, so the schema defaults only ever fire for a stale or hand-crafted POST
            // - and they mint the narrowest useful token, not the widest.
            const choice = {};
            for (const key of Object.keys(MCP_SECTIONS)) {
                choice[key] = request.payload[key];
            }
            ...
            validate: {
                ...
                payload: Joi.object(
                    Object.assign({}, oauthParamsSchema, {
                        ...
                        // One field per section, validated against the levels the table has plus
                        // `none`, defaulting to the section's own default - the narrowest useful
                        // choice: a payload that names no level (a stale form, a hand-crafted POST)
                        // mints a management token that can only observe, and no mail access
                        ...Object.fromEntries(
                            Object.entries(MCP_SECTIONS).map(([key, section]) => [
                                key,
                                Joi.string()
                                    .valid(...mcpLevelNames(section))
                                    .default(section.defaultLevel)
                            ])
                        )
                    })
                )
