...
const { MCP_ACCESS_LEVELS, MCP_ACCESS_LEVEL_OPTIONS, MCP_GRANT_WORDING } = require('../token-permission-view');
...
async function init(args) {
    ...
    async function renderConsent(h, { client, redirectUri, canProvision, values, errors }) {
        ...
        // Read-only unless the operator picked otherwise; a re-render after a validation error
        // keeps whatever they chose. The template compares this against each option itself
        // (the `equals` helper), so the level is one value rather than three booleans.
        const accessLevel = values && Object.hasOwn(MCP_ACCESS_LEVELS, values.access) ? values.access : 'read';
        ...
        return h.view(
            'mcp/authorize',
            {
                ...
                // What approving actually grants, in plain words, derived once at load from the
                // same tables the enforcement reads - hand-written prose here would drift the
                // first time the tool set changes
                grantWording: MCP_GRANT_WORDING,

                accessLevel,
                // The radio group itself renders through the shared mcp_access_levels partial
                accessOptions: MCP_ACCESS_LEVEL_OPTIONS,

                // The tool count under the radios, through the shared mcp_tool_count partial. The
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            // the least controllable devices. The radio always posts a value, so the schema
            // default only ever fires for a stale or hand-crafted POST - and defaulting that to
            // 'read' means an omitted field mints the narrowest token, not the widest.
            const access = request.payload.access;
            ...
            const code = await createAuthorizationCode({
                ...
                account,
                permissions: MCP_ACCESS_LEVELS[access],
            ...
            validate: {
                ...
                payload: Joi.object(
                    Object.assign({}, oauthParamsSchema, {
                        ...
                        // The keys of MCP_ACCESS_LEVELS, defaulting to the narrowest: a payload
                        // that names no level (a stale form, a hand-crafted POST) mints the
                        // read-only token, never the wide one
                        access: Joi.string()
                            .valid(...Object.keys(MCP_ACCESS_LEVELS))
                            .default('read')
                    })
                )
            }
