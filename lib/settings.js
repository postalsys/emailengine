...
// Settings a narrowed access token may not read or write, whatever its record grants. `write/settings` is
// a grantable permission, and most of the settings blob is what it says: webhook targets,
// notification shaping, sync strategy, listener ports. These keys are different in kind - each one
// would make the holder more than a settings editor - so assertNoPrivilegedSettings() in
// lib/api-routes/route-helpers.js refuses them to a restricted credential, and the MCP settings
// tool never offers them. An unnarrowed API token is unaffected, as it is for `proxy`.
//
// Listed beside SECRET_KEYS because the three lists are read together: a key that is a secret is
// privileged, but the privileged list is wider than the secrets.
const PRIVILEGED_SETTINGS_KEYS = [
    ...
    // and which stored certificate each listener presents, the API's included, and the mode that
    ...
    // The deprecated Document Store's endpoint and credentials, and the error reporting target
...
module.exports = {
    ...
    // What a narrowed token may never read or write, see PRIVILEGED_SETTINGS_KEYS above
    privilegedKeys: PRIVILEGED_SETTINGS_KEYS,
