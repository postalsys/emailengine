    ...
    async getValue(encryptSecret, key, value) {
        ...
        if (typeof value === 'string') {
            try {
                ...
                logger.debug({ msg: 'Failed to parse stored setting', key, encrypted: ENCRYPTED_KEYS.includes(key), err });
                return null;
            }
        }
...
-----END LICENSE-----`;
    ...
    async setLicense(licenseData, licenseFile) {
        ...
        // operator has never touched the sentryEnabled setting (atomic through setIfMissing),
        // tracked through the internal sentryAutoEnabled marker: activating a full license or
        // removing the license clears the default again as long as the marker still owns the
        // state, and any explicit operator write to sentryEnabled drops the marker (see
        // set()/setMulti()), making that choice permanent. A SENTRY_DSN environment value pins
        // the Sentry configuration regardless of these settings. License activation must never
        // fail on this convenience, hence the catch-all.
        try {
            if (licenseData.trial) {
                if (await module.exports.setIfMissing('sentryEnabled', true)) {
                    await module.exports.set('sentryAutoEnabled', true);
