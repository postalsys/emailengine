 ...
 *
 * A DSN body is not a single header block. RFC 3464 splits it into a per-message block
 * and a separate per-recipient block for every reported address, with an empty line between
 * them. libmime.decodeHeaders() stops at the first empty line, as that is where a header
 * block ends, so every block has to be decoded on its own and the results merged.
 *
...
function decodeDeliveryStatus(content) {
    // Prototype-less, the keys come straight from the message and may include "__proto__"
    let entries = Object.create(null);

    // Group the lines into blocks at the empty lines, splitting on the line breaks libmime splits on
    let blocks = [[]];
    ...
    for (let block of blocks) {
        ...
        let blockEntries = libmime.decodeHeaders(block.join('\r\n'));
        for (let key of Object.keys(blockEntries)) {
            if (!entries[key]) {
                entries[key] = [];
            }
            entries[key].push(...blockEntries[key]);
        }
    }
...
module.exports = { bounceDetect, decodeDeliveryStatus };
