...
const { isUnsafeTransformedKey } = require('./utils/unsafe-key');
const { pushHeaderValue } = require('./utils/header-map');
const simpleParser = require('mailparser').simpleParser;
const { toCamelCase } = require('./utils/to-camel-case');

// Wording of a notice that reports a delay rather than a failure. Sendmail and qmail-style MTAs
// send these as plain text, with the same "<user@example.com>:" recipient block a bounce carries
const DELAY_NOTICE_RE =
    /WARNING MESSAGE ONLY|DO NOT NEED TO RESEND|has not yet been delivered|will keep trying|delivery attempts will continue|delivery (?:is|has been) delayed/i;

// Wording that says the MTA gave up. A final bounce often quotes the last temporary reply the
// remote host gave ("Sorry, I wasn't able to establish an SMTP connection. (#4.4.1)" followed by
// "I'm not going to try again"), so a 4.x.x status on its own does not make a notice a delay
const GAVE_UP_RE = /permanent error|given up|not going to try again|retry timeout exceeded|will not be retried|could not be delivered/i;

/**
 * Tells whether a notice the text heuristics matched reports a delay rather than a failure
 *
 * @param {string} text - Body text the heuristics ran over
 * @param {Object} [response] - The response the heuristics extracted
 * @returns {Boolean} True for a temporary failure the MTA is still retrying
 */
function isDelayNotice(text, response) {
    if (DELAY_NOTICE_RE.test(text)) {
        return true;
    }

    let status = (response && response.status) || '';
    let message = (response && response.message) || '';
    let temporary = /^4\./.test(status) || /^4\d{2}[\s-]/.test(message);

    return temporary && !GAVE_UP_RE.test(text);
}
...
function parseDeliveryReport(content) {
    ...
    for (let key of Object.keys(entries)) {
        ...
        //
        // Checked on the camelCased key rather than the raw name: decodeHeaders() already dropped
        // the raw name at the parse boundary, but toCamelCase() re-creates it out of a name that
        // boundary had no reason to refuse - "_-_proto__" camelCases straight into "__proto__",
        // and "to-string" into "toString", which as an own property shadows the method on the
        // published report - hence the transformed-key test covering every Object.prototype member.
        let displayKey = toCamelCase(key);
        if (isUnsafeTransformedKey(displayKey)) {
            continue;
        }
...
async function bounceDetect(sourceStream) {
            ...
            if (parsedOriginal) {
                ...
                if (parsedOriginal.headerLines && Array.isArray(parsedOriginal.headerLines)) {
                    parsedOriginal.headerLines.forEach(entry => {
                        ...
                        // pushHeaderValue() skips names that resolve to an Object.prototype member,
                        // which a bare `!headers[key]` probe would have pushed onto, throwing out of
                        // the whole block and losing every header of the bounced message
                        pushHeaderValue(headers, entry.key, entry.line.substring(entry.key.length + 1).trim());
                    });
                }
    ...
    // What the structured parts said, if anything; the text heuristics below only fill gaps, and
    // only a claim they made is reconsidered at the end
    let structuredAction = result.action;
    ...
    // The heuristics above claim 'failed' for any recipient block or quoted reply they recognize,
    // but a delay notice has the same shape: a Sendmail-style "THIS IS A WARNING MESSAGE ONLY"
    // carries a "<user@example.com>: ... (#4.4.1)" block, and a plain-text notice may quote a
    // "said: 450 4.2.1 ..." reply. A delay reported as a DSN never gets here (it is applied as a
    // delivery report first, see applyDeliveryReport), so only a claim the heuristics made is
    // reconsidered. The action is cleared rather than set to 'delayed': every caller treats a set
    // action as a bounce to store and to fire emailBounce for, and a message the remote host is
    // still retrying is not one
    if (!structuredAction && result.action === 'failed' && isDelayNotice(text, result.response)) {
