...
function decodeDeliveryStatusBlocks(content) {
    return (
        (content || '')
            ...
            .replace(/\r\n|\r/g, '\n')
            .split(/\n{2,}/)
            .map(block => libmime.decodeHeaders(block))
 ...
 *
 * Takes decoded entries rather than the raw attachment, as every caller has just decoded
 * the same content to keep as the reported message headers.
 *
 * @param {Object} entries - Header entries from libmime.decodeHeaders()
        ...
        try {
            ...
            if (parsedOriginal) {
                ...
                if (!messageHeaders) {
                    ...
                    if (messageHeaders) {
                        let headerEntries = libmime.decodeHeaders((messageHeaders.content || '').toString().trim());
