 ...
 * @requires postal-mime - For parsing email messages
 */

const libmime = require('libmime');
const { parseEmail, getHeader, hasHeader, contentToString } = require('./parse-helpers');
...
async function bounceDetect(sourceStream) {
    ...
    if (sourceStream.parsed) {
        ...
        // Parse the email (postal-mime auto-detects DSN emails)
        parsed = await parseEmail(sourceStream);
    }
    ...
    let deliveryStatus = attachments.find(attachment => attachment.mimeType?.toLowerCase() === 'message/delivery-status');
    let messageHeaders = attachments.find(attachment => attachment.mimeType?.toLowerCase() === 'text/rfc822-headers');
    let originalMessage = attachments.find(attachment => attachment.mimeType?.toLowerCase() === 'message/rfc822');

    // Zoho Mail uses non-standard content type for original message
    let zohoOriginalMessage = attachments.find(attachment => attachment.mimeType?.toLowerCase() === 'text/rfc822');

    let parsedMessageHeaders;

    // Handle Exchange/Outlook specific header for failed recipients
    if (hasHeader(parsed.headers, 'x-failed-recipients')) {
        // Parse comma and space separated list of failed recipients
        let list = getHeader(parsed.headers, 'x-failed-recipients')
    ...
    if (hasHeader(parsed.headers, 'x-mailer')) {
        let xMailer = (getHeader(parsed.headers, 'x-mailer')[0] || '').trim();

        let threadTopic = (getHeader(parsed.headers, 'thread-topic')[0] || '').trim();
    ...
    if (deliveryStatus) {
        result = Object.assign(result, parseDeliveryStatus(contentToString(deliveryStatus.content)) || {});
    }

    // Process RFC822 headers attachment (contains headers of the original message)
    if (messageHeaders) {
        parsedMessageHeaders = libmime.decodeHeaders(contentToString(messageHeaders.content).trim());
        result = Object.assign(result, parseDeliveryHeaders(contentToString(messageHeaders.content)) || {});
    }
    ...
    if (originalMessage) {
        try {
            ...
            let parsedOriginal = await parseEmail(Buffer.from(originalMessage.content));
            if (parsedOriginal) {
                // Build headers object from postal-mime headers array for consistent access
                let headers = {};

                if (parsedOriginal.headers && Array.isArray(parsedOriginal.headers)) {
                    parsedOriginal.headers.forEach(entry => {
                        if (!headers[entry.key]) {
                            headers[entry.key] = [];
                        }
                        headers[entry.key].push(entry.value);
                    });
                }
                ...
                if (!deliveryStatus) {
                    deliveryStatus = originalAttachments.find(attachment => attachment.mimeType?.toLowerCase() === 'message/delivery-status');
                    if (deliveryStatus) {
                        result = Object.assign(result, parseDeliveryStatus(contentToString(deliveryStatus.content)) || {});
                    }
                }
                ...
                if (!messageHeaders) {
                    messageHeaders = originalAttachments.find(attachment => attachment.mimeType?.toLowerCase() === 'text/rfc822-headers');
                    if (messageHeaders) {
                        result = Object.assign(result, parseDeliveryHeaders(contentToString(messageHeaders.content)) || {});
                    }
                }
    ...
    if (zohoOriginalMessage) {
        ...
        if (parsed.headers) {
            ...
            const xMailer = getHeader(parsed.headers, 'x-mailer').join(' ').toLowerCase();
            // Check From header for Zoho domain
            const fromHeader = getHeader(parsed.headers, 'from')
                .map(f => (f.address || f.text || f || '').toLowerCase())
                .join(' ');
            // Check User-Agent header
            const userAgent = getHeader(parsed.headers, 'user-agent').join(' ').toLowerCase();
        ...
        if (isZohoBounce) {
            try {
                let parsedOriginal = await parseEmail(Buffer.from(zohoOriginalMessage.content));
                // Zoho sometimes includes headers in the text body with no actual header structure
                if (parsedOriginal && parsedOriginal.text && (!parsedOriginal.headers || !parsedOriginal.headers.filter(h => h.key).length)) {
    ...
    if (!text && !parsed.html && attachments.length) {
        let emptyContent = attachments.find(attachment => !attachment.mimeType);
        if (emptyContent) {
            text = contentToString(emptyContent.content);
        }
    }
