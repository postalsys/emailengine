...
/**
 * Decodes the body of a message/delivery-status part into a header entries object
 *
 ...
 * libmime.decodeHeaders() stops at the first empty line, as that is where a header block ends,
 * so the separators are collapsed and the whole body is decoded as one block.
 *
 ...
 *
 * Where decodeDeliveryStatus() returns the raw header entries, this turns them into the
 * shape published on messageData.deliveryReport: keys camelCased and Arrival-Date
 * normalized to an ISO timestamp. A field reported for several recipients keeps the last
 * value, which is what the DSN paths have always read.
 *
 * NB! A field that carries an address-type prefix, eg. "Final-Recipient: rfc822; user@example.com",
 * is dropped rather than reported as { label, value }. That is not intentional, it is what
 * this code has always done, and the report is a published webhook payload - so changing it
 * is a deliberate decision and not a refactoring detail.
 *
 * @param {string|Buffer} content - Raw message/delivery-status body
...
function parseDeliveryReport(content) {
    ...
    for (let key of Object.keys(entries)) {
        ...
        if (/^[^\s;]+;/.test(value)) {
            // carries an address type, see the note above
            continue;
        }
...
module.exports = { bounceDetect, decodeDeliveryStatus, parseDeliveryReport };
