...
//
// Everything here is pure and dependency-free so it can be unit tested against the
// committed swagger.json fixture without booting a server.
//
...
// Human-readable type for a schema node. Arrays name their item type so a property
// table row reads "array of object" instead of a bare "array".
//
// This works on a plain schema and cannot follow $refs - resolving those needs the
// whole document, which is buildNode()'s job in schema-tree.js.
function typeLabel(schema) {
    ...
    if (Array.isArray(schema.anyOf) && schema.anyOf.length) {
        return schema.anyOf.map(entry => typeLabel(entry)).join(' | ');
    }

    if (schema.type === 'array') {
        const itemType = schema.items ? typeLabel(schema.items) : 'any';
        return `array of ${itemType}`;
    }

    if (schema.enum && schema.enum.length && !schema.type) {
        return 'enum';
    }
...
// keywords and the `x-*` extensions joi-to-swagger emits, which swagger-ui drops on
// the floor even though they carry real validation rules (accepted URI schemes,
// email format, value coercion).
