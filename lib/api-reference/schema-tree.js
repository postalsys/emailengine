...
const { formatDescription, typeLabel, constraintList } = require('./format');
...
function buildNode(spec, rawSchema, options) {
    ...
    node.typeLabel = typeLabel(schema);

    // typeLabel() cannot follow $refs (it has no access to the document), so an array
    // whose items are a $ref would read "array of any". 46 properties in the current
    // document are shaped that way, so the item type is resolved here instead.
    if (schema.type === 'array' && schema.items) {
        const item = resolve(spec, schema.items, resolved.refStack);
        node.typeLabel = `array of ${item.schema ? typeLabel(item.schema) : 'any'}`;
    }
    ...
    if (Array.isArray(schema.anyOf) && schema.anyOf.length) {
        ...
        // typeLabel() cannot follow $refs, and the anyOf branches are $refs into
        // x-alt-definitions, so the summary line is rebuilt from the resolved branches
        node.typeLabel = node.variants.map(variant => variant.typeLabel).join(' or ');
        return node;
    }
