...
const { buildSchemaTree, buildExample, typeLabel, enumInfo } = require('./schema-tree');
...
function buildOperation(spec, path, method, operation, tagName) {
    ...
    // which hapi-swagger joins into the standard `description` - so every consumer of
    ...
    // hapi-swagger joins an array `notes` with this separator (lib/paths.js)
    const paragraphs = (operation.description || '').split('<br/><br/>');
