...
const { formatDescription, slugify, methodVariant, typeLabel, constraintList, stringifyExample } = require('./format');
const { buildSchemaTree, buildExample } = require('./schema-tree');
...
// The five streaming and binary downloads (SSE changes feed, raw message source,
// attachment, log stream, export download) declare only error responses - hapi-swagger
// has no schema to attach for a raw stream, so the success case is missing from the
// spec entirely and the old swagger-ui page showed nothing for it. The content type is
// still recoverable: the route's `produces` is what got stamped onto every sibling
// response, so a success row is synthesized from it rather than left blank.
function synthesizeSuccess(entries) {
    if (entries.some(entry => entry.variant === 'success')) {
        return entries;
    }

    const contentType = entries.map(entry => entry.contentType).find(type => type && type !== JSON_CONTENT_TYPE);
    ...
    return [
        {
            code: '200',
            variant: 'success',
            descriptionHtml: formatDescription(`Successful response. The body is returned as ${contentType} and has no JSON schema.`),
            contentType,
            tree: null,
            exampleJson: '',
            synthesized: true
...
// Tab ids are built here rather than in the template because ui/tabs neutralizes its
// own hash params inside the partial block, so the operation id is not reachable from
// there to compose them with.
function buildResponses(spec, responses, operationId) {
    ...
    return synthesizeSuccess(entries).map((entry, index) =>
        Object.assign(entry, {
...
function buildOperation(spec, path, method, operation, tagName) {
    const parameters = (operation.parameters || []).map(buildParameter);
