...
// What the Try it panel says before you press Send, keyed on the operation's declared
// impact (lib/api-routes/operation-impact.js) and falling back to the HTTP method.
//
...
const TRY_IMPACT = {
    destructive: { warn: true, confirm: true, text: 'This permanently removes data from this instance. It cannot be undone from here.' },
    sends: { warn: true, confirm: true, text: 'This sends real email. Messages reach the recipients in the request, not a sandbox.' },
    readonly: null,
    write: { warn: false, text: 'Runs against live data on this instance.' }
};

function tryImpact(operation, method) {
    const declared = operation['x-ee-impact'];
    if (declared) {
        return TRY_IMPACT[declared] || null;
    }

    // Undeclared: a GET reads, anything else writes. Operations where the method is a bad
    // proxy for the effect declare an impact instead of relying on this.
    return method.toLowerCase() === 'get' ? null : TRY_IMPACT.write;
}
