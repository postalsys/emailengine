...
const { formatDescription, slugify, tagUrl, operationUrl, methodVariant, constraintList, stringifyExample } = require('./format');
const { buildSchemaTree, buildExample, typeLabel, enumInfo, requiredFirst, rowExample, itemExampleFor } = require('./schema-tree');
const { readCodeSamples } = require('./code-samples');
const { NOTES_SEPARATOR } = require('../openapi');
const { IMPACT, resolveImpact } = require('../api-routes/operation-impact');

const METHOD_ORDER = ['get', 'post', 'put', 'patch', 'delete', 'options', 'head'];

const JSON_CONTENT_TYPE = 'application/json';

// What the Try it panel says before you press Send, keyed on the operation's resolved impact.
// The resolution - declared `x-ee-impact` if there is one, otherwise the HTTP method default -
// belongs to lib/api-routes/operation-impact.js, because the token permission model narrows on
// the same value. Deriving the method default here as well is how a route ends up classified one
// way for this banner and another way for access control.
//
// Every panel used to carry the same warning - "requests run against this instance with real
// data" - on all 82 operations, 42 of which only read. A warning that fires on GET /v1/stats
// is how a reader learns to skim past the one on DELETE /v1/account/{account}, and it
// described the mechanism (this is a real POST) rather than the consequence. Weight now
// tracks consequence: nothing on a read, one muted line on an ordinary write, a real warning
// where the effect cannot be taken back.
// `warn` picks the rendering in reference/try-it: an alert box when true, one muted line
// otherwise. A boolean rather than a level string because the template can branch on it with
// a plain {{#if}}, and this codebase has no comparison helper to test a string against.
// `confirm` puts a confirmation step in front of Send, on the operations whose effect cannot
// be taken back. Every field arrives prefilled from the schema example, so
// DELETE /v1/account/{account} sits one click from a real deletion with nothing in between.
//
// Sending is behind the same dialog, and the draft submit is the reason. Its body is
// entirely optional overrides - the stored draft already carries its own recipients, and
// lib/api-routes/submit-routes.js says as much - so Send there mails people the reader never
// typed and cannot see anywhere in the panel. Handing that a lighter treatment than a DELETE
// would have the guard weakest exactly where the request says least about itself.
//
// The dialog shows the resolved request, method and path with the values actually in the
// form, plus the body when there is one: "delete this account?" is a weaker question than
// "DELETE /v1/account/user123?".
const TRY_IMPACT = {
    [IMPACT.DESTRUCTIVE]: { warn: true, confirm: true, text: 'This permanently removes data from this instance. It cannot be undone from here.' },
    [IMPACT.SENDS]: { warn: true, confirm: true, text: 'This sends real email. Messages reach the recipients in the request, not a sandbox.' },
    [IMPACT.READONLY]: null, // reads only, so the panel says nothing at all
    [IMPACT.WRITE]: { warn: false, text: 'Runs against live data on this instance.' }
};

function tryImpact(operation, method) {
    // An unresolvable impact lands on the same `null` as READONLY, which is safe only because a
    // route cannot declare a bad impact and reach a release: test/api-reference-test.js asserts
    // every declaration against DECLARABLE_IMPACTS.
    return TRY_IMPACT[resolveImpact(operation['x-ee-impact'], method)] || null;
}

// A parameter group this long, whose members are all the same type and carry no
// documentation of their own, is one decision rather than N of them. GET /v1/settings takes
// 87 boolean query parameters that select which settings come back; rendered a row each they
// were roughly 260 lines of page at the same visual weight as parameters that carry a real
// choice, and they pushed the response schema three screens down. Matched on shape rather
// than on the endpoint, so anything similar added later is covered too.
const FLAG_GROUP_MIN = 12;
...
function buildParameter(spec, parameter, operationId) {
    ...
    const view = {
        ...
        usageHtml: formatDescription(schema['x-meta'] && schema['x-meta'].usage),
        constraints: constraintList(schema),
        deprecated: !!parameter.deprecated,
        // Shown on the row, under the same rules a schema property gets. A parameter is
        // built with a tree node's field names on purpose - reference/params renders it
        // through reference/schema-node - so the example belongs here too. Nothing
        // inherits: a parameter has no ancestor to inherit from.
        example: rowExample(schema, { itemExample: itemExampleFor(spec, schema, []), enums }),
    ...
    if (enums && enums.alsoAccepts.length) {
        view.alsoAccepts = enums.alsoAccepts;
    }
...
// Names of a parameter group that is a flag list rather than N documented parameters, or
// null when the group is anything else. See FLAG_GROUP_MIN.
//
// The boolean and default-false tests are not incidental: reference/params renders the group
// under a fixed sentence telling the reader to set the ones they want to `true`. A predicate
// looser than that sentence would eventually match a group of undocumented strings and
// describe it wrongly, having also dropped each member's type, default and anchor. Predicate
// and prose have to mean the same thing.
function flagNames(parameters) {
    if (parameters.length < FLAG_GROUP_MIN) {
        return null;
    }

    const uniform = parameters.every(
        parameter =>
            parameter.typeLabel === 'boolean' &&
    ...
    return uniform ? parameters.map(parameter => parameter.name) : null;
}
...
function buildBody(spec, requestBody, operationId) {
    ...
    // Whether the route declared the whole payload itself, in which case neither view is
    // synthesized and there is nothing to offer a second tab for
    const curated = typeof picked.media.example !== 'undefined';
    const exampleValue = curated ? picked.media.example : buildExample(spec, schema);
    const tree = buildSchemaTree(spec, schema, `${operationId}.body`);

    // The curated payload answers "what does a normal call look like" and is the right
    // default. It cannot also answer "what else can I send", because an object-level
    // `.example()` stops the descent, so every key its author left out is invisible: 10 of
    // 71 fields on POST /v1/account/{account}/submit, 8 of 62 on PUT /v1/account/{account},
    // 466 of 651 across all request bodies. The complete view fills the rest in from the
    // per-property examples, curated values still winning wherever they exist.
    //
    // Only offered when it actually differs, which is 8 bodies of the document's 34 - on the
    // rest the typical payload already names every field and a second identical tab would be
    // a control that does nothing.
    const completeValue = curated ? undefined : buildExample(spec, schema, { complete: true });
    const exampleJson = stringifyExample(exampleValue);
    const completeJson = stringifyExample(completeValue);

    return {
        contentType: picked.contentType,
        descriptionHtml: formatDescription(requestBody.description),
        tree,
        scopeId: `${operationId}-body-schema`,
        exampleValue,
        exampleJson,
        completeJson: completeJson !== exampleJson ? completeJson : '',
        // Panel ids for the two views. Built here for the same reason the response tab ids
        // are: ui/tabs neutralizes its own hash params inside the partial block, which puts
        // the operation id out of reach for composing them there.
        exampleTabId: `${operationId}-body-example`,
        completeTabId: `${operationId}-body-example-full`
    };
}
...
function buildResponses(spec, responses, operationId) {
    ...
    const entries = Object.keys(responses).map(code => {
        ...
        const tree = schema && variant === 'success' ? buildSchemaTree(spec, schema, `${operationId}.resp.${code}`) : null;
        ...
        return {
            ...
            tree,
            scopeId: `${operationId}-resp-${code}-schema`,
...
function buildOperation(spec, path, method, operation, tagName) {
    ...
    // 87 query parameters, and the handful that are mandatory should not be buried.
    const pathParams = parameters.filter(parameter => parameter.in === 'path');
    const queryParams = requiredFirst(parameters.filter(parameter => parameter.in === 'query'));
    const headerParams = requiredFirst(parameters.filter(parameter => parameter.in === 'header'));
    ...
    return {
        ...
        // What the Try it panel warns about, if anything
        tryImpact: tryImpact(operation, method),
        path,
        ...
        queryParams,
        headerParams,
        // Set only when the group is really a flag list; reference/params renders those as a
        // chip grid instead of a row each. Query only: flagNames() needs an optional group,
        // and lib/openapi/build-document.js marks every path parameter required, while the
        // largest header group in the document is two entries.
        queryFlags: flagNames(queryParams),
        body,
...
function buildModel(spec) {
    ...
    return {
        ...
        searchIndex: allOperations.map(operation => ({
            ...
            url: operationUrl(operation.tagSlug, operation.id)
        }))
    };
}
