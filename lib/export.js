...
// Lua script: atomically check concurrent limits AND add to active set if under limits
// Returns 1 if added successfully, 0 if at/over either limit
// This eliminates TOCTOU race by combining check and add in a single atomic operation
...
// Atomically check concurrent limits and add to active set
// Returns true if added, throws TooManyExports error if limits exceeded
...
class Export {
    static async create(account, options) {
        ...
        // Atomically check concurrent limits AND add to active set to prevent race conditions
        ...
        try {
            // Create BullMQ job (active set entry was already added atomically above)
            await exportQueue.add('export', { account, exportId }, { jobId: exportId, removeOnComplete: true, removeOnFail: true });
        } catch (err) {
            // Cleanup on failure: remove from active set and delete export data
    ...
    static async listActive(account) {
        const activeSet = await redis.smembers(ACTIVE_EXPORTS_KEY);
        const accountPrefix = `${account}:`;
    ...
    static async resume(account, exportId) {
        ...
        // Reset status and re-queue the job (active set entry was already added atomically above)
        await redis
            .multi()
            .hmset(exportKey, {
                status: 'queued',
        ...
        try {
            ...
            // Rollback on failure: remove from active set and restore failed status
            await redis
                .multi()
                .hmset(exportKey, {
                    status: 'failed',
