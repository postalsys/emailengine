...
// Lua script: check if account is under concurrent export limit
...
class Export {
    static async create(account, options) {
        ...
        const underLimit = await redis.eval(CONCURRENT_COUNT_SCRIPT, 1, activeKey, maxConcurrent, accountPrefix);
