...
// Lua script: check if account is under both per-account and global concurrent export limits
// Returns 1 if under both limits, 0 if at/over either limit
// SADD is done separately after job creation to avoid TOCTOU race
const CONCURRENT_COUNT_SCRIPT = `
...
class Export {
    static async create(account, options) {
        ...
        // Check concurrent limit atomically (SADD is done after job creation to avoid TOCTOU race)
        const activeKey = `${REDIS_PREFIX}exp:active`;
        const accountPrefix = `${account}:`;
        const activeEntry = `${account}:${exportId}`;
        const underLimit = await redis.eval(CONCURRENT_COUNT_SCRIPT, 1, activeKey, maxConcurrent, maxGlobalConcurrent, accountPrefix);
        ...
        // Determine if encryption should be enabled (only if secret is available)
        ...
        try {
            // Create BullMQ job first, then add to active set
            // This order prevents failed job creation from inflating the concurrent count
            await exportQueue.add('export', { account, exportId }, { jobId: exportId, removeOnComplete: true, removeOnFail: true });
            await redis.sadd(activeKey, activeEntry);
        } catch (err) {
            // Cleanup export data on failure (active set entry was never added)
    ...
    static async queueMessage(account, exportId, messageInfo) {
        ...
        // Combine timestamp (seconds) with hash-based tiebreaker to create unique scores
        // This prevents duplicate timestamps from causing messages to be skipped at batch boundaries
        // Score format: baseSeconds * 1000000 + tiebreaker (stays within JS safe integer range)
        const baseTimestamp = messageInfo.date instanceof Date ? messageInfo.date.getTime() : Number(messageInfo.date) || Date.now();
        const baseSeconds = Math.floor(baseTimestamp / 1000);

        // SHA-256 hash of message identity ensures unique tiebreaker without collision risk
        ...
        // Synchronize queue TTL with export key to prevent orphaned data
    ...
    static async fail(account, exportId, error) {
        ...
        // Check if export is resumable (has progress and queued messages remaining)
        ...
        // Export is resumable if it made progress and has remaining messages
        const isResumable = lastProcessedScore > 0 && messagesQueued > 0 && messagesExported < messagesQueued;

        if (isResumable) {
            // Preserve queue for resumable exports - don't delete queueKey
            await redis
                .multi()
                .hmset(exportKey, {
                    status: 'failed',
            ...
            logger.error({ msg: 'Export failed (resumable)', account, exportId, error, isResumable: true });
        } else {
            // Non-resumable - delete queue
            await redis
                .multi()
                .hmset(exportKey, {
                    status: 'failed',
            ...
            logger.error({ msg: 'Export failed', account, exportId, error, isResumable: false });
        }
    }
    ...
    static async resume(account, exportId) {
        ...
        // Validate export exists and is resumable
        const exportData = await redis.hgetall(exportKey);
        if (!exportData || !exportData.exportId) {
            const err = new Error('Export not found');
            err.code = 'ExportNotFound';
            err.statusCode = 404;
            throw err;
        }

        if (exportData.status !== 'failed') {
            const err = new Error(`Cannot resume export with status "${exportData.status}"`);
            err.code = 'InvalidExportStatus';
            err.statusCode = 400;
            throw err;
        }

        if (exportData.isResumable !== '1') {
            const err = new Error('Export is not resumable. The export must have made progress before failing.');
            err.code = 'ExportNotResumable';
            err.statusCode = 400;
            throw err;
        }

        // Check that queue still exists
        const queueSize = await redis.zcard(queueKey);
        if (queueSize === 0) {
            const err = new Error('Export queue no longer exists');
        ...
        // Extend TTL on export key and queue key
        const maxAge = await getExportMaxAge();
        const ttl = Math.ceil(maxAge / 1000);
        const now = Date.now();
        const newExpiresAt = now + maxAge;

        // Reset status and re-queue the job
        await redis
            .multi()
            .hmset(exportKey, {
                status: 'queued',
            ...
            .expire(queueKey, ttl)
            .sadd(activeKey, activeEntry)
            .exec();

        try {
            // Create BullMQ job with isResumed flag
    ...
    static async cleanup() {
        ...
        try {
            ...
            for (const file of files) {
                ...
                // Handle both encrypted (.ndjson.gz.enc) and unencrypted (.ndjson.gz) extensions
