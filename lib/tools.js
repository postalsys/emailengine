...
`
    ...
    notifyError(e, opts) {
        console.log(123, e);
        if (module.exports.readEnvValue('BUGSNAG_API_KEY')) {
            console.log(124);
            Bugsnag.notify(e, event => {
                console.log(125);
                event.severity = opts.severity || 'info';
                event.context = opts.context || 'ee';
                if (opts.account) {
                    event.setUser(opts.account);
                }
                if (opts.msg || opts.path) {
                    event.addMetadata('msg', { msg: opts.msg, path: opts.path });
                }
            });
        }
    }
