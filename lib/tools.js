...
-----END PUBLIC KEY-----`);
    ...
    async verifyAccountInfo(accountData) {
        ...
        if (accountData.smtp) {
            try {
                let smtpConfig = Object.assign({}, accountData.smtp);

                // set up proxy if needed
                if (proxyEnabled && proxyUrl && !smtpConfig.proxy) {
    ...
    async getStats(redis, call, seconds) {
        ...
        for (let queue of ['notify', 'submit']) {
            try {
                const [resActive, resDelayed, resWaiting] = await redis
                    .multi()
                    .llen(`bull:${queue}:active`)
                    .zcard(`bull:${queue}:delayed`)
                    .llen(`bull:${queue}:wait`)
                    .exec();
                if (resActive[0] || resDelayed[0] || resWaiting[0]) {
                    // counting failed
                ...
                queues[queue].total = queues[queue].active + queues[queue].delayed + queues[queue].waiting;
            } catch (err) {
