'use strict';

const libmime = require('libmime');
const addressparser = require('nodemailer/lib/addressparser');
const { toCamelCase } = require('./utils/to-camel-case');
const { decodeHeaders } = require('./utils/decode-headers');
const { isUnsafeTransformedKey } = require('./utils/unsafe-key');

// The attachment types a complaint is read from: the ARF report itself, and the copy of the
// original message beside it, which some feedback loops send as text/rfc822-headers. Both arrival
// paths gate and download on these lists, and the switch in arfDetect() parses every type in them
const FEEDBACK_REPORT_TYPE = 'message/feedback-report';
const ORIGINAL_MESSAGE_TYPES = ['message/rfc822', 'message/rfc822-headers', 'text/rfc822-headers', 'text/rfc822-header'];
const COMPLAINT_ATTACHMENT_TYPES = [FEEDBACK_REPORT_TYPE, ...ORIGINAL_MESSAGE_TYPES];

const ARF_SINGLE = [
    'feedback-type',
    'user-agent',
    'version',
    'original-envelope-id',
    'original-mail-from',
    'abuse-type',
    'arrival-date',
    'received-date',
    'reporting-mta',
    'source-ip',
    'source',
    'subscription-link',
    'incidents'
];

const arfDetect = async messageInfo => {
    const report = {
        arf: {},
        headers: {}
    };

    let returnPath;

    for (let attachment of messageInfo.attachments || []) {
        switch (attachment.contentType.toLowerCase()) {
            case 'message/feedback-report': {
                // found a feedback report

                const contents = decodeHeaders((attachment.content || '').toString());

                Object.keys(contents).forEach(key => {
                    let value = contents[key];

                    if (!key || !value) {
                        return;
                    }

                    if (ARF_SINGLE.includes(key)) {
                        value = Array.isArray(contents[key]) ? contents[key].join(',') : (contents[key] || '').toString();
                    } else if (!Array.isArray(value)) {
                        value = []
                            .concat(value || [])
                            .map(element => (element || '').toString().trim())
                            .filter(element => element);
                    }

                    if (!value || !value.length) {
                        return;
                    }

                    switch (key) {
                        case 'original-mail-from':
                            value = value.replace(/^[\s<]*|[>\s]*$/g, '');
                            break;

                        case 'arrival-date':
                        case 'received-date':
                            key = 'arrival-date';
                            value = new Date(value);
                            if (value.toString() === 'Invalid Date') {
                                return;
                            }
                            value = value.toISOString();
                            break;
                    }

                    report.arf[key] = value;
                });

                break;
            }

            case 'message/rfc822':
            case 'message/rfc822-headers':
            case 'text/rfc822-headers':
            case 'text/rfc822-header': {
                let contents = (attachment.content || '').toString();
                const headerPos = contents.match(/\r?\n\r?\n/);

                if (headerPos) {
                    contents = contents.substr(0, headerPos.index);
                }

                const headers = decodeHeaders(contents);

                // Hotmail original recipient X-HmXmrOriginalRecipient
                const addresses = addressparser([].concat(headers['x-hmxmroriginalrecipient'] || []).join(', '))
                    .map(addr => addr.address)
                    .filter(addr => addr && !(report.arf['original-rcpt-to'] && report.arf['original-rcpt-to'].includes(addr)));

                report.arf['original-rcpt-to'] = [].concat(report.arf['original-rcpt-to'] || []).concat(addresses);

                if (headers['return-path']) {
                    returnPath = addressparser([].concat(headers['return-path'] || []).join(', '))
                        .map(addr => addr.address)
                        .filter(addr => addr)
                        .pop();
                }

                let messageId = [].concat(headers['message-id'] || []).pop();
                if (messageId && !report.headers['message-id']) {
                    report.headers['message-id'] = messageId;
                }

                let sourceIp = [].concat(headers['x-sender-ip'] || []).pop();
                if (sourceIp && !report.arf['source-ip']) {
                    report.arf['source-ip'] = sourceIp;
                }

                let originalArrivalTime = [].concat(headers['x-ms-exchange-crosstenant-originalarrivaltime'] || []).pop();
                if (originalArrivalTime && !report.arf['arrival-date']) {
                    let originalArrivalTimeDate = new Date(originalArrivalTime);
                    if (originalArrivalTimeDate && originalArrivalTimeDate.toString() !== 'Invalid Date') {
                        report.arf['arrival-date'] = originalArrivalTimeDate.toISOString();
                    }
                }

                for (let headerKey of ['from', 'to', 'cc', 'bcc', 'subject', 'date']) {
                    let headerLines = []
                        .concat(headers[headerKey] || [])
                        .map(val => val && typeof val === 'string' && val.trim())
                        .filter(val => val);
                    if (!headerLines.length) {
                        continue;
                    }

                    switch (headerKey) {
                        case 'date':
                            {
                                let value = new Date(headerLines.pop());
                                if (value.toString() !== 'Invalid Date') {
                                    report.headers[headerKey] = report.headers[headerKey] || value.toISOString();
                                }
                            }
                            break;

                        case 'subject':
                            {
                                let value = libmime.decodeWords(headerLines.pop()).trim();
                                if (value) {
                                    report.headers[headerKey] = report.headers[headerKey] || value;
                                }
                            }
                            break;

                        case 'from':
                        case 'to':
                        case 'cc':
                        case 'bcc':
                            {
                                let value = headerLines
                                    .flatMap(addressparser)
                                    .map(entry => entry.address)
                                    .filter(addr => addr);

                                if (value && value.length) {
                                    if (headerKey === 'from') {
                                        value = value.pop();
                                    }
                                    report.headers[headerKey] = report.headers[headerKey] || value;
                                }
                            }
                            break;
                    }
                }

                break;
            }

            default:
                break;
        }
    }

    let reportDefaults = {
        arf: {},
        headers: {}
    };

    if (!report.source && messageInfo.from && messageInfo.from.address === 'staff@hotmail.com') {
        reportDefaults.arf.source = 'Hotmail';
        reportDefaults.arf['feedback-type'] = reportDefaults.arf['feedback-type'] || 'abuse';
        reportDefaults.arf['abuse-type'] = reportDefaults.arf['abuse-type'] || 'complaint';
        if (returnPath) {
            reportDefaults.arf['original-mail-from'] = returnPath;
        }
    }

    for (let key of Object.keys(reportDefaults)) {
        report[key] = Object.assign(reportDefaults[key], report[key]);
    }

    return report;
};

/**
 * Turns an ARF report into the camelCased shape published in a complaint notification
 *
 * @param {Object} report - Report from arfDetect()
 * @returns {Object} The arf and headers sections with camelCased keys
 */
function camelCaseComplaint(report) {
    let complaint = {};

    for (let subKey of ['arf', 'headers']) {
        for (let key of Object.keys(report[subKey])) {
            // decodeHeaders() dropped the prototype-shaped names at the parse boundary, but
            // toCamelCase() re-creates one out of a name that boundary had no reason to refuse:
            // "_-_proto__" camelCases into "__proto__" (a prototype swap) and "to-string" into
            // "toString" (an own property shadowing the method), hence the transformed-key test
            // covering every Object.prototype member. A feedback report is an attacker-supplied
            // attachment, so the name is theirs to choose.
            let displayKey = toCamelCase(key);
            if (isUnsafeTransformedKey(displayKey)) {
                continue;
            }

            if (!complaint[subKey]) {
                complaint[subKey] = {};
            }
            complaint[subKey][displayKey] = report[subKey][key];
        }
    }

    return complaint;
}

module.exports = { arfDetect, camelCaseComplaint, FEEDBACK_REPORT_TYPE, ORIGINAL_MESSAGE_TYPES, COMPLAINT_ATTACHMENT_TYPES };
