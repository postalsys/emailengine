...
function camelCaseComplaint(report) {
    ...
    for (let subKey of ['arf', 'headers']) {
        for (let key of Object.keys(report[subKey])) {
            ...
            // "_-_proto__" camelCases into "__proto__", and assigning that swaps the prototype of
            // the section being built rather than storing a field. A feedback report is an
            // attacker-supplied attachment, so the name is theirs to choose.
            let displayKey = toCamelCase(key);
            if (isUnsafeKey(displayKey)) {
                continue;
            }
