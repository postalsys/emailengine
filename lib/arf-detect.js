        ...
        switch (attachment.contentType.toLowerCase()) {
            case 'message/feedback-report': {
                ...
                Object.keys(contents).forEach(key => {
                    ...
                    // The field names come from the message. "__proto__" names no ARF field, while
                    // assigning it would swap this object's prototype instead of storing anything.
                    // The reportDefaults merge below already keeps that out of the returned report,
                    // so this only keeps the intermediate sane, as in bounce-detect.js
                    if (!key || key === '__proto__' || !value) {
                        return;
                    }
...
function camelCaseComplaint(report) {
    ...
    for (let subKey of ['arf', 'headers']) {
        for (let key of Object.keys(report[subKey])) {
            ...
            complaint[subKey][toCamelCase(key)] = report[subKey][key];
        }
    }
