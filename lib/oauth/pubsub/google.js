...
class PubSubInstance {
    ...
    async processPulledMessage(messageId, data) {
        ...
        console.log('PROCESSING PUSH MESSAGE', data);
    }
    ...
    async run() {
        ...
        /*
        let pushUrl = `https://pubsub.googleapis.com/v1/${this.appData.pubSubTopic}:publish`;
        let pubRes = await this.client.request(accessToken, pushUrl, 'POST', {
            messages: [{ data: Buffer.from(`Current time is ${new Date().toISOString()}`).toString('base64') }]
        });
        console.log('PUSH RES', require('util').inspect(pubRes, false, 22, true));
        */
