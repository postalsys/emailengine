...
class PubSubInstance {
    constructor(parent, opts) {
        ...
        this.checkSchemaVersions()
            .catch(err => {
                logger.error({ msg: 'Failed to process schema versions', app: this.app, err });
            })
            .finally(() => {
                if (!this.stopped) {
                    this.startLoop();
        ...
        this.run()
            ...
            .catch(err => {
                ...
                if (this._lastLoopError !== errKey) {
                    if (err.eeLogged) {
                        // already logged with full context where it happened; keep only a trace of the loop retry
                        logger.debug({ msg: 'Failed to process subscription loop', app: this.app, err });
                    } else {
        ...
        for (let subscriberApp of subscriberApps) {
            ...
            if (schemaVersion < currentSchemaId) {
                ...
                try {
                    ...
                    logger.info({ msg: 'Normalized hash keys', app: subscriberApp, schemaVersion, currentSchemaId, normalizationResults });
                } catch (err) {
                    logger.error({ msg: 'Failed to normalize hash keys', app: subscriberApp, schemaVersion, currentSchemaId, err });
                }
            }
        }
    }
    ...
    async processPulledMessage(messageId, data) {
        logger.debug({ msg: 'Processing subscription message', app: this.app, messageId, data });
        ...
        try {
            ...
            logger.error({
                ...
                dataPreview: (data || '').toString().slice(0, 256),
                err
            });
            return;
        }
        ...
        if (!accountIds.size) {
            logger.debug({ msg: 'No account matched for Pub/Sub notification', app: this.app, subscriberApps, messageId, emailAddress: payload.emailAddress });
            return;
        }
    ...
    async run() {
        ...
        if (!accessToken) {
            // not logged here: the thrown error is logged by the startLoop catch
