    ...
    async run() {
        ...
        try {
            ...
            if (err.statusCode === 404 || err?.oauthRequest?.response?.error?.code === 404) {
                let now = Date.now();
                let backoffMs = Math.min(3000 * Math.pow(2, this.recoveryAttempts), 5 * 60 * 1000);
                if (now - this.lastRecoveryAttempt < backoffMs) {
                    logger.warn({ msg: 'Skipping subscription recovery (backoff)', app: this.app, recoveryAttempts: this.recoveryAttempts, backoffMs });
                    throw err;
                }
                ...
                logger.warn({ msg: 'Subscription not found (404), attempting recovery', app: this.app, recoveryAttempts: this.recoveryAttempts });

                try {
                    await this.getApp(true);
                    await oauth2Apps.ensurePubsub(this.appData);
                    // Refresh appData and client after recreation (subscription name may have changed)
                    await this.getClient(true);
                    await oauth2Apps.setMeta(this.app, { pubSubFlag: null });
                    this.recoveryAttempts = 0;
                    logger.info({ msg: 'Successfully recovered Pub/Sub subscription', app: this.app });
                    return; // re-enter the pull loop
                } catch (recoveryErr) {
                    logger.error({ msg: 'Failed to recover Pub/Sub subscription', app: this.app, err: recoveryErr });
                    throw err; // throw the original 404 error to trigger startLoop retry
                }
            }
