...
const { REDIS_PREFIX, TRANSIENT_NETWORK_CODES, GMAIL_PUBSUB_MAX_RECOVERY_ATTEMPTS } = require('../../consts');
...
class PubSubInstance {
    ...
    startLoop() {
        ...
        this.run()
            ...
            .catch(err => {
                ...
                // AbortSignal.timeout safety net - not an error, just restart the loop
    ...
    async run() {
        ...
        try {
            ...
            let signal = AbortSignal.any([this._abortController.signal, AbortSignal.timeout(5 * 60 * 1000)]);
            let pullRes;
            try {
                pullRes = await this.client.request(accessToken, pullUrl, 'POST', { returnImmediately: false, maxMessages: 100 }, { signal });
            } finally {
                this._abortController = null;
            }
            if (this.stopped) {
            ...
            if (ackIds.length > 0) {
                ...
                try {
                    ...
                    logger.error({ msg: 'Failed to batch ack subscription messages', app: this.app, count: ackIds.length, err });
                }
            }
            ...
            // AbortSignal.timeout safety net fired - not an error, just restart the loop
            ...
            // Authentication or authorization failure -- set operator-visible flag and attempt recovery
            if (err.statusCode === 401 || err.statusCode === 403) {
                ...
                await this._setPubSubFlag({
                    message:
                        err.statusCode === 401 ? 'Service account access token expired or revoked' : 'Insufficient permissions to pull from the subscription',
                    description: [err.message, err.reason, err.code].filter(val => val).join('; ')
                });
                await this.attemptRecovery(err.statusCode === 401 ? 'Access token rejected (401)' : 'Permission denied (403)');
                return;
            }
    ...
    async attemptRecovery(reason) {
        let now = Date.now();
        let backoffMs = this._backoffMs(this.recoveryAttempts);
        ...
        if (this.recoveryAttempts > GMAIL_PUBSUB_MAX_RECOVERY_ATTEMPTS) {
            logger.error({
                msg: 'Recovery circuit breaker tripped, stopping recovery attempts',
                app: this.app,
                reason,
                recoveryAttempts: this.recoveryAttempts
            });
            await this._setPubSubFlag({
                message: 'Pub/Sub recovery failed repeatedly. Manual intervention required. Re-save the OAuth2 app to reset.',
                description: `${this.recoveryAttempts} recovery attempts exhausted for: ${reason}`
            });
            this.stopped = true;
            this.parent.remove(this.app);
            return;
        ...
        try {
            ...
            let nextBackoffMs = this._backoffMs(this.recoveryAttempts);
