...
class ExternalAccountSigner {
    constructor(opts) {
        ...
        let { targetServiceAccountEmail } = validateConfig(opts.config, { allowedTokenHosts: opts.allowedTokenHosts });
    ...
    async _readSubjectToken() {
        ...
        if (source.file) {
            ...
            try {
                ...
                // Report the errno rather than the raw message: the path came from the operator so
                // echoing it back is useful, but the full fs message adds nothing and this string
                // is surfaced in the verify report.
                throw makeError(`Failed to read subject token file ${source.file}${err.code ? ` (${err.code})` : ''}`, 'ESubjectTokenRead', null, {
                    cause: err
                });
            }
