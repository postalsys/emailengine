...
// Individual Gmail API scopes for detection
...
const formatFetchBody = (searchParams, logRaw) => {
    ...
    for (let key of ['refresh_token', 'client_secret', 'code']) {
        if (data[key]) {
...
const formatFetchResponse = (responseObj, logRaw) => {
    ...
    for (let key of ['access_token', 'refresh_token', 'id_token']) {
        if (data[key]) {
...
class GmailOauth {
    constructor(opts) {
        ...
        let requestedScopes = [].concat(opts.scopes || defaultBaseScopes);

        // Only include OpenID Connect scopes for regular OAuth (not service accounts)
    ...
    getTokenRequest(opts) {
        ...
        // Note: scope parameter should NOT be included in token exchange requests
        // The scopes are determined by what was approved during authorization
        // and are returned in the token response
    ...
    async getToken(code) {
        ...
        try {
            ...
            if (this.logger) {
                this.logger.error({ msg: 'Failed to parse JSON response', url: requestUrl, status: res.status, err });
            }
        }
        ...
        if (!res.ok) {
            ...
            err.tokenRequest = {
                ...
                scopes: this.scopes,
                code: formatPartialSecretKey(code)
            };
            try {
    ...
    async refreshToken(opts) {
        ...
        if (this.serviceClient) {
            ...
            url.searchParams.set('client_id', this.clientId);
            // Note: scope parameter should NOT be included in refresh token requests
    ...
    async request(accessToken, url, method, payload, options) {
        ...
        if (!res.ok) {
            ...
            try {
                ...
                if (this.logger) {
                    this.logger.warn(Object.assign({ msg: 'API request failed' }, err.oauthRequest));
                }
            }
    ...
    async revokeToken(token) {
        ...
        try {
            ...
            if (this.logger) {
                this.logger.warn({ msg: 'OAuth2 token revocation error', err });
            }
        }
    }
}
