...
async function collectBeacon({ redis, logger }) {
    ...
    const safe = async fn => {
        try {
            ...
            if (logger) {
                logger.debug({ msg: 'Beacon field collection failed', err });
            }
            return undefined;
        }
    };
    ...
    try {
        ...
        if (logger) {
            logger.warn({ msg: 'Beacon collection failed', err });
        }
        return null;
    }
}
...
async function attachBeacon(body, { redis, logger, now }) {
    try {
        ...
        if (logger) {
            logger.warn({ msg: 'License beacon collection failed', err });
        }
    }
}
...
async function persistBeaconMarkers({ redis, logger, body, now, needFull }) {
    try {
        ...
        if (logger) {
            logger.warn({ msg: 'Failed to persist license beacon markers', err });
        }
    }
}
