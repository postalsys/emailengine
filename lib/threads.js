        ...
        if (missingRefs.length) {
            ...
            try {
                ...
                if (err.meta && err.meta.body && err.meta.body.status === 404) {
                    // thread entry not found, add as new

                    let threadCreateQuery = {
                        index: `${index}.threads`,
                        id: threadEntry._id,
                        document: Object.assign(threadEntry._source, {
                            references: threadEntry._source.references.concat(missingRefs)
                        })
                    };

                    try {
                        let createResult = await client.create(threadCreateQuery);
                        logger.error({
                            msg: 'Re-create thread entry',
                            thread: threadCreateQuery,
                            createResult
                        });
                    } catch (err) {
                        logger.error({
                            msg: 'Failed to re-create thread entry',
                            thread: threadCreateQuery,
                            err
                        });
                    }
