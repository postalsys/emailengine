...
async function processMessage(data) {
    let raw = Buffer.from(data.raw, 'base64');
    let hasBcc = false;
    ...
    return { raw: message, hasBcc, messageId, envelope };
}
...
async function getRawEmail(data) {
    ...
    return { raw: email, hasBcc: !!bcc.length, messageId, envelope: compiled.getEnvelope() };
}
