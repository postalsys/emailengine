...
const { parseEmail, contentToBase64 } = require('./parse-helpers');
...
async function getRawEmail(data, licenseInfo, options) {
    ...
    if (data.raw) {
        ...
        if (options.returnObject) {
            ...
            // postal-mime doesn't do HTML/text transformations by default, so no skip options needed
            const parsed = await parseEmail(response.raw, {
                maxHeadersSize: 2 * 1024 * 1024
            });

            // postal-mime already returns simplified address format:
            // - parsed.from is already { address, name } (not wrapped in .value)
            // - parsed.to/cc/bcc are already arrays of { address, name }
            // No conversion needed

            delete parsed.headers; // conflicts

            for (let attachment of parsed.attachments || []) {
                attachment.content = contentToBase64(attachment.content);
                attachment.encoding = 'base64';
                if (attachment.contentId) {
                    attachment.cid = attachment.contentId;
                }
                // postal-mime uses 'disposition' instead of 'contentDisposition'
                attachment.isInline = attachment.disposition === 'inline';
            }
