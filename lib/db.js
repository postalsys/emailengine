...
// duplicate function declaration to avoid requiring tools.js
...
const REDIS_CONF = Object.assign(
    {
        // some defaults
        maxRetriesPerRequest: 20, // Fail after 20 retries to detect issues faster
...
module.exports.setReconnectionHandler = handler => {
...
redis.on('ready', () => {
    if (redisWasDisconnected) {
        logger.info({ msg: 'Redis connection recovered, triggering reconnection handler', isMainThread, threadId });
        redisWasDisconnected = false;
        
        if (reconnectionHandler && typeof reconnectionHandler === 'function') {
            // Delay slightly to ensure Redis is fully ready
            setTimeout(() => {
                reconnectionHandler();
            }, 1000);
        }
...
redis.on('end', () => {
    if (redisConnected) {
        logger.warn({ msg: 'Redis connection lost', isMainThread, threadId });
