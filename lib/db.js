...
const showRedisError = (msg, err) => {
    if (!redisConnected && process.stdout.isTTY && isMainThread && process.env.EENGINE_SILENT_INSTALL !== 'true') {
...
redis.on('error', err => {
    if (/NOAUTH/.test(err.message)) {
        if (REDIS_CONF.password) {
            return showRedisError('Redis requires a valid password', err);
        } else {
            return showRedisError('Redis password is required but not provided', err);
        }
    }

    if (/WRONGPASS/.test(err.message)) {
        return showRedisError('Provided Redis password was not accepted', err);
    }

    switch (err.code) {
        case 'ECONNREFUSED':
            return showRedisError('Can not connect to the database. Redis might not be running. Are you using correct hostname and port values?', err);
        case 'ETIMEDOUT':
            return showRedisError('Connection to the database timed out. Seems like you are firewalled. Are you using correct hostname and port values?', err);
        default:
