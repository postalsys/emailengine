...
class WebhooksHandler {
    ...
    async get(id) {
        ...
        tcount = Number(getResult[4] && getResult[2].toString()) || 0;
    ...
    async pushToQueue(event, originalPayload, opts = {}) {
        ...
        for (let route of webhookRoutes) {
            if (route.enabled && route.targetUrl && typeof route.filterFn === 'function') {
                ...
                if (canSend && payload) {
                    logger.error({ msg: 'Triggered custom webhook route', webhook: route.id });
                    ...
                    try {
                        await this.redis.hget(this.getWebhooksContentKey(), `${route.id}:tcount`);
                    } catch (err) {
                        // ignore
                    }
                }
