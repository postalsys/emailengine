...
class WebhooksHandler {
    ...
    async storeLog(id, type, payload, error) {
        ...
        try {
            await redis.multi().rpush(this.getWebhooksLogKey(id), logRow).ltrim(this.getWebhooksLogKey(id), -maxLogLines, -1).exec();
        } catch (err) {
    ...
    async getHandler(id) {
        ...
        try {
            if (webhookData.content.fn) {
                webhookData.compiledFn = new vm.Script(`result = (()=>{
${webhookData.content.fn}
})(payload);`);
            } else {
        ...
        if (webhookData.compiledFn) {
            try {
                if (webhookData.content.map) {
                    webhookData.compiledMap = new vm.Script(`result = (()=>{
${webhookData.content.map}
})(payload);`);
                } else {
        ...
        if (webhookData.compiledFn) {
            webhookData.filterFn = payload => {
                ...
                try {
                    ...
                    this.storeLog(id, 'filter', payload, err.stack).catch(() => false);
        ...
        if (webhookData.compiledMap) {
            webhookData.mapFn = payload => {
                ...
                try {
                    ...
                    this.storeLog(id, 'map', payload, err.stack).catch(() => false);
    ...
    async formatPayload(event, originalPayload) {
        ...
        if (event === 'messageNew' && payload && payload.data && payload.data.attachments) {
