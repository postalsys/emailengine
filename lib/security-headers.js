...
const MACHINE_TAGS = Object.freeze(['api', 'external', 'static']);

// A preset's `nonceDirectives` get the request's nonce appended after any route override, so
// an override of one of them keeps the nonce and a missing nonce fails closed
const CSP_PRESETS = Object.freeze({
    admin: Object.freeze({
        nonceDirectives: Object.freeze(['script-src', 'style-src-elem']),
        directives: Object.freeze({
            'default-src': "'self'",
            'script-src': "'self'",
            // Stylesheets need the nonce like scripts: nothing first-party appends a <style>
            // element (ACE runs with useStrictCSP and links its sheets, the message browser
            // widget is handed the nonce). Style attributes stay allowed: sanitised email HTML
            // arrives with every rule inlined into them, and a style attribute cannot load or
            // run anything the rest of the policy does not already permit. `style-src` is the
            // fallback for a browser that predates the -elem/-attr split.
...
// What an admin page has to declare when it carries a form whose POST is answered with a redirect
// to an OAuth provider's authorization page. Chrome checks a form submission's redirect target
// against form-action, so `'self'` blocks the submission even though the form posts to this very
// instance. The hosted setup form gets the same relaxation from the public preset - an admin page
// renders the same form and has to ask for it, which is what the account page's "Re-authenticate"
// button needs.
//
// `https:` rather than the five authorization origins the providers actually use today
// (accounts.google.com, oauth.mail.ru and the three Entra clouds in lib/oauth/outlook.js). Listing
// them would be accurate right now and would silently reintroduce this very bug the day a provider
// or a Microsoft cloud is added - as a CSP refusal in the browser console, which is how it was
// found the first time. It is also the value every public page already carries.
const OAUTH_REDIRECT_FORM_DIRECTIVES = Object.freeze({ 'form-action': "'self' https:" });
...
function profileNameFor(path, tags = []) {
    ...
    if (path.startsWith('/v1/') || isMachineRoute(tags)) {
        return 'api';
    }
    return 'public';
}
...
function policyFor({ path, tags = [], layout, override, nonce, secureOrigin = false, cspMode = 'enforce' }) {
    ...
    if (profile.csp) {
        ...
        if (nonce) {
            for (const directive of preset.nonceDirectives || []) {
