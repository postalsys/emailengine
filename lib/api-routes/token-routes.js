...
async function init(args) {
    ...
    server.route({
        ...
        async handler(request) {
            try {
                // By id only. getRawData() would also accept the plaintext value, but a value in a
                // URL path lands in the access log and in any reverse proxy in front of it - and
                // there is no need for it here, since POST /v1/tokens returns the id.
                const tokenData = await tokens.get(request.params.token, true, { allowExpired: true }).catch(() => null);
                if (!tokenData) {
    ...
    server.route({
        ...
        async handler(request) {
            try {
                ...
                // By id, for the same reason as the getter above
                const tokenData = await tokens.get(request.params.token, true, { allowExpired: true }).catch(() => null);
                if (!tokenData) {
                    throwNotFound('Requested access token was not found');
                }
