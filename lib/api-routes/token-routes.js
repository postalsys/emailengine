...
async function init(args) {
    ...
    const createTokenPayloadSchema = Joi.object({
        ...
        scopes: Joi.array()
            .items(
                Joi.string()
                    .valid('api', 'smtp', 'imap-proxy', 'mcp', 'mcp-manage')
                    .meta({ enumDescriptions: ENUM_DESCRIPTIONS.tokenScope })
                    .label('TokenScope')
            )
            .single()
            .default(['api'])
            .required()
            .description(
                'Token permission scopes: "api" for REST API access, "smtp" for SMTP submission, "imap-proxy" for IMAP proxy authentication, "mcp-manage" for the instance management tools of the MCP endpoint, "mcp" for its mail tools'
            )
            .label('Scopes'),
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    ...
                    description:
                        'Revoke an access token by its id. Every client using it is cut off immediately and there is no undo. Confirm with the user before calling this.'
                },
                openapi: {
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    ...
                    description:
                        'List the access tokens on this instance, paged: description, scopes, restrictions, the account each is bound to and when it was last used. Token values are never shown. Filter with query, or with account for the tokens bound to one account.',
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    ...
                    description:
                        'Get one access token by its id (from list_tokens): scopes, permissions, restrictions and last use. Never the token value itself.'
                },
                openapi: {
    ...
    server.route({
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    name: 'get_token_log',
                    title: 'Get access token audit log',
                    description:
                        'Read the audit log of one access token, paged: which requests it made, against which account, from which address, and which were refused and why. Only kept while the tokenAuditLog setting is on.',
                    bounds: { pageSize: MCP_MAX_PAGE_SIZE }
                },
                openapi: {
