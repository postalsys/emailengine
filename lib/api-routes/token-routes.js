...
async function init(args) {
    ...
    const createTokenPayloadSchema = Joi.object({
        ...
        scopes: Joi.array()
            .items(Joi.string().valid('api', 'smtp', 'imap-proxy', 'mcp').meta({ enumDescriptions: ENUM_DESCRIPTIONS.tokenScope }).label('TokenScope'))
            .single()
            .default(['api'])
            .required()
            .description(
                'Token permission scopes: "api" for REST API access, "smtp" for SMTP submission, "imap-proxy" for IMAP proxy authentication, "mcp" for the MCP endpoint only'
            )
            .label('Scopes'),
