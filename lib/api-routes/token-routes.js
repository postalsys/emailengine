...
async function init(args) {
    ...
    server.route({
        ...
        async handler(request) {
            ...
            try {
                // Nothing invalidates access tokens when the first admin password is set, so a token
                // minted while the instance is unprotected outlives that window silently.
                //
                // This route normally requires a real credential (`auth: {strategy: 'api-token',
                // mode: 'required'}`), and a caller presenting one is trusted - minting tokens with
                // no admin password set is a supported headless configuration. But the
                // `disableTokens` setting rewrites a credential-less request to `access_token=preauth`
                // and the strategy accepts it, so that specific combination has to be refused here.
                if (request.auth.credentials?.preauth && !(await settings.isInstanceSecured())) {
