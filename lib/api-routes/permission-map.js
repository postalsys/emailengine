...
const PER_REQUEST_SURFACES = new Set(['mcp', 'mcp-manage']);

function grantListAdmits(grants, grant) {
    return (grants || []).some(entry => entry.action === grant.action && entry.group === grant.group);
}
...
function surfaceAdmits(scope, grant) {
    return grantListAdmits(SURFACE_GRANTS[scope], grant);
}
...
function sessionTokenAdmits(grant, opts) {
    ...
    return grantListAdmits(SESSION_TOKEN_GRANTS, grant);
}
