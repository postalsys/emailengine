...
const GROUP = {
    ...
    // that would make the holder more than a settings editor are refused to a narrowed token by
    // assertNoPrivilegedSettings() in ./route-helpers.js, keyed on PRIVILEGED_SETTINGS_KEYS in
    // lib/privileged-settings.js, which also renders the phrase every description of the rule uses.
    SETTINGS: 'settings',
...
// What a two-axis record that omits `groups` reaches: the sections that were grantable when the
// two-axis form shipped in 2.79.0, and nothing added since. The record's schema promised "omit to
// allow all of them", and every record written under that promise was written when these were all
// there was. Reading an absent axis as the live GRANTABLE_GROUPS would have handed every such
// record the five instance sections the admin split created - a `{ actions: ['read', 'write'] }`
// token minted through the API on 2.79 would have woken up on 2.80 able to retarget an account's
// host and move the webhook target. So the instance sections are reachable only by a record that
// names them, and a section added later joins GRANTABLE_GROUPS and not this list.
// test/token-permissions-test.js pins it to these thirteen names.
const IMPLICIT_GROUPS = Object.freeze([
    GROUP.ACCOUNT,
    GROUP.MAILBOX,
    GROUP.MESSAGE,
    GROUP.SUBMIT,
...
const GROUP_ROUTES = {
    ...
    // already existed: a record cannot name a group that did not exist when it was written, and a
    // record that names no groups at all reaches IMPLICIT_GROUPS, which stops at the split, so no
    ...
    [GROUP.PROVISIONING]: [
        ...
        // Offers supplied credentials to the autodiscovery host the address domain's own DNS names,
        // which is why it sits here and not with the credential-free `GET /v1/autoconfig` under
        // diagnostics. It stores nothing, so it is still a read.
        'POST /v1/autoconfig'
    ],
