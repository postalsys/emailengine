...
const { accountIdSchema, blocklistListIdSchema, apiResponses } = require('../schemas');
...
async function init(args) {
    ...
    server.route({
        ...
        options: {
            ...
            notes: 'List suppression lists (blocklists) with the number of blocked addresses on each',
            tags: ['api', 'Blocklists'],

            plugins: {
                mcp: {
                    name: 'list_blocklists',
                    title: 'List suppression lists',
                    description: 'List the suppression lists (blocklists) and how many addresses each holds, paged.',
                    bounds: { pageSize: MCP_MAX_PAGE_SIZE }
                },
                openapi: {
                    responses: apiResponses('Returns every blocklist with its entry count.', 400, 401, 403, 429, 500)
                }
            },
    ...
    server.route({
        ...
        options: {
            ...
            notes: 'List suppressed addresses for a list',
            tags: ['api', 'Blocklists'],
            ...
            plugins: {
                mcp: {
                    ...
                    description: 'List the addresses on one suppression list, paged, with the reason and source of each entry.',
                    bounds: { pageSize: MCP_MAX_PAGE_SIZE }
                },
                openapi: {
                    responses: apiResponses('Returns a page of entries on the blocklist.', 400, 401, 403, 404, 429, 500)
                }
            },
    ...
    server.route({
        ...
        async handler(request) {
            ...
            try {
                ...
                let added = await lists.add(request.params.listId, request.payload.recipient, {
                    account: request.payload.account,
                    source: 'api',
        ...
        options: {
            ...
            notes: 'Add an email address to a suppression list. Sending with this listId skips the address',
            tags: ['api', 'Blocklists'],
            ...
            plugins: {
                mcp: {
                    ...
                    description:
                        'Add a recipient address to a suppression list, so messages sent with that list id are not delivered to it. Needs the account the block was raised for.'
                },
                openapi: {
                    responses: apiResponses('Returns whether the address was added or was already listed.', 400, 401, 403, 404, 429, 500)
                }
            },
            ...
            validate: {
                ...
                payload: Joi.object({
                    ...
                    reason: Joi.string().empty('').default('block').example('unsubscribe').description('Identifier for the blocking reason')
    ...
    server.route({
        ...
        async handler(request) {
            try {
                let exists = await lists.exists(request.params.listId);
                if (!exists) {
                ...
                let deleted = await lists.remove(request.params.listId, request.query.recipient);
        ...
        options: {
            ...
            notes: 'Remove a suppressed email address from a list',
            tags: ['api', 'Blocklists'],
            ...
            plugins: {
                mcp: {
                    ...
                    description: 'Remove a recipient address from a suppression list, so messages to it are delivered again.'
                },
                openapi: {
