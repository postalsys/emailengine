...
const { handleError, throwNotFound, flattenOAuthAppMeta, MASKED } = require('./route-helpers');
const { ENCRYPTED_APP_KEYS, MCP_MAX_PAGE_SIZE } = require('../consts');
...
const oauth2AppExtraFields = {
    ...
    skipScopes: Joi.array()
        ...
        .label('AppSkipScopes'),
    baseScopes: oauthCreateSchema.baseScopes
        .description('OAuth2 base scopes for this app')
        .meta({ enumDescriptions: ENUM_DESCRIPTIONS.appBaseScopes })
        .label('AppBaseScopes'),
...
async function init(args) {
    ...
    server.route({
        ...
        async handler(request) {
            try {
                ...
                for (let app of response.apps) {
                    for (let secretKey of ENCRYPTED_APP_KEYS) {
                        if (app[secretKey]) {
                            app[secretKey] = MASKED;
                        }
                    }
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    name: 'list_oauth2_apps',
                    title: 'List OAuth2 applications',
                    description:
                        'List the OAuth2 applications (Gmail, Outlook and other providers) that accounts can sign in through, paged. Client secrets are masked.',
                    bounds: { pageSize: MCP_MAX_PAGE_SIZE }
                },
                openapi: {
                    responses: apiResponses('Returns a page of OAuth2 applications.', 400, 401, 403, 429, 500)
                }
            },
    ...
    server.route({
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    name: 'get_oauth2_app',
                    title: 'Get OAuth2 application',
                    description: 'Get one OAuth2 application: its provider, client id, scopes, redirect URL and Pub/Sub settings. Client secrets are masked.'
                },
                openapi: {
                    responses: apiResponses('Returns the OAuth2 application. Client secrets are masked.', 400, 401, 403, 404, 429, 500)
                }
            },
            ...
            response: {
                schema: Joi.object({
                    ...
                    accounts: Joi.number().integer().example(12).description('The number of accounts registered with this application'),
    ...
    server.route({
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    name: 'create_oauth2_app',
                    title: 'Create OAuth2 application',
                    description:
                        'Register an OAuth2 application that accounts can sign in through. Needs the provider and the client credentials created at that provider; a client secret or service key can be written but never read back. Confirm the redirect URL with the user: it is where the provider sends authorization codes and has to match what the provider has on file.'
                },
                openapi: {
                    responses: apiResponses('Returns the registered OAuth2 application.', 400, 401, 403, 429, 500)
                }
            },
    ...
    server.route({
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    name: 'update_oauth2_app',
                    title: 'Update OAuth2 application',
                    description:
                        'Update an OAuth2 application. Send only the fields to change; a client secret can be replaced but not read back. Changing the redirect URL moves where the provider sends authorization codes, so confirm that with the user.'
                },
                openapi: {
                    responses: apiResponses('Returns the updated OAuth2 application.', 400, 401, 403, 404, 429, 500)
                }
            },
    ...
    server.route({
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    name: 'delete_oauth2_app',
                    title: 'Delete OAuth2 application',
                    description:
                        'Remove an OAuth2 application. Accounts that signed in through it can no longer refresh their access tokens and stop syncing. Confirm with the user before calling this.'
                },
                openapi: {
                    'x-ee-impact': IMPACT.DESTRUCTIVE,
                    responses: apiResponses('Returns a deletion marker.', 400, 401, 403, 404, 429, 500)
                }
            },
    ...
    server.route({
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    name: 'verify_oauth2_app',
                    title: 'Verify OAuth2 application',
                    description:
                        'Check an OAuth2 application against its provider: the credentials, the Pub/Sub setup where it has one, and optionally the connection of one account that uses it.',
                    openWorld: true
                },
                openapi: {
                    'x-ee-impact': IMPACT.READONLY,
                    responses: apiResponses('Returns the outcome of the credential check against the provider.', 400, 401, 403, 404, 429, 500)
                }
            },
