...
const gatewayLastErrorSchema = Joi.object({
    ...
    networkRouting: Joi.object()
        ...
        .label('GatewayErrorNetworkRouting')
})
    .unknown()
    ...
    server.route({
        ...
        async handler(request) {
            try {
                ...
                const listing = await gatewayObject.listGateways(request.query.page, request.query.pageSize);

                for (const entry of listing.gateways || []) {
                    maskSecrets(entry);
                }
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    name: 'list_gateways',
                    title: 'List SMTP gateways',
                    description:
                        "List the SMTP gateways (relay servers messages can be sent through instead of the account's own SMTP server), paged. Passwords are masked.",
                    bounds: { pageSize: MCP_MAX_PAGE_SIZE }
                },
                openapi: {
                    responses: apiResponses('Returns a page of configured SMTP gateways.', 400, 401, 403, 429, 500)
                }
            },
    ...
    server.route({
        ...
        async handler(request) {
            ...
            try {
                ...
                // remove secrets
                if (gatewayData.pass) {
                    gatewayData.pass = MASKED;
                }

                // A failed delivery records the proxy it used under lastError.networkRouting, which
                // is projected below - the password beside it was already masked
                maskSecrets(gatewayData);
        ...
        options: {
            ...
            notes: `Returns stored information about the gateway. ${MASK_NOTE}`,
            tags: ['api', 'SMTP Gateway'],

            plugins: {
                mcp: {
                    name: 'get_gateway',
                    title: 'Get SMTP gateway',
                    description: 'Get one SMTP gateway: host, port, TLS mode, username and the last delivery error. The password is masked.'
                },
                openapi: {
                    responses: apiResponses('Returns the gateway configuration. The password is masked.', 400, 401, 403, 404, 429, 500)
                }
            },
    ...
    server.route({
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    name: 'create_gateway',
                    title: 'Create SMTP gateway',
                    description:
                        'Register an SMTP gateway, a relay server messages can be sent through. Needs an id, a name, the host and the port; a password can be written but never read back.'
                },
                openapi: {
                    responses: apiResponses('Returns the registered gateway.', 400, 401, 403, 429, 500)
                }
            },
            ...
            validate: {
                ...
                payload: Joi.object({
                    ...
                    user: Joi.string()
                        ...
                        .description('SMTP authentication username')
                        .label('UserName'),
                    pass: Joi.string().empty('').max(1024).default(null).example('verysecret').description('SMTP authentication password').label('Password'),
            ...
            response: {
                schema: Joi.object({
                    ...
                    state: Joi.string()
                        ...
                        .valid('existing', 'new')
                        .example('new')
                        .description('Whether the gateway was created or updated')
    ...
    server.route({
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    name: 'update_gateway',
                    title: 'Update SMTP gateway',
                    description:
                        'Update an SMTP gateway. Send only the fields to change. Changing the host makes the next delivery authenticate to the new host with the stored password, so confirm host changes with the user.'
                },
                openapi: {
                    responses: apiResponses('Returns the updated gateway.', 400, 401, 403, 404, 429, 500)
                }
            },
            ...
            validate: {
                ...
                payload: Joi.object({
                    ...
                    user: Joi.string()
                        ...
                        .description('SMTP authentication username')
                        .label('UserName'),
                    pass: Joi.string().empty('').max(1024).allow(null).example('verysecret').description('SMTP authentication password').label('Password'),
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    ...
                    description: 'Remove an SMTP gateway. Messages queued through it fail to deliver. Confirm with the user before calling this.'
                },
                openapi: {
