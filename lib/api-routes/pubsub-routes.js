...
const { pubSubErrorSchema, apiResponses } = require('../schemas');
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    ...
                    title: 'Get Gmail Pub/Sub status',
                    description:
                        'List the Gmail Pub/Sub push subscriptions of the OAuth2 applications and their state, paged. A Gmail account falls back to polling while its subscription is missing.',
                    bounds: { pageSize: MCP_MAX_PAGE_SIZE }
                },
                openapi: {
                    responses: apiResponses('Returns the Pub/Sub subscription state for each configured Gmail application.', 400, 401, 403, 429, 500)
                }
            },
