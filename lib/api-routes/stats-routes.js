    ...
    server.route({
        ...
        async handler(request) {
            try {
                ...
                // The metrics come from a main-thread RPC, so a timeout there carries its own
                // status code, which the shared handler keeps instead of a generic 500
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    ...
                    description:
                        'Get instance-wide statistics: the running version, the license, how many accounts are connected, disconnected or failing, and message and webhook counters for the last `seconds` (default one day). The first call to make when asked how the instance is doing.'
                },
                openapi: {
                    responses: apiResponses('Returns counters, connection states and process information for this instance.', 400, 401, 403, 429, 500)
                }
            },
                    ...
                    redisWarnings: Joi.array()
                        .items(
                            Joi.object({
                                ...
                                details: Joi.array().items(Joi.string().example('maxmemory-policy is allkeys-lru')).description('Warning details')
                            })
                                .unknown()
                        ...
                        .label('RedisWarnings'),
                    redisPing: Joi.number().example(1.2).description('Redis latency in milliseconds'),
