...
// The body budget of a single get_message call. The tool returns the message body inline so that
// reading one message is one call rather than two, and this is what keeps that from being an
// unbounded amount of work: the whole body is fetched over IMAP and run through the sanitizer on
// the same worker thread that syncs every other account on the instance. A message longer than
// this comes back with text.hasMore set, and get_message_text fetches the rest under the caller's
// own maxBytes.
const MCP_MESSAGE_BODY_MAX_BYTES = 64 * 1024;
            ...
            plugins: {
                mcp: {
                    ...
                    description:
                        ...
                        `It is capped at ${MCP_MESSAGE_BODY_MAX_BYTES} bytes - text.hasMore true means the rest is available from get_message_text. ` +
                    ...
                    // that exists for every message. Attached images stay out of it: inlining
                    // them as data URIs would multiply the size of a tool result that already
                    // carries the whole body.
                    omit: ['textType', 'webSafeHtml', 'preProcessHtml', 'embedAttachedImages', 'maxBytes', 'documentStore'],
                    force: { textType: '*', preProcessHtml: true, maxBytes: MCP_MESSAGE_BODY_MAX_BYTES }
                },
                openapi: {
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    ...
                    title: 'Create draft',
                    description:
                        'Store a new message in a folder, typically a draft in the Drafts folder. Takes structured fields (from, to, subject, text, html). Nothing is sent - use send_message for that.'
                },
                openapi: {
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    ...
                    omit: ['textType', 'webSafeHtml', 'documentStore'],
                    force: { webSafeHtml: true }
                },
                openapi: {
