...
const { oauthOrigin, mcpResource, registerClient, redeemAuthorizationCode, MAX_REDIRECT_URIS, MCP_OAUTH_SCOPES } = require('../mcp/oauth');
...
async function assertRateLimit(request, bucket, allowed) {
    ...
    if (!limit.success) {
        const ttl = Math.ceil(limit.ttl);
        const err = Boom.tooManyRequests('Rate limit exceeded');
        err.output.payload.ttl = ttl;
        // The conventional header for a 429, so a generic HTTP client backs off without
        // knowing about the ttl field in the body
        err.output.headers['Retry-After'] = ttl;
        throw err;
    }
}
...
async function init(args) {
    ...
    server.ext('onPreResponse', async (request, h) => {
        ...
        if (origin) {
            ...
            // so writing any other key would be dropped in the rebuild. The strategy's challenge
            // params (error="...") are kept - a client debugging a bad token needs both.
            const pointer = `resource_metadata="${origin}/.well-known/oauth-protected-resource/mcp"`;
            const existing = response.output.headers['WWW-Authenticate'];
            response.output.headers['WWW-Authenticate'] =
                typeof existing === 'string' && /^Bearer\s+\S/.test(existing) ? `${existing}, ${pointer}` : `Bearer ${pointer}`;
        }
    ...
    server.route({
        ...
        options: {
            ...
            validate: {
                payload: Joi.object({
                    redirect_uris: Joi.array().items(Joi.string().max(2048)).min(1).max(MAX_REDIRECT_URIS).required(),
