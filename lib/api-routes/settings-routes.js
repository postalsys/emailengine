...
// ones (PRIVILEGED_SETTINGS_KEYS in lib/privileged-settings.js) and the ones the schema hides from the
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    name: 'get_settings',
                    title: 'Get settings',
                    description: `Read instance settings. Set each setting you want returned to true. Secrets come back as a boolean saying whether a value is stored, credentials inside URLs are masked, and eventTypes lists the webhook event names the instance supports. Settings that would widen this credential (${PRIVILEGED_SETTINGS_SUMMARY}) are not offered.`,
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    ...
                    description: `Change instance settings. Send only the keys to change; they take effect immediately on every worker. Covers where webhooks are delivered and which events are sent, notification shaping, branding, sync strategy, queue retention and AI processing. Confirm with the user before changing the webhook target. The settings that would widen this credential (${PRIVILEGED_SETTINGS_SUMMARY}) are not offered.`,
