        ...
        options: {
            ...
            plugins: {
                mcp: {
                    ...
                    title: 'Get settings',
                    description:
                        'Read instance settings. Set each setting you want returned to true. Secrets come back as a boolean saying whether a value is stored, credentials inside URLs are masked, and eventTypes lists the webhook event names the instance supports. Settings that would widen this credential (operator scripts, proxies, secrets, the built-in listeners) are not offered.',
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    ...
                    description:
                        'Change instance settings. Send only the keys to change; they take effect immediately on every worker. Covers where webhooks are delivered and which events are sent, notification shaping, the service URL and branding, sync strategy, queue retention, AI processing and TLS provisioning. Confirm with the user before changing the webhook target, the service URL or the TLS mode. The settings that would widen this credential (operator scripts, proxies, secrets, the built-in listeners, the MCP and audit switches) are not offered.',
