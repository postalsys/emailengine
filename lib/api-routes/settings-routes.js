...
async function init(args) {
    ...
    server.route({
        ...
        async handler(request) {
            ...
            // The global equivalents of the credential-bearing account fields. encryptedKeys above
            // covers what is encrypted at rest, which is not the same set: proxyUrl, httpProxyUrl,
            // webhooks and webhooksCustomHeaders are stored in the clear and were returned in full,
            // so this endpoint disclosed exactly what the account getters were masking. It sits in
            // the never-grantable admin group, so no narrowed token reaches it - but leaving the
    ...
    server.route({
        ...
        async handler(request) {
            ...
            // the 'settings' message handler, so no inline reload is needed here. Only the keys
            // that were actually written: a skipped masked echo must not trigger reloads either.
