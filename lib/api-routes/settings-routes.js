...
const { normalizeHostname } = require('../tls/store');
const { queueStats } = require('../queue-stats');
const Boom = require('@hapi/boom');
const { handleError, maskSecrets, containsMaskedSecret, isMaskedRoundTrip, MASKED } = require('./route-helpers');
...
// A write is not a change: settings.set() is an unconditional hset, so `applied` lists every key
// the payload carried, and a read-modify-write client posts the whole settings object back on
// every call. Reloading on that alone handed three listeners a new certificate every time somebody
// changed the timezone. tlsHostnames is a set of names, so a reordered list is the same list.
function tlsSettingChanged(key, storedValue, value) {
    if (key === 'tlsHostnames') {
        // Not folded by formatSettingValue(), so the list is compared the way every other reader of
        // it compares a name
