...
const { logApiError } = require('./route-helpers');
const settings = require('../settings');

const LOG_VERBOSE = getBoolean(process.env.EE_OPENAPI_VERBOSE);

const { accountIdSchema, messageSpecialUseSchema, responseAddressSchema, responseFromAddressSchema, apiResponses } = require('../schemas');
...
async function init(args) {
    ...
    async function processChatRequest(opts) {
        let { account, question, index, client, logger } = opts;
        ...
        if (queryResponse?.messageId) {
            ...
            if (LOG_VERBOSE) {
                logger.trace({ msg: 'Chat search result', searchQuery, searchResult });
            }
    ...
    server.route({
        ...
        options: {
            ...
            plugins: {
                openapi: {
                    responses: apiResponses('Returns the generated answer and the messages it was drawn from.', 400, 401, 403, 404, 429, 500)
                }
            },
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                ...
                request.logger.error({ msg: 'Failed to run chat test', err });
                return {
                    success: false,
                    error: err.message
                };
