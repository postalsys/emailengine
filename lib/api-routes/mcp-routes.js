...
const { getToolRegistry, callTool, listToolsFor, toolGrants } = require('../mcp/tools');
...
async function init(args) {
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            // around tools that would only ever answer it with a 403: a bound credential loses
            // the instance-wide listings (list_accounts, get_outbox) and the `account` argument
            // of everything else, a token holding one MCP scope loses the other scope's tools,
            // and a permission-narrowed token loses the tools whose grant its record refuses -
            // the same checks the injected inner request would enforce, asked ahead of time.
            // Advertisement only: tools/call still dispatches whatever is asked, and the inner
            // request stays the enforcement.
            //
            // Auth mode is required and every strategy success path sets artifacts (the token
            // record, or {} for the preauth caller).
            const catalog = listToolsFor(server, { tokenData: request.auth.artifacts, boundAccount });

            // Which of the two tool sets this credential actually holds, read off the catalog it
            // is advertised rather than off its scopes: a narrowed record can empty a scope out,
            // and the instructions should describe the tools the agent will see. A tool both
            // tables cover (the account reads) says nothing about either.
            const profile = { account: boundAccount, manage: false, mail: false };
            for (const { name, surfaces } of toolGrants(server)) {
                if (!catalog.some(tool => tool.name === name)) {
                    continue;
                }
                if (surfaces.includes('mcp-manage') && !surfaces.includes('mcp')) {
                    profile.manage = true;
                }
                if (surfaces.includes('mcp') && !surfaces.includes('mcp-manage')) {
                    profile.mail = true;
                }
            }
            ...
            const ctx = {
                ...
                // Written for this credential: which tool sets it holds, and whether it is bound
                // to one account (in which case the generic opening - call list_accounts, then
                // pass the id - describes a workflow it cannot perform)
        ...
        options: {
            ...
            // `external`: no CSRF crumb and no OpenAPI operation. Three scopes: `api` tokens can
            // do over MCP whatever they can do over REST, and `mcp` (mail) and `mcp-manage`
            // (instance management) tokens are surface-bound credentials this endpoint admits
            // and plain REST refuses (see the scope check in workers/api.js).
            tags: ['external', 'scope:api', 'scope:mcp', 'scope:mcp-manage'],
