...
const { getToolRegistry, callTool, toolVisibleTo } = require('../mcp/tools');
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            const ctx = {
                ...
                // loses the instance-wide listings (list_accounts, get_outbox), and a
                // permission-narrowed token loses the tools whose grant its record refuses -
                // the same check the injected inner request would enforce, asked ahead of time.
                // Advertisement only: tools/call still dispatches whatever is asked, and the
                // inner request stays the enforcement.
                listTools: () => {
                    const { tools, byName } = getToolRegistry(server);
                    // Auth mode is required and every strategy success path sets artifacts
                    // (the token record, or {} for the preauth caller)
                    const tokenData = request.auth.artifacts;
                    return tools.filter(tool => toolVisibleTo(byName.get(tool.name), { tokenData, boundAccount }));
                },
