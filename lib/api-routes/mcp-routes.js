...
const { getToolRegistry, callTool, listToolsFor, catalogSurfaces } = require('../mcp/tools');
...
async function init(args) {
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            //
            // Computed on first use and at most once per request: only tools/list and the
            // discovery methods read the catalog, and only the latter read the instructions, so
            // a tools/call or a ping pays for neither.
            //
            ...
            const listTools = () => {
                if (!catalog) {
                    catalog = listToolsFor(server, { tokenData: request.auth.artifacts, boundAccount });
                }
                return catalog;
            };
            ...
            const ctx = {
                ...
                // Written for this credential: which tool sets it holds, read off the catalog it
                // is advertised rather than off its scopes (a narrowed record can empty a scope
                // out, and the instructions should describe the tools the agent will see), and
                // whether it is bound to one account (in which case the generic opening - call
                // list_accounts, then pass the id - describes a workflow it cannot perform)
                get instructions() {
                    const advertised = listTools();
                    return serverInstructions({ account: boundAccount, surfaces: catalogSurfaces(server, advertised), hasTools: advertised.length > 0 });
                },
