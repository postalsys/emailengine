...
// The shared no-metadata answer. This accessor runs in the api-token strategy on every API
// request, and nearly every route declares no plugins.mcp block, so allocating a fresh empty
// object per call would be steady-state garbage. Frozen so a caller that tries to write into
// it fails loudly instead of poisoning every other route's answer.
...
function pluginOptions(route, namespace) {
    return (route && route.settings && route.settings.plugins && route.settings.plugins[namespace || PLUGIN_OPTIONS_KEY]) || NO_OPTIONS;
}
