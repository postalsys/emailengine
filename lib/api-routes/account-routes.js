...
const { Account, accountExists, authFailureDisabledAt } = require('../account');
const { formatGmailWatch } = require('../account/account-state');
...
const { handleError, maskSecrets, assertNoNetworkOverride, MASKED, MASK_NOTE } = require('./route-helpers');
...
const { REDIS_PREFIX, MAX_FORM_TTL, NONCE_BYTES, MCP_MAX_PAGE_SIZE } = require('../consts');
...
// The answer autodiscovery gives, shared by GET and POST /v1/autoconfig. One definition rather than
// two identical inline copies: the OpenAPI converter collapses structurally identical labelled
// schemas into a single component, so the two routes publish one `DiscoveredEmailSettings` type.
// One discovered server block. The IMAP and SMTP halves carry the same fields and differ only in
// their examples and their (historically inconsistent) label, so they are built rather than written
// out twice - the same reason verifyResultSchema above is a factory.
const discoveredServerSchema = (proto, label, hostExample, portExample) =>
    Joi.object({
        auth: Joi.object({
            user: Joi.string().max(256).example('myuser@gmail.com').description('Account username')
        }).label('DetectedAuthenticationInfo'),

        host: Joi.string().hostname().example(hostExample).description('Hostname to connect to'),
        port: Joi.number()
            .integer()
            .min(1)
            .max(64 * 1024)
        ...
        secure: Joi.boolean().default(false).example(true).description(`Should connection use TLS. Usually true for port ${portExample}`)
    })
        .allow(false)
        .description(`Discovered ${proto} settings. False if ${proto} settings were not found`)
        .label(label);

const discoveredEmailSettingsSchema = Joi.object({
    imap: discoveredServerSchema('IMAP', 'ResolvedServerSettings', 'imap.gmail.com', 993),
    smtp: discoveredServerSchema('SMTP', 'DiscoveredServerSettings', 'smtp.gmail.com', 465),
...
// Address to discover settings for. Same field on the GET query and the POST payload.
const autodiscoverEmailSchema = Joi.string()
    .email()
    .required()
...
// Shared by both autoconfig routes, which answer the same question and publish the same contract -
// including the empty result, which is the normal answer for an address whose domain describes no
// mail server anywhere. Credentials are optional and only ever reach an autodiscovery endpoint that
// refuses to answer anonymously (lib/autodetect-imap-settings.js).
const runAutoconfig = async (request, email, credentials) => {
    try {
        return await autodetectImapSettings(email, request.app.gt, credentials);
    } catch (err) {
        // autodiscovery fails routinely for unknown domains and the handler answers with a
        // regular "not found" response, so this is not an operator-visible failure
        request.logger.debug({ msg: 'API request failed', err });
        if (Boom.isBoom(err)) {
            throw err;
        }
...
const AUTOCONFIG_RESPONSES = apiResponses(
...
async function init(args) {
    ...
    server.route({
        ...
        async handler(request) {
            ...
            try {
                // A narrowed provisioning token may add an account, but not route its connection
                // through a host of its own choosing - the same rule the submit routes apply
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    ...
                    description:
                        'Register an email account. For an OAuth2 provider set oauth2.provider to the application id (from list_oauth2_apps) and oauth2.authorize to true: the response carries a redirect URL for the user to open and sign in, and no credentials are needed. For IMAP set imap and smtp with the host, port and auth the user supplied (autodiscover_settings finds the servers). Prefer create_account_setup_link when the user can open a link themselves, so no password passes through this conversation.',
                    omit: ['proxy', 'smtpEhloName', 'webhooksCustomHeaders', 'copy', 'locale', 'tz']
                },
                openapi: {
            ...
            validate: {
                ...
                payload: Joi.object({
                    account: Joi.string()
                        ...
                        .allow(null)
                        .example('example')
                        .description('Account ID. If set to `null`, a unique ID will be generated automatically')
                        .meta({ usage: FIELD_USAGE.accountId })
                        .required(),
                    ...
                    imap: Joi.object(imapSchemaArg)
                        .allow(false)
                        .description('IMAP configuration. Set to `false` when there is no IMAP access to configure.')
                        .label('ImapConfiguration'),

                    smtp: Joi.object(smtpSchemaArg)
                        .allow(false)
                        .description('SMTP configuration. Set to `false` when there is no SMTP access to configure.')
                        .label('SmtpConfiguration'),

                    oauth2: Joi.object(oauth2SchemaArg)
                        .allow(false)
                        .description('OAuth2 configuration. Set to `false` when the account does not authenticate with OAuth2.')
                        .label('OAuth2'),
            ...
            response: {
                schema: Joi.object({
                    ...
                    state: Joi.string()
                        .valid('existing', 'new')
                        .example('new')
                        .description('Whether the account was created or updated. Not present when a redirect URL is returned')
                        .meta({ enumDescriptions: ENUM_DESCRIPTIONS.entityState })
    ...
    server.route({
        ...
        async handler(request) {
            ...
            try {
                // The stored credential survives a partial update, so where the connection goes is
                // the one thing a narrowed token must not decide (see the provisioning group)
                assertNoNetworkOverride(request);
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    ...
                    description:
                        "Change an account's configuration: name, email, sync and notification settings, special folders, the webhook target, and the IMAP, SMTP or OAuth2 connection settings. Send only the fields to change. Changing a host makes the next connection authenticate to the new host with the stored credential, so confirm connection changes with the user.",
                    omit: ['proxy', 'smtpEhloName', 'webhooksCustomHeaders', 'copy', 'locale', 'tz']
                },
                openapi: {
                    'x-ee-behavior': [BEHAVIOR.CREDENTIALS_VERIFIED_LATER],
            ...
            validate: {
                ...
                payload: Joi.object({
                    ...
                    imap: Joi.object(imapUpdateSchema)
                        .allow(false)
                        .description('IMAP configuration. Set to `false` to remove the stored IMAP configuration from the account.')
                        .label('IMAPUpdate'),
                    smtp: Joi.object(smtpUpdateSchema)
                        .allow(false)
                        .description('SMTP configuration. Set to `false` to remove the stored SMTP configuration from the account.')
                        .label('SMTPUpdate'),
                    oauth2: Joi.object(oauth2UpdateSchema)
                        .allow(false)
                        .description('OAuth2 configuration. Set to `false` to remove the stored OAuth2 configuration from the account.')
                        .label('OAuth2Update'),
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    ...
                    description:
                        "Ask the account's worker to close and re-open its connection. Use after fixing credentials or server settings, or when an account is stuck connecting.",
                    omit: ['reconnect'],
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    ...
                    description: "Ask the account's worker to run a sync now instead of waiting for the next scheduled one.",
                    omit: ['sync'],
                    force: { sync: true }
                },
                openapi: {
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    ...
                    description:
                        'Remove an account from this instance, with its synced index and the tokens bound to it. The mailbox at the provider is untouched. Set revoke to true to also revoke the OAuth2 grant at the provider. Confirm with the user before calling this.'
                },
                openapi: {
            ...
            validate: {
                ...
                query: Joi.object({
                    revoke: Joi.boolean()
                        ...
                        .falsy('N', 'false', 0)
                        .default(false)
                        .description('Revoke the upstream OAuth2 grant at the provider before deleting the account')
                        .meta({ usage: FIELD_USAGE.revokeGrant })
                })
            },
    ...
    server.route({
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    name: 'flush_account',
                    title: 'Flush and re-sync account',
                    description:
                        "Discard the account's local index and sync it again from scratch. Every existing message counts as new again, so webhooks fire for all of them; use only when the index is broken or the sync settings changed. Confirm with the user before calling this.",
                    omit: ['flush'],
                    force: { flush: true }
                },
                openapi: {
    ...
    server.route({
        ...
        async handler(request) {
            try {
                ...
                const listing = await accountObject.listAccounts(request.query.state, request.query.query, request.query.page, request.query.pageSize);

                // The proxy and webhook URLs can carry inline credentials, and the listing returns
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    ...
                    title: 'List accounts',
                    description:
                        'List the email accounts on this instance, paged. Use the returned account id as the account argument of the other tools. Check the state field: only connected accounts can be read.',
                    ...
                    accounts: Joi.array()
                        .items(
                            Joi.object({
                                ...
                                authFailureDisabledAt: accountSchemas.authFailureDisabledAt,
    ...
    server.route({
        ...
        async handler(request) {
            ...
            try {
                    ...
                    if (accountData[type] && accountData[type].auth) {
                        for (let key of ['pass', 'accessToken', 'refreshToken']) {
                            if (key in accountData[type].auth) {
                                accountData[type].auth[key] = MASKED;
                            }
                        }
                    }
                    ...
                    if (accountData[type]) {
                        for (let key of ['accessToken', 'refreshToken']) {
                            if (key in accountData[type]) {
                                accountData[type][key] = MASKED;
                            }
                        }
                    }
                }

                // Credentials for systems other than the mail server, which the loop above does not
                // reach: the proxy and webhook URLs, the webhook headers, and the proxy recorded a
                // second time inside smtpStatus.networkRouting by a failed submission.
                maskSecrets(accountData);
                ...
                let gmailWatch = formatGmailWatch(accountData);
                if (gmailWatch) {
                    result.gmailWatch = gmailWatch;
                }

                // Stated rather than inferred; see the field's description in lib/schemas.js
                result.authFailureDisabledAt = authFailureDisabledAt(accountData);
        ...
        options: {
            ...
            notes: [`Returns stored information about the account. ${MASK_NOTE}`, BEHAVIOR.NO_QUOTA_ON_API_ACCOUNTS],
            tags: ['api', 'Account'],
            ...
            plugins: {
                mcp: {
                    name: 'get_account',
                    title: 'Get account',
                    description: 'Get one account: its display name, email address, connection state and sync status. Credentials are masked.'
                },
                openapi: {
            ...
            response: {
                schema: Joi.object({
                    ...
                    gmailWatch: Joi.object({
                        state: Joi.string()
                            .valid('active', 'expired', 'error')
                            .example('active')
                            .description('Whether the Pub/Sub watch is armed, lapsed, or failed to renew')
                            .label('AccountGmailWatchState'),
                        lastCheck: Joi.date().iso().allow(null).example('2021-07-08T07:06:34.336Z').description('When the watch was last renewed or attempted'),
                        expires: Joi.date().iso().allow(null).example('2021-07-15T07:06:34.336Z').description('When the current watch lapses')
                    })
                        .description(
                            'State of the Gmail Pub/Sub watch (Gmail API accounts only). A watch that is not active does not stop the account syncing - the client falls back to polling, so new mail is delayed rather than lost. Why a renewal failed is shown on the account page rather than here, because that text is written by the provider'
                        )
                        .label('AccountGmailWatch'),
                    ...
                    outlookSubscription: Joi.object({
                        id: Joi.string().example('0fc0f6bb-a3e1-4f7a-9d3a-8e2f1f8b9c11').description('Microsoft Graph subscription ID'),
                        expirationDateTime: Joi.date().iso().allow(null).example('2021-07-11T07:06:34.336Z').description('When the subscription expires'),
                        state: Joi.object({
                            state: Joi.string()
                                .valid('creating', 'created', 'renewing', 'error')
                                .description('Subscription state')
                                .meta({ enumDescriptions: ENUM_DESCRIPTIONS.pubsubState }),
                            time: Joi.number().example(1625727994336).description('Timestamp of last state change'),
                            error: Joi.string()
                                .allow(null)
                                .example('Subscription renewal was rejected')
                                .description('Error message if state is error, null after a successful renewal'),
                            retryCount: Joi.number().integer().example(0).description('How many times the subscription renewal has been retried'),
                            createRetryCount: Joi.number().integer().example(0).description('How many times the subscription creation has been retried')
                        })
                            .unknown()
    ...
    server.route({
        ...
        async handler(request) {
            ...
            try {
                ...
                // provider name and expose the app ID separately. Migrated legacy app IDs equal
                // their provider name, so the lookup is a no-op rename for these.
    ...
    server.route({
        ...
        async handler(request) {
            try {
                let { data, signature } = await getSignedFormData({
                    ...
                    skipServerSettings: request.payload.skipServerSettings,
        ...
        options: {
            ...
            notes: [
                ...
                'An IMAP setup completes without asking the user for mail server settings whenever EmailEngine can discover and verify them from the address and password alone. Set `skipServerSettings` to false to always show that step.',
            ...
            plugins: {
                mcp: {
                    ...
                    title: 'Create account setup link',
                    description:
                        'Create a one-time link to the hosted setup form, where the user signs in to their mailbox themselves, so no password passes through this conversation. Give the link to the user. Set redirectUrl to where they should land afterwards; optionally account (the id to assign), name, email, expectedEmail (the setup is refused if they sign in as someone else) and type (an OAuth2 application id, or imap).',
                    keep: ['account', 'name', 'email', 'expectedEmail', 'redirectUrl', 'type']
                },
                openapi: {
            ...
            validate: {
                ...
                payload: Joi.object({
                    ...
                    skipServerSettings: Joi.boolean()
                        .empty('')
                        .truthy('Y', 'true', '1')
                        .falsy('N', 'false', 0)
                        .default(true)
                        .description(
                            'If true, the mail server settings step is skipped whenever the IMAP and SMTP settings for the address can both be discovered and verified with the password the user enters. The step is still shown if discovery or verification does not fully succeed. Set to false to always ask the user for the server settings. Applies to the IMAP setup path only.'
                        ),
    ...
    server.route({
        ...
        async handler(request) {
            try {
                // Existence probe, so an unknown account is a 404 rather than an empty download.
                // The same error loadAccountData() throws for a missing record, without the
                // record load and decrypts that only the existence answer is wanted from
                if (!(await accountExists(redis, request.params.account))) {
                    throw Boom.notFound('Account record was not found for requested ID');
                }

                return await getLogs(redis, request.params.account);
            } catch (err) {
                handleError(request, err);
            }
        },
        options: {
            description: 'Return stored logs for an account',
            notes: [
                'Output is a downloadable text file with one JSON log entry per line, or a plain-text notice when nothing is stored',
                BEHAVIOR.ACCOUNT_LOG_SCOPE
            ],
            tags: ['api', 'Logs'],
            ...
            plugins: {
                mcp: {
                    ...
                    title: 'Get account connection log',
                    description:
                        'Read the stored connection log of an account: the raw IMAP or provider API trace, which names folders, message ids and subjects. Use it to find out why an account fails to connect or sync. Only kept while logging is enabled for the account or for the whole instance.'
                },
                openapi: {
                    'x-ee-behavior': [BEHAVIOR.ACCOUNT_LOG_SCOPE],
                    produces: ['text/plain'],
                    responses: apiResponses('Returns the account log as a plain text stream.', 400, 401, 403, 404, 429, 500)
                }
            },
    ...
    server.route({
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    name: 'verify_account_settings',
                    title: 'Verify account settings',
                    description:
                        'Test IMAP and SMTP settings without saving them: connects and authenticates with the supplied host, port and credentials and reports what failed. Set mailboxes to true to also list the folders. Useful before create_account or update_account.',
                    omit: ['proxy', 'smtpEhloName'],
                    openWorld: true
                },
                openapi: {
    ...
    server.route({
        ...
        async handler(request) {
            return await runAutoconfig(request, request.query.email);
        },
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    name: 'autodiscover_settings',
                    title: 'Discover mail server settings',
                    description:
                        "Discover the IMAP and SMTP server settings for an email address from DNS and the provider's autoconfiguration. The natural first step before create_account or verify_account_settings for a mailbox that does not use an OAuth2 application.",
                    openWorld: true
                },
                openapi: {
                    responses: AUTOCONFIG_RESPONSES
                }
            },
            ...
            validate: {
                ...
                query: Joi.object({
                    email: autodiscoverEmailSchema
                }).label('AutodiscoverQuery')
            },

            response: {
                schema: discoveredEmailSettingsSchema,
                failAction: 'log'
            }
        }
    });
    ...
    server.route({
        ...
        async handler(request) {
            // The credentials are offered to the autodiscovery host only after that host has refused
            // an anonymous request with a Basic challenge over https, and only to the origin that
            // issued it (lib/autodetect-imap-settings.js runAutodiscovery). Every other resolver
            // stays anonymous, so an address that resolves without a password answers the same here
            // as it does on the GET.
            return await runAutoconfig(request, request.payload.email, request.payload.auth);
        },

        options: {
            description: 'Discover Email settings with credentials',
            notes: [
                'Try to discover IMAP and SMTP settings for an email account, optionally authenticating against an autodiscovery server that refuses to answer anonymously',
                "Hosted Exchange answers autodiscovery only for an authenticated request, which is why this endpoint exists alongside `GET /v1/autoconfig`. The password is sent to the autodiscovery host named by the address domain's own DNS records, and only after that host has asked for it with a Basic challenge over HTTPS."
            ],
            tags: ['api', 'Settings'],

            // Deliberately not exposed as an MCP tool. `autodiscover_settings` (the GET) covers
            // discovery for an agent; this one takes a mailbox password as an argument, which is not
            // something to put in a tool manifest for a model to fill in.
            plugins: {
                openapi: {
                    'x-ee-impact': IMPACT.READONLY,
                    responses: AUTOCONFIG_RESPONSES
                ...
                payload: Joi.object({
                    ...
                    auth: Joi.object({
                        user: Joi.string()
                            ...
                            .description('Username for the autodiscovery request. Defaults to the email address.'),
                        pass: Joi.string().max(1024).required().example('verysecret').description('Password for the mailbox')
                    })
                        .description('Credentials to offer if the autodiscovery server refuses to answer anonymously')
                        .label('AutodiscoverCredentials')
                }).label('AutodiscoverRequest')
            },
