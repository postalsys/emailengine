...
const URL_SECRET_KEYS = ['proxy', 'proxyUrl', 'httpProxyUrl', 'webhooks', 'targetUrl', 'sentryDsn', 'documentStoreUrl'];
...
function maskSecrets(entry) {
    ...
    // A failed delivery records the target it was aimed at on the webhook route, the account or
    // the settings. The worker stores it with the credentials already stripped, but a flag written
    // by an older release still carries the raw URL - the same one field over from the masked
    // `targetUrl` / `webhooks` beside it, readable by any token that can list webhook routes.
    if (entry.webhookErrorFlag && typeof entry.webhookErrorFlag === 'object' && entry.webhookErrorFlag.url) {
        entry.webhookErrorFlag.url = maskUrlPassword(entry.webhookErrorFlag.url);
    }
 ...
 *
 * Refused for two kinds of credential. A narrowed token, for the reason above. And a browse-page
 * session token, which is bounded by SESSION_TOKEN_GRANTS rather than by a permissions record and
 * so reads as unnarrowed here - it holds `send/submit` but is refused `PUT /v1/account/{account}`,
 * which is exactly the shape this guard exists for. Without it the one credential that lives in
 * page HTML could still post the account's SMTP session to an address of its choosing.
 *
 * Everything else is left alone: an unnarrowed API token already reaches PUT /v1/account/{account}
 * and can point the account's own proxy wherever it likes, so refusing it here would cost a
 * documented capability and buy nothing.
 *
...
function assertNoNetworkOverride(request) {
    ...
    if (!isRestrictedCredential(request)) {
        return;
    }
...
/**
 * Whether the credential behind a request is bounded below the full API.
 *
 * Two kinds are. A narrowed token, one carrying a `permissions` record. And a browse-page session
 * token, which is bounded by SESSION_TOKEN_GRANTS rather than by a record and so reads as
 * unnarrowed to the permission check - it is marked on the request by the api-token strategy
 * rather than faked into a record, because the session bound is a grant list the two axes cannot
 * express without widening it. The permission verdict is asked of the module that enforces it
 * rather than read off `permissions` for truthiness, so `{}` and an unreadable record count here
 * exactly as they do at the strategy. `artifacts` is the token record the strategy resolved.
 *
 * An unnarrowed API token is not restricted: it already reaches everything, so the payload rules
 * that key on this would cost it a documented capability and buy nothing.
 *
 * @param {Object} request - the Hapi request
 * @returns {Boolean}
 */
function isRestrictedCredential(request) {
    if (request.app && request.app.sessionToken) {
        return true;
    }
    return tokenPermissions.inspect(request.auth && request.auth.artifacts).narrowed;
}

/**
 * Refuses a settings request that names a privileged key, when the credential making it is
 * narrowed.
 *
 * `read/settings` and `write/settings` are grantable permissions, and the settings blob is the
 * one endpoint whose fields span every trust level: `serviceUrl` beside `openAiPreProcessingFn`,
 * which is operator code run inside the instance, and `scriptEnv`, where the admin UI tells
 * operators to keep the API keys their scripts use. The second payload rule in the model after
 * assertNoNetworkOverride(), and like it a static list rather than an interpretation of a value:
 * PRIVILEGED_SETTINGS_KEYS in lib/privileged-settings.js says which keys, and the MCP settings tools hide the
 * same keys from their schemas, so the grants mean the same thing on both surfaces. Applied to
 * reads as well as writes, because several of the keys are secrets a narrowed credential is
 * promised it cannot read, and the write guard alone left them readable.
 *
 * Refused as a whole rather than applied minus the privileged keys, because a partial answer that
 * silently dropped fields would read to the caller as success.
 *
 * @param {Object} request - the Hapi request
 * @param {Array<String>} keys - the settings the request names (payload keys, or the query flags set)
 * @param {String} [verb] - 'change' or 'read', for the refusal
 */
function assertNoPrivilegedSettings(request, keys, verb = 'change') {
    const privileged = settings.privilegedKeys.filter(key => keys.includes(key));
    if (!privileged.length || !isRestrictedCredential(request)) {
        return;
    }

    throw Boom.forbidden(`A token with restricted permissions can not ${verb} ${privileged.join(', ')}`);
}
