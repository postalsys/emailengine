...
function assertNoNetworkOverride(request) {
    ...
    // Set by the api-token strategy for a `sess_` credential. Marked on the request rather than
    // faked into a permissions record: the session bound is a grant list, which the two axes cannot
    // express without widening it, and a record that says more than the rule would be read as
    // policy by the next thing that looks at it.
    const sessionToken = !!(request.app && request.app.sessionToken);
    ...
    if (!sessionToken && !tokenPermissions.inspect(request.auth && request.auth.artifacts).narrowed) {
        return;
    }
