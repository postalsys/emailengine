...
/**
 * Refuses a settings write that names a privileged key, when the credential making it is narrowed.
 *
 * `write/settings` is a grantable permission, and the settings blob is the one endpoint whose
 * fields span every trust level: `serviceUrl` beside `openAiPreProcessingFn`, which is operator
 * code run inside the instance. The second payload rule in the model after assertNoNetworkOverride(),
 * and like it a static list rather than an interpretation of a value: PRIVILEGED_SETTINGS_KEYS in
 * lib/settings.js says which keys, and the MCP settings tool hides the same keys from its schema,
 * so the grant means the same thing on both surfaces.
 *
 * Refused as a whole rather than applied minus the privileged keys, because a partial write that
 * silently dropped fields would read to the caller as success.
 *
 * @param {Object} request - the Hapi request
 */
function assertNoPrivilegedSettings(request) {
    const payload = request.payload;
    if (!payload || typeof payload !== 'object' || Array.isArray(payload)) {
        return;
    }

    const privileged = settings.privilegedKeys.filter(key => Object.hasOwn(payload, key));
    if (!privileged.length || !isRestrictedCredential(request)) {
        return;
    }

    throw Boom.forbidden(`A token with restricted permissions can not change ${privileged.join(', ')}`);
}
