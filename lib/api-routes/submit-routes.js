        ...
        options: {
            ...
            plugins: {
                mcp: {
                    ...
                    // An allowlist, not a deny list. The submit payload is the widest one in the
                    // API and it grows; with `omit` a field added here later would join the
                    // agent-facing schema by default, which fails open on the one tool that
                    // reaches people outside the mailbox.
                    //
                    // What is kept is the message an agent composes: who it goes to, what it
                    // says, what it answers, what it carries and when it leaves. What is not is a
                    // decision the operator made when they connected the account - routing,
                    // tracking, bounce handling, whether the sent copy is filed - plus every way
                    // around the structured fields (`raw` replaces the whole message, `envelope`
                    // overrides the SMTP conversation, `headers`/`messageId` set headers
                    // EmailEngine derives) and `mailMerge`, which would turn one call into a bulk
                    // send to a recipient list - not what "confirm before sending" protects
                    // against. An agent that should mail several people calls the tool several
                    // times, where each send is visible as a send.
                    keep: [
                        'account',
                        'reference',
                        'from',
    ...
    server.route({
        ...
        options: {
            ...
            validate: {
                ...
                params: Joi.object({
                    account: accountIdSchema.required(),
                    message: messageIdSchema.required().description('Message ID of the draft')
                }),
