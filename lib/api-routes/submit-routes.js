...
const proxySchema = settingsSchema.proxyUrl.description('Optional proxy URL to use when connecting to the SMTP server');
            ...
            validate: {
                ...
                payload: Joi.object({
                    envelope: Joi.object({
                        ...
                        .description(
                            "An optional object specifying the SMTP envelope used during email transmission. If not provided, the envelope is derived from the draft's message headers. Only used when the message is delivered over SMTP."
                        )
                    ...
                    copy: Joi.boolean()
                        ...
                        .example(null)
                        .description(
                            "If set then either copies the message to the Sent Mail folder or not. If not set then uses the account's default setting. Only applies to SMTP deliveries - Gmail and MS Graph file sent drafts into the Sent Mail folder themselves."
                        ),
