...
//   - appended to the route's `notes` array, which the generator joins into the standard
//     `description` field, so every consumer of /swagger.json sees it;
//   - listed under `plugins.openapi['x-ee-behavior']`, so the API reference can tell
...
//   NO_LABEL_SET_ON_GMAIL     lib/email-client/gmail-client.js, assertLabelSetSupported throw
...
//   ACCOUNT_LOG_SCOPE         lib/email-client/base-client.js, getLogger; workers/imap.js, getAccountLogger
...
module.exports = {
    ...
    NO_QUOTA_ON_API_ACCOUNTS:
        'Quota reporting is not available for Gmail API or MS Graph accounts; the quota field is returned as false for those, the same value an IMAP server without QUOTA support produces.',
