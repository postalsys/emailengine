        ...
        levels: [
            {
                ...
                // Less read/provisioning, which is only POST /v1/verifyAccount: a connection to a
                // host the caller names, authenticated with credentials the caller supplies. The
                // other reads that leave the instance (autodiscovery, the OAuth2 application
                // check) go where the caller's own domain or a stored record points; this one is
                // a port and password probe for whatever network the instance sits on, which a
                // level sold as unable to change anything must not be. test/mcp-tools-test.js
                // pins the pair to that one route, so the group is the whole of the exclusion.
