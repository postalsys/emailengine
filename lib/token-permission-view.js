...
const MCP_SECTIONS = {
    manage: {
        ...
        levels: {
            none: [],
            observe: pairsOf('mcp-manage', grant => grant.action === ACTION.READ),
            operate: pairsOf('mcp-manage', grant => grant.action !== ACTION.DESTRUCTIVE),
            administer: pairsOf('mcp-manage', () => true)
        },
        // In the order they are offered. `caveat` is the sentence the consent prompt shows under
        // the derived grant list: what the level still cannot do, and what it can that a reader
        // might not expect.
        options: [
            {
                value: 'observe',
        ...
        // Offered where the section can be declined outright: the consent prompt and the
        // generator. The token form declines it by leaving the scope unticked instead.
        noneOption: { value: 'none', label: 'No management access', hint: 'The client gets only what is granted below.' }
    },
    mail: {
        scope: 'mcp',
        label: 'Email access',
        ...
        levels: {
            none: [],
            read: pairsOf('mcp', grant => grant.action === ACTION.READ),
            mail: pairsOf('mcp', grant => grant.action !== ACTION.DESTRUCTIVE),
            full: pairsOf('mcp', () => true)
        },
        options: [
            {
                value: 'read',
        ...
        // The section is off until a person switches it on: the checkbox the consent prompt and
        // the generator render above the levels
        toggle: {
            label: 'Also let the client work with mailbox contents',
            hint: 'Off by default. Email content then flows into whatever model the connected client runs.'
        }
    }
};

// What each level's grants mean in plain words, and the keys the pages count tools against -
// computed once at load onto the option itself, so a level cannot be offered without wording.
for (const section of Object.values(MCP_SECTIONS)) {
    for (const option of section.options) {
        const pairs = section.levels[option.value];
        option.actions = humanGrantList([...new Set(pairs.map(grant => grant.action))], ACTION_LABELS);
        option.groups = humanGrantList([...new Set(pairs.map(grant => grant.group))], GROUP_LABELS);
        option.grants = pairs.map(grantKey);
    }
}
...
function mcpGrantsFor(choice) {
    ...
    for (const [key, section] of Object.entries(MCP_SECTIONS)) {
        const level = choice && choice[key];
        if (!level || level === 'none') {
            continue;
        }
        if (!Object.hasOwn(section.levels, level)) {
            throw new Error(`Unknown ${key} access level: ${level}`);
        }
        scopes.push(section.scope);
        for (const grant of section.levels[level]) {
