...
const { ACTION, GROUP, GRANTABLE_GROUPS, SURFACE_GRANTS, PER_REQUEST_SURFACES, MCP_SCOPES } = require('./api-routes/permission-map');
...
function summarize(permissions) {
    ...
    if (!Array.isArray(permissions.grants)) {
        // The two-axis form, read out as written: an absent axis is not a restriction and stays out
        // of the sentence, which is why this does not go through effectiveGrants() - that expands an
        // absent axis to every value, and "every section" would imply a grant the record does not make
        const actions = labelAxis(permissions.actions, ACTION_LABELS);
        const groups = labelAxis(permissions.groups, GROUP_LABELS);
