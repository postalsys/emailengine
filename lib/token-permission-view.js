        ...
        levels: [
            {
                ...
                // Less read/provisioning, which is POST /v1/verifyAccount and POST /v1/autoconfig:
                // both take credentials the caller supplies and offer them to a host the caller's
                // input decides. The other reads that leave the instance (the credential-free
                // GET /v1/autoconfig, the OAuth2 application check) go where the caller's own
                // domain or a stored record points and carry no password; these two are a port and
                // password probe for whatever network the instance sits on, which a level sold as
                // unable to change anything must not be. test/mcp-tools-test.js pins the pair to
                // those routes, so the group is the whole of the exclusion.
                pairs: pairsOf(MCP_SCOPES.manage, grant => grant.action === ACTION.READ && grant.group !== GROUP.PROVISIONING)
            },
            {
                value: 'operate',
                label: 'Operate',
                hint: 'Also reconnect, add and test accounts, change settings, and manage OAuth2 applications, gateways and templates.',
                caveat: 'Settings changes take effect at once, and moving the webhook target sends future message notifications, message text included, elsewhere. Reconfiguring an account or gateway can send its stored credential to a new host, and reconfiguring an OAuth2 application can redirect where future sign-ins deliver their authorization codes. It cannot delete anything, create tokens or read stored credentials.',
