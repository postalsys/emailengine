...
// prose (see MCP_GRANT_WORDING below), so each one has to work as a sentence fragment too.
const ACTION_LABELS = {
    [ACTION.READ]: 'Read',
...
// The narrowings behind the MCP access-level choice, offered identically by the OAuth consent
// page and the admin generator. Derived once at load from the same table the scope enforcement
// reads, nothing else. Lives here rather than on the pages that post it because this module owns
// how permission records are presented, and a second derivation had already drifted (all groups
// instead of the readable ones).
//
// Three levels, because that is where consent-screen comprehension and real agent shapes meet:
// read-only for browsing, "mail agent" for the triage-and-reply agent that must never reach the
// delete tool, and full for everything the surface exposes. Full is the absence of a permissions
// record - the mcp scope itself is the bound.
const MCP_READ_ONLY_PERMISSIONS = {
    actions: [ACTION.READ],
    groups: SURFACE_GRANTS.mcp.filter(grant => grant.action === ACTION.READ).map(grant => grant.group)
};

// The permission record's two axes are independent allowlists, so deriving both from a pair list
// widens it (read x submit, write x template, ...). Inert here: an mcp-scoped token is usable
// only through /mcp, where surfaceAdmits() re-checks every request against the exact grant pairs.
const mailAgentGrants = SURFACE_GRANTS.mcp.filter(grant => grant.action !== ACTION.DESTRUCTIVE);
const MCP_MAIL_AGENT_PERMISSIONS = {
    actions: [...new Set(mailAgentGrants.map(grant => grant.action))],
    groups: [...new Set(mailAgentGrants.map(grant => grant.group))]
};

// What each posted access-level slug mints. The consent route validates against these keys, so a
// level added here is automatically accepted there - and the fail-safe direction is fixed by the
// route defaulting to 'read' when no level is posted at all.
const MCP_ACCESS_LEVELS = {
    read: MCP_READ_ONLY_PERMISSIONS,
    mail: MCP_MAIL_AGENT_PERMISSIONS,
    full: null
};

// How the levels are offered on screen, in the order they are listed. The consent prompt and
// the admin generator render the same radio group from this table (the mcp_access_levels
// partial), because two hand-written fieldsets had already drifted on the read-only wording.
// A level added to MCP_ACCESS_LEVELS needs an entry here to be offered anywhere.
// What each level's grants mean in plain words, computed once at load and shown wherever a level
// is approved (today the OAuth consent prompt). Keyed off MCP_ACCESS_LEVELS itself so a level
// cannot exist without wording; the full level stores null - no permissions record, the mcp
// scope is the bound - so its wording derives from the surface grant table directly. The mail
// and full group lists currently come out identical (the destructive grants add no group of
// their own), but each level derives its own list on purpose - they diverge the moment a
// destructive-only group joins the surface.
const MCP_GRANT_WORDING = Object.fromEntries(
    Object.entries(MCP_ACCESS_LEVELS).map(([level, permissions]) => {
        const actions = permissions ? permissions.actions : [...new Set(SURFACE_GRANTS.mcp.map(grant => grant.action))];
        const groups = permissions ? permissions.groups : [...new Set(SURFACE_GRANTS.mcp.map(grant => grant.group))];
        return [level, { actions: humanGrantList(actions, ACTION_LABELS), groups: humanGrantList(groups, GROUP_LABELS) }];
    })
);

const MCP_ACCESS_LEVEL_OPTIONS = [
    {
        value: 'read',
        label: 'Read-only',
...
function summarize(permissions) {
    ...
    if (Array.isArray(permissions.grants)) {
        return summarizeGrants(permissions.grants, label, covers);
    }
...
// The pair-list form of the record, read out loud. A list that happens to be a full cross product
// is the two-axis record in another spelling and gets that record's sentence; anything else is
// described section by section, in the order the sections are declared, because "read on
// Messages, write on Templates" is the shape the form exists for and it has no two-axis sentence.
function summarizeGrants(grants, label, covers) {
    // In declaration order rather than in list order, so two records granting the same pairs read
    // the same however they were written
    const actionOrder = Object.values(ACTION);
    const groupOrder = Object.values(GROUP);

    const actions = actionOrder.filter(action => grants.some(grant => grant.action === action));
    const groups = groupOrder.filter(group => grants.some(grant => grant.group === group));

    const summary = {
        actions: label(actions, ACTION_LABELS),
        groups: label(groups, GROUP_LABELS),
        unreadable: false
    };

    if (!grants.length || grants.length === actions.length * groups.length) {
        summary.sentence = sentenceFor(summary.actions, summary.groups, covers(actions, actionOrder), covers(groups, GRANTABLE_GROUPS));
        return summary;
    }
    ...
    summary.sentence = groups
        .map(group => {
            ...
            const where = Object.hasOwn(GROUP_LABELS, group) ? GROUP_LABELS[group] : group;
            return `${where}: ${humanGrantList(granted, ACTION_LABELS)}`;
        })
        .join('; ');
