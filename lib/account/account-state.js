...
/**
 * Whether a mail connection can be opened for this account at all.
 *
 * Shared by the connect gates so the same two conditions are not spelled out at each one. This is
 * a liveness question, not a capability one - do not confuse it with Account.isSendOnlyAccount(),
 * which asks whether an account was ever configured to read mail.
 *
 * NB: this is deliberately NOT used by the admin UI. It returns true for an SMTP-only send-only
 * account (no imap, no oauth2), which is a perfectly healthy account the UI must not label as
 * disabled - the views test `imap.disabled` directly for that reason.
 *
