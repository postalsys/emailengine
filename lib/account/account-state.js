...
/**
 * Whether a mail connection can be opened for this account at all: no transport is configured, or
 * the operator ticked "Disable IMAP".
 *
 * The single spelling of that test, shared by the connect gates (imap-client, message-builder) and
 * by the admin UI's `canReadMail` / "Disable IMAP" defaults. Answering it here rather than at each
 * call site is what keeps them from drifting.
 *
 * NB: "no connection can be opened" is not "this account is broken". An SMTP-only send-only account
 * returns true here and is perfectly healthy, so the state badge and the Enabled row in the views
 * must keep testing `imap.disabled` directly rather than reusing this.
 *
...
function isConnectionBlocked(accountData) {
    ...
    return !!(accountData.imap && accountData.imap.disabled);
}
