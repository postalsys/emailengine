...
// park in general. A password account carries a real IMAP configuration and has been wearing this
// same threshold error state since v2.46.0 (2024-08-28), where the safety net first shipped; it is
// deliberately not matched, because it was never stuck - clearing its checkbox is an explicit
// `disabled: false`, which is not marker-gated - and matching it would stamp accounts nobody has
// touched in two years with a park time of today.
//
...
const { REDIS_PREFIX, AUTH_FAILURE_DISABLED_FIELD } = require('../consts');

// Frozen copy of the description the safety net stored, NOT the live
// AUTH_FAILURE_DISABLED_DESCRIPTION constant. This matches data written by a past release, so it has
// to keep matching that text after somebody rewords the sentence the current release writes -
// sharing the constant would let a copy edit silently disarm the migration.
const LEGACY_PARK_DESCRIPTION = 'IMAP was disabled for the account due to exceeding the authentication error threshold';
...
function isLegacyPark([imapInfo, marker, errorStateInfo, oauth2Info]) {
    ...
    return !!errorState && errorState.description === LEGACY_PARK_DESCRIPTION;
}
