...
function parseImapData(imapInfo, logger) {
    ...
    // `null` parses fine and is an absent blob, which is what it is. So is `false`: it is what POST
    // and PUT /v1/account store when the payload says there is no IMAP access to configure
    // (`imap: false`), the documented way to register an OAuth2 account without one. Reporting it
    // as unreadable would exempt exactly those accounts from the safety net and log an error on
    // every init.
    if (imapData === null || imapData === false) {
        return { imapData: null, invalid: false };
    }
    ...
    // replaced with a bare flag.
    if (typeof imapData !== 'object' || Array.isArray(imapData)) {
