...
const { filterListPage } = require('./tools');
...
class TemplateHandler {
    ...
    async list(account, page, pageSize, query) {
        ...
        let selectedIds = query ? templateIds : templateIds.slice(startPos, startPos + pageSize);
        if (!selectedIds.length) {
            return response;
        }

        let keys = selectedIds.map(id => `${id}:meta`);
        ...
        for (let [i, entry] of list.entries()) {
            try {
                ...
                logger.error({ msg: 'Failed to process template', template: selectedIds[i], entry: entry && entry.toString('base64'), err });
                continue;
            }
        }

        if (query) {
            let paged = filterListPage(response.templates, ['id', 'name', 'description'], query, startPos, pageSize);
            response.templates = paged.entries;
    ...
    // The inverse of generateId(): an 8-byte timestamp, a 4-byte counter, then the account id.
    // Anything shorter than the fixed 12-byte header cannot be an id this module issued, and the
    // routes accept any string up to 256 characters, so a malformed id is refused as "not found"
    // here rather than failing on the header read.
    unpackId(id) {
    ...
    async get(id) {
        ...
        try {
            ...
            logger.error({ msg: 'Failed to process template', template: id, entry: getResult[0].toString('base64'), err });
        }
        ...
        try {
            ...
            logger.error({ msg: 'Failed to process template', template: id, entry: getResult[1].toString('base64'), err });
        }
