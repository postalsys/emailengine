...
class TemplateHandler {
    ...
    async generateId(account) {
        let idNum = await this.redis.hincrby(this.getTemplatesContentKey(account), 'id', 1);
        console.log('IDNUM', account, idNum);

        let idBuf = Buffer.alloc(8 + 4 + 1);
        idBuf.writeBigUInt64BE(BigInt(Date.now()), 0);
        idBuf.writeUInt32BE(idNum, 8);

        idBuf[idBuf.length - 1] = !account ? 0x01 : 0;

        const id = idBuf.toString('base64url');

        console.log('ID', account, idNum);
    ...
    unpackId(id) {
        ...
        return {
            ...
            public: idBuf[idBuf.length - 1] === 0x01
        };
    }
    ...
    async update(account, id, meta, content) {
    ...
    async get(account, id) {
        // all accounts can read public templates
        ...
        let meta, content;
        if (getResult && getResult.length === 2) {
            try {
                if (getResult[0]) {
...
/*
async function main() {
    let p = await module.exports.templates.create(
        null,
        {
            title: 'hallo hallo public'
        },
        {
            html: '<b>aaa public</b>',
            text: 'vvvv public',
    ...
    let c = await module.exports.templates.create(
        'test',
        {
            title: 'hallo hallo'
        },
    ...
    console.log(c);
    let r = await module.exports.templates.get('test', c.id);
    console.log(r);

    let list = await module.exports.templates.list('test');
    console.log(list);

    let entry = await module.exports.templates.get('test', list.templates[0].id);
    console.log(entry);

    let u = await module.exports.templates.update('test', list.templates[0].id, {
        title: 'hallo hallo upated'
    });

    console.log(u);

    entry = await module.exports.templates.get('test', list.templates[0].id);
    console.log(entry);

    entry = await module.exports.templates.get('test', p.id);
    console.log(entry);

    entry = await module.exports.templates.del('test', list.templates[0].id);
    console.log(entry);

    entry = await module.exports.templates.del('test', p.id);
    console.log(entry);
}
