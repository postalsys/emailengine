...
function formatExtraScopes(extraScopes, defaultScopes) {
    ...
    for (let extraScope of extraScopes) {
        if (
            defaultScopes.includes(extraScope) ||
            defaultScopes.includes(`https://graph.microsoft.com/${extraScope}`) /* || defaultScopes.includes(`https://outlook.office.com/${extraScope}`)*/
        ) {
            // skip existing
            continue;
        }
