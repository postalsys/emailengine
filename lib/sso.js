...
// Optional return-to-app target after RP-initiated logout. Must be registered at the
// IdP as a valid post-logout redirect URI. When unset, no post_logout_redirect_uri is
// sent and the IdP shows its own logged-out page (so OIDC_LOGOUT needs no IdP config).
// Point it at `<serviceUrl>/admin/login?loggedout=1` for an in-app "signed out" screen.
const OIDC_POST_LOGOUT_REDIRECT_URI = readEnvValue('OIDC_POST_LOGOUT_REDIRECT_URI') || null;

const USE_OIDC_AUTH = !!(OIDC_ISSUER && OIDC_CLIENT_ID && OIDC_CLIENT_SECRET);

const DISCOVERY_TIMEOUT = 10 * 1000;

// Backoff bounds for retryDiscovery(). Quick at first, because the common failure is
// an IdP that is merely slower to boot than we are (a shared host restarting both at
// once, where the IdP can be a minute behind), then doubling to one probe every five
// minutes so a genuine multi-hour outage costs almost nothing.
const DISCOVERY_RETRY_MIN = 5 * 1000;
const DISCOVERY_RETRY_MAX = 5 * 60 * 1000;
...
// containing "@" are exact email matches, unless the "@" comes first, which makes
// the entry a domain match. An entry with no "@" at all cannot be an email, so it
// is read as a domain too: as an exact-email rule it could never match, and a bare
// "example.com" would have locked every user out with a generic "not authorized".
...
function parseAllowList(str) {
    return (str || '')
        ...
        .map(entry => {
            ...
            if (at > 0) {
                return { type: 'email', value: entry };
            }
            return { type: 'domain', value: at === 0 ? entry.slice(1) : entry };
        })
...
function matchesEmailAllowList(profile, allowList) {
    // Never honor an email/domain match for an address the IdP explicitly reports as
    // unverified - otherwise an IdP that allows self-registration (or unverified email
    // changes) would let an attacker claim any allow-listed address and gain admin. A
    // missing email_verified claim (many IdPs omit it) keeps the prior trust behavior;
    // group-based authorization is unaffected and still evaluated in isAuthorized().
...
// Normalize the OIDC `email_verified` claim to a strict tri-state used by the email
// allow-list. The spec says it is a JSON boolean, but some IdPs emit the string
// "true"/"false" (or, via SAML bridges, 1/0). An absent claim stays undefined so an
// IdP that never asserts verification keeps the prior trust behavior; any value that
// IS present resolves to a strict boolean, so a non-standard "unverified" signal (e.g.
// 0) can never be mistaken for "unknown" and silently trusted.
...
function mapUserinfoProfile(userinfo, groupsClaim) {
    ...
    return {
        ...
        emailVerified: normalizeEmailVerified(userinfo.email_verified),
        groups: extractGroups(userinfo, groupsClaim)
    };
}

// Fetch and validate the discovery document for the configured issuer. Defaults to the
// shared RetryAgent (transient DNS/socket retries) with a hard timeout so a
// slow/unreachable IdP cannot stall API-worker startup indefinitely. Callers that
// retry on their own schedule pass a plain dispatcher instead - see retryDiscovery().
async function fetchOidcDiscovery(options) {
    options = options || {};
    ...
    let res = await fetchCmd(url, {
        ...
        dispatcher: options.dispatcher || httpAgent.retry,
...
// Keep probing the discovery endpoint until it answers, resolving with the document.
// Callers use this after a failed startup fetch: discovery is what the bell strategy is
// built from, so a failure that is never retried leaves SSO switched off for the
// lifetime of the process - silently, and on a forced instance that looks like the
// admin panel reverting to password login on its own.
//
// Two deliberate departures from a plain fetch-with-retry. It waits before the first
// attempt, because this continues a fetch that has just failed rather than starting a
// fresh one. And each probe uses the plain dispatcher instead of the retrying one
// fetchOidcDiscovery() defaults to: this loop is itself the retry layer, and undici's
// ladder underneath it would turn every "cheap probe" into five connect attempts
// holding a socket for the whole 10s timeout.
async function retryDiscovery(options) {
    options = options || {};

    let fetchDiscovery = options.fetchDiscovery || (() => fetchOidcDiscovery({ dispatcher: httpAgent.fetch }));

    return backOff(fetchDiscovery, {
        delayFirstAttempt: true,
        startingDelay: options.minDelay || DISCOVERY_RETRY_MIN,
        maxDelay: options.maxDelay || DISCOVERY_RETRY_MAX,
