...
async function runAutodiscovery(baseUrl, email, credentials, fetchResource) {
    ...
    const hasSettings = result => result && (result.imap || result.smtp);
    ...
    // Both are in flight before either is awaited, so the slower one costs nothing when the first
    // already answered. POX is awaited first because it is the only one that names the login to use.
    ...
    for (let result of [await pox, await soap]) {
        if (hasSettings(result)) {
            return result;
        }
    }
