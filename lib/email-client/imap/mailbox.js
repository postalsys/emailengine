...
class Mailbox {
    ...
    async getAttachment(message, part, options, connectionOptions) {
        ...
        try {
            ...
            // The lock is held for the lifetime of the stream, so it can only be given back once
            // the stream is done - by any route. Enumerating those routes by hand is what let a
            // consumer that simply destroys the stream hold the lock forever: an abort emits
            // neither 'end' nor 'error', only 'close'. finished() covers end, error, destroy and
            // premature close by contract, and fires exactly once, so there is no set of terminal
            // events left to get wrong. A completed download reports no error and leaves the
            // connection where it is; anything else ended mid-FETCH, so the main mailbox has to be
            // re-selected.
