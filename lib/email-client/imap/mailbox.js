 ...
 *
 * An ENVELOPE list can come back holding no usable address at all: RFC 9051 7.5.2 group
 * markers carry a group name and nothing else, and a server that failed to parse a
 * malformed header reports the wreckage the same way, as a display name with an empty
 * address. The raw header is the better answer in that case, because addressparser sees
 * the original value and can still recover the recipient. Falling back on emptiness alone
 * was not enough - the broken list is not empty, just useless.
 *
...
function preferAddressList(envelopeList, headers, key) {
    if (envelopeList?.some(entry => entry.address)) {
        return envelopeList;
    }
