...
const { samePath } = require('../../utils/mailbox-path');
const { connectionNotAvailableError } = require('./connection-errors');
...
const { isUnsafeKey } = require('../../utils/unsafe-key');
const { arfDetect, camelCaseComplaint, COMPLAINT_ATTACHMENT_TYPES } = require('../../arf-detect');
...
/**
 * Whether the connection is gone, rather than this message being a problem. An enrichment step that
 * hits one must let it propagate: publishSyncedEvents() keeps the message queued for the next pass
 * instead of announcing it with its inline images pointing at content that was never downloaded.
 * No notification has been sent by then - processNew() emits all of them at the end - so the retry
 * delivers exactly one webhook rather than a second copy.
 *
 * Deliberately narrower than the set the outer catch in publishSyncedEvents() tests: it covers only
 * "the connection went away", which ends the pass anyway and clears itself on the reconnect. A
 * transient network code is not obviously self-correcting, and because that loop always reads the
 * head of the queue, a message failing the same way on every pass would block every message behind
 * it in the folder for good.
 *
 * Work earlier in the pass does run twice when a later step re-queues the message: the seen-messages
 * HyperLogLog (so the retry reports `seemsLikeNew: false`, an informational field) and, the
 * expensive one, the LLM summary and embeddings, which sit just before the inline-image download.
 * All of it needs the connection to
 * die in one narrow window, and a re-queue was already reachable from every error thrown outside
 * these catches
 * @param {Error} err - The error to check
 * @returns {Boolean} True when the connection, not the message, is what failed
 */
function isConnectionGone(err) {
    return !!err && CONNECTION_CLOSING_CODES.has(err.code);
}

/**
 * Compact fingerprint of the STATUS counters stored in the phantom folder
 * marker - a parked phantom folder is re-probed as soon as this changes
 * @param {Object} status - STATUS response counters
 * @returns {String} Fingerprint string
 */
function phantomStatusKey(status) {
    return `${status.uidValidity || ''}:${status.uidNext || 0}:${status.messages || 0}`;
}

/**
 * Headers the notification path needs for its own processing, whatever the notifyHeaders
 * setting asks for. They travel in the one BODY.PEEK[HEADER.FIELDS (...)] section that path
 * already requests, and are filtered back down before anything is published: processNew()
 * narrows the exposed set to what the caller asked for once its own checks have run, and
 * Webhooks.formatPayload() drops the block entirely unless notifyHeaders names something.
 */
const NOTIFICATION_HEADERS = [
    ...
    // Bounce and complaint detection
    'content-type',

    // Fallback for an ENVELOPE that carries no usable address. Lark Mail omits the address
    // fields outright, and any server can mis-parse a malformed header into a display name
    // with no address - see preferAddressList()
    'from',
    'to',
    'cc',
    'bcc'
...
/**
 * Builds the header field list for a notification fetch.
 *
 ...
 */
function notificationHeaderFields(configuredHeaders) {
    let fetchHeaders = new Set(Array.isArray(configuredHeaders) ? configuredHeaders : []);

    for (let key of NOTIFICATION_HEADERS) {
...
// Runs for every address of every message, so both patterns are built once
const HAS_WHITESPACE = /\s/;
// An addr-spec that carries its whitespace legally, inside a quoted local part
const QUOTED_LOCAL_ADDR = /^"(?:[^"\\]|\\[\s\S])*"@\S+$/;

/**
 * Tells whether an address is usable as a mailbox.
 *
 * A server that failed to parse a malformed header does not always report the wreckage as
 * an empty address - Gmail hands back "user@example.com user@example.com" when the header
 * separated the two copies with a non-breaking space. Unquoted whitespace is not addr-spec,
 * so a value carrying it is no more usable than an empty one, and the raw header is the
 * better source. Whitespace inside a quoted local part is legal and stays usable.
 *
 * @param {String} address - Address to check
 * @returns {Boolean} True if the address can be used as a mailbox
 */
function isUsableAddress(address) {
    ...
    return !HAS_WHITESPACE.test(address) || QUOTED_LOCAL_ADDR.test(address);
}

/**
 * Parses a raw address header into the address objects the API exposes.
 *
 * @param {String[]} values - Raw header values for one address field
 * @returns {Object[]|undefined} Parsed addresses, or undefined if none could be read
 */
function parseAddressHeader(values) {
    if (!values?.length) {
        return undefined;
    }

    try {
        let addressList = addressparser(values)
            .filter(address => isUsableAddress(address.address))
            .map(address => {
        ...
        return addressList.length ? addressList : undefined;
    } catch (err) {
        // a header we cannot read is worth no more than one that is not there
        return undefined;
    }
}

/**
 * Picks the address list to expose for a header, preferring the server's ENVELOPE.
 *
 * An ENVELOPE list can come back holding no address anyone could use: RFC 9051 7.5.2 group
 * markers carry a group name and nothing else, and a server that failed to parse a malformed
 * header reports the wreckage in the shapes isUsableAddress() turns down. The raw header is
 * the better answer then, because addressparser sees the original value and can still recover
 * the recipient. A list has to be judged on what its entries hold, not on being non-empty.
 *
 * The header is parsed only when the ENVELOPE turns out to be unusable, which is rare.
 * Parsing all four address fields up front would be work discarded on every message.
 *
 * @param {Object[]} envelopeList - Address list from the IMAP ENVELOPE
 * @param {Object} headers - Decoded message headers, if any were fetched
 * @param {String} key - Header name to fall back to
 * @returns {Object[]|undefined} The list to expose, or undefined if neither source has one
 */
function preferAddressList(envelopeList, headers, key) {
    if (envelopeList?.some(entry => isUsableAddress(entry.address))) {
        return envelopeList;
    }

    let headerList = parseAddressHeader(headers?.[key]);
    if (headerList?.length) {
        return headerList;
    }

    // Nothing usable anywhere - keep the ENVELOPE entries so a group name still shows up
    return envelopeList?.length ? envelopeList : undefined;
}
...
class Mailbox {
    constructor(connection, entry) {
        ...
        this.redisKey = mailboxPathHash(this.path);

        // Built once, through the shared helper: Account.getMailboxListing() reads these counters
        // back through the same helper and the two have to agree byte for byte, but the key is
        // also read several times per folder on every sync pass and rebuilding it would re-run
        // the SHA1 + BigInt + base36 derivation every time.
        this.mailboxStatusKey = getMailboxStatusKey(this.connection.account, this.path);

        this.runPartialSyncTimer = false; // Timer for delayed partial sync after EXISTS

        this.synced = false; // Resolver of the sync() promise while a pass is waiting for its SELECT
        this.syncChain = null; // Promise of the latest sync() pass, which the next caller waits for
        this.syncing = false; // Whether currently syncing
        this.processingOpen = false; // Whether onOpen() is currently running for this mailbox
        this.processingOpenClient = null; // The IMAP client whose open event holds the processingOpen latch
    ...
    /**
     * The connection's IMAP client, or the shared connection-closed throw.
     * `connection.imapClient` is null between a teardown and the
     * next connect, and a sync pass already in flight reaches these calls in that window - which
     * surfaced as `Cannot read properties of null (reading 'downloadMany')` in the error log and in
     * Sentry, and, because a TypeError carries no code, got the message announced with its inline
     * images stripped instead of re-queued.
     *
     * Named apart from the `connectionClient` parameter that getMailboxStatus() and
     * getMailboxLock() bind, and from IMAPClient.getImapConnection(), which answers a bigger
     * question (pooled or secondary connections) that these callers have no business asking
     * @returns {Object} The live ImapFlow client
     */
    requireImapClient() {
        if (!this.connection.imapClient) {
            throw connectionNotAvailableError();
        }
        return this.connection.imapClient;
    }
    ...
    getMailboxStatus(connectionClient) {
        connectionClient = connectionClient || this.requireImapClient();
        ...
        if (!samePath(mailboxInfo.path, this.path)) {
            // The counters would describe whatever folder the connection has open. Persisted
            // as this folder's state, the next open would read them as a UIDVALIDITY change
            // and rebuild the index. Whether the mismatch is a problem is the caller's call
            this.logger.debug({ msg: 'IMAP connection has a different mailbox selected', path: this.path, selectedPath: mailboxInfo.path });
            let err = new Error('Requested mailbox is not selected');
            err.code = 'MailboxNotSelected';
            err.statusCode = 503;
            throw err;
        }

        return this.mailboxStatusFromInfo(mailboxInfo, connectionClient);
    }

    /**
     * Normalizes an ImapFlow mailbox object into the status shape the sync state machine compares
     * @param {Object} mailboxInfo - ImapFlow mailbox object: the selected mailbox, or the mailboxOpen event payload
     * @param {Object} [connectionClient] - IMAP client the object came from, for diagnostics
     * @returns {Object} Status object with path, highestModseq, uidValidity, uidNext, messages
     */
    mailboxStatusFromInfo(mailboxInfo, connectionClient) {
        let status = {
            path: this.path
        };
    ...
    async getStoredStatus() {
        ...
        // Log diagnostic info if the stored status exists but its uidValidity is invalid
        // or missing. A folder that has never been synced has no stored state at all,
        // which is expected and not worth a warning
        if (hasStoredState && !validUidValidity(data.uidValidity)) {
            this.logger.warn({
    ...
    async packUidWithLogging(uid, notificationType) {
        ...
        if (!packedUid) {
            this.logger.warn({
                msg: 'Skipping notification, packUid returned invalid result',
                notificationType,
                uid,
                path: this.path
            });
    ...
    isSelected() {
        return (
            this.selected && this.connection.imapClient && this.connection.imapClient.mailbox && samePath(this.connection.imapClient.mailbox.path, this.path)
        );
    }

    /**
     * Settles the sync() pass waiting for this mailbox to be opened, if one armed a
     * resolver. Calling it twice is harmless - the second call lands on a settled promise
     */
    settleSyncPass() {
        if (this.synced) {
            this.synced();
    ...
    async clear(opts) {
        ...
        // Dropping the folder from the stored listing is also the claim to announce it. That
        // hash is the only record that the folder was ever listed, and HDEL reports whether
        // this call is the one that removed it, so exactly one caller announces a deletion.
        // Every announcement is otherwise made off a listing snapshot: deleteMailbox() clears
        // the folder while the stored listing still holds it, so the next getCurrentListing()
        // finds it missing from the server and announces it a second time, and two overlapping
        // passes that read the hash before either wrote it announce it once each.
        //
        // One transaction, so the claim can not outlive the teardown it guards: with the
        // listing entry gone and the index behind it left in place, nothing enumerates the
        // folder again - it would neither be cleared nor announced by any later pass, and a
        // folder recreated under the same name would sync against the stale index.
        let result = await this.connection.redis
            .multi()
            .hdel(this.connection.getMailboxListKey(), normalizedPath)
            .del(this.getMailboxKey(), this.getMessagesKey(), this.getNotificationsKey())
            .exec();
        ...
        let removedFromListing = !!result[0][1];

        // Removing the folder from the connection's own map is the other half of that claim, and
        // just as one-shot. A folder can be tracked here while its listing entry is already gone:
        // processListing() registers a Mailbox without writing the hash, and a pass that had to
        // defer the entry (no primary connection) or purged it as corrupt writes the hash back
        // without it. Gating on the HDEL alone, a folder in that state was torn down in silence
        // and no later pass could announce it either - the deleted entries a pass reports come
        // from the very hash the folder is missing from.
        const removedFromTracking = this.connection.mailboxes.delete(normalizedPath);

        this.logger.debug({ msg: 'Deleted mailbox', path: this.listingEntry.path });

        if (!opts.skipNotify && (removedFromListing || removedFromTracking)) {
            this.connection
    ...
    /**
     * Syncs the mailbox, running overlapping calls one after another. A pass awaits a
     * promise with a single resolver slot (this.synced): a second call arming it while
     * the first still waits for its SELECT would take the slot over, and the first
     * caller - the resync loop, which re-arms its timer only once every folder has
     * returned - would never be settled. Calls overlap whenever a subconnection reports
     * a change or a listing refresh registers folders during a resync pass
     * @param {Boolean} forceEmpty - Force sync even if mailbox appears unchanged
     * @returns {Boolean} True if synced
     */
    async sync(forceEmpty) {
        // A rejected pass was reported to its own caller; the next pass runs regardless
        const current = (this.syncChain || Promise.resolve()).catch(() => {}).then(() => this.syncOnce(forceEmpty));
        this.syncChain = current;
        return await current;
    }

    /**
     * One sync pass: compares the STATUS counters with the stored state and selects the
     * mailbox when they drifted, which runs the state machine in onOpen()
     * @param {Boolean} forceEmpty - Force sync even if mailbox appears unchanged
     * @returns {Boolean} True if synced
     */
    async syncOnce(forceEmpty) {
        ...
        try {
            ...
            if (err.code === 'NotFound') {
                // Folder is missing, refresh the folder listing and process the
                // result so deletions get cleared and newly found folders are
                // registered and synced (emitting their mailboxNew notification)
                await this.connection.refreshAndProcessListing();
                return;
            } else {
                throw err;
            }
        ...
        // Re-check after the awaits above: a concurrent operation may have
        // selected this mailbox while we were reading STATUS / stored state.
        // In that case onOpen() owns the sync work, and select() below
        // would skip the SELECT without another mailboxOpen event ever firing
        if (this.selected || !this.connection.imapClient) {
            // expect current folder to be already synced
            return false;
        }

        // A phantom folder (listed but repeatedly rejecting SELECT, e.g. a
        // Dovecot shared namespace root without \Noselect) is treated like
        // \Noselect instead of re-attempting the SELECT on every sync pass -
        // on some servers the failed SELECT poisons the session, which then
        // gets killed, tearing down the whole account connection each cycle.
        // The folder is re-probed once the interval passes, right away if its
        // STATUS counters changed (something appeared in the folder), and the
        // marker is cleared on a successful SELECT
        let phantomState = await this.getPhantomState();
        if (
            phantomState &&
            phantomState.count >= PHANTOM_SELECT_FAIL_THRESHOLD &&
            phantomState.statusKey === phantomStatusKey(status) &&
            Date.now() - phantomState.time < PHANTOM_REPROBE_INTERVAL
        ) {
            this.logger.debug({
                msg: 'Skipped select of a phantom mailbox',
                path: this.path,
                selectFailures: phantomState.count,
                lastFailure: new Date(phantomState.time).toISOString()
            });
            return;
        }
        ...
        let syncedPromise = new Promise((resolve, reject) => {
            this.synced = resolve;
            this.select().catch(err => reject(err));
        });
        ...
        try {
            ...
            if (err.mailboxMissing) {
                // The folder was deleted after it was listed (e.g. by another client
                // between LIST and SELECT). Refresh the listing right away so the
                // deletion gets processed instead of waiting for the next resync cycle,
                // and process the result so folders that appeared in the same change
                // (e.g. a rename) are registered and emit their mailboxNew notification.
                // A failing refresh is a connection-level error and propagates, same
                // as in the NotFound branch above.
                this.logger.info({ msg: 'Mailbox is missing, refreshing folder listing', path: this.path });
                await this.connection.refreshAndProcessListing();
                return;
            }
            if (err.responseStatus === 'NO') {
                // The folder is listed but can not be selected. Either a permanent
                // phantom folder (e.g. a shared namespace root that the server lists
                // without \Noselect) or a transient RFC 5530 condition like INUSE -
                // both are skipped and retried on the next resync cycle instead of
                // failing the entire account sync. Network-level errors do not have
                // responseStatus set and keep propagating.
                this.logger.warn({
                    msg: 'Skipped mailbox that can not be selected',
                    path: this.path,
                    serverResponseCode: err.serverResponseCode,
                    responseText: err.responseText
                });
                // Count the failure so a permanent phantom stops being probed on
                // every sync pass once it hits PHANTOM_SELECT_FAIL_THRESHOLD
                await this.setPhantomState(phantomState, status);
                return;
            }
            throw err;
        } finally {
            // Settled either way, so the slot is free for the next pass. select() and
            // onOpen() call the resolver only while one is armed
            this.synced = false;
        }

        if (phantomState) {
            // the folder can be selected again, drop the failure marker
            await this.clearPhantomState();
        }
    }

    /**
     * Reads the phantom folder marker: how many times in a row a SELECT of
     * this listed folder has been rejected, when the last rejection happened,
     * and a fingerprint of the STATUS counters observed at that time
     * @returns {Object|false} Marker object {count, time, statusKey} or false
     */
    async getPhantomState() {
        let raw = await this.connection.redis.hget(this.getMailboxKey(), 'phantomState');
        if (!raw) {
            return false;
        }
        try {
            let value = JSON.parse(raw);
            if (value && typeof value.count === 'number' && typeof value.time === 'number') {
                return value;
            }
        } catch (err) {
            // corrupted marker, treat as unset
        }
        return false;
    }

    /**
     * Records a failed SELECT of a listed folder
     * @param {Object|false} phantomState - Previous marker from getPhantomState()
     * @param {Object} status - STATUS counters from the failing sync pass
     */
    async setPhantomState(phantomState, status) {
        try {
            await this.connection.redis.hset(
                this.getMailboxKey(),
                'phantomState',
                JSON.stringify({
                    count: (phantomState ? phantomState.count : 0) + 1,
                    time: Date.now(),
                    statusKey: phantomStatusKey(status)
                })
            );
        } catch (err) {
            // failing to persist the marker must not mask the skip handling
            this.logger.error({ msg: 'Failed to store phantom folder marker', path: this.path, err });
        }
    }

    /**
     * Removes the phantom folder marker
     */
    async clearPhantomState() {
        await this.connection.redis.hdel(this.getMailboxKey(), 'phantomState');
    }

    /**
     * Selects the mailbox on the primary connection, so IDLE watches it. Taking the
     * lock is the SELECT: ImapFlow opens the path before granting, so once granted
     * the mailbox is open and the lock is given straight back - its release re-arms
     * auto-IDLE, and holding it any longer would only queue the other operations
     */
    async select() {
        // Captured before the await: this is the pass select() was called for, and by the
        // time it returns the slot can hold the resolver of a later one
        const settle = this.synced;

        if (!this.isSelected()) {
            // getMailboxLock() throws when there is no connection to select on
            let lock = await this.getMailboxLock(null, { description: `Select mailbox: ${this.path}` });
            lock.release();
        }

        // An open handler settles the pass itself, once its sync work is done - which
        // connection it belongs to does not matter, the resolver is per mailbox. With none
        // running, no mailboxOpen event is coming for this already-open mailbox and the
        // pass would wait forever
        if (settle && !this.processingOpen) {
            settle();
        }
    }
     ...
     *
     * A lock on the primary connection takes it away from the mailbox it watches, so the
     * pending return to the main mailbox is cancelled here and re-armed by the returned
     * release(). Both halves live in this one place on purpose: when each holder had to
     * report the task itself, the sync operations never did, and an API command's SELECT
     * whose open handler ran a sync left the connection parked on that folder until the
     * next resync pass. A lock on a pooled secondary never moved the primary and reports nothing
    ...
    async getMailboxLock(connectionClient, options) {
        ...
        return {
            ...
            release: () => {
                if (released) {
                    // ImapFlow ignores a repeated release; a repeated report would arm a timer for nothing
                    return;
                }
                released = true;
                lock.release();
    ...
    logEvent(msg, event) {
        ...
        Object.keys(logObj).forEach(key => {
            ...
            if (logObj[key] && typeof logObj[key].has === 'function') {
    ...
    async onExists(event) {
        ...
        if (this.syncDisabled) {
            // An API command opened a folder that is excluded from syncing. Its untagged
            // responses arrive like any other, but nothing in it may be indexed or advertised
            return;
        }
        ...
        this.runPartialSyncTimer = setTimeout(() => {
            ...
            this.shouldRunPartialSyncAfterExists()
                ...
                .catch(err => {
                    if (CONNECTION_CLOSING_CODES.has(err.code)) {
                        this.logger.debug({ msg: 'Sync skipped, connection closing', err });
                    } else {
                        this.logger.warn({ msg: 'Failed to sync mailbox after EXISTS', err });
                    }
                });
        }, 1000);
    }
    ...
    async onExpunge(event) {
        ...
        if (imapIndexer !== 'full' || this.syncDisabled) {
    ...
    async shouldRunPartialSyncAfterExists() {
        let storedStatus = await this.getStoredStatus();
        if (!this.isSelected()) {
            // An API command opened another folder while the stored state was read. The
            // connection now reports that folder's counters; the drift this folder
            // accumulated is picked up by the state machine when it is opened again
            return false;
        }
    ...
    /**
     * Processes a deleted message and sends the notification
     * @param {Object} messageData - Deleted message data
     */
    async processDeleted(messageData) {
        this.logger.debug({ msg: 'Message deleted', uid: messageData.uid });
        ...
        try {
            ...
            this.logger.error({ msg: 'Failed to remove deleted message from notifications set', uid: messageData.uid, err });
        }
    }

    /**
     * Downloads one attachment's body and stores it on the attachment as base64. Three enrichment
     * passes need this - the notifyAttachments loop, the inline images referenced from the HTML,
     * and calendar parts - and each carried its own copy of the decode, the download and the error
     * handling.
     *
     * A gone connection is rethrown so processNew() re-queues the message rather than announcing it
     * with the content missing; anything else is this attachment's problem and is logged. A
     * zero-length body leaves `content` unset rather than storing an empty string, which is what
     * the calendar copy always did and what the other two meant to
     * @param {Object} messageInfo - The message being enriched
     * @param {Object} attachment - Attachment entry to fill in
     * @param {Object} options - Processing options, for the download chunk size
     * @returns {Promise<void>}
     */
    async loadAttachmentContent(messageInfo, attachment, options) {
        try {
            let buf = Buffer.from(attachment.id, 'base64url');
            let part = buf.subarray(8).toString();

            let { content: downloadStream } = await this.requireImapClient().download(messageInfo.uid, part, {
                uid: true,
    ...
    async processNew(messageData, options, canSync, storedStatus) {
        ...
        if (this.previouslyConnected && !samePath(this.path, 'INBOX') && !this.isAllMail && storedStatus.initialUidNext > messageData.uid) {
            this.logger.debug({
        ...
        if (this.mightBeAComplaint(messageInfo)) {
            try {
                ...
                for (let attachment of messageInfo.attachments || []) {
                    ...
                    let { content: sourceStream } = await this.requireImapClient().download(messageInfo.uid, part, {
                        uid: true,
        ...
        if (this.mightBeDSNResponse(messageInfo)) {
            try {
                let { content: sourceStream } = await this.requireImapClient().download(messageInfo.uid, false, {
                    uid: true,
                    chunkSize: options.chunkSize,
                    maxBytes: MAX_ALLOWED_DOWNLOAD_SIZE
                });
                ...
                if (parsed) {
                    content = { parsed };
                    applyDeliveryReport(parsed, messageInfo, this.logger);
                }
            } catch (err) {
                if (isConnectionGone(err)) {
                    throw err;
        ...
        if (this.mightBeABounce(messageInfo)) {
            ...
            try {
                if (!content) {
                    let result = await this.requireImapClient().download(messageInfo.uid, false, {
                        uid: true,
                        chunkSize: options.chunkSize,
                        maxBytes: MAX_ALLOWED_DOWNLOAD_SIZE
                    });
        ...
        // Narrow the headers to what the caller asked for. This runs after the complaint, delivery
        // report and bounce checks above: they read headers fetched for them (see
        // notificationHeaderFields()) that the published payload may not carry
        ...
        if (
            ...
            for (let category of ['primary', 'social', 'promotions', 'updates', 'forums', 'reservations', 'purchases']) {
                try {
                    let results = await this.requireImapClient().search(
                        {
                            uid: messageInfo.uid,
                    ...
                    this.logger.warn({ msg: 'Failed to resolve category for message', err, category, uid: messageData.uid, id: messageInfo.id });
                    break;
                }
            }
        }
        ...
        if (notifyAttachments && messageInfo.attachments?.length) {
            for (let attachment of messageInfo.attachments || []) {
                ...
                if (!attachment.content) {
                    await this.loadAttachmentContent(messageInfo, attachment, options);
                }
            }
        }
        ...
        if (messageInfo.attachments?.length && messageInfo.text?.html) {
            ...
            let cidReferences = collectCidReferences(messageInfo.text.html);
            for (let attachment of messageInfo.attachments || []) {
        ...
        this.setMessageSpecialUse(messageInfo);
        ...
        if (notifyWebSafeHtml && messageInfo.text && (messageInfo.text.html || messageInfo.text.plain)) {
            // convert to web safe
            messageInfo.text._generatedHtml = await messageWebSafeHtml(messageInfo);
            ...
            if (messageInfo.text.html && messageInfo.attachments) {
                ...
                if (attachmentList.size) {
                    if (partList.length) {
                        ...
                        try {
                            let contentParts = await this.requireImapClient().downloadMany(messageInfo.uid, partList, {
                                uid: true
                            });
                            ...
                            this.logger.error({ msg: 'Failed to download attachments', uid: messageInfo.uid, partList, err });
                        }
                    }

                    // Replace CID references with data URIs
                    messageInfo.text.html = messageInfo.text.html.replace(cidReferenceRegex(), (fullMatch, cidMatch) => {
    ...
    /**
     * Sets the special use of the folder a message belongs to: the mailbox's own, or for Gmail
     * over IMAP the label, since the message lives in All Mail there. Junk wins over Inbox
     * @param {Object} messageInfo - Formatted message, updated in place
     */
    setMessageSpecialUse(messageInfo) {
    ...
    async getMessageInfo(messageData, extended) {
        ...
        // The address fields fall back to these when the ENVELOPE carries no usable address,
        // which Lark Mail does by omitting them and any server can do by mis-parsing a
        // malformed header. See preferAddressList(). Normally only the ENVELOPE is used, and
        // a message listing fetches no headers at all
        let headers;
        if (messageData.headers) {
            headers = decodeHeaders(messageData.headers.toString().trim());
        }
        ...
        const result = {
            ...
            // Prefer the ENVELOPE, fall back to the raw header when it holds no usable address
            from: preferAddressList(envelope.from, headers, 'from')?.[0] || undefined,
            ...
            to: preferAddressList(envelope.to, headers, 'to'),
            cc: preferAddressList(envelope.cc, headers, 'cc'),

            bcc: extended ? preferAddressList(envelope.bcc, headers, 'bcc') : undefined,
     ...
     * Delegates to SyncOperations, which reads the stored state itself under the lock
     */
    async fullSync() {
     ...
     */
    async partialSync() {
        return this.syncOps.partialSync();
    }
    ...
    async publishSyncedEvents(storedStatus) {
        ...
        // Also request the headers this path needs for its own processing. Skipped when the
        // setting already asks for every header, since there is nothing left to add
        if (messageFetchOptions.headers !== true) {
            messageFetchOptions.fetchHeaders = notificationHeaderFields(messageFetchOptions.headers);
        }
        ...
        while ((queuedEntry = await this.connection.redis.zrange(notificationsKey, 0, 0, 'WITHSCORES')) && queuedEntry.length) {
            ...
            try {
                ...
                // Wider than the enrichment steps on purpose: this is the last chance to keep a
                // message that failed on a transient network error, and it is not a step that can
                // re-run the same failure indefinitely - the pass ends here either way
                if (isConnectionGone(err) || TRANSIENT_NETWORK_CODES.has(err.code)) {
                    throw err;
                }
                await this.connection.redis.zrem(notificationsKey, rawMember);
     ...
     */
    async onOpen(event, openedClient) {
        clearTimeout(this.runPartialSyncTimer);
        this.selected = true;

        // The latch is scoped to the connection that emitted the open event.
        // A re-entrant event from the SAME connection must bail out, but an
        // event from a NEW connection must not be swallowed while a previous
        // connection's handler is still unwinding (its Redis-heavy phases keep
        // running after the socket died) - settling the fresh resolver here
        // would mark the post-reconnect sync as done without any sync work
        openedClient = openedClient || this.connection.imapClient;

        if (this.processingOpen && this.processingOpenClient === openedClient) {
            // Re-entrant open event: a sync operation running inside this very
            // handler (e.g. seedMailboxIndex acquiring its mailbox lock)
            // re-SELECTed this mailbox. Running the state machine again from
            // here recurses - on a server where the seeding FETCH keeps
            // failing it becomes an endless reseed storm of back-to-back
            // SELECTs. The outer invocation owns the sync work; settle an
            // armed sync() resolver and bail out
            this.settleSyncPass();
            return false;
        }
        // Take the latch (over) - if a previous connection's handler is still
        // unwinding, it detects the new owner in its finally and backs off
        this.processingOpen = true;
        this.processingOpenClient = openedClient;

        // Assigned inside the try - everything that can throw must run there,
        // because the finally below is the sole resolver of the sync() promise
        // (this.synced). A throw before the try would skip the finally and
        // permanently wedge the awaiting sync()
        let mailboxStatus;
        ...
        try {
            ...
            // The event payload is the mailbox object of the SELECT that triggered this
            // handler, frozen once another folder is selected - the snapshot this decision
            // wants, whatever an API command opens on the connection during the await above
            mailboxStatus = event ? this.mailboxStatusFromInfo(event, openedClient) : this.getMailboxStatus();

            if (this.syncDisabled) {
                // Excluded from syncing but opened by an API command: refresh the listing
                // counters as syncOnce() does and index nothing
                await this.updateStoredStatus(mailboxStatus);
                return false;
            }

            // Read outside the mailbox lock, so it only decides which sync to run. The sync
            // itself reads the stored state again once it holds the lock: a pass ahead of it in
            // the queue (an EXISTS debounce, a previous connection's open still unwinding) has
            // moved uidNext on by then, and re-fetching the range it already handled is wasted
            // work on every folder the account has
            let storedStatus = await this.getStoredStatus();
            ...
            // Check for UIDVALIDITY change (mailbox recreated). Checked ahead of the
            // queued notifications: those reference UIDs of the previous incarnation,
            // and the reseed below drops them together with the index
            ...
            if (err.mailboxMissing) {
                ...
                try {
                    ...
                    this.logger.error({ msg: 'Failed to refresh listing after missing mailbox', err: E });
                }
            }
            throw err;
        } finally {
            if (this.processingOpenClient !== openedClient) {
                // A newer connection's open handler took the latch over while
                // this invocation was unwinding. The new owner runs the actual
                // sync now, so leave the latch, the isNew notification and the
                // sync promise settlement to it - settling here would resolve
                // the new owner's freshly armed promise before its work ran
                this.logger.debug({ msg: 'Mailbox open handler superseded by a newer connection', path: this.path });
            } else {
                this.processingOpen = false;
                this.processingOpenClient = null;

                // Send new mailbox notification if this is the first sync. When the
                // open failed before the mailbox state could be read, mailboxStatus
                // is undefined - keep isNew set so the notification is emitted on
                // the next successful open instead of sending a broken payload now
                if (this.listingEntry.isNew && mailboxStatus) {
                ...
                // IDLE needs no restart: ImapFlow re-arms auto-IDLE on every lock release
                this.settleSyncPass();
            }
        }
    }
    ...
    // User methods
    // Every method takes its lock through getMailboxLock(), whose release() reports the task
    // to the connection - nothing else has to be called once the lock is given back
    ...
    async getText(message, textParts, options, connectionOptions) {
        ...
        try {
            for (let part of textParts) {
                ...
                // Group by content type (plain, html, etc). The subtype comes from the message,
                // so a sender picks it: for "text/__proto__" the `!result[typeKey]` probe below
                // finds Object.prototype rather than an array, skips the initializer and then
                // throws on .reduce() - the same shape lib/utils/header-map.js exists to guard.
                // Neither name is a real MIME subtype, so the part is grouped as plain text
                // instead of being dropped.
                let typeKey = (meta.contentType && meta.contentType.split('/')[1]) || 'plain';
                if (isUnsafeKey(typeKey)) {
                    typeKey = 'plain';
                }
                if (!result[typeKey]) {
                    result[typeKey] = [];
            ...
            // Paired with the lock: a FETCH that throws still selected a mailbox on this
            // connection, so it still has to be sent back to the one it is watching.
            if (lock) {
                lock.release();
            }
        }
    ...
    async getAttachment(message, part, options, connectionOptions) {
        ...
        try {
            ...
            // Both halves are required: meta describes the part, content is what finished() is
            // handed below, and getText() already guards the same pair
            if (!meta || !content) {
                return false;
            }
            ...
            // The lock is held for the lifetime of the stream, so it can only be given back once the
            // stream is done - by any route. Enumerating those routes by hand is what let a consumer
            // that simply destroys the stream hold the lock forever: an abort emits neither 'end' nor
            // 'error', only 'close'. finished() covers end, error, destroy and premature close by
            // contract, and fires exactly once, so there is no set of terminal events left to get
            // wrong. However the download ended, the release sends the connection back to the
            // mailbox it is watching.
            finished(content, () => {
                lock.release();
            });

            // Only once finished() has taken over. Set before the call, a synchronous throw from it
            // - anything not a stream reaching this far - would leave the finally below believing
            // the stream owns the lock, and nothing would ever release it.
            streaming = true;
    ...
    async getMessage(message, options, connectionOptions) {
        ...
        try {
            ...
            if (options.markAsSeen && !messageData.flags?.has('\\Seen')) {
                try {
                    let res = await connectionClient.messageFlagsAdd(message.uid, ['\\Seen'], { uid: true });
                    // Mirror the flag locally so the returned message reflects it. A
                    // caller-provided field list can leave FLAGS out and a server can
                    // answer the FETCH without them - in that case there is nothing to
                    // mirror into, and inventing a set here would report the message as
                    // carrying \Seen and no other flag
                    if (res && messageData.flags) {
                        messageData.flags.add('\\Seen');
                    }
                } catch (err) {
                    this.logger.debug({ msg: 'Failed to mark message as Seen', uid: message.uid, err });
                }
            }
            ...
            if (messageInfo) {
                ...
                // The details route asks for the content check; its download rides on this lock
                // instead of taking one of its own, which on the primary connection is a SELECT
                // away from the watched folder and back
                if (options.detectBounce) {
                    await this.connection.detectBounce(messageInfo, async () => {
                        let { content } = await connectionClient.download(message.uid, false, {
                            uid: true,
                            maxBytes: MAX_ALLOWED_DOWNLOAD_SIZE,
                            chunkSize: options.chunkSize
                        });
        ...
        if (options.preProcessHtml && messageInfo.text && (messageInfo.text.html || messageInfo.text.plain)) {
            messageInfo.text.html = await messageWebSafeHtml(messageInfo);
            messageInfo.text.webSafe = true;
        }
        ...
        if (options.embedAttachedImages && messageInfo.text && messageInfo.text.html && messageInfo.attachments) {
            ...
            if (partList.length) {
                let lock;
                try {
                    // The lock taken for the FETCH above is released by now. Without one
                    // another task can select a different mailbox on this connection first,
                    // and the UID would then address whatever message carries it there
                    if (!options.skipLock) {
                        lock = await this.getMailboxLock(connectionClient, { description: `Download inline images: ${message.uid}` });
                    }
    ...
    async deleteMessage(message, force, connectionOptions) {
        ...
        try {
            ...
            if (['\\Trash', '\\Junk'].includes(this.listingEntry.specialUse) || force) {
                ...
                if (!trashMailbox || samePath(trashMailbox.path, this.path)) {
    ...
    async listMessages(options, connectionOptions) {
        ...
        try {
            ...
            // This stays provider-gated, unlike the notification path: a listing fetches no
            // headers at all otherwise, so asking for them here adds a BODY.PEEK section to
            // every message of every page. Lark needs it because its ENVELOPE omits addresses
            // outright; for everyone else a mis-parsed header still yields a display name, and
            // paying for it on every listing is not worth the rare recovery.
            if (this.isLarkSuite) {
    ...
    async markUpdated() {
        try {
            ...
            this.logger.error({ msg: 'Failed to update sync timestamp', err });
        }
    }

    /**
     * Checks if a newly arrived message might be a bounce: one in the Inbox or Junk folder (or, on
     * Gmail, carrying that label) that was not already read as a delivery report, and that looks
     * like a bounce. The shape check is the shared one, so IMAP and the API clients agree on it
    ...
    mightBeABounce(messageInfo) {
        ...
        return this.connection.looksLikeABounce(messageInfo);
    }

    /**
     * The Inbox-only gate (the Inbox label on Gmail) for the shared complaint shape check
     * @param {Object} messageInfo - Message information
     * @returns {Boolean} True if likely a complaint
     */
    mightBeAComplaint(messageInfo) {
        if (!samePath(this.path, 'INBOX') && !(this.isAllMail && messageInfo.labels && messageInfo.labels.includes('\\Inbox'))) {
            return false;
        }

        return this.connection.looksLikeAComplaint(messageInfo);
    }

    /**
     * The Inbox-only gate (the Inbox label on Gmail) for the shared delivery-report shape check
     * @param {Object} messageInfo - Message information
    ...
    mightBeDSNResponse(messageInfo) {
        ...
        return this.connection.looksLikeDSNResponse(messageInfo);
    }
    ...
    decodeCursorStr(cursorStr) {
        ...
        if (cursorStr) {
            ...
            try {
                ...
                // Well-formed but not a cursor this client issued - reported like a broken one
                throw new Error('Cursor does not carry a page number');
            } catch (err) {
                this.logger.warn({ msg: 'Failed to parse paging cursor', cursorStr, err });
...
// Exported for tests: publishSyncedEvents() cannot be driven without a live connection
module.exports.notificationHeaderFields = notificationHeaderFields;
