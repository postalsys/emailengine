...
class Mailbox {
    ...
    /**
     * The connection's IMAP client, or a throw that reads like every other "the connection went
     * away" failure. Named apart from the `connectionClient` parameter two methods here already
     * bind, and from IMAPClient.getImapConnection(), which answers the same question for the
     * pooled/secondary cases this one has no business picking between. `connection.imapClient` is null between a teardown and the next connect, and
     * a sync pass already in flight reaches these calls in that window - which surfaced as
     * `Cannot read properties of null (reading 'downloadMany')` in the error log and in Sentry,
     * and, because a TypeError carries no code, got the message announced with its inline images
     * stripped instead of re-queued
     * @returns {Object} The live ImapFlow client
     */
    requireImapClient() {
        if (!this.connection.imapClient) {
            let error = new Error('IMAP connection is closed');
            error.code = 'NoConnection';
            // Matches what IMAPClient.getImapConnection() and the two inline throws in this file
            // report, so the same failure is a 503 wherever it surfaces rather than a 500
