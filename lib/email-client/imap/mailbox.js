    ...
    async publishSyncedEvents(storedStatus) {
        ...
        while ((queuedEntry = await this.connection.redis.zpopmin(this.getNotificationsKey(), 1)) && queuedEntry.length) {
            ...
            try {
                ...
                // Connection-level errors should abort the loop
                if (err.code === 'NoConnection' || err.code === 'IMAPConnectionClosing' || err.code === 'EConnectionClosed') {
                    throw err;
                }
        ...
        try {
            ...
            if (this.synced) {
                ...
                try {
                    ...
                    this.logger.debug({ msg: 'Failed to select after sync', err });
                }
            }
        }
    }
