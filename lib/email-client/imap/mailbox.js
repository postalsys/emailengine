...
class Mailbox {
    ...
    async clear(opts) {
        ...
        this.connection.mailboxes.delete(normalizedPath);

        this.logger.debug({ msg: 'Deleted mailbox', path: this.listingEntry.path });

        if (!opts.skipNotify && removedFromListing) {
            this.connection
     ...
     */
    async fullSync(storedStatus) {
        return this.syncOps.fullSync(storedStatus);
    }
    ...
    async onOpen(event, openedClient) {
        ...
        try {
            ...
            if (hasQueuedNotifications) {
                return await this.fullSync(storedStatus);
            }
