...
class Mailbox {
    ...
    // User methods
    // getMailboxLock() clears connection.completedTimer for the primary connection
    // Call this.connection.onTaskCompleted(connectionClient) after the selected mailbox is processed and the lock is released
    ...
    async getAttachment(message, part, options, connectionOptions) {
        ...
        try {
            ...
            // wrong. Reporting the task either way is what every other method here does: however the
            // download ended, the connection has to go back to the mailbox it is watching.
            finished(content, () => {
                lock.release();
                this.connection.onTaskCompleted(connectionClient);
            });
