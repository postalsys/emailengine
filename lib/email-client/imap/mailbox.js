...
class Mailbox {
    ...
    async syncOnce(forceEmpty) {
        ...
        // In that case onOpen() owns the sync work, and select(true) below
    ...
    async select(skipIdle) {
        ...
        if (this.isSelected()) {
            // Already watching this mailbox - taking a lock only to find that out is a
            // lock cycle on the idling connection for nothing. A resolver armed here
            // belongs to a sync() pass that raced an API command's SELECT; settle it
            // like the lock-held branch below does
            if (this.synced) {
                this.synced();
            }
            return;
        ...
        if (currentLock) {
            if (this.path === currentLock.path) {
                ...
                if (this.synced) {
                    // Locks are only granted after the mailbox is open, so no
                    // further mailboxOpen event is coming for this path. Settle
                    // the resolver armed by sync() instead of leaving it waiting
                    // forever. If the open's onOpen() is still running, its own
                    // resolve call later is a no-op on the settled promise
                    this.synced();
                }
                return;
            }
        ...
        if (this.connection.imapClient?.mailbox && this.connection.imapClient.mailbox.path === this.path) {
            ...
            if (this.synced) {
                // The lock was granted against the already-open mailbox without
                // a new SELECT, so no mailboxOpen event fires - settle the
                // resolver armed by sync() instead of leaving it waiting forever
                this.synced();
            }
            lock.release();
            return;
    ...
    async onOpen(event, openedClient) {
        ...
        try {
            ...
            if (this.processingOpenClient !== openedClient) {
                ...
                // Settle the sync() pass waiting for this open, if any. IDLE needs no
                // restart: ImapFlow re-arms auto-IDLE whenever a lock is released
                if (this.synced) {
                    this.synced();
                }
            }
