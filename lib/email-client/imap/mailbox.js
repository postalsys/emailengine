 ...
 * HyperLogLog (so the retry reports `seemsLikeNew: false`, an informational field), a stored bounce
 * record (a duplicate row in the message's `bounces` array), and, the expensive one, the LLM summary
 * and embeddings, which sit just before the inline-image download. All of it needs the connection to
 ...
 * narrows the exposed set to what the caller asked for, and Webhooks.formatPayload() drops
 * the block entirely unless notifyHeaders names something.
 */
...
class Mailbox {
    ...
    async processDeleted(messageData) {
        ...
        if (messageData.messageId) {
            try {
                let deleted = await appendList.clear(this.connection.redis, this.connection.getBounceKey(), messageData.messageId);
                if (deleted) {
                    this.logger.error({
            ...
            try {
                ...
                if (content) {
                    ...
                    if (bounce.action && bounce.recipient && bounce.messageId) {
                        ...
                        stored = await this.connection.storeBounce(messageInfo.id, bounce);
    ...
    async getMessageInfo(messageData, extended) {
        ...
        await this.connection.attachBounces(result);
