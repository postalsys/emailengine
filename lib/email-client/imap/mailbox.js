...
const { parseEmail, contentToString } = require('../../parse-helpers');
    ...
    async processNew(messageData, options, canSync, storedStatus) {
        ...
        if (this.mightBeDSNResponse(messageInfo)) {
            try {
                ...
                let parsed = await parseEmail(sourceStream);
                if (parsed) {
                    content = { parsed };

                    // Extract delivery status information
                    let deliveryStatus = (parsed.attachments || []).find(attachment => attachment.mimeType === 'message/delivery-status');
                    if (deliveryStatus) {
                        let deliveryEntries = libmime.decodeHeaders(contentToString(deliveryStatus.content).trim());
                        let structured = {};
