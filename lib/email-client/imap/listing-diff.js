...
// Pure helpers behind the mailbox listing: the diff IMAPClient.getCurrentListing() runs, and
// the comparisons that decide which listed folders an account watches.
//
...
// reports "Inbox" one pass and "INBOX" the next does not look like a delete plus an add. Every
// comparison of a path against another path goes through samePath() (lib/utils/mailbox-path.js)
// for that reason, which is why the ones that match stored account configuration against a
// listing live here too rather than as an open-coded `===` at each call site.

const msgpack = require('../../msgpack');
const { normalizePath, samePath } = require('../../utils/mailbox-path');
...
/**
 * Does a configured selector pick out this listed folder? Account `path` entries and
 * `subconnections` entries are stored exactly as the API received them and are either a folder
 * path or a special-use token like "\Sent". Tokens are matched literally - they are a fixed
 * vocabulary the server never spells differently - while paths go through samePath().
 *
 * @param {String} configuredPath - One entry of an account's `path` or `subconnections`
 * @param {Object} entry - Mailbox entry from the server listing
 * @returns {Boolean} True when the selector picks out this folder
 */
function matchesListingEntry(configuredPath, entry) {
    // The token compare is guarded on the token existing: most folders carry no special use, and
    // an undefined configured entry must not match an undefined specialUse
    return samePath(configuredPath, entry.path) || (!!entry.specialUse && configuredPath === entry.specialUse);
}

/**
 * Index of the first configured selector that picks out this folder, or -1. The position is what
 * orders the candidates for the folder the primary connection watches, so the lowest-numbered
 * selector wins.
 *
 * @param {Array} configuredPaths - An account's `path` list
 * @param {Object} entry - Mailbox entry from the server listing
 * @returns {Number} Index into configuredPaths, or -1 when the folder is not selected
 */
function configuredPathIndex(configuredPaths, entry) {
    return configuredPaths.findIndex(configuredPath => matchesListingEntry(configuredPath, entry));
}
 ...
 * @returns {Object} { hasChanges, hasChangesBesidesNew, deletedEntries } - deletedEntries are
 *                   stored folders the server no longer lists, whose message index the caller must
 *                   clear. The second flag is for the caller that defers the folders that appeared
 *                   (see IMAPClient.getCurrentListing): without it, a pass whose only change was a
 *                   deferred folder still rewrites the whole listing hash with what it just read
 */
function diffMailboxListing(listing, storedListing) {
    let hasChanges = false;
    let hasChangesBesidesNew = false;
    ...
    return { hasChanges, hasChangesBesidesNew, deletedEntries };
}
