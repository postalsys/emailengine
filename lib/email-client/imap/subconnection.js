...
const { detachImapClient } = require('./detach-client');
const { connectionUnavailableError } = require('./connection-errors');
const { isRefusedImapLogin } = require('./login-failure');
...
class Subconnection extends EventEmitter {
    constructor(opts) {
        ...
        this.reconnectBackoff = new ReconnectBackoff({ baseDelay: 2000, maxDelay: 30000, jitter: 1000 });
    }
    ...
    async reconnect() {
        ...
        try {
            // Calculate delay with cap and jitter (never stops trying)
            const totalDelay = this.reconnectBackoff.nextDelay();
            ...
            await backOff(() => this.start(), {
                maxDelay: this.reconnectBackoff.maxDelay,
        ...
        try {
            ...
            if (err.responseStatus === 'NO' && this.imapClient?.usable) {
                ...
                try {
                    ...
                    // The path goes into the LIST command as the match pattern
                    // verbatim, so IMAP wildcards (% and *) in the folder name
                    // can match sibling folders. Only an entry naming the same
                    // folder proves the monitored mailbox still exists - matched
                    // through samePath() because a server is free to answer with
                    // a different case for INBOX than the one it listed earlier,
                    // and reading that as a missing folder disables the
                    // subconnection until the folder is seen again
                    if (!folders || !folders.some(folder => samePath(folder.path, this.mailbox.path))) {
            ...
            if (err.mailboxMissing) {
                ...
                // mailboxMissing is the machine-readable marker consumed by the
                // revival trigger in syncMailboxes() - the disabledReason text
                // is display copy and must not carry behavior
                this.disabled = true;
                this.state = 'disabled';
                this.disabledReason = 'Mailbox folder not found';
                this.mailboxMissing = true;
    ...
    async start() {
        if (this.imapClient) {
            ...
            try {
                detachImapClient(prevImapClient, this.logger, { type: 'subConnection', previous: true, account: this.account });
                ...
                await this.parent.untrackConnection(prevImapClient);
            }
        }

        // Built as this subconnection, not as the primary connection: a credential failure of its
        // own must not notify or park the account, which is why notify() below is a no-op. The
        // parent used to be reached through an injected zero-argument arrow that silently dropped
        // this context
        let imapConfig = await this.parent.getImapConfig(null, this);
        ...
        imapClient.on('close', async () => {
            try {
                await this.parent.untrackConnection(imapClient);
                imapClient.log.info({ msg: 'Connection closed', account: this.account });
            } catch (err) {
                this.logger.error({ msg: 'Failed to update connection count on close', account: this.account, err });
            }
            ...
            try {
                ...
                // Nothing else re-arms a subconnection after an unexpected close, so a failure
                // here leaves this mailbox without its own IDLE channel until the account is
                // reconnected - never demote this below error.
                this.logger.error({ msg: 'Reconnection error', account: this.account, err });
            }
        ...
        try {
            ...
            if (connectResponse === false) {
                // connect() bailed because close() landed while the connection
                // was being set up - no socket was opened, so the 'close'
                // listener that maintains the connection count never fires.
                // Drop the tracking entry manually
                await this.parent.untrackConnection(imapClient);
            }
        } catch (err) {
            if (isRefusedImapLogin(err)) {
                this.logger.warn({ msg: 'Failed to authenticate subconnection', account: this.account, err });
                this.state = 'authenticationError';
                this.disabledReason = 'Authentication failed';
            } else {
                this.logger.warn({ msg: 'Failed to connect subconnection', account: this.account, err });
    ...
    async connect() {
        ...
        imapClient.on('exists', event => {
            try {
                ...
                this.logger.trace({ msg: 'Exists notification', account: this.account, event });
        ...
        imapClient.on('flags', event => {
            try {
                ...
                this.logger.trace({ msg: 'Flags notification', account: this.account, event });
    ...
    async expungeHandler(payload) {
        this.logger.trace({ msg: 'Expunge notification', account: this.account, payload });
    ...
    close() {
        ...
        // Drop the pending 'changes' debounce. The mailboxMissing disable path
        // closes the subconnection while the parent's listeners are still
        // attached (they are only detached at the next reconcile), so a timer
        // armed just before close would otherwise still emit on the closed
        // instance and trigger a pointless sync for the removed folder
        clearTimeout(this.emitTimer);
        this.emitTimer = false;
