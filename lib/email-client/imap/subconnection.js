        ...
        try {
            ...
            if (err.responseStatus === 'NO' && this.imapClient?.usable) {
                ...
                try {
                    ...
                    // can match sibling folders. Only an entry with the exact
                    // same path proves the monitored mailbox still exists
                    // (monitored paths come from the account listing, so the
                    // stored spelling matches what the server returns)
                    if (!folders || !folders.some(folder => folder.path === this.mailbox.path)) {
