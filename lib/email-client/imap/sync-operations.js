...
// Signals that the sync was interrupted rather than that the command itself failed. The original
// failure is kept as `cause`, because on this path it is the only record of why the server dropped
// the connection (NO [OVERQUOTA], * BYE Too many connections, [SERVERBUG]).
...
/**
 * Checks if UIDVALIDITY has changed (mailbox was recreated). getStoredStatus() reports a
 * missing UIDVALIDITY as false, never as an absent key, so the value is tested rather than
...
function hasUidValidityChanged(storedStatus, mailboxStatus) {
    return validUidValidity(storedStatus.uidValidity) && mailboxStatus.uidValidity !== storedStatus.uidValidity;
}
...
class SyncOperations {
    ...
    /**
     * Ends a sync operation: gives the mailbox lock back (its release reports the task
     * to the connection, see Mailbox.getMailboxLock()) and clears the syncing flags
     * @param {Object} lock - Lock returned by Mailbox.getMailboxLock()
     */
     ...
     * @param {Object} [storedStatus] - Stored mailbox status, when the caller already read it
        ...
        try {
            ...
            if (imapIndexer === 'fast') {
                ...
                for await (let messageData of imapClient.fetch('1:*', fields, { uid: true })) {
                    // A server can answer a FETCH without the fields it was asked for, and
                    // everything below this point dereferences the flag set
                    if (!messageData || !messageData.uid || !messageData.flags) {
                        continue;
                    }
                    // ignore Recent flag
    ...
    async runFastSync(storedStatus) {
        ...
        try {
            ...
            // Both snapshots are read under the lock, which guarantees this folder is the
            // selected one: read before it, the mailbox counters could belong to a folder
            // an API command opened in the meantime, and the stored state could predate a
            // sync of this folder that ran just ahead of this one
                ...
                while (!fetchCompleted) {
                    try {
                        ...
                        for await (let messageData of imapClient.fetch(range, fields, opts)) {
                            if (!messageData || !messageData.uid || (fields.flags && !messageData.flags)) {
                        ...
                        if (!imapClient.usable) {
                            ...
                            throw connectionClosedError(err);
                        }

                        const fetchRetryDelay = calculateFetchBackoff(++fetchRetryCount);
                        this.logger.warn({ msg: 'FETCH failed, retrying', retryDelay: Math.round(fetchRetryDelay), err });
    ...
    async runPartialSync(storedStatus) {
        ...
        try {
            ...
            // Read under the lock for the same reason as in runFastSync()
            storedStatus = storedStatus || (await this.mailbox.getStoredStatus());
            let mailboxStatus = this.mailbox.getMailboxStatus();
            ...
            if (mailboxStatus.messages) {
                ...
                while (!fetchCompleted) {
                    ...
                    if (!imapClient || !imapClient.usable) {
                        this.logger.debug({ msg: 'IMAP client not available for partial sync' });
                        let err = new Error('IMAP connection not available');
                        err.code = 'IMAPConnectionClosing';
                        err.statusCode = 503;
                        throw err;
            ...
            if (mailboxStatus.messages) {
                ...
                while ((range = getFetchRange(mailboxStatus.messages, range))) {
                    ...
                    while (!fetchCompleted) {
                        ...
                        if (!imapClient || !imapClient.usable) {
                            this.logger.debug({ msg: 'IMAP client not available for FETCH' });
                            let err = new Error('IMAP connection not available');
                            err.code = 'IMAPConnectionClosing';
                            err.statusCode = 503;
                            throw err;
                        ...
                        try {
                            ...
                            if (!imapClient.usable) {
                                // A connection dropped mid-FETCH is the most common way a full sync
                                // dies, and this path sets no syncError flag, so report it here
                                this.logger.warn({
                                    msg: 'FETCH failed, connection is no longer usable',
                                    code: 'fetch_error',
                                    loopId,
                                    batchNumber,
                                    range,
                            ...
                            this.logger.warn({
                                ...
                                retryCount: fetchRetryCount,
                                totalMessages: mailboxStatus.messages,
                                retryDelay: Math.round(fetchRetryDelay),
                                err
                            });
                            await new Promise(r => setTimeout(r, fetchRetryDelay));
                            ...
                            if (!currentMailbox || !samePath(currentMailbox.path, this.mailbox.path)) {
                                this.logger.warn({
