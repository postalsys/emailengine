 ...
 * connected session, yet this folder has no stored state while the server still reports
 * messages. That mismatch means EmailEngine's per-mailbox index was lost (e.g. Redis evicted
 * it) - replaying those already-synced messages as new would flood the webhook queue. A
 * genuine first sync keeps `previouslyConnected <= 1` and is left to the normal
 * notifyFrom-bounded path so intentional backfills still work.
 *
 * @param {Object} storedStatus - Stored mailbox status (uidNext is `false` when state is missing)
 * @param {Object} mailboxStatus - Current mailbox status from IMAP
...
function shouldSeedLostIndex(storedStatus, mailboxStatus, previouslyConnected) {
    return Number(previouslyConnected) > 1 && storedStatus.uidNext === false && Number(mailboxStatus.messages) > 0;
}
...
class SyncOperations {
    ...
    /**
     * Rebuilds the message index from the server's current state WITHOUT emitting messageNew
     * notifications. Used when the stored sync state was lost (e.g. Redis evicted it) or
     * invalidated (UIDVALIDITY change). Records the messages currently on the server as the new
     * baseline so only mail arriving afterwards is advertised as new, then emits a single
     * mailboxReset event so integrators can reconcile.
     *
     * @param {Object} mailboxStatus - Current mailbox status from IMAP
     ...
     * @returns {Number} Count of indexed messages
     */
    async seedMailboxIndex(mailboxStatus, options) {
        ...
        try {
            ...
            this.logger.warn({
                msg: 'Rebuilt mailbox index without notifications',
