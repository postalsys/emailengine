...
const { checkAccountScopes, isSendOnlyByScopes } = require('../oauth/scope-checker');
...
const { emitChangeEvent, toCamelCase, collectCidReferences, cidReferenceRegex, truncateTextContents } = require('../tools');
...
const {
    ...
    GMAIL_MAX_HISTORY_ENTRY_ATTEMPTS
} = require('../consts');

const settings = require('../settings');

const { GMAIL_API_BASE, LIST_BATCH_SIZE, request: gmailApiRequest, executeBatchRequests } = require('./gmail/gmail-api');
const { buildSendRequest, buildDraftSendRequest } = require('./gmail/send-request');
const { pushHeaderValue } = require('../utils/header-map');

const MAX_GMAIL_BATCH_SIZE = 50;

// Gmail rejects batchModify calls carrying more ids than this
const BATCH_MODIFY_MAX_IDS = 1000;

// System labels that act as folders and that the API lets a client remove: SENT and DRAFT can
// not be, and the remaining system labels are markers (STARRED, IMPORTANT) or Inbox tabs
const FOLDER_SYSTEM_LABELS = new Set(['INBOX', 'SPAM', 'TRASH']);
...
// Extract the Message-ID header value (including angle brackets) from a raw RFC822 message,
// tolerating folded headers. Returns null if the message has no Message-ID header.
function extractMessageIdHeader(raw) {
    let headerEnd = raw.indexOf('\r\n\r\n');
    if (headerEnd < 0) {
        headerEnd = raw.indexOf('\n\n');
    }
    let headerBlock = raw
        .subarray(0, headerEnd >= 0 ? headerEnd : raw.length)
        .toString()
        // unfold wrapped header lines
        .replace(/\r?\n[ \t]+/g, ' ');
    let match = headerBlock.match(/^Message-ID:\s*(<[^>]+>)/im);
...
// 10min default - fallback polling if no Pub/Sub notifications. Overridable via
// EENGINE_GMAIL_FALLBACK_POLL_INTERVAL (ms) so integration tests can exercise the poller
// without waiting the full interval.
const FALLBACK_POLLING_INTERVAL = Number((process.env.EENGINE_GMAIL_FALLBACK_POLL_INTERVAL || '').toString().trim()) || 10 * 60 * 1000;
...
class GmailClient extends BaseClient {
    ...
    async init(opts) {
        ...
        if (!isSendOnly) {
            ...
            // Lets getToken() re-arm the timers after an authentication error clears
            this.backgroundTimersArmed = true;
        }

        // Before the state flip below - see notifyAuthenticationSuccess()
    ...
    async close() {
        ...
        // A graceful close means the in-flight history entry did not kill this
        // worker, so it must not keep the strike a deploy or a pause would
        // otherwise leave behind. Guarded because the caller does not await
        // close() - a rejection here would surface as an unhandled rejection
        try {
            await this.redis.hdel(this.getAccountKey(), 'googleHistoryProcessing');
        } catch (err) {
            this.logger.debug({ msg: 'Failed to clear history processing marker', account: this.account, err });
        }
     ...
     * (PUT /v1/account/{account}/sync). Gmail has no folder-walk sync like IMAP; instead it
     * reads the current history checkpoint and processes anything that arrived since the last
     * stored historyId, reusing the same code path as live Pub/Sub notifications.
     */
    async syncMailboxes() {
        if (this.closed) {
            return null;
        ...
        // Send-only accounts have no mailbox to sync
        if (isSendOnlyByScopes('gmail', accountData)) {
            return null;
        }
        ...
        try {
            ...
            this.logger.error({ msg: 'Failed to fetch profile for manual sync', account: this.account, err });
            return null;
        }

        let currentHistoryId = Number(profileRes?.historyId) || null;
        // googleHistoryId is already loaded with the account data; reuse it (matches init())
        let storedHistoryId = Number(accountData.googleHistoryId) || null;

        if (currentHistoryId && storedHistoryId && currentHistoryId > storedHistoryId) {
            // Reuse triggerSync so the processingHistory guard prevents overlap with a
            // concurrent Pub/Sub-driven sync. triggerSync is fire-and-forget but catches its
            // own errors, so it cannot produce an unhandled rejection.
            this.triggerSync(storedHistoryId, currentHistoryId);
        }
    ...
    async reconnect() {
        // A closed instance can be revived here, as IMAPClient.reconnect(force) does: every timer
        // and the watch setup return on `closed`, and init() does not reset it, so without this a
        // reconnect after close() reported a connected account that never synced. The auth-failure
        // safety net produces exactly that instance - the park closes the client but leaves it
        // assigned, and switching the flag back off through PUT /v1/account reconnects it in place.
        this.closed = false;
    ...
    async updateMessages(path, search, updates) {
        ...
        // Step 1. Resolve matching messages. Pages through the whole result set, as the IMAP
        // client does - the ids are applied in batchModify-sized chunks below
        let emailIds = search.emailIds;
        if (!emailIds) {
            emailIds = [];
            let cursor;
            do {
        ...
        // Batch modify all messages, in the chunks the endpoint accepts
        for (let i = 0; i < emailIds.length; i += BATCH_MODIFY_MAX_IDS) {
            await this.request(
                `${GMAIL_API_BASE}/gmail/v1/users/me/messages/batchModify`,
                'post',
                Object.assign({ ids: emailIds.slice(i, i + BATCH_MODIFY_MAX_IDS) }, labelUpdates),
                { returnText: true }
            );
        }
    ...
    async moveMessage(emailId, target, options) {
        ...
        if (sourcePath) {
            let sourceLabel = await this.getLabel(sourcePath);
            if (!sourceLabel) {
                throw this.unknownPathError(sourcePath);
            }
            labelsUpdate.delete = sourceLabel.id;
        } else {
            // No source folder given: leave the folder the message is filed in the way Gmail's
            // own "move to" does, by dropping the system folder labels it carries. User labels
            // stay - without a source there is no telling which of them the caller sees as the
            // folder. Adding the target alone used to leave the message in the Inbox
            let currentData = await this.request(`${GMAIL_API_BASE}/gmail/v1/users/me/messages/${emailId}`, 'get', { format: 'minimal' });
            let folderLabels = (currentData?.labelIds || []).filter(labelId => FOLDER_SYSTEM_LABELS.has(labelId) && labelId !== label.id);
            if (folderLabels.length) {
                labelsUpdate.delete = folderLabels;
            }
        }
    ...
    async getMessage(emailId, options) {
        ...
        let formattedMessage = this.formatMessage(messageData, { extended: true, textType: options.textType, maxBytes: options.maxBytes });
            ...
            const batchResults = await Promise.all(
                batch.map(async emailId => {
                    try {
                        ...
                        const messageData = await this.request(`${GMAIL_API_BASE}/gmail/v1/users/me/messages/${emailId}`, 'get', requestQuery);
                        const formattedMessage = this.formatMessage(messageData, { extended: true, textType: options.textType, maxBytes: options.maxBytes });
    ...
    async getText(textId, options) {
        ...
        const [emailId, textParts] = this.unpackApiId(
            textId,
            parts => parts.length >= 2 && Array.isArray(parts[1]) && Array.isArray(parts[1][0]) && Array.isArray(parts[1][1])
        );
     ...
     * Verifies that the message carries the DRAFT label, downloads its raw content and
     * looks up the Gmail draft resource id so the delivery step can use drafts.send
     * @param {string} emailId - Gmail message id of the draft
     * @returns {Object} { raw: Buffer, draft: { message, draftId }, threadId }
     */
    async resolveDraftForSubmit(emailId) {
        await this.prepare();

        // A single format=raw fetch provides the label check, the thread id and the raw content
        let messageData;
        try {
            messageData = await this.request(`${GMAIL_API_BASE}/gmail/v1/users/me/messages/${emailId}`, 'get', { format: 'raw' });
        } catch (err) {
            if (err?.oauthRequest?.status === 404 || err?.oauthRequest?.response?.error?.code === 404) {
        ...
        let raw = messageData?.raw ? Buffer.from(messageData.raw, 'base64url') : null;
        if (!raw) {
        ...
        // Find the draft resource that wraps this message (Gmail wraps every DRAFT-labeled
        // message in one, even messages.insert uploads). The lookup can still come up empty -
        // no Message-ID header, or the rfc822msgid search index lagging behind a fresh upload -
        // and such a message is sent as a raw copy instead of drafts.send at delivery time
        let draftId = null;
        let messageIdHeader = extractMessageIdHeader(raw);
        if (messageIdHeader) {
            try {
                let listing = await this.request(`${GMAIL_API_BASE}/gmail/v1/users/me/drafts`, 'get', { q: `rfc822msgid:${messageIdHeader}` });
                draftId = listing?.drafts?.find(draftEntry => draftEntry?.message?.id === emailId)?.id || null;
            } catch (err) {
                // draft id resolution is best effort - fall back to sending the raw copy
                this.logger.warn({ msg: 'Failed to resolve Gmail draft id', emailId, err });
            }
        }

        return { raw, draft: { message: emailId, draftId }, threadId: messageData?.threadId };
    }
    ...
    async submitMessage(data) {
        ...
        // A resolved draft is sent through drafts.send (Gmail moves the message from Drafts to
        // Sent itself); everything else goes out as a raw message - endpoint, content type and
        // payload encoding then depend on message size and threading, see
        // lib/email-client/gmail/send-request.js
        const { targetEndpoint, contentType, payload } = data.draft?.draftId
            ? buildDraftSendRequest(data.draft.draftId)
            : buildSendRequest(raw, data?.reference?.threadId);

        let submitInfo;
        try {
            submitInfo = await this.request(`${GMAIL_API_BASE}${targetEndpoint}`, 'post', payload, { contentType });
        } catch (err) {
        ...
        if (data.draft && !data.draft.draftId) {
            // The draft resource could not be resolved at queue time, so the draft was sent as
            // a raw copy - remove the original draft message (the Gmail API can only move it
            // to Trash)
            await this.cleanupSubmittedDraft(data.draft, { permanent: false, queueId, messageId });
        }
        /*
            SEND RESPONSE {
            id: '18f85d2eb6adb232',
    ...
    async getTokenData() {
        ...
        try {
            ...
            if (this.state === 'authenticationError') {
                ...
                // outlive the error it describes. Deliberately only the authentication error, the
                // same rule the Graph client follows: a token says nothing about `unset` (an
                // account the safety net parked) or `disconnected` (one that was closed), and
                // flipping either to `connected` for having fetched one was never right
                await this.notifyAuthenticationSuccess(tokenData.user);

                this.state = 'connected';
                await this.setStateVal();

                // Both background timers stop re-arming themselves on an authentication error,
                // so the recovery has to start them again or the watch would never be renewed
                // and nothing would poll for missed notifications until a manual reconnect.
                // Each setup clears its previous timer, so a still-armed one is not doubled.
                if (this.backgroundTimersArmed) {
                    this.setupRenewWatchTimer();
                    this.setupFallbackPollingTimer();
                }
            }
            ...
            if (E.code === 'ETokenRefresh') {
                await this.reportTokenRefreshFailure(E, 'gmail');
            }
        ...
        this.renewWatchTimer = setTimeout(() => {
            ...
            this.renewWatch()
                .catch(err => {
                    ...
                    // Check if this is a permanent auth failure. A token endpoint that answered 429
                    // or a 5xx has not refused the credential, and stopping the timer on one would
                    // leave the watch unrenewed until something else happens to fetch a token
                    if ((err.code === 'ETokenRefresh' && !isTransientCredentialError(err)) || this.state === 'authenticationError') {
                        authError = true;
                    }
                })
                .finally(() => {
    ...
    async renewWatch(accountData, opts) {
        ...
        if (accountData._app?.pubSubApp && (forceWatchRenewal || !accountData.lastWatch || accountData.lastWatch < new Date(now - MIN_WATCH_TTL))) {
            ...
            if (appData?.pubSubTopic && appData.pubSubIamPolicy) {
                ...
                try {
                    ...
                    // NB! `lastWatch` is deliberately NOT moved here, so it now means "last
                    // successful arm" rather than "last attempt". It gates the renewal at the top of
                    // this method against MIN_WATCH_TTL, so writing it on a failure bought a broken
                    // watch a full day of silence even though the renewal timer fires hourly - and a
                    // Gmail watch lapses seven days after the last successful call, so a week of
                    // daily failures ended push entirely. The attempt time goes on the failure
                    // record instead, which is what reports how stale this is. A failing watch now
                    // retries at the timer's own cadence, hourly, with no backoff and no cap: one
                    // call per account per hour is orders of magnitude under the Gmail quota, and
                    // the alternative is the seven-day lapse this replaces
                    await this.accountObject.update({
                        watchFailure: {
                            err: err.message,
                ...
                // Recorded, not just logged: this is the one failure mode that armed no watch and
                // wrote nothing, so an operator polling the account saw what a healthy one shows.
                // Written every cycle, unlike the warning below - the record's whole job is to say
                // when the state was last observed, and a latch would freeze that at the first
                // sighting while the check went on running hourly. Best effort, because init()
                // awaits this method and a Redis hiccup must not fail the account setup
                try {
                    await this.accountObject.update({
                        watchFailure: {
                            err: 'Pub/Sub topic and IAM markers are not recorded on the linked application',
                            time: new Date(now).toISOString()
                        }
                    });
                } catch (err) {
                    this.logger.error({ msg: 'Failed to record the missing watch markers', account: this.account, err });
                }
        ...
        } else if (!accountData._app?.pubSubApp && (accountData.watchFailure || accountData.watchResponse)) {
            // Nothing pushes to this account any more: the Pub/Sub app was unlinked, or the account
            // was re-authorized onto an app that has none. The watch record is only ever cleared by
            // a successful arm, so an account with no push configured went on reporting the last
            // failure - state 'error' with a frozen lastCheck - for good. Keyed on the pubSubApp
            // term alone, never on the whole condition above: a still-linked account inside
            // MIN_WATCH_TTL takes the same else and its failure has to stay.
            // Best effort, because init() awaits this method and a Redis hiccup must not fail the
            // account setup
            ...
            try {
                ...
                this.logger.error({ msg: 'Failed to clear the stored Gmail watch record', account: this.account, err });
            }
        }
    }
    ...
    getEnvelope(messageData) {
        ...
        for (let key of ['from', 'to', 'cc', 'bcc', 'sender', 'reply-to']) {
            for (let header of messageData.payload.headers.filter(header => header.name.toLowerCase() === key)) {
                ...
                let envelopekey = toCamelCase(key.toLowerCase());
    ...
    formatMessage(messageData, options) {
        let { extended, path, textType, minimalFields, maxBytes } = options || {};
        ...
        // Extract all headers. pushHeaderValue() guards against names that resolve to
        // Object.prototype members, which would otherwise throw for every fetch of that message
        let headers = {};
        for (let header of messageData.payload.headers) {
            pushHeaderValue(headers, header.name.toLowerCase(), header.value);
        }
        ...
        const textHasMore = truncateTextContents(textValues, maxBytes);
        ...
        const result = {
            ...
            text: textId
                ? {
                      ...
                      plain: textValues.plain,
                      html: textValues.html,
                      hasMore: textContents?.[0] || textContents?.[1] ? textHasMore : undefined
                  }
                : undefined,
    ...
    async getAttachmentContent(attachmentId, options) {
        options = options || {};
        const [emailId, contentType, disposition, filename, id] = this.unpackApiId(attachmentId, parts => parts.length >= 5);
    ...
    prepareQuery(search) {
        ...
        for (let key of ['larger', 'smaller']) {
            if (search[key]) {
                queryParts.push(`${key}:${Number(search[key]) || 0}`);
            }
        }
        ...
        this.processHistory(currentHistoryId, updatedHistoryId)
            ...
            .finally(() => {
                ...
                if (this.pendingHistoryId && this.pendingHistoryId > processedHistoryId) {
                    ...
                    this.pendingHistoryId = null;
                    // From the stored cursor, not from where the finished run was headed: a run that
                    // failed partway left the cursor behind that, and the entries in between would
                    // never be fetched
                    this.triggerSyncFromCursor(pending);
                } else {
                    this.pendingHistoryId = null;
                }
            });
     ...
     */
    async processHistoryEntry(historyEntry, context) {
        const { labels, newMessageOptions } = context || (await this.getHistoryContext());
        ...
        // Process new messages. Each is fetched on its own, so one that fails is retried on its own
        // and does not take the rest of the entry with it
        for (let entry of historyEntry?.messagesAdded || []) {
            ...
            await this.processAddedMessage(eventEntry, newMessageOptions);
        }
    }

    /**
     * Fetches a message reported as added and announces it, deferring it for a retry on failure
     * @param {Object} eventEntry - Message event data
     * @param {Object} options - Message fetch options
     * @param {number} [attempt] - How many times it has failed before
     */
    async processAddedMessage(eventEntry, options, attempt) {
        try {
            const messageData = await this.prepareNewMessage(eventEntry, options);
            if (messageData) {
                await this.processNew(messageData, options);
            }
        } catch (err) {
            await this.deferFailedChangeEvent({ type: 'created', message: eventEntry, attempt }, err, [eventEntry.id]);
        }
    }

    /**
     * What every history entry of a run needs from the API and the settings, resolved once per
     * run. Fetched before any entry is processed, so a failure here ends the run with the cursor
     * where it was and the next run fetches the same entries again: deferring each entry instead
     * would replay its label changes hours later, over whatever changed since
     * @returns {Promise<{labels: Object[], newMessageOptions: Object}>}
     */
    async getHistoryContext() {
        const [labels, newMessageOptions] = await Promise.all([this.getLabels(), this.getMessageFetchOptions()]);
        return { labels, newMessageOptions };
    }

    retryDeferredEvents() {
        // the same run a fallback poll makes, which picks up the due changes
        this.triggerSyncFromCursor();
    }

    /**
     * Starts a history run from the stored cursor
     * @param {number} [updatedHistoryId] - Where the run is headed, the cursor itself by default
     */
    triggerSyncFromCursor(updatedHistoryId) {
        this.redis
            .hget(this.getAccountKey(), 'googleHistoryId')
            .then(historyId => {
                if (historyId && !this.closed) {
                    this.logger.debug({ msg: 'Processing history from the stored cursor', account: this.account, historyId, updatedHistoryId });
                    this.triggerSync(Number(historyId), Math.max(Number(historyId), updatedHistoryId || 0));
                }
            })
            .catch(err => {
                this.logger.error({ msg: 'Failed to read the history cursor', account: this.account, err });
            });
    }
    ...
    async processHistory(currentHistoryId, updatedHistoryId) {
        ...
        // Resolved on first use, and once for the whole run
        let context;
        const getContext = async () => context || (context = await this.getHistoryContext());

        // Messages deferred for a retry, every one of them a message reported as added
        try {
            await this.processDueDeferredEvents(async event => this.processAddedMessage(event.message, (await getContext()).newMessageOptions, event.attempt));
        } catch (err) {
            this.logger.error({ msg: 'Failed to process deferred mailbox changes', account: this.account, err });
        }

        // The history cursor only advances after an entry is processed, so an
        // entry that kills the worker (OOM, heartbeat restart) is refetched by
        // the restarted worker and would loop forever. The in-flight marker
        // records which entry processing last started on and how many times;
        // once the limit is hit the entry is skipped with a syncWarning
        let processingMarker = null;
        try {
            processingMarker = JSON.parse(await this.redis.hget(this.getAccountKey(), 'googleHistoryProcessing'));
        } catch (err) {
            // no marker or an unreadable one, nothing to resume
        }

        // advance the durable cursor once an entry is handled (processed or skipped)
        let advanceHistoryCursor = async historyEntryId => {
        ...
        let getHistoryPage = async pageToken => {
            ...
            try {
                ...
                switch (err?.oauthRequest?.response?.error?.code) {
                    case 404: {
                        ...
                        // Set to newest known value. A fallback poll or a retry is headed nowhere past
                        // the expired cursor, so ask for the current one, or every run would 404 again
                        newestHistoryId = updatedHistoryId;
                        if (!(updatedHistoryId > currentHistoryId)) {
                            try {
                                const profileRes = await this.request(`${GMAIL_API_BASE}/gmail/v1/users/me/profile`);
                                newestHistoryId = Number(profileRes?.historyId) || newestHistoryId;
                            } catch (profileErr) {
                                this.logger.warn({ msg: 'Failed to fetch the current history ID', account: this.account, err: profileErr });
                            }
                        }
                        return;
                    }
            ...
            if (historyRes?.history?.length) {
                // Before any entry is marked in flight: a failure here ends the run with the cursor
                // where it was, see getHistoryContext()
                await getContext();
            }
            ...
            for (let entry of historyRes?.history || []) {
                ...
                let attempts = processingMarker && entry?.id && String(processingMarker.id) === String(entry.id) ? Number(processingMarker.count) || 0 : 0;
                if (attempts >= GMAIL_MAX_HISTORY_ENTRY_ATTEMPTS) {
                    // Every attempt to process this entry has taken the worker
                    // down with it. Skip it and advance the cursor so the
                    // account can make progress; the warning names the messages
                    // whose events were lost so they can be recovered manually
                    let messageIds = (entry?.messagesAdded || []).map(added => added?.message?.id).filter(id => id);
                    this.logger.error({
                        msg: 'Skipping history entry that repeatedly killed the worker',
                        account: this.account,
                        entryId: entry?.id,
                        attempts,
                        messageIds
                    ...
                    await emitChangeEvent(this.logger, this.account, 'syncWarning', {
                        type: 'historyEntrySkipped',
                        message: 'Processing a history entry repeatedly killed the sync worker, its changes were skipped',
                        historyId: entry?.id,
                        messageIds
                    });
                    await advanceHistoryCursor(historyEntryId);
                    continue;
                }

                if (entry?.id) {
                    // mark the entry as in flight before touching it, so a
                    // worker death mid-entry is attributable on restart
                    await this.redis.hset(this.getAccountKey(), 'googleHistoryProcessing', JSON.stringify({ id: entry.id, count: attempts + 1 }));
                }

                try {
                    // A message in the entry that fails to fetch is deferred on its own inside
                    await this.processHistoryEntry(entry, context);
                    await advanceHistoryCursor(historyEntryId);
                } catch (err) {
                    this.logger.error({
                        msg: 'Failed to process history entry',
                        account: this.account,
                        entryId: entry?.id,
                        messagesAdded: entry?.messagesAdded?.length,
                        messagesDeleted: entry?.messagesDeleted?.length,
                        labelsAdded: entry?.labelsAdded?.length,
                        labelsRemoved: entry?.labelsRemoved?.length,
                        err
                    });
                }
            }
        ...
        // the run finished without dying, drop the in-flight marker
        await this.redis.hdel(this.getAccountKey(), 'googleHistoryProcessing');
    }
    ...
    async prepareNewMessage(eventEntry, options) {
        ...
        let messageData;
        try {
            messageData = await this.getMessage(eventEntry.id, options);
        } catch (err) {
            if (err.statusCode !== 404) {
                throw err;
            }
            // deleted before the history entry was processed - reported as missing below, the
