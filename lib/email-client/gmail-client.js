...
const he = require('he');
const { BaseClient, metricsMeta, isTransientNetworkError } = require('./base-client');
    ...
    async init(opts) {
        ...
        try {
            ...
            if (isTransientNetworkError(err)) {
                // The provider could not be reached - a connection problem, not a refused
                // credential. Left unflagged so the worker retries init() instead of reporting
                // an authentication error that only re-authorization could clear
                throw err;
            }
