    ...
    async request(...args) {
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            ...
            try {
                ...
                // Check if this is a rate limit error (429 or specific rate limit reasons)
                const isRateLimited = status === 429 || errorReason === 'rateLimitExceeded' || errorReason === 'userRateLimitExceeded';

                if (isRateLimited && attempt < maxRetries) {
                    // Calculate delay with exponential backoff
                    const retryAfter = err.oauthRequest?.headers?.['retry-after'];
                    let delay = retryAfter ? parseInt(retryAfter, 10) * 1000 : baseDelay * Math.pow(2, attempt);
                    ...
                    metricsMeta({ account: this.account }, this.logger, 'oauth2ApiRequest', 'inc', {
                        status: 'rate_limited',
                        provider: 'gmail',
                        statusCode: '429'
                    });
