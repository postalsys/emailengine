...
class GmailClient extends BaseClient {
    ...
    async init(opts) {
        ...
        // Checked before the state is stamped 'connecting' and before any token fetch, so a parked
        // account makes no provider calls at all.
        if (await this.parkIfAuthFailureDisabled()) {
            return;
        }
        ...
        try {
            ...
            if (E.code === 'ETokenRefresh') {
                ...
                await this.notify(false, CONNECT_ERROR_NOTIFY, {
                    response: E.message,
                    serverResponseCode: E.code || 'TokenGenerationError'
                });
            }
