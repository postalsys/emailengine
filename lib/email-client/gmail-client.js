...
class GmailClient extends BaseClient {
    ...
    async submitMessage(data) {
        ...
        if (raw.length <= JSON_SEND_LIMIT || data?.reference?.threadId) {
            // Use JSON endpoint: base64url string body is retry-safe (no ArrayBuffer)
            // threadId metadata requires JSON endpoint regardless of size
            ...
            // Large non-threaded message: use upload endpoint with raw Buffer
            // OAuth request layer uses fetchAgent (non-retry) for Buffer bodies
