        ...
        try {
            ...
            if (E.code === 'ETokenRefresh') {
                ...
                if (!E.errorNotified) {
                    ...
                    await this.notify(false, AUTH_ERROR_NOTIFY, {
                        // A token error carries `tokenRequest`, not `oauthRequest` - that field is
                        // only set on the API request path. Reading oauthRequest alone made
                        // `response` undefined, so this webhook shipped with no detail at all.
                        response: E.tokenRequest?.response?.error_description || E.oauthRequest?.response?.error?.message || E.response || E.message,
                        serverResponseCode: 'TokenGenerationError'
                    });
                }
            } else if (!E.errorNotified) {
                // Anything that is not an ETokenRefresh - an undici network failure (which leaves
                // E.code undefined), a Redis lock failure, or a Workload Identity Federation error
                // (ESignJwt / ESTSExchange / ESubjectTokenRead) - used to fall straight through to
                // the rethrow below. init() is dispatched fire-and-forget by workers/imap.js, so
                // nothing caught it: no state was written, no webhook fired, and there is no retry
                // loop on this transport. The account simply sat at 'connecting' forever, which the
                // admin UI renders as an ordinary info badge, indistinguishable from a healthy
                // account starting up. Report it as a connection error so it is at least visible.
                E.errorNotified = true;
                this.state = 'connectError';
                await this.setStateVal();

                await this.notify(false, CONNECT_ERROR_NOTIFY, {
                    // The errno goes in `response`, not in `serverResponseCode`. setErrorState()
                    // treats a changed serverResponseCode as a new first occurrence, so varying it
                    // per errno would reset the auth-failure window and fire a webhook on every
                    // flip for a provider alternating between, say, ETIMEDOUT and ECONNRESET.
                    response: E.code ? `${E.code}: ${E.message}` : E.message,
                    serverResponseCode: 'TokenGenerationError'
                });
            }
