...
// require the other.
//
// The IMAP server's own verdict on a login (a NO to AUTHENTICATE that is the server's problem
// rather than the credential's) is decided in ./imap/login-failure.js, kept dependency-free so the
// pure subconnection tests do not inherit the Redis connections this module's imports open.
 ...
 * only carry the request record, and so does the external authentication server (`authRequest`,
 * attached by resolveCredentials() in lib/tools.js - it must not put an operator's status on
 * err.statusCode, which is read as the status of the API request being served). Boom wrappers are
 * deliberately not consulted: Account.getActiveAccessTokenData() boomifies every renewal failure
 * as 403, which would mask the status the token endpoint actually sent
...
function credentialErrorStatus(err) {
    return Number(err?.statusCode || err?.tokenRequest?.status || err?.oauthRequest?.status || err?.authRequest?.status || 0);
}
