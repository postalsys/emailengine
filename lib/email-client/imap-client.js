...
const { shouldInvalidateAccessToken } = require('./oauth-errors');
const { isConnectionBlocked } = require('../account/account-state');
...
class IMAPClient extends BaseClient {
    constructor(account, options) {
        ...
        //
        // Jitter matters more here than the curve does: every account belonging to one OAuth2 app
        // shares its client credentials and therefore one provider quota, so a provider-side outage
        // or rate limit hits them all at the same instant. Without jitter they then retry in
        // lockstep, arriving as synchronized waves that keep the limit tripped instead of draining
        // it. A shared circuit breaker is not an option here (CLAUDE.md: one failing account must
        // not block the others), so spreading the retries is the only fleet-level remedy available.
        // imap/subconnection.js already does this.
        this.reconnectBackoff = new ReconnectBackoff({ baseDelay: 1000, maxDelay: 30000, jitter: 1000 });
        ...
        this.errorReconnectBackoff = new ReconnectBackoff({ baseDelay: 3000, maxDelay: 30000, jitter: 1000 });

        // Per-instance spread for the backOff() loop in reconnect(), which is the pacer for a failing
        // token refresh. exponential-backoff only offers jitter:'none' or jitter:'full', and 'full'
        // is `Math.random() * delay` - it shortens delays rather than spreading them, so it cannot be
        // used on a path this hot. Randomizing the curve's own parameters instead spreads the fleet
        // while guaranteeing every delay stays at or above the un-jittered floor: the start is never
        // below 2s and the steady state never below 90% of the cap.
        this.backoffStartingDelay = 2000 + Math.round(Math.random() * 1000);
        this.backoffMaxDelay = Math.round(MAX_BACKOFF_DELAY * (0.9 + Math.random() * 0.1));
    ...
    async getCommandConnection(reason) {
        ...
        try {
            ...
            if (isConnectionBlocked(accountData)) {
                return null;
            }
    ...
    async reconnect(force) {
        ...
        try {
            ...
            // external trigger
            // This loop re-runs the OAuth2 token refresh on every attempt, so it is the pacer for a
            // provider outage. The per-instance spread computed in the constructor de-synchronizes
            // accounts that share an OAuth2 app without ever shortening a delay.
            //
            // NB: do NOT pass jitter:'full' here. The library implements it as
            // `Math.random() * delay`, which replaces the delay rather than spreading it - measured
            // sub-second retries on a curve whose floor is meant to be 2s. Each retry re-runs the
            // token refresh and setErrorState()'s unconditional Redis writes, which is the churn
            // that presented as 100% CPU in 2.73.0.
            await backOff(() => this.start(), {
                maxDelay: this.backoffMaxDelay,
                numOfAttempts: Infinity,
                retry: () => !this.isClosing && !this.isClosed,
                startingDelay: this.backoffStartingDelay
            });
            this.logger.debug({
        ...
        try {
            ...
            imapClient.on('error', err => {
                ...
                if (!this.reconnectTimer) {
                    ...
                    const errorRetryDelay = this.errorReconnectBackoff.nextDelay();
                    ...
                    this.reconnectTimer = setTimeout(() => {
                        this.reconnectTimer = null;
                        // The backoff delay is reset after a successful sync, not here -
                        // reconnect() also resolves when the setup failed and was rescheduled
                        this.reconnect().catch(err => {
                            this.logger.warn({
                                ...
                                account: this.account,
                                err,
                                attempt: this.errorReconnectBackoff.attempts
                            });
                        });
                    }, errorRetryDelay);
                }
            ...
            try {
                ...
                // See lib/email-client/oauth-errors.js for which SASL statuses count as stale.
                if (shouldInvalidateAccessToken(err.oauthError)) {
                    // access token is invalid, clear it so the next attempt fetches a fresh one
                    try {
    ...
    async processSubConnections() {
        ...
        for (const mailbox of mailboxes) {
            ...
            const subconnection = new Subconnection({
                ...
                account: this.account,
                mailbox,
                // Deliberately takes no parameters, so the `(null, this)` that Subconnection passes
                // is dropped and `ctx` defaults to the primary client. That means a subconnection
                // whose token refresh fails marks the whole ACCOUNT as failing, not just itself.
                //
                // Keep it that way. The OAuth2 token is account-level: if renewing it fails for a
                // subconnection it is failing for the primary too, so the account really is broken.
                // Forwarding `this` would route the failure to Subconnection.notify(), which is a
                // no-op - the failure would become invisible rather than better-scoped.
