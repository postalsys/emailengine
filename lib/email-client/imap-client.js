...
class IMAPClient extends BaseClient {
     ...
     * sync emits the mailboxNew notification, so every listing refresh done to
     * process folder changes must route its result through here (callers that
     * only read the listing, like listMailboxes(), do not).
        ...
        try {
            ...
            if (err.mailboxMissing) {
                ...
                try {
                    ...
                    // Register and sync newly found folders without delaying the
                    // upload error response - the initial sync of a new folder
                    // can take minutes
                    this.processListing(listing).catch(listingErr => {
                        this.logger.error({ msg: 'Failed to process listing', err: listingErr });
                    });
                } catch (E) {
