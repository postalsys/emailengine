...
class IMAPClient extends BaseClient {
    ...
    async reconnect(force) {
        ...
        try {
            ...
            // that presented as 100% CPU in 2.73.0. Randomizing the parameters keeps every delay at
            // or above the un-jittered floor: the start never below 2s, the cap never below 90%.
            await backOff(() => this.start(), {
                maxDelay: Math.round(MAX_BACKOFF_DELAY * (0.9 + Math.random() * 0.1)),
