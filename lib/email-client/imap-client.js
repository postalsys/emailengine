...
class IMAPClient extends BaseClient {
    ...
    async reconnect(force) {
        ...
        try {
            ...
            // provider outage. Every account of one OAuth2 app shares a provider quota, so the
            // curve's own parameters are randomized per call to de-synchronize the fleet.
            //
            // NB: do NOT pass jitter:'full' instead. The library implements it as
            ...
            // token refresh, which is half of the churn that presented as 100% CPU in 2.73.0; the
            // other half was setErrorState()'s per-attempt Redis writes, which it now skips when
            // re-entering the state it is already in.
            //
            // Both parameters are jittered ADDITIVELY, matching ReconnectBackoff.nextDelay() and
            // imap/subconnection.js. A multiplicative cap (`MAX_BACKOFF_DELAY * (0.9 + random*0.1)`)
            // only ever shortens it, which raises steady-state retry frequency by up to 11% across
            // the whole fleet - the opposite of what de-synchronization is for. Spread the delays,
            // never shorten them: no delay here is below the un-jittered floor.
            await backOff(() => this.start(), {
                maxDelay: MAX_BACKOFF_DELAY + Math.round(Math.random() * 60000),
                numOfAttempts: Infinity,
                retry: () => !this.isClosing && !this.isClosed,
                startingDelay: 2000 + Math.round(Math.random() * 1000)
            });
            this.logger.debug({
    ...
    async processSubConnections() {
        ...
        for (const mailbox of mailboxes) {
            ...
            const subconnection = new Subconnection({
                ...
                // is dropped and `ctx` defaults to the primary client. The OAuth2 token is
                // account-level, so a renewal failure here is the account's problem, not the
                // subconnection's - and forwarding `this` would route it to Subconnection.notify(),
                // which is a no-op, making the failure invisible rather than better-scoped.
                //
                // What keeps that from mislabelling a healthy account is the guard in
                // notifyTransientTokenFailure(): a transient failure raised while the primary is
                // connected is logged and dropped rather than persisted. Only a credential
                // rejection, which really is account-level, still marks the account from here.
