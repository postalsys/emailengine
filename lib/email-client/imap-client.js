...
const { decodeStoredListing, diffMailboxListing, buildStoredListingObject } = require('./imap/listing-diff');
...
class IMAPClient extends BaseClient {
    ...
    hasListedMailbox(path) {
        ...
        for (let mailbox of this.mailboxes.values()) {
            if (mailbox.listingEntry && mailbox.listingEntry.specialUse === path) {
                return true;
            }
        }
        return false;
    ...
    async processSubConnections() {
        ...
        for (let i = this.subconnections.length - 1; i >= 0; i--) {
            ...
            const mailbox = mailboxes.find(mailbox => mailbox.path === subconnection.path);
