...
class IMAPClient extends BaseClient {
    ...
    async syncMailboxes() {
        ...
        if (
            this.subconnections.some(
                subconnection =>
                    subconnection.disabled && subconnection.disabledReason === 'Mailbox folder not found' && this.hasListedMailbox(subconnection.path)
            )
        ) {
    ...
    async start() {
        ...
        if (this.imapClient) {
            ...
            try {
                ...
                if (this.connections.delete(prevImapClient)) {
                    try {
                        await this.redis.hSetExists(this.getAccountKey(), 'connections', this.connections.size.toString());
                    } catch (err) {
    ...
    async setupSubConnections() {
        if (this._settingUpSubConnections) {
            ...
            // runs would race the shared subconnections array
            return null;
        }
        this._settingUpSubConnections = true;
        try {
            return await this.processSubConnections();
        } finally {
