...
class IMAPClient extends BaseClient {
    ...
    async reconnect(force) {
        ...
        try {
            // Loading account data must happen inside try/finally - a throw here
            // (e.g. a transient Redis error) would otherwise leave _connecting set
            // and permanently block all later reconnection attempts
            let accountData = await this.accountObject.loadAccountData();
