...
class OutlookClient extends BaseClient {
    constructor(account, options) {
        ...
        // When the earliest change notification deferred for a retry is due: undefined until Redis
        // has been asked (so events deferred before a restart are found), null when none is waiting
        this.deferredDueAt = undefined;
        this.queueRetryTimer = null;
        this.queueRetryTimerDueAt = null;
    ...
    async processHistory() {
        ...
        // A drain with nothing deferred costs no extra Redis round trip
        if (this.deferredDueAt === undefined || (this.deferredDueAt !== null && this.deferredDueAt <= Date.now())) {
            this.deferredDueAt = await this.accountObject.promoteDueQueueEvents();
        }

        while ((event = await this.accountObject.pullQueueEvent()) !== null) {
            processedAny = true;

            // The event is already off the queue, so a failure here must not abort the drain and
            // lose the events behind it. A transient one is deferred and retried instead
            try {
                switch (event.type) {
                    case 'updated':
                ...
                await this.deferFailedQueueEvent(event, err);
            }
        }
        ...
        this.scheduleQueueRetryTimer();
    }

    /**
     * Schedules a retry for a change notification that failed to process. Graph answers 5xx and
     * times out often enough that dropping the event on the first failure lost a messageNew
     * webhook every few hours on a busy instance, with nothing left to recover it from. Only a
     * failure of the service is retried (the same line the credential paths draw); anything else
     * would fail the same way again
     * @param {Object} event - The queue event that failed
     * @param {Error} err - Why it failed
     */
    async deferFailedQueueEvent(event, err) {
        const attempt = (event.attempt || 0) + 1;
        const delay = OUTLOOK_QUEUE_RETRY_DELAYS[attempt - 1];
        const logEntry = { type: 'history-event', eventType: event.type, emailId: event.message, attempt, err };

        if (!delay || !isTransientCredentialError(err)) {
            this.logger.error({ msg: 'Failed to process change notification, giving up', ...logEntry });
            if (event.type === 'created') {
                // the only record left that this message was never announced
                await emitChangeEvent(this.logger, this.account, 'syncWarning', {
                    type: 'changeNotificationSkipped',
                    message: 'A new message could not be fetched, no messageNew webhook was sent for it',
                    messageIds: [event.message]
                });
            }
            return;
        }

        this.logger.warn({ msg: 'Failed to process change notification, retrying later', delay, ...logEntry });

        const dueTime = Date.now() + delay;
        try {
            await this.accountObject.deferQueueEvent({ type: event.type, message: event.message, attempt }, dueTime);
            if (!this.deferredDueAt || dueTime < this.deferredDueAt) {
                this.deferredDueAt = dueTime;
            }
        } catch (deferErr) {
            this.logger.error({ msg: 'Failed to defer change notification', deferErr, ...logEntry });
        }
    }

    // Also forgets the due time, so the next drain asks Redis again
    ...
    /**
     * Arms a timer for the earliest deferred change notification, unless one is already set for it
     */
    scheduleQueueRetryTimer() {
        if (this.closed || !this.deferredDueAt || this.deferredDueAt === this.queueRetryTimerDueAt) {
            return;
        }

        clearTimeout(this.queueRetryTimer);
        this.queueRetryTimerDueAt = this.deferredDueAt;
        this.queueRetryTimer = setTimeout(() => {
            this.queueRetryTimerDueAt = null;
            this.triggerSync();
        }, this.deferredDueAt - Date.now());
    ...
    async syncMissedMessages(opts) {
        ...
        // message by rollingBucketLock below, so forcing cannot re-emit seen messages.
