...
class OutlookClient extends BaseClient {
    ...
    async delete() {
        ...
        // Best-effort: delete MS Graph subscription so it stops sending notifications
        try {
        ...
        // Clean up lock keys
        try {
            await this.redis.del(`${this.getAccountKey()}:subscription-renew-lock`, `${this.getAccountKey()}:subscription-ensure-lock`);
        } catch (err) {
            // ignore
        }
        ...
        this.renewWatchTimer = setTimeout(async () => {
            ...
            try {
                ...
                if (!renewalResult.success && (renewalResult.reason === 'expired' || renewalResult.reason === 'no_subscription')) {
                    ...
                    let subscriptionData = await this.redis.hget(this.getAccountKey(), 'outlookSubscription');
                    if (subscriptionData) {
                        try {
    ...
    async ensureSubscription() {
        ...
        // Acquire distributed lock to prevent concurrent subscription creation
        const lockKey = `${this.getAccountKey()}:subscription-ensure-lock`;
        const lockTTL = OUTLOOK_SUBSCRIPTION_LOCK_TTL;
        const lockAcquired = await this.redis.set(lockKey, Date.now(), 'EX', lockTTL, 'NX');
        if (!lockAcquired) {
            this.logger.info({
            ...
            if (!outlookSubscription.id) {
                ...
                try {
                    ...
                    outlookSubscription.state = {
                        ...
                        time: Date.now(),
                        createRetryCount: (outlookSubscription.state?.createRetryCount || 0) + 1
                    };
                    this.subscriptionState = 'failed';
                } finally {
                    await this.redis.hSetExists(this.getAccountKey(), 'outlookSubscription', JSON.stringify(outlookSubscription));
