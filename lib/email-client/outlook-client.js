...
class OutlookClient extends BaseClient {
    ...
    async request(...args) {
        ...
        try {
            ...
            const mappedError = graphErrorCode && GRAPH_ERROR_MAP[graphErrorCode];
            ...
            if (mappedError) {
                this.logger.debug({
                    ...
                    graphErrorCode,
                    internalCode: mappedError.code
                });
                let error = new Error(err.oauthRequest?.response?.error?.message || graphErrorCode);
    ...
    async requestWithRetry(url, method, payload, options = {}) {
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                return await this.request(url, method, payload, options);
            } catch (err) {
                ...
                // Default: 30s, 60s, 120s (capped at 120s)
                const retryAfter = err.retryAfter || Math.min(30 * Math.pow(2, attempt), 120);
