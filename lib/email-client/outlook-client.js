    ...
    async getToken() {
        ...
        try {
            ...
            if (E.code === 'ETokenRefresh') {
                ...
                // E.code undefined) or a Redis lock failure - used to fall straight through to the
                // rethrow below. init() is dispatched fire-and-forget by workers/imap.js, so nothing
                // caught it: no state was written, no webhook fired, and there is no retry loop on
                // this transport. The account simply sat at 'connecting' forever, which the admin UI
                // renders as an ordinary info badge, indistinguishable from a healthy account
                // starting up. Report it as a connection error so it is at least visible.
                E.errorNotified = true;
