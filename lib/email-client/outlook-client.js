...
class OutlookClient extends BaseClient {
     ...
     * again. Only touches a failure this client reported, so an unrelated error state is left for
     * whatever owns it
     * @returns {Promise<void>}
     */
    async clearSubscriptionFailure() {
        // In-process, so an ordinary renewal costs nothing. A worker that restarted since the report
        // reads `connecting` here and skips - harmless, because init() clears the stored error state
        // through notifyAuthenticationSuccess() on the way back up
        ...
        if (stored && stored.serverResponseCode !== 'SubscriptionSetupError') {
            return;
        }

        // The clear goes first, and it only applies while the error is still the one judged above:
        // a failure written since then is a different one, and reporting `connected` over it is
        // exactly the state this whole path exists to stop
     ...
     * waits. The counters are cleared first, so an hour that could have recovered gets the fast
     * ladder back instead of one attempt every hour forever.
     *
