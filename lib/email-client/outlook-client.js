...
class OutlookClient extends BaseClient {
    constructor(account, options) {
        ...
        this.oauth2Username = 'me'; // `users/${encodeURIComponent('shared@example.com')}`;
    ...
    async init() {
        ...
        let profileRes;
        try {
            profileRes = await this.request(`/${this.oauth2Username}`);
        } catch (err) {
    ...
    async listMessages(query, options) {
        ...
        // list messages
        try {
            let listing = await this.request(`/${this.oauth2Username}/${folder ? `mailFolders/${folder.id}/` : ''}messages`, 'get', requestQuery);
    ...
    async getRawMessage(emailId) {
        ...
        try {
            raw = await this.request(`/${this.oauth2Username}/messages/${emailId}/$value`, 'get', Buffer.alloc(0), { returnText: true });
        } catch (err) {
            switch (err.oauthRequest?.status) {
                case 404: {
    ...
    async deleteMessage(emailId, force) {
        ...
        if (force) {
            try {
                await this.request(`/${this.oauth2Username}/messages/${emailId}`, 'delete', Buffer.alloc(0), { returnText: true });
            } catch (err) {
                this.logger.error({
        ...
        let messageData;
        try {
            messageData = await this.request(`/${this.oauth2Username}/messages/${emailId}/move`, 'post', {
        ...
        let formatRequest = emailId => {
            ...
            if (force) {
                return {
                    id: reqId,
                    method: 'DELETE',
                    url: `/${this.oauth2Username}/messages/${emailId}`
                };
            } else {
                return {
                    id: reqId,
                    method: 'POST',
                    url: `/${this.oauth2Username}/messages/${emailId}/move`,
    ...
    async updateMessage(emailId, updates) {
        ...
        try {
            modifyResult = await this.request(`/${this.oauth2Username}/messages/${emailId}`, 'patch', flagUpdates);
        } catch (err) {
            this.logger.error({
    ...
    async getMessage(emailId, options) {
        ...
        try {
            messageData = await this.request(`/${this.oauth2Username}/messages/${emailId}`, 'get', {
    ...
    async getText(textId, options) {
        ...
        try {
            messageData = await this.request(`/${this.oauth2Username}/messages/${textId}`, 'get', {
                $select: 'body'
            });
        } catch (err) {
            switch (err.oauthRequest?.status) {
    ...
    async uploadMessage(data) {
        ...
        let messageData;
        try {
            messageData = await this.request(`/${this.oauth2Username}/messages`, 'post', Buffer.from(raw.toString('base64')), {
        ...
        if (messageData && messageData.parentFolderId !== targetFolder.id) {
            ...
            try {
                messageData = await this.request(`/${this.oauth2Username}/messages/${messageData.id}/move`, 'post', {
                    destinationId: targetFolder.id
                });
                if (!messageData) {
    ...
    async submitMessage(data) {
        ...
        try {
            ...
            await this.request(`/${this.oauth2Username}/sendMail`, 'post', Buffer.from(raw.toString('base64')), {
                contentType: 'text/plain',
                returnText: true
            });
        } catch (err) {
    ...
    async createMailbox(path) {
        ...
        if (parentFolder) {
            // child folder
            reqUrl = `/${this.oauth2Username}/mailFolders/${parentFolder.id}/childFolders`;
        } else {
            // root folder
            reqUrl = `/${this.oauth2Username}/mailFolders`;
        }
    ...
    async renameMailbox(path, newPath) {
        ...
        if (sourceName !== destinationName) {
            let mailbox;
            try {
                mailbox = await this.request(`/${this.oauth2Username}/mailFolders/${sourceFolder.id}`, 'patch', {
        ...
        if (destinationParentFolder) {
            let mailbox;
            try {
                mailbox = await this.request(`/${this.oauth2Username}/mailFolders/${sourceFolder.id}/move`, 'post', {
    ...
    async deleteMailbox(path) {
        ...
        try {
            await this.request(`/${this.oauth2Username}/mailFolders/${folder.id}`, 'delete', Buffer.alloc(0), { returnText: true });
        } catch (err) {
            this.logger.error({
    ...
    async renewMailboxFolderCache() {
        ...
        let deltaReqUrl = (await this.redis.hget(this.getAccountKey(), 'outlookMailFoldersDeltaUrl')) || `/${this.oauth2Username}/mailFolders/delta`;
    ...
    async getMailboxListing() {
        ...
        for (let specialUseKey of specialUseKeys) {
            ...
            let reqUrl = `/${this.oauth2Username}/mailFolders/${specialUseKey}`;
            try {
        ...
        let traverse = async (pathNamePrefix, folderId) => {
            let list = [];
            let done = false;
            let reqUrl = `/${this.oauth2Username}/mailFolders${folderId ? `/${folderId}/childFolders` : ''}`;
            while (!done) {
    ...
    async ensureSubscription() {
        ...
        if (!outlookSubscription.id) {
            ...
            const subscriptionPayload = {
                ...
                resource: `/${this.oauth2Username}/messages`,
                expirationDateTime: expirationDateTime.toISOString(),
    ...
    async getAttachmentContent(attachmentId, options) {
        ...
        try {
            attachmentResponse = await this.request(`/${this.oauth2Username}/messages/${emailId}/attachments/${id}`);
        } catch (err) {
            switch (err.oauthRequest?.status) {
                case 404: {
