...
class OutlookClient extends BaseClient {
    ...
    async getMessage(emailId, options) {
        ...
        // Bounces recorded against this message by processNew(). The sync path fetches new
        // messages through here as well and never reads the field, so it skips the lookup
        if (!options.skipBounces) {
            await this.attachBounces(formattedMessage);
        }
            ...
            switch (event.type) {
                ...
                case 'created': {
                    ...
                    try {
                        ...
                        // The event is already off the queue, so a failure here must not abort the
                        // drain and lose the events behind it (the other branches catch their own)
                        this.logger.error({ msg: 'Failed to process new message event', type: 'history-event', emailId: event.message, err });
                    }
                    break;
                }
    ...
    // genuine deletion the existence probe answers 404. Only unexpected failures deserve error.
    logMessageFetchFailure(emailId, err) {
        if (err.code === 'MessageNotFound' || err.statusCode === 404) {
            this.logger.debug({ msg: 'Failed to fetch message data', emailId, err });
        } else {
            this.logger.error({ msg: 'Failed to fetch message data', emailId, err });
        }
    }
    ...
    async processDeletedMessage(emailId) {
        ...
        try {
            ...
            this.logMessageFetchFailure(emailId, err);
        }
        if (messageData) {
    ...
    async prepareNewMessage(emailId, options) {
        ...
        // The sync path never reads the bounces field
        options.skipBounces = true;
