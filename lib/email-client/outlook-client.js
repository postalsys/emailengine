...
class OutlookClient extends BaseClient {
    ...
    async submitMessage(data) {
        ...
        // Check if this is a calendar invite (text/calendar attachment)
        // Calendar invites need to be sent as raw MIME to preserve proper formatting
        let hasCalendarInvite = emailObject.attachments?.some(att =>
            att.contentType && att.contentType.toLowerCase().includes('text/calendar')
        );

        if (hasCalendarInvite) {
            // Send calendar invites as raw MIME to preserve functionality
            try {
                ...
                switch (err.oauthRequest?.status) {
                    ...
                    default:
                        this.logger.error({
                            msg: 'Failed to submit calendar invite',
                            messageId,
                            err
                        });
                        throw err;
