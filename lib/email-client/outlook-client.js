...
class OutlookClient extends BaseClient {
    ...
    async getMessages(emailIds, options) {
        ...
        const results = [];
        const BATCH_SIZE = 20; // MS Graph supports higher concurrency
        ...
        for (let i = 0; i < emailIds.length; i += BATCH_SIZE) {
            ...
            const batchResults = await Promise.all(
                batch.map(async emailId => {
                    try {
                        const messageData = await this.request(
                            `/${this.oauth2UserPath}/messages/${emailId}`,
                            'get',
                            {
                                $select: [
                        ...
                        return {
                            messageId: emailId,
                            data: null,
                            error: { message: err.message, code: err.code, statusCode: err.oauthRequest?.status || err.statusCode }
                        };
                    }
                })
            );
