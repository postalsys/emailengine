...
class OutlookClient extends BaseClient {
        ...
        try {
            ...
            switch (err.oauthRequest?.status) {
                case 400: {
                    ...
                    error.statusCode = 500; // reported in the API response and webhook payload
                    // A malformed message fails every retry. Stated explicitly rather than implied
                    // by the 500 above: the submit worker classifies real SMTP replies only.
                    error.permanentDeliveryError = true;
                    throw error;
                }
            ...
            try {
                ...
                switch (err.oauthRequest?.status) {
                    case 400: {
                        ...
                        error.code = err.oauthRequest?.response?.error?.code || 'InvalidMessage';
                        error.statusCode = 500; // reported in the API response and webhook payload
                        // A malformed message fails every retry. Stated explicitly rather than
                        // implied by the 500 above: the submit worker classifies SMTP replies only.
                        error.permanentDeliveryError = true;
                        throw error;
                    }
    ...
    async getToken() {
        ...
        try {
            ...
            await this.reportTokenAcquisitionSuccess(tokenData, this.oAuth2Client?.provider || 'outlook');
        } catch (E) {
            await this.reportTokenAcquisitionFailure(E, this.oAuth2Client?.provider || 'outlook');
            throw E;
        }
