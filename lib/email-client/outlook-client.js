...
class OutlookClient extends BaseClient {
    ...
    async init() {
        ...
        let profileRes;
        try {
            profileRes = await this.request(`/me`);
        } catch (err) {
    ...
    async listMessages(query, options) {
        ...
        if (query.search) {
            if (query.useOutlookSearch) {
                ...
                if ($search) {
                    ...
                    for (let disabledParam of ['$skip', '$orderBy']) {
        ...
        try {
            ...
            totalMessages = !isNaN(listing['@odata.count']) ? Number(listing['@odata.count']) : listing?.value?.length || 0;
    ...
    async getRawMessage(emailId) {
        ...
        try {
            raw = await this.request(`/me/messages/${emailId}/$value`, 'get', Buffer.alloc(0), { returnText: true });
        } catch (err) {
            switch (err.oauthRequest?.status) {
                case 404: {
    ...
    async deleteMessage(emailId, force) {
        ...
        if (force) {
            try {
                await this.request(`/me/messages/${emailId}`, 'delete', Buffer.alloc(0), { returnText: true });
            } catch (err) {
                this.logger.error({
        ...
        let messageData;
        try {
            messageData = await this.request(`/me/messages/${emailId}/move`, 'post', {
    ...
    async updateMessage(emailId, updates) {
        ...
        try {
            modifyResult = await this.request(`/me/messages/${emailId}`, 'patch', flagUpdates);
        } catch (err) {
            this.logger.error({
    ...
    async getMessage(emailId, options) {
        ...
        try {
            messageData = await this.request(`/me/messages/${emailId}`, 'get', {
    ...
    async getText(textId, options) {
        ...
        try {
            messageData = await this.request(`/me/messages/${textId}`, 'get', {
                $select: 'body'
            });
        } catch (err) {
            switch (err.oauthRequest?.status) {
    ...
    async uploadMessage(data) {
        ...
        let messageData;
        try {
            messageData = await this.request(`/me/messages`, 'post', Buffer.from(raw.toString('base64')), {
        ...
        if (messageData && messageData.parentFolderId !== targetFolder.id) {
            ...
            try {
                messageData = await this.request(`/me/messages/${messageData.id}/move`, 'post', {
                    destinationId: targetFolder.id
                });
                if (!messageData) {
    ...
    async submitMessage(data) {
        ...
        try {
            ...
            await this.request(`/me/sendMail`, 'post', Buffer.from(raw.toString('base64')), {
                contentType: 'text/plain',
                returnText: true
            });
        } catch (err) {
    ...
    async createMailbox(path) {
        ...
        if (parentFolder) {
            // child folder
            reqUrl = `/me/mailFolders/${parentFolder.id}/childFolders`;
        } else {
            // root folder
            reqUrl = `/me/mailFolders`;
        }
    ...
    async renameMailbox(path, newPath) {
        ...
        if (sourceName !== destinationName) {
            let mailbox;
            try {
                mailbox = await this.request(`/me/mailFolders/${sourceFolder.id}`, 'patch', {
        ...
        if (destinationParentFolder) {
            let mailbox;
            try {
                mailbox = await this.request(`/me/mailFolders/${sourceFolder.id}/move`, 'post', {
    ...
    async deleteMailbox(path) {
        ...
        try {
            await this.request(`/me/mailFolders/${folder.id}`, 'delete', Buffer.alloc(0), { returnText: true });
        } catch (err) {
            this.logger.error({
    ...
    async renewMailboxFolderCache() {
        ...
        let deltaReqUrl = (await this.redis.hget(this.getAccountKey(), 'outlookMailFoldersDeltaUrl')) || `/me/mailFolders/delta`;
    ...
    async getMailboxListing() {
        ...
        for (let specialUseKey of specialUseKeys) {
            ...
            let reqUrl = `/me/mailFolders/${specialUseKey}`;
            try {
        ...
        let traverse = async (pathNamePrefix, folderId) => {
            let list = [];
            let done = false;
            let reqUrl = `/me/mailFolders${folderId ? `/${folderId}/childFolders` : ''}`;
            while (!done) {
    ...
    async getAttachmentContent(attachmentId, options) {
        ...
        try {
            attachmentResponse = await this.request(`/me/messages/${emailId}/attachments/${id}`);
        } catch (err) {
            switch (err.oauthRequest?.status) {
                case 404: {
    ...
    prepareSearchQuery(search) {
        ...
        for (let key of Object.keys(search)) {
            if (!enabledKeys.includes(key)) {
                let error = new Error(`Unsupported search term "${key}"`);
