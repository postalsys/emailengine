...
class OutlookClient extends BaseClient {
    ...
    async init() {
        ...
        if (!isSendOnly) {
            ...
            if (storedSub.state && (storedSub.state.createRetryCount > 0 || storedSub.state.retryCount > 0)) {
                ...
                await this.redis.hSetExists(this.getAccountKey(), 'outlookSubscription', JSON.stringify(storedSub));
            }
    ...
    async renewSubscription({ force = false, skipLock = false } = {}) {
        ...
        try {
            ...
            // Capture previous retry count before overwriting state
            ...
            try {
                ...
                if (subscriptionRes?.expirationDateTime) {
                    ...
                    outlookSubscription.state = {
                        state: 'created',
                        time: Date.now(),
                        // Clear any previous error state and retry counts
                        retryCount: 0,
                        createRetryCount: 0,
                        error: null
                    };
                ...
                if (retryCount <= OUTLOOK_MAX_RETRY_ATTEMPTS) {
                    ...
                    if (this.renewRetryTimer) {
                        this.logger.debug({ msg: 'Replacing pending subscription retry timer', account: this.account, reason: 'renewal_failure' });
                    }
                    clearTimeout(this.renewRetryTimer);
    ...
    async ensureSubscription({ clearExisting = false } = {}) {
        ...
        try {
            ...
            if (this.renewRetryTimer) {
                this.logger.debug({ msg: 'Replacing pending subscription retry timer', account: this.account, reason: 'lock_error' });
            }
            clearTimeout(this.renewRetryTimer);
            this.renewRetryTimer = setTimeout(() => {
                if (!this.closed) {
                    this.ensureSubscription().catch(retryErr => {
                        this.logger.error({
                            msg: 'Subscription ensure retry after lock error failed',
                            account: this.account,
                            err: retryErr
                        });
                    });
                }
            }, OUTLOOK_RETRY_BASE_DELAY * 1000);
            this.renewRetryTimer.unref();
            return;
        }
            ...
            if (!outlookSubscription.id) {
                ...
                if (outlookSubscription.state?.state === 'error') {
                    ...
                    if (createRetryCount <= OUTLOOK_MAX_RETRY_ATTEMPTS) {
                        ...
                        if (this.renewRetryTimer) {
                            this.logger.debug({ msg: 'Replacing pending subscription retry timer', account: this.account, reason: 'creation_failure' });
                        }
                        clearTimeout(this.renewRetryTimer);
                        this.renewRetryTimer = setTimeout(() => {
                            if (this.closed) {
                            ...
                            this.ensureSubscription().catch(err => {
                                this.logger.error({
                                    msg: 'Subscription creation retry failed',
                                    account: this.account,
