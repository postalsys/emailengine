...
const { isTransientCredentialError } = require('./credential-errors');
...
const he = require('he');
const { emitChangeEvent, prepareUrl, collectCidReferences, cidReferenceRegex, truncateTextContents } = require('../tools');
const { messageWebSafeHtml, applyWebSafeHtmlOptions } = require('../web-safe-html');
...
const { buildRawSendMailRequest, buildStructuredSendMailRequest, buildDraftSendRequest } = require('./outlook/send-request');
const { pushHeaderValue } = require('../utils/header-map');
...
// The `serverResponseCode` a failed change subscription is reported with, and the marker that
// tells the report apart from anything else the account may have stored. Only this client reads
// it: reportSubscriptionFailure() writes it, clearSubscriptionFailure() lifts it, and
// isSelfClearedErrorState() is what keeps a login from taking it along
const SUBSCRIPTION_SETUP_ERROR_CODE = 'SubscriptionSetupError';
 ...
 */
// Status for a Graph failure the caller did not map to a code of its own. The transport passes
// the provider status through (a mapped Graph error carries its own), so a throttled or failing
// provider is reported as such instead of as the caller's bad request
function graphFailureStatus(err) {
    return Number(err?.statusCode) >= 400 ? Number(err.statusCode) : 400;
}
...
class OutlookClient extends BaseClient {
    constructor(account, options) {
        ...
        // A failed init() is scheduled again by the worker, see BaseClient.scheduleInitRetry()
        this.retriesInit = true;
        ...
        // Whether clearSubscriptionFailure() has looked for a report written before this process
        // started. Deliberately not persisted: it asks a question only a restart can raise
        this.storedSubscriptionReportChecked = false;
    }
    ...
    async init() {
        ...
        clearTimeout(this.renewRetryTimer);
        this.cancelDeferredRetryTimer();
        this.cancelInitRetry();
        await this.setStateVal();

        await this.getAccount();

        // The auth-failure safety net switches an account off by setting `imap.disabled`, and
        // an API-backed account reaches it the same way an IMAP one does - its failures run
        // through the same setErrorState(). Checked before any credential work, because the
        // work that keeps failing for these accounts is the token refresh below.
        if (await this.isSyncDisabled()) {
            this.logger.info({ msg: 'Skipped connection setup', account: this.account, reason: 'disabled' });
            this.state = 'unset';
            await this.setStateVal();
            return;
        }
        ...
        try {
            ...
            if (isTransientCredentialError(err)) {
                // Not a refused credential - left unflagged so the worker retries init() instead of
                // reporting an authentication error only re-authorization could clear
                throw err;
            }
        ...
        // Before the state flip below - see notifyAuthenticationSuccess(). This used to run after
        // it, which is why a Graph account never announced its first login at all
        await this.notifyAuthenticationSuccess(accountData.oauth2?.auth?.user);
        ...
        if (!isSendOnly) {
            ...
            await this.resetSubscriptionRetryState();
            ...
            try {
                ...
                this.logger.warn({ msg: 'Failed to renew mailbox folder cache', err });
            }

            // Start processing any pending webhook notifications
            // (this also picks up the ones deferred for a retry before a restart)
            this.triggerSync();
        }
    }
    ...
    /**
     * Performs an on-demand catch-up sync. Triggered by the "Run sync" action
     * (PUT /v1/account/{account}/sync). Outlook syncs via webhook subscriptions and delta
     * queries rather than a folder walk, so a manual sync refreshes the folder cache and runs
     * the missed-notification recovery (with the cooldown bypassed for this explicit action).
     */
    async syncMailboxes() {
        if (this.closed) {
            return null;
        ...
        try {
            ...
            this.logger.warn({ msg: 'Failed to refresh folder cache for manual sync', account: this.account, err });
        }

        await this.syncMissedMessages({ force: true });
    ...
    async delete() {
        ...
        try {
            ...
            this.logger.warn({ msg: 'Failed to delete MS Graph subscription', account: this.account, err });
        }
     ...
     */
    async reconnect() {
        // A closed instance can be revived here, as IMAPClient.reconnect(force) does: the
        // subscription setup and the sync paths return on `closed`, and init() does not reset it,
        // so without this a reconnect after close() reported a connected account that never
        // received a notification. The auth-failure safety net produces exactly that instance - the
        // park closes the client but leaves it assigned, and switching the flag back off through
        // PUT /v1/account reconnects it in place.
        this.closed = false;
    ...
    async listMailboxes(options) {
        ...
        if (!cachedListing || options?.statusQuery?.messages || (await this.renewMailboxFolderCache())) {
            ...
            await this.setCachedMailboxListing(mailboxListing);
    ...
    async deleteMessage(emailId, force) {
        ...
        // A message already in Deleted Items is purged instead of being "moved" there again,
        // matching what deleteMessages() does for that folder. The folder listing is read once
        // for both resolutions, the way deleteMessages() reads it
        let mailboxListing;
        let sourceFolder;
        try {
            let currentData = await this.request(`/${this.oauth2UserPath}/messages/${emailId}`, 'get', { $select: 'parentFolderId' });
            if (currentData?.parentFolderId) {
                mailboxListing = (await this.getCachedMailboxListing()) || (await this.getMailboxListing());
                sourceFolder = await this.resolveFolder(currentData.parentFolderId, { byId: true, mailboxListing });
            }
        } catch (err) {
            // the move below reports a missing message on its own
            this.logger.debug({ msg: 'Failed to resolve the source folder of a message', emailId, err });
        }

        if (sourceFolder?.specialUse === '\\Trash') {
            return await this.deleteMessage(emailId, true);
        }

        // Move to trash (soft delete)
        let messageData;
        try {
            messageData = await this.requestWithRetry(`/${this.oauth2UserPath}/messages/${emailId}/move`, 'post', {
        ...
        let folder;
        try {
            folder = await this.resolveFolder(messageData.parentFolderId, { byId: true, mailboxListing });
        } catch (err) {
            this.logger.warn({
    ...
    async updateMessages(path, search, updates) {
        ...
        if (needsCurrentCategories) {
            ...
            let submitFetchBatch = async () => {
                ...
                try {
                    ...
                    // Without the current categories the PATCH below would replace every
                    // message's category list with just the added labels, so a failed fetch
                    // has to abort the update instead of being logged past
                    this.logger.error({
        ...
        // Process messages in batches
        for (let emailId of emailIds) {
            if (needsCurrentCategories && !currentCategories.has(emailId)) {
                // The category fetch for this message failed (logged above) - skipping it
                // keeps its existing categories instead of overwriting them with the additions
                continue;
            }
            batch.push(formatRequest(emailId));
            // submit batch when it reaches max size
        ...
        try {
            ...
            switch (err?.oauthRequest?.response?.error?.code) {
                ...
                default: {
                    ...
                    let error = new Error('Move failed');
                    error.info = {
                        response: err?.oauthRequest?.response?.error?.message || err?.message
                    };
                    error.code = err?.oauthRequest?.response?.error?.code || err?.code;
                    error.statusCode = graphFailureStatus(err);
                    throw error;
                }
            }
        }
    ...
    async getMessage(emailId, options) {
        ...
        if (messageData.internetMessageHeaders) {
            ...
            for (let header of messageData?.internetMessageHeaders || []) {
                ...
                if (!pushHeaderValue(headers, name, value)) {
                    continue;
                }
        ...
        if (options.detectBounce) {
            await this.detectBounce(formattedMessage);
        }
        ...
        if (options.preProcessHtml && formattedMessage.text && (formattedMessage.text.html || formattedMessage.text.plain)) {
            formattedMessage.text.html = await messageWebSafeHtml(formattedMessage);
        ...
        if (options.embedAttachedImages && formattedMessage.text?.html && formattedMessage.attachments) {
            ...
            let cidReferences = collectCidReferences(formattedMessage.text.html);
            ...
            for (let attachment of formattedMessage.attachments) {
                ...
                if (contentId && !attachmentMap.has(contentId) && cidReferences.has(contentId)) {
            ...
            formattedMessage.text.html = formattedMessage.text.html.replace(cidReferenceRegex(), (fullMatch, cidMatch) => {
    ...
    async getText(textId, options) {
        ...
        response.hasMore = truncateTextContents(response, options.maxBytes);
        ...
        for (let key of Object.keys(emailObject)) {
            switch (key) {
                ...
                case 'attachments': {
                    ...
                    let cidReferences = collectCidReferences(emailObject.html);
                    for (let attachment of emailObject.attachments || []) {
                        ...
                        if (attachment.cid) {
                            ...
                            if (cidReferences.has(attachmentEntry.contentId)) {
     ...
     * Verifies that the message is a Graph draft and downloads its raw content
     * @param {string} emailId - Graph message id of the draft
     * @returns {Object} { raw: Buffer, draft: { message } }
     */
    async resolveDraftForSubmit(emailId) {
        await this.prepare();

        // The draft check and the raw download have no data dependency, so run them in parallel
        let messageData;
        let raw;
        try {
            [messageData, raw] = await Promise.all([
                this.request(`/${this.oauth2UserPath}/messages/${emailId}`, 'get', { $select: 'id,isDraft' }),
                this.getRawMessage(emailId)
            ]);
        } catch (err) {
            if (err.oauthRequest?.status === 404 || err.code === 'NotFound') {
                let error = new Error('Requested message was not found');
        ...
        return { raw, draft: { message: emailId } };
    }

    /**
     * Sends a queued message through Graph. Three delivery shapes: an existing draft is sent
     * via /messages/{id}/send, otherwise the message goes through /sendMail as raw MIME
     * (default - preserves calendar invites and special MIME types) or as a structured JSON
     * message (opt-in - respects the from field for shared mailboxes)
     * @param {Object} data - Submission payload
     * @param {Buffer} raw - Raw message source
     */
    async sendGraphMessage(data, raw) {
        let { messageId } = data;

        if (data.draft) {
            // Send the existing draft message - Graph files it into Sent Items itself
            try {
                let sendRequest = buildDraftSendRequest(data.draft.message, this.oauth2UserPath);
                await this.request(sendRequest.path, 'post', sendRequest.body, sendRequest.options);
            } catch (err) {
                switch (err.oauthRequest?.status) {
                    case 400:
                    case 404: {
                        // the draft is gone or not sendable - retrying would not help
                        let error = new Error(err.oauthRequest.status === 404 ? 'Draft message was not found' : 'Draft message can not be sent');
                        error.info = {
                            response: err.oauthRequest?.response?.error?.message || error.message
                        };
                        error.code = err.oauthRequest?.response?.error?.code || 'MessageNotFound';
                        error.statusCode = 500; // do not retry sending
                        throw error;
                    }
                    ...
                    default:
                        this.logger.error({
                            msg: 'Failed to submit draft message',
                            messageId,
                            draft: data.draft.message,
                            err
                        });
                        throw err;
                }
            }
        } else if (data.useStructuredFormat) {
            ...
            try {
                ...
                let sendRequest = buildStructuredSendMailRequest(messagePayload, this.oauth2UserPath);
                await this.request(sendRequest.path, 'post', sendRequest.body, sendRequest.options);
            } catch (err) {
                switch (err.oauthRequest?.status) {
                    case 400: {
            ...
            try {
                let sendRequest = buildRawSendMailRequest(raw, this.oauth2UserPath);
                await this.request(sendRequest.path, 'post', sendRequest.body, sendRequest.options);
            } catch (err) {
                switch (err.oauthRequest?.status) {
                    case 400: {
    ...
    async submitMessage(data) {
        ...
        try {
            await this.sendGraphMessage(data, raw);
        } catch (err) {
            // Report the failure the way the SMTP path does (messageDeliveryError webhook,
            // feedback key) before the job decides whether to retry
            await this.handleSubmitError(err, { data, jobData, queueId, envelope });
            throw err;
        }
    ...
    async createMailbox(path) {
        ...
        let mailbox;
        try {
            mailbox = await this.requestWithRetry(reqUrl, 'post', {
                displayName,
                isHidden: false
            });
            if (!mailbox) {
        ...
        setImmediate(() => {
            ...
            this.listMailboxes().catch(err => {
                this.logger.error({ msg: 'Failed to refresh mailbox listing cache', err });
            });
        });
    ...
    async renameMailbox(path, newPath) {
        ...
        if (sourceParentPath !== destinationParentPath) {
            if (!destinationParentPath) {
                // Moving to the top level: Graph addresses the root through its well-known name
                destinationParentFolder = { id: 'msgfolderroot' };
            } else {
                destinationParentFolder = await this.resolveFolder(destinationParentPath);
                if (!destinationParentFolder) {
                    // Resolved before the rename step so a bad target leaves the folder untouched
                    let error = new Error('Rename failed');
                    error.info = {
                        response: 'Not able to find the destination parent folder'
                    };
                    error.code = 'NotFound';
                    error.statusCode = 404;
                    throw error;
    ...
    async getAccount() {
        ...
        // Pass `call` so Account.update() can dispatch a reconnect back to this worker when
        // OAuth credentials are re-authorized for an account in an error state (same channel the
        // IMAP path uses).
        this.accountObject = new Account({ redis: this.redis, account: this.account, secret: await getSecret(), call: msg => this.call(msg) });
    ...
    /**
     * Reports that the account has no working change subscription left to fall back on - and for a
     * Graph account there is no fallback: unlike the Gmail client there is no polling timer, so a
     * subscription that cannot be established means the account never sees new mail again. Both the
     * creation and the renewal paths retry with backoff and then give up; giving up used to write a
     * log line and nothing else, so the account went on reporting `connected`. A tenant that
     * disables the Exchange Online service principal (AADSTS500014) hits this without ever failing a
     * token refresh, so no authentication error fires either - reported in the field after nine
     * months of accounts looking healthy and syncing nothing.
     *
     * Sent as `connectError` rather than `authenticationError` on purpose: the credential is fine,
     * and re-authorizing does not lift a disabled service principal. It also keeps the failure away
     * from the auth-failure safety net, which would switch the account off after three days and
     * offer the operator only a re-authorization that cannot help.
     *
     * NB! `connectError` is not in VALID_STATES, so this also makes the account's own operations
     * (reading a message, moving one) answer 503 until a subscription works again. That is the
     * intended reading for an account that cannot sync at all, and it is the only reading this
     * state machine offers - there is no degraded-but-usable state.
     *
     * setErrorState() suppresses a repeat of the same code, so a permanently broken tenant produces
     * one webhook per distinct failure rather than one per renewal cycle
     * @param {String} stage - 'creation' or 'renewal', for the log line and the fallback message
     * @param {Object} state - The stored subscription state, carrying the last error text
     * @param {Number} retryCount - Attempts made, for the log line
     * @returns {Promise<void>}
     */
    async reportSubscriptionFailure(stage, state, retryCount) {
        this.logger.error({ msg: `Max subscription ${stage} retries exceeded`, account: this.account, retryCount });

        try {
            await this.notify(false, CONNECT_ERROR_NOTIFY, {
                response: state?.error || `MS Graph subscription ${stage} failed`,
                serverResponseCode: SUBSCRIPTION_SETUP_ERROR_CODE
            });
        } catch (err) {
            // Best effort. This is awaited from inside ensureSubscription(), which init() awaits in
            // turn, so letting a Redis hiccup out of here would fail the whole account setup over a
            // report about a failure that has already been logged
            this.logger.error({ msg: 'Failed to report the subscription failure', account: this.account, err });
        }
    }

    /**
     * A failed change subscription is this client's own report to lift. A tenant that disabled the
     * Exchange Online service principal keeps refreshing tokens and logging in perfectly while no
     * subscription can be created, so a login says nothing about it and clearing it on one leaves
     * the account reporting healthy while it syncs nothing
     * @param {Object} parsedState - The stored `lastErrorState`, parsed
     * @returns {boolean} True when the stored run is a reported subscription failure
     */
    isSelfClearedErrorState(parsedState) {
        return parsedState?.serverResponseCode === SUBSCRIPTION_SETUP_ERROR_CODE;
    }

    /**
     * Lifts the error state reportSubscriptionFailure() left behind, once a subscription is working
     * again - which is every path that concludes there is a working one, not only a renewal that
     * had to make a request. A subscription 60 hours from expiry is checked hourly and renewed
     * once, so an account reported while it had one would have stayed in `connectError` for two
     * days with notifications arriving the whole time.
     *
     * Only touches a failure this client reported, so an unrelated error state is left for whatever
     * owns it. A report this process wrote is visible in `this.state`; the one way a standing
     * report is not is a worker that restarted since it was written, which can only be true once
     * per account per worker - so a healthy account calling this every hourly pass reads the stored
     * state once and then costs nothing.
     *
     * @returns {Promise<void>}
     */
    async clearSubscriptionFailure() {
        if (this.state !== 'connectError' && this.storedSubscriptionReportChecked) {
            // Nothing of ours to take back
            return;
        }

        // Best effort throughout. Both callers report a throw here as a Graph subscription that
        // could not be renewed or created: it would count against the retry budget and, at the cap,
        // announce a Redis error text to the operator as the tenant's problem
        try {
            const storedValue = await this.redis.hget(this.getAccountKey(), 'lastErrorState');
            this.storedSubscriptionReportChecked = true;

            let stored;
            try {
                stored = storedValue && JSON.parse(storedValue);
            } catch (err) {
                stored = null;
            }

            const isOwnReport = this.isSelfClearedErrorState(stored);

            if (stored && !isOwnReport) {
                return;
            }

            if (this.state !== 'connectError' && !isOwnReport) {
                // Nothing of ours to take back
                return;
            }

            // The clear goes first, and it only applies while the error is still the one judged
            // above: a failure written since then is a different one, and reporting `connected`
            // over it is exactly the state this whole path exists to stop
            if (!(await this.clearStoredErrorState(storedValue))) {
                this.logger.info({ msg: 'Subscription recovered into a newer error state, leaving it alone', account: this.account });
                return;
            }
            ...
            this.logger.error({ msg: 'Failed to clear the reported subscription failure', account: this.account, err });
        }
    }
    ...
    async getTokenData() {
        ...
        try {
            ...
            if (this.state === 'authenticationError') {
                // A valid token means the credential recovered, so announce it here as well as in
                // init() - this is also what clears the stored error state, which would otherwise
                // outlive the error it describes. Deliberately only the authentication error: a
                // token says nothing about a `connectError` from a subscription that cannot be
                // established, and treating it as a recovery is what let a tenant with a disabled
                // service principal keep reporting `connected`
                await this.notifyAuthenticationSuccess(tokenData.user);
            ...
            if (E.code === 'ETokenRefresh') {
                await this.reportTokenRefreshFailure(E, this.oAuth2Client?.provider || 'outlook');
            }

            throw E;
        }

        return { accessToken: tokenData.accessToken, cached: !!tokenData.cached };
    }
    ...
    setupRenewWatchTimer() {
        ...
        this.renewWatchTimer = setTimeout(() => {
            ...
            // Never rejects - the pass reports its own failures
            this.renewOrCreateSubscription().finally(() => this.setupRenewWatchTimer());
        }, RENEW_WATCH_TTL);
        this.renewWatchTimer.unref();
    }

    /**
     * Clears the retry counters a failing subscription left on the stored record, so the next
     * attempt starts with a full fast-retry budget rather than the exhausted one it inherited.
     * Shared by the two occasions that count as a fresh start: a reconnect, and the hourly pass
     * @returns {Promise<void>}
     */
    async resetSubscriptionRetryState() {
    ...
    /**
     * One pass of the renewal timer: renews the change subscription, and recreates it when there is
     * nothing left to renew.
     *
     * This is the account's only slow retry. The fast ones ensureSubscription() schedules are
     * capped, so a tenant-side failure - a disabled service principal, a permission an admin
     * revoked - exhausts them within minutes and leaves the account with no subscription at all.
     * This pass used to read that count, find it at the cap and give up, waiting for a reconnect
     * that only a worker restart, an account update or a re-authorization brings; a tenant that
     * fixed the cause an hour later was never noticed. Unlike the Gmail client there is no poller
     * behind this to fall back on, so an account left unsubscribed syncs nothing at all while it
     * waits. The counters are cleared before anything else is attempted, so an hour that could have
     * recovered gets the fast ladder back instead of one attempt every hour forever - a renewal
     * ladder as much as a creation one, since a subscription inside the renew window whose retries
     * are spent is not recreated by this pass at all and would otherwise never get another.
     *
     * A failing account is not heard twice, but it is logged twice: reportSubscriptionFailure()
     * stores the same serverResponseCode every pass, which is what setErrorState() suppresses a
     * repeat notification on, while the log line it writes recurs hourly. The account stays in
     * `connectError` until a creation actually succeeds.
     *
     * @returns {Promise<void>}
     */
    async renewOrCreateSubscription() {
        try {
            await this.resetSubscriptionRetryState();

            const renewalResult = await this.renewSubscription({ force: false });

            // Only a subscription there is nothing left to renew is recreated here; any other
            // failure leaves one in place, and the reset above is what gives it another ladder
    ...
    async getCachedMailboxListing() {
        ...
        if (cachedListingValue) {
            try {
                ...
                this.logger.warn({ msg: 'Failed to parse cached mailbox listing', err });
            }
        }
        return cachedListing;
    }

    async setCachedMailboxListing(mailboxListing) {
        try {
    ...
    async resolveFolder(path, options) {
        ...
        let cachedListing = !options.mailboxListing && (await this.getCachedMailboxListing());
        let mailboxListing = options.mailboxListing || cachedListing || (await this.getMailboxListing());

        if (options.byId) {
            let folder = mailboxListing.find(entry => entry.id === path);
            if (!folder && cachedListing && (await this.renewMailboxFolderCache())) {
                // A folder created after the listing was cached: without the refresh a new message
                // in it was reported as missing instead of new. Only when the folder delta reports
                // a change, as a folder the listing never includes would otherwise cost a full
                // folder crawl per message
                try {
                    mailboxListing = await this.getMailboxListing();
                } catch (err) {
                    // the delta check consumed the change, forget it so the next miss asks again
                    await this.redis.hdel(this.getAccountKey(), 'outlookMailFoldersDeltaUrl');
                    throw err;
                }
                await this.setCachedMailboxListing(mailboxListing);
                folder = mailboxListing.find(entry => entry.id === path);
            }
            return folder;
        }
    ...
    async renewSubscription({ force = false, skipLock = false } = {}) {
        ...
        try {
            ...
            if (!shouldRenew) {
                ...
                // A subscription that is still good is the same recovery as one just renewed
                await this.clearSubscriptionFailure();
            ...
            try {
                ...
                if (retryCount < OUTLOOK_MAX_RETRY_ATTEMPTS) {
                    ...
                    await this.reportSubscriptionFailure('renewal', outlookSubscription.state, retryCount);
                }
    ...
    async ensureSubscription({ clearExisting = false } = {}) {
        ...
        if (!serviceUrl) {
            this.logger.error({ msg: 'Cannot create MS Graph subscription, service URL is not set' });
            return false;
        }
        ...
        try {
            ...
            if (outlookSubscription.id) {
                ...
                if (existingExpirationDateTime && existingExpirationDateTime.getTime() < now + OUTLOOK_CLOCK_SKEW_BUFFER) {
                    ...
                    this.subscriptionState = 'valid';
                    // A subscription that is still good is the same recovery as one just created
                    await this.clearSubscriptionFailure();
                    return;
                }
            }
            ...
            if (!outlookSubscription.id) {
                ...
                if (outlookSubscription.state?.state === 'error') {
                    ...
                    if (createRetryCount < OUTLOOK_MAX_RETRY_ATTEMPTS) {
                        ...
                        // End of the fast ladder, not the end of trying: renewOrCreateSubscription()
                        // clears the counters and starts a new one on the next hourly pass
                        await this.reportSubscriptionFailure('creation', outlookSubscription.state, createRetryCount);
                    }
                }
            }
        } finally {
    ...
    formatMessage(messageData, options) {
        let { extended, path, textType, maxBytes, showPath, minimalFields } = options || {};
        ...
        const textHasMore = truncateTextContents(textContents, maxBytes);
        ...
        let message = {
            ...
            text: {
                ...
                hasMore: textContents?.plain || textContents?.html ? textHasMore : undefined
            },
        ...
        if (messageData.attachments?.length) {
            let cidReferences = collectCidReferences(message.text.html);
            message.attachments = messageData.attachments.map(entry => {
                ...
                if (entry.contentId) {
                    ...
                    if (cidReferences.has(attachment.contentId.replace(/^[\s<]*|[\s>]*$/g, ''))) {
    ...
    async getAttachmentContent(attachmentId, options) {
        ...
        const [emailId, id] = this.unpackApiId(attachmentId, parts => parts.length >= 2);
    ...
    triggerSync() {
        ...
        this.processHistory()
            ...
            .finally(() => {
                ...
                this.scheduleDeferredRetryTimer();
            });
    }
    ...
    async processHistory() {
        ...
        // The event is already off the queue, so a failure must not abort the drain and lose the
        // events behind it. A transient one is deferred and retried instead
        const handle = async event => {
            try {
                await this.processChangeEvent(event, context);
            } catch (err) {
                await this.deferFailedChangeEvent(event, err, event.type === 'created' ? [event.message] : null);
            }
        };

        // Changes deferred for a retry go first, then the queue
        try {
            await this.processDueDeferredEvents(handle);
        } catch (err) {
            this.logger.error({ msg: 'Failed to process deferred mailbox changes', account: this.account, err });
        }
    ...
    /**
     * Processes one change notification, throwing when it could not be processed
     * @param {Object} event - Queue event
     * @param {Object} context - Shared by the events of one drain, caches the fetch options
     */
    async processChangeEvent(event, context) {
        switch (event.type) {
            case 'updated':
            ...
            case 'created': {
                if (!context.newMessageOptions) {
                    // cache options for efficiency
                    context.newMessageOptions = await this.getMessageFetchOptions();
                    context.newMessageOptions.showPath = true;
                }

                let messageData = await this.prepareNewMessage(event.message, context.newMessageOptions);
                if (messageData) {
                    ...
                    let dedupeKey = `${messageData.id}:created`;
                    if (await this.isRecentlySeen(dedupeKey, messageData.path)) {
                        this.logger.debug({ msg: 'Ignore recently seen new email event', type: 'history-event', emailId: event.message });
                        break;
                    }

                    await this.processNew(messageData, context.newMessageOptions);
                    await this.markSeen(dedupeKey, messageData.path);
                }
                break;
            }

            default:
                this.logger.debug({ msg: 'Ignoring unknown history event type', type: 'history-event', event });
                break;
        }
    }
     ...
     */
    async syncMissedMessages(opts) {
        let { force } = opts || {};

        // Debounce: skip if recovery already ran recently. A manual "Run sync" passes
        // force:true to bypass the cooldown; duplicate suppression is still handled per
        // message by the dedupe lock below, so forcing cannot re-emit seen messages.
        ...
        for (const msg of messages) {
            const dedupeKey = `${msg.id}:created`;
            try {
                // Pre-filter: check the dedupe lock BEFORE the expensive getMessage() call
                let folder = folderMap.get(msg.parentFolderId);
                if (folder && (await this.isRecentlySeen(dedupeKey, folder.pathName))) {
                    continue;
                }
                ...
                if (!folder && (await this.isRecentlySeen(dedupeKey, messageData.path))) {
                    continue;
                }
                ...
                await this.deferFailedChangeEvent({ type: 'created', message: msg.id }, err, [msg.id]);
            }
        }
    ...
    // A message gone by the time a change notification is processed is a routine race: for every
    // genuine deletion the existence probe answers 404. Any other failure says nothing about the
    // message, so it is thrown for processHistory() to retry - a 5xx here used to be reported as
    // a deletion, and an update was dropped.
    fetchChangedMessage(emailId, fields) {
        return this.request(`/${this.oauth2UserPath}/messages/${emailId}`, 'get', { $select: fields.join(',') }).catch(err => {
            if (err.code === 'MessageNotFound' || err.statusCode === 404) {
                this.logger.debug({ msg: 'Failed to fetch message data', emailId, err });
                return null;
            }
            throw err;
        });
    ...
    async processDeletedMessage(emailId) {
        ...
        let messageData = await this.fetchChangedMessage(emailId, ['id', 'parentFolderId']);
        if (messageData) {
    ...
    async processUpdatedMessage(emailId) {
        let messageData = await this.fetchChangedMessage(emailId, ['id', 'isRead', 'isDraft', 'flag', 'conversationId', 'parentFolderId']);
        if (!messageData) {
        ...
        // Check the dedupe lock to see if we recently processed the update event for the same email
        let dedupeKey = `${messageData.id}:updated`;
        let dedupeValue = JSON.stringify(changes.flags.value);
        if (await this.isRecentlySeen(dedupeKey, dedupeValue)) {
        ...
        await this.markSeen(dedupeKey, dedupeValue);
    }
    ...
    async prepareNewMessage(emailId, options) {
        this.logger.debug({ msg: 'New message', id: emailId });
        ...
        let messageData;
        try {
            messageData = await this.getMessage(emailId, options);
        } catch (err) {
            if (err.statusCode !== 404) {
                throw err;
            }
            // deleted before the notification was processed - reported as missing below, the
            // way the IMAP path reports a FETCH that finds nothing
        }
    ...
    async searchEmailIds(path, search) {
        // Pages through the whole result set, as the IMAP client does - the callers apply the
        // change in Graph-sized batches. Only ids are needed, so the listing is the minimal one
        let emailIds = [];
        let cursor;
        ...
        do {
            ...
            for (let message of messageListResult?.messages || []) {
    ...
    prepareSearchQuery(search) {
        ...
        const enabledKeys = ['from', 'to', 'cc', 'bcc', 'subject', 'larger', 'smaller', 'body', 'before', 'sentBefore', 'since', 'sentSince'];
        ...
        if (search.before) {
            searchParts.push(`received<=${escapeString(search.before)}`);
        }

        if (search.sentBefore) {
            searchParts.push(`sent<=${escapeString(search.sentBefore)}`);
        }

        if (search.since) {
            searchParts.push(`received>=${escapeString(search.since)}`);
        }

        if (search.sentSince) {
            searchParts.push(`sent>=${escapeString(search.sentSince)}`);
        }
    ...
    prepareFilterQuery(search) {
        ...
        if (search.before) {
            filterParts.push(`receivedDateTime lt ${this.formatSearchTerm(search.before, false)}`);
        }

        if (search.sentBefore) {
            filterParts.push(`sentDateTime lt ${this.formatSearchTerm(search.sentBefore, false)}`);
        }

        if (search.since) {
            filterParts.push(`receivedDateTime gt ${this.formatSearchTerm(search.since, false)}`);
        }

        if (search.sentSince) {
            filterParts.push(`sentDateTime gt ${this.formatSearchTerm(search.sentSince, false)}`);
        }
    ...
    /**
     * The dedupe buckets a key is looked up in, newest first: one bucket to the future, the
     * current 10 minute bucket, and one to the past
     */
    dedupeBucketKeys() {
        let currentBucketTime = new Date(`${new Date().toISOString().substring(0, 15)}0:00.000Z`).getTime();
        return [-1, 0, 1].map(i => `${REDIS_PREFIX}bck:${new Date(currentBucketTime - i * 10 * 60 * 1000).toISOString()}`);
    }

    /**
     * Whether the same change was recently processed with the same value
     */
    async isRecentlySeen(key, value = '1') {
        let pipeline = this.redis.multi();
        for (let bucketKey of this.dedupeBucketKeys()) {
        ...
        for (let [, bucketRes] of pipelineRes) {
            if (bucketRes) {
                // Always check newer first. If there is an outdated value in an older bucket, then we use the newer value
                return bucketRes === value;
            }
        }
        return false;
    }
    ...
    /**
     * Records a processed change for isRecentlySeen(). Callers mark a change only once it was
     * processed, so a change that failed is not skipped as "recently seen" on its retry
     */
    async markSeen(key, value = '1') {
        let bucketKey = this.dedupeBucketKeys()[1];
        await this.redis
            .multi()
