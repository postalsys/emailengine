...
class BaseClient {
    ...
    async reportTokenAcquisitionFailure(err, provider) {
        if (err.code === 'ETokenRefresh') {
            ...
            // treat as authentication failure. Set unconditionally: when the error was already
            // notified the branch below is skipped, so this is the only write of the state.
        ...
        // Anything that is not an ETokenRefresh - an undici network failure (which leaves err.code
        // undefined), a Redis lock failure, or a Workload Identity Federation error (ESignJwt /
        // ESTSExchange / ESubjectTokenRead) - used to fall straight through to the caller's rethrow.
        // init() is dispatched fire-and-forget by workers/imap.js, so nothing caught it: no state
        // was written, no webhook fired, and there is no retry loop on these transports. The account
        // simply sat at 'connecting' forever, which the admin UI renders as an ordinary info badge,
        // indistinguishable from a healthy account starting up. Report it as a connection error so
        // it is at least visible.
        //
        // No explicit setStateVal() here: this branch always reaches notify(), which routes through
        // setErrorState() and both assigns and persists the state. Writing it first as well cost a
        // second transaction and double-counted state:count:connectError.
        err.errorNotified = true;

        await this.notify(false, CONNECT_ERROR_NOTIFY, {
            // The errno goes in `response`, not in `serverResponseCode`. setErrorState() treats a
            // changed serverResponseCode as a new first occurrence, so varying it per errno would
            // reset the auth-failure window and fire a webhook on every flip for a provider
            // alternating between, say, ETIMEDOUT and ECONNRESET.
            response: err.code ? `${err.code}: ${err.message}` : err.message,
            serverResponseCode: 'TokenGenerationError'
        });
    }
     ...
     *
     * @param {Object} ctx - Client reporting the failure. Subconnection.notify() is a deliberate
     *   no-op, so a secondary connection failing raises no account-level event.
    ...
    async notifyTransientTokenFailure(ctx, err) {
        ...
        // notify() normally persists this through setErrorState(); assigned here too so the state
        // is still correct when notify() is a Subconnection no-op or threw above.
