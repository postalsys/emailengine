...
class BaseClient {
     ...
     * the raw message is downloaded only when the message looks like a bounce, and a delivery or
     * delay report is not a bounce, as on the arrival path. The folder is not consulted here, so
     * a bounce filed away is still reported as one. A failure is logged and leaves the fields
     * unset rather than failing the fetch
     * @param {Object} messageData - Formatted message details, updated in place
    ...
    async detectBounce(messageData, getContent = () => this.getRawMessage(messageData.id)) {
        if (!this.looksLikeABounce(messageData)) {
            return;
        }
