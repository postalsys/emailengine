...
function isTransientError(err) {
    ...
    // Check for network errors. undici's fetch rejects with a generic TypeError and hangs the real
    // errno off err.cause, so both have to be checked - see loadOAuth2LoginCredentials.
...
class BaseClient {
    ...
    /**
     * Reports a failure raised while acquiring an OAuth2 access token on an API transport, and
     * marks the error so a caller further up cannot report it a second time.
     *
     * Shared by GmailClient and OutlookClient, whose catch blocks were byte-identical. Keeping one
     * copy is what stops the two transports emitting different webhook payloads for the same
     * failure - the `oauthRequest` vs `tokenRequest` mixup this replaced had to be fixed twice.
     *
     * @param {Error} err - The error thrown while acquiring a token. Mutated: gains `errorNotified`
     *   and, for a credential rejection, `authenticationFailed`.
     * @param {String} provider - Provider label for the metrics counter
     * @returns {Promise<void>}
     */
    async reportTokenAcquisitionFailure(err, provider) {
        // Only a credential rejection is reported here. Anything else - an undici network failure, a
        // Redis lock failure, a Workload Identity Federation error - is deliberately left for the
        // caller's own catch to classify.
        //
        // A catch-all branch reporting those as connectError was tried and removed. getToken() runs
        // on EVERY Gmail/Graph API request, not just on connect, so a single blip during routine
        // polling persisted connectError over a healthy account and fired a false webhook; it also
        // caught errors that are not token failures at all (loadAccountData()'s Boom 404 for an
        // account deleted mid-request). Worse, setting `errorNotified` on that path silently
        // disabled the `if (!err.errorNotified)` reporters in GmailClient.init()/OutlookClient.init()
        // - which still persisted state='authenticationError' - leaving the stored state, the stored
        // lastErrorState and the delivered webhook describing three different things. Those init()
        // catches already report this class of failure with serverResponseCode 'ApiRequestError',
        // which is why nothing is lost by returning here.
        ...
        // Reporting must never replace the failure being reported. Both callers are
        // `await this.reportTokenAcquisitionFailure(E, ...); throw E;` - so a Redis blip in
        // setStateVal() or notify() would escape here, skip that rethrow, and hand the caller an
        // error carrying neither `code` nor `authenticationFailed`. Downstream that reads as an
        // unclassified failure, which is how a queued message ends up discarded over a Redis hiccup.
        try {
            // Marks the error so GmailClient.init()/OutlookClient.init(), which rethrow past their
            // own `if (!err.errorNotified)` reporters, do not report this a second time.
            err.errorNotified = true;

            // No setStateVal() here: notify() routes through setErrorState(), which both assigns and
            // persists the state. Writing it first as well cost a second Redis transaction and
            // double-counted state:count:authenticationError on every token rejection.
            await this.notify(false, AUTH_ERROR_NOTIFY, {
                // A token error carries `tokenRequest`, not `oauthRequest` - that field is only
                // set on the API request path. Reading oauthRequest alone made `response`
                // undefined, so this webhook shipped with no detail at all.
                response: err.tokenRequest?.response?.error_description || err.oauthRequest?.response?.error?.message || err.response || err.message,
                serverResponseCode: 'TokenGenerationError'
            });
        } catch (reportErr) {
            this.logger.error({ msg: 'Failed to report OAuth2 token acquisition failure', account: this.account, err: reportErr });
        }
    }

    /**
     * Records a successful OAuth2 access token acquisition on an API transport.
     *
     * The mirror of reportTokenAcquisitionFailure(), and shared for the same reason: the success
     * halves of GmailClient.getToken() and OutlookClient.getToken() were byte-identical apart from
     * the provider label, so the recovery clear below had to be written twice.
     *
     * @param {Object} tokenData - Result of getActiveAccessTokenData()
     * @param {Boolean} [tokenData.cached] - True when no refresh actually happened
     * @param {String} provider - Provider label for the metrics counter
     * @returns {Promise<void>}
     */
    async reportTokenAcquisitionSuccess(tokenData, provider) {
        if (!['init', 'connecting', 'connected'].includes(this.state)) {
            // We're in an error state (authenticationError, disconnected, etc.) but we just got a
            // valid token, so we've recovered.
            this.state = 'connected';
            await this.setStateVal();

            // For an API transport this is the only recovery route - init() may not run again for
            // hours. Without this the stored error payload and the auth-failure window survive the
            // failure they measured and resurface, misdated, on the next unrelated error days later.
            await this.clearAuthFailureState();
        }

        // Track successful token refresh (only if token was actually refreshed, not cached)
        if (!tokenData.cached) {
            postMetrics({ account: this.account }, this.logger, 'oauth2TokenRefresh', 'inc', { status: 'success', provider, statusCode: '200' });
        }
    }

    /**
     * Reports a transient token-refresh failure as a connection error rather than an auth failure.
     *
     * Lives here rather than at the call sites because none of the three places that reach
     * getImapConfig() has a `catch` - fixing it at the point the error originates is what makes all
     * of them safe at once.
     *
     * `serverResponseCode` is deliberately a single stable value for every transient token failure
     * rather than the underlying errno. setErrorState() treats a changed serverResponseCode as a new
     * first occurrence and resets the error window (base-client.js `isFirstOccurrence`), so varying
     * it per errno would make a flapping provider reset the auto-disable window forever and send a
     * webhook on every flip.
     *
     * @param {Object} ctx - Client reporting the failure. In practice always the primary client:
     *   the `getImapConfig` closure at imap-client.js drops the ctx a Subconnection passes, because
     *   Subconnection.notify() is a no-op and the failure would otherwise become invisible.
     * @param {Error} err - The originating error
     * @returns {Promise<void>}
     */
    async notifyTransientTokenFailure(ctx, err) {
        // Only the connect path may put the account into an error state. getImapConfig() is shared
        // with the command channel (getCommandConnection), with subconnections, and with the
        // outbound SMTP builder, and `ctx` is the primary client on all of them - so without this
        // guard a token blip while fetching a message or sending mail would persist connectError
        // over an account whose IMAP connection is alive and syncing, after a false webhook had
        // already gone out. Nothing rewrites 'connected' until the next 15-minute resync.
        //
        // The paused/closing half mirrors the existing suppression in the connect catch
        // (imap-client.js), so a deliberate pause is not reported as a failure.
        // 'syncing' counts as healthy alongside 'connected': it is held for the whole initial or
        // forced resync, which is exactly when a long-running account is most likely to be serving
        // command-channel traffic.
        if (ctx.paused || ctx.isClosing || ctx.isClosed || ['connected', 'syncing'].includes(ctx.state)) {
            ctx.logger.error({
                msg: 'Transient error during OAuth2 token refresh, not reported as an account error',
                account: ctx.account,
                state: ctx.state,
                err
            });
        ...
        let notifyData = {
            ...
            serverResponseCode: 'OauthRenewNetworkError'
        };
        ...
        try {
            await ctx.notify(false, CONNECT_ERROR_NOTIFY, notifyData);
        } catch (notifyErr) {
            // Reporting must never mask the original failure - the caller still rethrows `err`.
            // notify() did not get far enough to persist the state, so assign it here; on the
            // success path setErrorState() has already both assigned and persisted it.
            ctx.state = 'connectError';
            ctx.logger.error({ msg: 'Failed to report transient token refresh failure', account: ctx.account, err: notifyErr });
        }
    }

    /**
     * Clears the stored error state and the auth-failure counters after a successful connection.
     *
     * Must stay on the unconditional part of each client's success path - not behind an "is this
     * the first success" check - so the auto-disable window cannot outlive the failure it measures.
     *
     * @returns {Promise<void>}
     */
    async clearAuthFailureState() {
        await this.redis.hdel(this.getAccountKey(), 'lastErrorState', 'lastError:event', 'lastError:errorCount', 'lastError:first');
    }
    ...
    async setErrorState(event, data) {
        // Retrieve the previous error payload AND the event it was reported under. The event is part
        // of the dedup key: two different events can legitimately carry the same serverResponseCode,
        // and comparing the payload alone let a connectError suppress the authenticationError that
        // followed it - silently withholding the only signal an operator gets that an account needs
        // re-authorization.
        //
        // It is kept in its OWN field rather than read from `state`. `state` is not ours: connect()
        // and both API init()s persist 'connecting' immediately before the attempt that fails, so a
        // comparison against it is false on every pass of the reconnect loop - which would make every
        // retry a "first occurrence", firing a webhook each time and resetting lastError:first, so
        // the auth-failure window could never reach its threshold.
        let [prevLastErrorState, prevErrorEvent] = await this.redis.hmget(this.getAccountKey(), 'lastErrorState', 'lastError:event');
        ...
        // Check if this is a repeat of the same error, reported under the same event
        if (data && Object.keys(data).length && prevLastErrorState && prevErrorEvent === event) {
        ...
        // Track error occurrences. isFirstOccurrence is computed above so that an unchanged repeat
        // skips the payload write and the change event entirely: a reconnect loop calls this on
        // every attempt, and rewriting byte-identical JSON plus broadcasting a state change nothing
        // observed is the write amplification that presented as pinned CPU in 2.73.0. The counter
        // still advances on every occurrence - that is what the auth-failure window measures.
        if (isFirstOccurrence) {
            const txn = createTransaction(this.redis, { logger: this.logger });
            txn.add('hSetExists', this.getAccountKey(), 'lastErrorState', JSON.stringify(data));
            // The event this payload was reported under - the other half of the dedup key above.
            txn.add('hSetExists', this.getAccountKey(), 'lastError:event', event);
            ...
            //
            // This is keyed off `imap.disabled`, so it only ever fires for password IMAP accounts -
            // OAuth2 accounts have no `imap` object and fall straight through. That asymmetry is
            // DELIBERATE and was decided after building the alternative and reverting it. Do not
            // re-raise it in review; the reasoning is below.
            //
            // A transport-neutral park (a system-owned `authFailureDisabled` field honoured by all
            // three transports) shipped briefly and was backed out, because:
            //
            //   1. The premise did not hold. Gmail API and Outlook Graph do not retry at all -
            //      workers/imap.js dispatches `connection.init().catch(log)` fire-and-forget, there
            //      is no retry loop, and the polling/renewal timers are only armed after a
            //      successful token fetch. A revoked refresh token on those transports costs ONE
            //      failed request, not a stream of them. Only OAuth2-IMAP retries, and that loop is
            //      capped at MAX_BACKOFF_DELAY (10 min), i.e. ~144 token requests/day for a fully
            //      broken account - which is already visible as `authenticationError` with a
            //      webhook, and setErrorState() suppresses the repeats.
            //
            //   2. The failure modes are wildly asymmetric. Not parking means a broken account
            //      retries every 10 minutes, visibly. Parking means a HEALTHY account can go
            //      silently dead: the token-failure window that drove it was only cleared on a
            //      successful connect, while getToken() runs on every API call, so two unrelated
            //      blips days apart were enough to disable a working account.
            //
            //   3. A terminal, operator-gated disable is only correct if un-parking works on every
            //      recovery route. It has to be lifted in Account.create() (where the hosted OAuth
            //      consent redirect lands), in the admin Reconnect button, in the edit form, and in
            //      the REST reconnect endpoint. The attempt got the first three wrong, so
            //      re-authorizing through the consent screen left the account dead.
            //
            // If the retry load from permanently broken OAuth2-IMAP accounts ever becomes a real
            // problem, escalate the backoff instead of disabling: when the rejection is permanent
            // (invalid_grant / 400 rather than a network error or a 5xx), let the cap grow from
            // minutes to hours. That needs no marker and no un-park plumbing, and it stays
            // self-healing - the next retry after a re-authorization simply succeeds.
            switch (event) {
                case 'authenticationError':
                    if (errorCount > 0 && firstError && Date.now() - firstError.getTime() > MAX_IMAP_AUTH_FAILURE_TIME) {
                        ...
                        if (imapData && !imapData.disabled) {
                            ...
                            // lastError:event goes with the counters: it is the dedup partner of the
                            // lastErrorState being replaced below, and leaving it paired with a
                            // different payload is the drift this key exists to avoid.
                            disableTxn.add('hdel', this.getAccountKey(), 'lastError:errorCount', 'lastError:first', 'lastError:event');
                            disableTxn.add(
                                'hSetExists',
                                this.getAccountKey(),
                                'lastErrorState',
    ...
    async loadOAuth2LoginCredentials(accountObject, accountData, ctx, target) {
        ...
        if (!accountData.oauth2.accessToken || !accountData.oauth2.expires || accountData.oauth2.expires < new Date(now + 30 * 1000)) {
            ...
            try {
                ...
                //
                // The errno must be read from `err.cause.code` as well as `err.code`. Every token
                // endpoint is reached through undici's fetch, which rejects with a generic
                // `TypeError: fetch failed` and attaches the real DNS/socket error as `err.cause` -
                // so `err.code` is undefined for exactly the failures this branch exists to catch.
                // Testing `err.code` alone made the whole branch dead code and a DNS blip was still
                // reported to the operator as bad credentials. lib/logger.js:24 unwraps the same
                // shape for the same reason; TRANSIENT_NETWORK_CODES is the shared list.
                let isNetworkError = TRANSIENT_NETWORK_CODES.has(err.code) || TRANSIENT_NETWORK_CODES.has(err.cause?.code);
                ...
                if (isNetworkError) {
                    ...
                    // Not an authentication failure, so it must not be reported as one - but it
                    // still has to be reported. This throw leaves getImapConfig(), and all three of
                    // its call sites (imap-client.js start() and getCommandConnection(),
                    // imap/subconnection.js start()) sit in a `try` whose only handler is a
                    // `finally`. Rethrowing bare therefore set no state and fired no webhook, while
                    // backOff({numOfAttempts: Infinity}) kept retrying with `_connecting` latched
                    // true - leaving the account displayed as "Connecting", indistinguishable from a
                    // healthy account starting up, with nothing for an operator to alert on. That is
                    // the regression that got de27fca5 reverted; the state and webhook below are
                    // what make widening this classification safe later.
                    //
                    // notify() routes through setErrorState(), which writes the state, persists
                    // lastErrorState and suppresses the webhook for an unchanged repeat - so a
                    // reconnect loop does not turn into webhook spam.
                    await this.notifyTransientTokenFailure(ctx, err);
