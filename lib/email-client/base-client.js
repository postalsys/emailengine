...
class BaseClient {
    ...
    /**
     * Drops everything an error run stored. Shared so a caller that recovers by some other route
     * than a login - the Graph client's change subscription starting to work again - clears the
     * same set rather than its own idea of it
     * @returns {Promise<void>}
     */
    async clearStoredErrorState() {
        await this.redis.hdel(this.getAccountKey(), ...ERROR_STATE_FIELDS);
    }
    ...
    async setErrorState(event, data) {
        ...
        // is the webhook payload and a published response shape, and consumeErrorState() needs to
        // know whether the run that is ending was an authentication failure or a connection one
