...
class BaseClient {
    ...
    async clearAuthFailureState() {
        await this.redis.hdel(this.getAccountKey(), 'lastErrorState', 'lastError:errorCount', 'lastError:first', ACCOUNT_AUTH_FAILURE_DISABLED_KEY);
    }
            ...
            switch (event) {
                case 'authenticationError':
                    if (errorCount > 0 && firstError && Date.now() - firstError.getTime() > MAX_IMAP_AUTH_FAILURE_TIME) {
                        // Reset the counters unconditionally. This used to sit inside the
                        // `imap.disabled` write, so accounts that never reached it kept re-entering
                        // this branch with an ever-growing errorCount. It also makes the branch
                        // self-limiting: it cannot fire again until a fresh error window builds up.
                        const disableTxn = createTransaction(this.redis, { logger: this.logger });
                        disableTxn.add('hSetExists', this.getAccountKey(), ACCOUNT_AUTH_FAILURE_DISABLED_KEY, new Date().toISOString());
                        ...
                        this.logger.info({
                            msg: 'Account was disabled due to exceeding the authentication error threshold',
                            errorEvent: event,
