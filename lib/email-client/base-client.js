...
const { arfDetect, camelCaseComplaint } = require('../arf-detect');
...
class BaseClient {
    ...
    /**
     * Redis hash holding the bounces detected for this account, keyed by the Message-ID of
     * the message that bounced. Filled by the new-message processors of every client type
     * and read back into the `bounces` field of the message details response
     */
    getBounceKey() {
        return `${REDIS_PREFIX}iar:b:${this.account}`;
    }

    /**
     * Records a detected bounce against the Message-ID of the message that bounced
     * @param {string} bounceMessageId - EmailEngine id of the bounce message itself
     * @param {Object} bounce - Parsed bounce (recipient, action, messageId, response)
     * @returns {Promise<number|boolean>} appendList.append() result
     */
    async storeBounce(bounceMessageId, bounce) {
        let storedBounce = {
            i: bounceMessageId,
            r: bounce.recipient,
            t: Date.now(),
        ...
        return await appendList.append(this.redis, this.getBounceKey(), bounce.messageId, storedBounce);
    }

    /**
     * Lists the bounces recorded for a Message-ID in the shape of the `bounces` response field
     * @param {string} messageId - Message-ID header value of the sent message
     * @returns {Promise<Object[]>} Bounce entries, empty when none were recorded
     */
    async listBounces(messageId) {
        let rows = await appendList.list(this.redis, this.getBounceKey(), messageId);
        return rows.map(row => {
            let bounce = {
                message: row.i,
                recipient: row.r,
    ...
    /**
     * Adds the recorded bounces to a formatted message, when there are any. A lookup failure
     * is logged and leaves the message without the field rather than failing the fetch
     * @param {Object} messageData - Formatted message (needs `messageId`), updated in place
     */
    async attachBounces(messageData) {
        if (!messageData || !messageData.messageId) {
            return;
        }
        try {
            let bounces = await this.listBounces(messageData.messageId);
            if (bounces.length) {
                messageData.bounces = bounces;
            }
        } catch (err) {
            this.logger.error({
        ...
        if (this.mightBeABounce(messageData)) {
            ...
            try {
                ...
                if (content) {
                    ...
                    let stored = 0;
                    if (bounce.action && bounce.recipient && bounce.messageId) {
                        // Record it for the `bounces` field of the bounced message, as the IMAP path does
                        stored = await this.storeBounce(messageData.id, bounce);
