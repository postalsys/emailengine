...
const { isTransientTokenRefreshError } = require('./oauth-errors');
 ...
 *
 * Not to be confused with `isTransientTokenRefreshError` from ./oauth-errors, which this file also
 * imports. That one is deliberately narrower: it only treats a 5xx as transient when the error also
 * carries `code === 'ETokenRefresh'`, so an unrelated 5xx cannot be mistaken for a healthy token
 * endpoint having a moment. Use that one on the token-refresh path and this one elsewhere.
 *
            ...
            switch (event) {
                case 'authenticationError':
                    if (errorCount > 0 && firstError && Date.now() - firstError.getTime() > MAX_IMAP_AUTH_FAILURE_TIME) {
                        ...
                        // NB: OAuth2 accounts store no `imap` object - getImapConfig() builds the
                        // connection from the provider table - so this whole branch, including the
                        // close(), is a silent no-op for them and they retry indefinitely. The
                        // obvious patch (default imapData to {}) is wrong: `imap.disabled` is an
                        // operator-owned setting surfaced as the "Disable IMAP" checkbox and read by
                        // isSendOnlyAccount(), the API and the accounts list, and no OAuth2 re-auth
                        // path ever writes an `imap` key - so a system-written marker there could
                        // never be cleared, permanently bricking the account. Fixing this properly
                        // needs a dedicated account-level marker honoured by all three transports
                        // and cleared on successful authentication. Tracked separately.
    ...
    async loadOAuth2LoginCredentials(accountObject, accountData, ctx, target) {
        ...
        if (!accountData.oauth2.accessToken || !accountData.oauth2.expires || accountData.oauth2.expires < new Date(now + 30 * 1000)) {
            ...
            try {
                ...
                // Transient failures (socket/DNS blips, and a token endpoint answering 429 or 5xx)
                // are not authentication failures - see lib/email-client/oauth-errors.js
                if (isTransientTokenRefreshError(err)) {
                    ctx.logger.error({
                        msg: 'Transient error during OAuth2 token refresh',
