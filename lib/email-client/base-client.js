...
class BaseClient {
    ...
    /**
     * Redis hash marking the messages of this account that were themselves detected as bounces:
     * the Message-ID of the bounce message maps to the Message-ID of the message that bounced.
     * Written together with the bounce record and read back into the `isBounce` and
     * `relatedMessageId` fields of the message response, so the API reports the same detection
     * the messageNew webhook carried
     */
    getBounceMessagesKey() {
        return `${REDIS_PREFIX}iar:bm:${this.account}`;
    }

    /**
     * Records a detected bounce against the Message-ID of the message that bounced, and marks
     * the bounce message itself
     * @param {Object} bounceMessage - Formatted bounce message (`id`, and `messageId` when it has one)
     ...
     */
    async storeBounce(bounceMessage, bounce) {
        let storedBounce = {
            i: bounceMessage.id,
            r: bounce.recipient,
            t: Date.now(),
        ...
        if (bounceMessage.messageId) {
            await this.redis.hset(this.getBounceMessagesKey(), bounceMessage.messageId, bounce.messageId);
        }
    ...
    /**
     * Adds the recorded bounces to a formatted message, when there are any, and its own detection
     * as a bounce (`isBounce`, `relatedMessageId`). A lookup failure is logged and leaves the
     * message without the fields rather than failing the fetch
    ...
    async attachBounces(messageData) {
        ...
        try {
            let [bounces, relatedMessageId] = await Promise.all([
                this.listBounces(messageData.messageId),
                this.redis.hget(this.getBounceMessagesKey(), messageData.messageId)
            ]);
            if (bounces.length) {
            ...
            if (relatedMessageId) {
                ...
                messageData.relatedMessageId = relatedMessageId;
            }
        } catch (err) {
            this.logger.error({
            ...
            try {
                ...
                if (content) {
                    ...
                    if (bounce.action && bounce.recipient && bounce.messageId) {
                        ...
                        stored = await this.storeBounce(messageData, bounce);
