...
class BaseClient {
    ...
    async isSyncDisabled() {
        const imapInfo = await this.redis.hget(this.getAccountKey(), 'imap');
        if (!imapInfo) {
            return false;
        }

        try {
            return !!JSON.parse(imapInfo).disabled;
        } catch (err) {
            // An unreadable blob is not a disable; stay noisy and let the connection proceed.
    ...
    async disableAfterAuthFailures({ event, firstError, errorCount, data }) {
        ...
        let imapData;
        const imapInfo = await this.redis.hget(accountKey, 'imap');
        if (imapInfo) {
            try {
                imapData = JSON.parse(imapInfo);
            } catch (err) {
                // Rewriting a blob that does not parse would replace the account's IMAP
                // configuration with a bare flag, so leave it alone and stay noisy instead.
                this.logger.error({ msg: 'Failed parsing IMAP data', err });
                return false;
            }
        }
