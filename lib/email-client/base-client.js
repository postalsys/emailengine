...
const {
    ...
    EMAIL_SENT_NOTIFY,
    ACCOUNT_AUTH_FAILURE_DISABLED_KEY,
    ACCOUNT_TOKEN_ERROR_FIRST_KEY,
    TOKEN_FAILURE_RESPONSE_CODES
} = require('../consts');
...
/**
 * Parks an account: stops it connecting until an operator intervenes, and closes the client.
 *
 * Deliberately a module-level function taking the client explicitly, NOT a method. setErrorState()
 * is driven through a minimal fake receiver in test/set-error-state-test.js so the park logic can be
 * exercised against real Redis without constructing a client; anything it reaches for via `this`
 * that the fake does not define fails with a TypeError instead of a useful assertion.
 *
 * Shared by both park paths so they cannot drift - the counters cleared here are what makes a park
 * self-limiting, and missing one of them is how the pre-77f78b4b code leaked.
 *
 * @param {Object} ctx - Client (or fake) exposing redis, logger, getAccountKey() and close()
 * @param {Object} opts
 * @param {String} opts.description - Operator-facing reason, stored in lastErrorState
 * @param {String} [opts.response] - Underlying server response, if any
 * @param {Date} opts.firstError - When the failure window opened
 * @param {String} opts.errorEvent - What triggered the park, for logging
 * @param {Number} [opts.errorCount] - Errors seen in the window, for logging
 * @returns {Promise<void>}
 */
async function parkAccount(ctx, { description, response, firstError, errorEvent, errorCount }) {
    const disableTxn = createTransaction(ctx.redis, { logger: ctx.logger });
    disableTxn.add('hSetExists', ctx.getAccountKey(), ACCOUNT_AUTH_FAILURE_DISABLED_KEY, new Date().toISOString());
    disableTxn.add('hdel', ctx.getAccountKey(), 'lastError:errorCount', 'lastError:first', ACCOUNT_TOKEN_ERROR_FIRST_KEY);
    disableTxn.add('hSetExists', ctx.getAccountKey(), 'lastErrorState', JSON.stringify({ description, response }));
...
/**
 * Tracks how long the OAuth2 token endpoint has been failing for an account, and parks it once that
 * exceeds MAX_IMAP_AUTH_FAILURE_TIME.
 *
 * Module-level for the same reason as parkAccount above.
 *
 * @param {Object} ctx - Client (or fake) exposing redis, logger, getAccountKey() and close()
 * @param {Object} data - Error payload, as passed to setErrorState
 * @returns {Promise<boolean>} True when this call parked the account
 */
async function trackTokenEndpointFailure(ctx, data) {
    const accountKey = ctx.getAccountKey();
    const stored = await ctx.redis.hget(accountKey, ACCOUNT_TOKEN_ERROR_FIRST_KEY);
    ...
    if (!firstFailure) {
        // Opens the window. hSetExists rather than hset so a concurrently deleted account is not
        // resurrected as a ghost hash.
        await ctx.redis.hSetExists(accountKey, ACCOUNT_TOKEN_ERROR_FIRST_KEY, new Date().toISOString());
        return false;
    }

    if (Date.now() - firstFailure.getTime() <= MAX_IMAP_AUTH_FAILURE_TIME) {
        return false;
    }

    await parkAccount(ctx, {
        description: 'Account was disabled because the OAuth2 token endpoint kept failing',
        response: data.response,
        firstError: firstFailure,
        errorEvent: 'tokenEndpointFailure'
    });
...
class BaseClient {
    ...
    /**
     * Stamps the account as not syncing when it is parked after sustained authentication failures,
     * so the caller can bail before doing any provider work.
     *
     * For the API transports only. IMAP already holds the account record at its connect gates and
     * asks isConnectionBlocked(accountData) instead, which costs no extra round trip.
     *
     * @returns {Promise<boolean>} True when the caller should return without connecting
     */
    async parkIfAuthFailureDisabled() {
        if (!(await this.redis.hget(this.getAccountKey(), ACCOUNT_AUTH_FAILURE_DISABLED_KEY))) {
            return false;
        }
        this.state = 'unset';
     ...
     * @param {String} account - Account id, for logging only
     * @returns {Promise<void>}
     */
    async notifyTransientTokenFailure(ctx, err, account) {
        let notifyData = {
        ...
        try {
            ...
            ctx.logger.error({ msg: 'Failed to report transient token refresh failure', account, err: notifyErr });
        }
    ...
    /**
     * Clears the parked marker and the error counters after a successful authentication.
     *
     * This is the only path that un-parks an account, so it must stay on the unconditional part of
     * each client's success path - not behind an "is this the first success" check.
     *
     * @returns {Promise<void>}
     */
     ...
     * Parks the account after repeated authentication failures
    ...
    async setErrorState(event, data) {
        ...
        if (isFirstOccurrence) {
            ...
            // Park the account after sustained authentication failures
            switch (event) {
                case 'authenticationError':
                    if (errorCount > 0 && firstError && Date.now() - firstError.getTime() > MAX_IMAP_AUTH_FAILURE_TIME) {
                        await parkAccount(this, {
                            description: 'Account was disabled due to exceeding the authentication error threshold',
                            response: data.response,
                            firstError,
                            errorEvent: event,
                            errorCount
        ...
        // Independently of the lastError:* window above, track how long the OAuth2 token endpoint
        // itself has been failing. That window is reset whenever `serverResponseCode` changes, so a
        // provider alternating between error classes - 503 one minute, 400 the next - would keep
        // restarting it and could never reach the threshold. This one is set only when unset and
        // cleared only on a successful authentication, so it survives reclassification.
        //
        // It also covers failures reported as `connectError`, which the switch above cannot: a
        // token endpoint that is down rather than rejecting credentials still has to reach a
        // terminal state instead of retrying forever against a shared provider quota.
        if (data && TOKEN_FAILURE_RESPONSE_CODES.has(data.serverResponseCode)) {
            await trackTokenEndpointFailure(this, data);
        }
        ...
        if (!accountData.oauth2.accessToken || !accountData.oauth2.expires || accountData.oauth2.expires < new Date(now + 30 * 1000)) {
            ...
            try {
                ...
                if (isNetworkError) {
                    ...
                    await this.notifyTransientTokenFailure(ctx, err, accountObject.account);
