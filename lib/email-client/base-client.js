 ...
 * server) use this to avoid reporting an unreachable host as an authentication error, which
 * would webhook and park every account on the instance over a brief outage, and the API
 * transports use it to decide which failed requests to repeat. undici reports a connection
 * failure as a generic TypeError ("fetch failed") with the real DNS/socket error attached as
 * err.cause, so the code is looked up there as well
...
class BaseClient {
    ...
    async loadOAuth2LoginCredentials(accountObject, accountData, ctx, target) {
        ...
        if (await useAuthServerForOAuth2(accountData.oauth2, ctx.logger, { account: accountObject.account, target })) {
            try {
                let authData = await resolveCredentials(accountObject.account, target);
                ...
                // A transient failure to reach the auth server is a connection problem, not a
                // rejected credential. Reporting it as an authentication error would webhook every
                // account on the instance and park them in an error state over a brief blip.
                ...
                ctx.logger.error({
                    msg: 'Failed to resolve OAuth2 credentials from the authentication server',
                    account: accountObject.account,
                    target,
                    err
                });
