...
/**
 * Whether a stored `lastErrorState` describes an error that actually happened. An empty object is
 * what a cleared-but-rewritten state looks like, and a value that will not parse still counts
 * @param {String} storedState - Raw JSON read from the account hash
 * @returns {boolean} True when an error run was in progress
 */
function parsedErrorStateIsSet(storedState) {
    if (!storedState) {
        return false;
    }
    try {
...
class BaseClient {
    ...
    /**
     * Drops everything an error run stored, for a caller that recovers by some other route than a
     * login - the Graph client's change subscription starting to work again. Lives here so the set
     * of fields is the one notifyAuthenticationSuccess() clears rather than a second idea of it.
     *
    ...
    async notifyAuthenticationSuccess(user) {
        ...
        // One round trip: read what the previous attempt left behind, then drop the whole run. The
        // commands in a MULTI execute in order, so the reads still see the pre-delete values
        const txn = createTransaction(this.redis, { logger: this.logger });
        txn.add('hmget', accountKey, 'lastErrorState', LAST_ERROR_EVENT_FIELD, 'state:count:connected');
        txn.add('hdel', accountKey, ...ERROR_STATE_FIELDS);
        ...
        if (error) {
            ...
            this.logger.error({ msg: 'Failed to consume the stored error state', account: this.account, err: error });
            return false;
        }

        const [storedState, storedEvent, prevConnectedCount] = values[0];
        ...
        const errorEvent = parsedErrorStateIsSet(storedState) ? storedEvent || AUTH_ERROR_NOTIFY : null;
