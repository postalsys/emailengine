...
/**
 * Builds the request that sends an existing draft message. Graph requires an empty body
 * (Content-Length: 0), answers 202 Accepted, and files the message into Sent Items itself.
 *
 * @param {String} emailId - Graph message id of the draft
 * @param {String} oauth2UserPath - Graph user path ("me" or "users/<id>")
 * @returns {Object} { path, body, options } for OutlookOauth.request()
 */
function buildDraftSendRequest(emailId, oauth2UserPath) {
    return {
        path: `/${oauth2UserPath}/messages/${emailId}/send`,
        // An empty Buffer keeps the request body at Content-Length: 0
        body: Buffer.alloc(0),
        options: { returnText: true }
    };
}

module.exports = { buildRawSendMailRequest, buildStructuredSendMailRequest, buildDraftSendRequest };
