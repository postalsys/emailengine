...
class NotificationHandler {
    ...
    async processWithFlow(event, payload, queueKeep) {
        ...
        // A child that fails for good (the document store job or a custom-route delivery out
        // of attempts) has to be moved to the parent's failed-dependencies set, otherwise the
        // main webhook job sits in waiting-children until retention prunes it
        for (let child of queueFlow) {
            child.opts = Object.assign({ ignoreDependencyOnFailure: true }, child.opts);
        }
     ...
     */
    async processDocumentOnly(event, payload, queueKeep) {
        // Goes through buildJobOptions like every other enqueue. Passing DOCUMENT_JOB_OPTIONS
        // straight through left this path on the hardcoded baseline: it ignored the queueKeep
        // setting and, more importantly, never picked up the failed-entry floor.
        await this.documentsQueue.add(event, payload, this.buildJobOptions(queueKeep).documents);
    }
    ...
    buildJobOptions(queueKeep) {
        const retention = buildRetentionPolicy(queueKeep);

        const notifyOptions = Object.assign({}, DEFAULT_JOB_OPTIONS, retention);

        const documentOptions = Object.assign({}, DOCUMENT_JOB_OPTIONS, retention);
    ...
    async notify(mailbox, event, data, extraOpts) {
        ...
        if (!skipWebhook && addDocumentQueueJob) {
            ...
            // Document store only
            await this.processDocumentOnly(event, payload, queueKeep);
        }
    }
}
