...
/**
 * Builds the Gmail request that sends an existing draft resource. Gmail moves the message
 * from Drafts to Sent itself, so no cleanup call is needed afterwards.
 *
 * @param {String} draftId - Gmail draft resource id (NOT the message id)
 * @returns {Object} { targetEndpoint, contentType, payload } - endpoint is relative to GMAIL_API_BASE
 */
function buildDraftSendRequest(draftId) {
    return {
        targetEndpoint: `/gmail/v1/users/me/drafts/send`,
        contentType: 'application/json',
        payload: { id: draftId }
    };
}

module.exports = { buildSendRequest, buildDraftSendRequest, JSON_SEND_LIMIT };
