    ...
    async processNew(messageData, options, canSync) {
        ...
        if (notifyCalendarEvents && messageInfo.attachments && messageInfo.attachments.length) {
                        ...
                        try {
                            ...
                            if (downloadStream) {
                                ...
                                if (contentBuf && contentBuf.length) {
                                    ...
                                    try {
                                        const directEvents = ical.sync.parseICS(contentBuf.toString());
                                        // find first event key
                                        const eventKey = Object.keys(directEvents).find(key => directEvents[key] && directEvents[key].type === 'VEVENT');

                                        if (eventKey && !calendarEventMap.has(eventKey)) {
            ...
            if (calendarEventMap && calendarEventMap.size) {
                messageInfo.calendarEvents = Array.from(calendarEventMap.values());
            }
        }
        ...
        if (messageInfo.messageSpecialUse === '\\Inbox') {
            ...
            if (generateEmailSummary) {
                ...
                if (openAiAPIKey && (messageInfo.text.plain || messageInfo.text.html)) {
                    try {
