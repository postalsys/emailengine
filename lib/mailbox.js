...
class Mailbox {
    ...
    async getMailboxLock(storedStatus) {
        storedStatus = storedStatus || (await this.getStoredStatus());

        let lock = await this.imapClient.getMailboxLock(this.path, {
            uidValidity: storedStatus && storedStatus.uidValidity,
            changedSince: storedStatus && storedStatus.highestModseq
        });
    ...
    async onFlags(event) {
        ...
        if (!storedMessage) {
            ...
            if (event.modseq) {
                // bump highestmodseq as well
                let modseqUpdated = await this.connection.redis.hSetBigger(this.getMailboxKey(), 'highestModseq', event.modseq.toString());
                if (modseqUpdated) {
                    this.logger.trace({
                        msg: 'Bumped HIGHESTMODSEQ after FETCH',
                        prev: this.imapClient.mailbox.highestModseq && this.imapClient.mailbox.highestModseq.toString(),
                        current: event.modseq.toString()
                    });

                    if (this.imapClient.mailbox.highestModseq < event.modseq) {
                        this.imapClient.mailbox.highestModseq = event.modseq;
                    }
                }
            }
    ...
    async partialSync(storedStatus) {
        ...
        let lock = await this.getMailboxLock(storedStatus);
        try {
            let newMessages = [];
    ...
    async onOpen(event) {
        ...
        try {
            ...
            if (event.qresync && event.highestModseq > storedStatus.highestModseq) {
                // mailbox was opened in qresync mode, so it should be already in sync?
                await this.updateStoredStatus(this.getMailboxStatus());

                this.logger.trace({
                    msg: 'Opened QRESYNC mailbox with HIGHESTMODSEQ change',
                    prev: storedStatus.highestModseq.toString(),
                    current: this.imapClient.mailbox.highestModseq.toString()
                });
                    ...
                    if (textParts && textParts.length) {
                        ...
                        if (
                            ...
                            try {
                                ...
                                if (plaintext) {
                                    messageInfo.text.visible = linkify(textToHtml(plaintext), {
    ...
    async deleteMessage(message, force) {
        ...
        try {
            ...
            let updatedModseq = false;
            let prevModseq;

            if (['\\Trash', '\\Junk'].includes(this.listingEntry.specialUse) || force) {
                // delete
                prevModseq = this.imapClient.mailbox.highestModseq;
                result.deleted = await this.imapClient.messageDelete(message.uid, { uid: true });
                if (this.imapClient.mailbox.highestModseq && this.imapClient.mailbox.highestModseq > prevModseq) {
                    // highest modseq was updated
                    updatedModseq = true;
                }
            } else {
                // move to trash
            ...
            if (updatedModseq) {
                try {
                    let modseqUpdated = await this.connection.redis.hSetBigger(
                        this.getMailboxKey(),
                        'highestModseq',
                        this.imapClient.mailbox.highestModseq.toString()
                    );
                    if (modseqUpdated) {
                        this.logger.trace({
                            msg: 'Bumped HIGHESTMODSEQ after EXPUNGE',
                            prev: prevModseq.toString(),
                            current: this.imapClient.mailbox.highestModseq.toString()
                        });
                    }
                } catch (err) {
