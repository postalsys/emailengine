...
const { serialize, unserialize, compareExisting, normalizePath, download, filterEmptyObjectValues } = require('./tools');
...
const { mimeHtml, generateSummary, riskAnalysis } = require('@postalsys/email-text-tools');
...
class Mailbox {
    ...
    async processNew(messageData, options, canSync) {
        ...
        if (messageInfo.messageSpecialUse === '\\Inbox' && (!this.connection.notifyFrom || messageData.internalDate >= this.connection.notifyFrom)) {
            let generateEmailSummary = await settings.get('generateEmailSummary');
            if (generateEmailSummary) {
                let openAiAPIKey = await settings.get('openAiAPIKey');
                if (openAiAPIKey && messageInfo.text && (messageInfo.text.plain || messageInfo.text.html)) {
                    let requestOpts = {};
                    ...
                    switch (openAiModel) {
                        case 'gpt-4':
                            requestOpts.maxTokens = 6000;
                            break;
                        case 'gpt-3.5-turbo':
                        default:
                            requestOpts.maxTokens = 4000;
                            break;
                    }
            ...
            let generateRiskAssessment = await settings.get('generateRiskAssessment');
            if (generateRiskAssessment && (!this.connection.notifyFrom || messageData.internalDate >= this.connection.notifyFrom)) {
                let openAiAPIKey = await settings.get('openAiAPIKey');
                if (openAiAPIKey && messageInfo.text && (messageInfo.text.plain || messageInfo.text.html)) {
                    let requestOpts = {};
                    let openAiModel = await settings.get('openAiModel');
                    ...
                    try {
                        messageInfo.riskAssessment = await riskAnalysis(
                            {
                        ...
                        if (messageInfo.riskAssessment) {
                            for (let key of Object.keys(messageInfo.riskAssessment)) {
                                // remove meta keys from output
                                if (key.charAt(0) === '_' || messageInfo.riskAssessment[key] === '') {
                                    delete messageInfo.riskAssessment[key];
                                }
                            }

                            this.logger.trace({ msg: 'Fetched risk assessment from OpenAI', assessment: messageInfo.riskAssessment });
                        }
                    } catch (err) {
                        this.logger.error({ msg: 'Failed to fetch risk assessment from OpenAI', err });
                    }
                }
            }
        }
        ...
        try {
            ...
            try {
                ...
                if (options.markAsSeen && (!messageData.flags || !messageData.flags.has('\\Seen'))) {
                    //
                    try {
                        let res = await this.connection.imapClient.messageFlagsAdd(message.uid, ['\\Seen'], { uid: true });
                        if (res) {
