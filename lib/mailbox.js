...
class Mailbox {
    ...
    async sync() {
        ...
        let status;
        try {
            status = await this.imapClient.status(this.path, {
                uidNext: true,
                messages: true,
                highestModseq: true,
                uidValidity: true
                    ...
                    if (!storedMessage) {
                        ...
                        if (seq) {
                            await this.connection.redis.zadd(
                                ...
                                messageData.uid,
                                JSON.stringify({ uid: messageData.uid, flags: messageData.flags })
                            );
                        }
    ...
    async fullSync() {
        ...
        try {
            ...
            let notifyHeaders = await settings.get('notifyHeaders');
            if (documentStoreEnabled || notifyHeaders) {
                messageFetchOptions.headers =
                    (notifyHeaders && notifyHeaders.includes('*')) || documentStoreEnabled
                        ? true
                        : notifyHeaders && notifyHeaders.length
                        ? notifyHeaders
                        : false;
            }
