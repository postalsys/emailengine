...
class Mailbox {
    ...
    async partialSync(storedStatus) {
        ...
        try {
            ...
            let notifyTextSize;
            if (notifyText) {
                notifyTextSize = await settings.get('notifyTextSize');
            }

            // have to call after fetch is finished
            for (let messageData of newMesages) {
                await this.processNew(messageData, { notifyText, notifyTextSize });
            }
        } finally {
            lock.release();
        }
    ...
    async processNew(messageData, options) {
        ...
        let messageInfo = await this.getMessage(messageData, {
            textType: options.notifyText ? '*' : false,
            maxBytes: options.notifyTextSize
        });
