...
class Mailbox {
    ...
    async getMailboxLock(storedStatus) {
        ...
        let lock = await this.imapClient.getMailboxLock(this.path, {
            uidValidity: storedStatus && storedStatus.uidValidity && storedStatus.uidValidity.toString(),
            changedSince: storedStatus && storedStatus.highestModseq && storedStatus.highestModseq.toString()
        });
        ...
        try {
            ...
            if (updatedModseq) {
                try {
                    await this.updateStoredStatus(this.getMailboxStatus());
                    this.logger.trace({
                        msg: 'HIGHESTMODSEQ change after EXPUNGE',
