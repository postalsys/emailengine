...
// ({ name, title, description, keep?/omit?, force?, bounds?, binary?, resourceUriTemplate? }), and
// this module derives everything else from what
...
const { routeGrant, surfaceAdmits, surfaceBoundAdmits, PER_REQUEST_SURFACES, ACTION } = require('../api-routes/permission-map');
...
//
// Read as tokens rather than as bytes: this is roughly 32k of them, which is already a large
// share of a context window to spend on one call, and it is a ceiling rather than a target -
// every tool that can return a lot keeps its own, smaller budget (the forced maxBytes on the two
// body tools, the `bounds` below on the listings), so a truncated result means something went
// wrong rather than something got big.
...
// Every key a plugins.mcp block may carry. The one place the block's shape is written down, so a
// misspelling is a build failure rather than a knob that quietly does nothing.
const MCP_OPTION_KEYS = new Set(['name', 'title', 'description', 'keep', 'omit', 'force', 'bounds', 'binary', 'resourceUriTemplate', 'endpoint', 'openWorld']);
...
function buildToolRegistry(routes) {
    ...
    for (const route of routes) {
        ...
        // Nothing else checks the shape of a plugins.mcp block, so a misspelled key would be a
        // silent no-op - and every knob here is one that hides or pins something on the
        // agent-facing schema, where a no-op means the opposite of what was declared
        const unknownOptions = Object.keys(options).filter(key => !MCP_OPTION_KEYS.has(key));
        if (unknownOptions.length) {
            throw new Error(`MCP tool ${options.name}: unknown plugins.mcp option${unknownOptions.length > 1 ? 's' : ''}: ${unknownOptions.join(', ')}`);
        }

        const validate = (route.settings && route.settings.validate) || {};

        // Which arguments the tool offers, expressed either way round. `keep` is the allowlist
        // form and is what the composition tools use: their exposed set is small, the route's is
        // large and grows, and with a deny list a field added to the submit route later would
        // join send_message's schema by default - failing open on the one tool that reaches
        // people outside the mailbox. `omit` stays for the tools that hide two or three knobs
        // out of a stable handful, where an allowlist would be a list of everything.
        if (options.keep && options.omit) {
            throw new Error(`MCP tool ${options.name}: declare either keep or omit, not both`);
        }
        ...
        // Every request argument of the route, hidden ones included. `sources` is what a caller
        // may address; this is what the route actually accepts, and it is what a forced value has
        // to be looked up in - a forced argument is almost always a hidden one.
        const argSources = new Map();
        // The arguments the tool does not offer, and the subset of those the route requires
        const hidden = new Set();
        const hiddenRequired = new Set();
        ...
        for (const [sourceName, source] of [
            ...
            for (const [key, propertySchema] of Object.entries(source.properties)) {
                ...
                if (keep ? !keep.has(key) : omit.has(key)) {
                    hidden.add(key);
                    if (source.required.includes(key)) {
                        // A required argument the tool does not offer has to be supplied by
                        // `force`, or every call dies on the same joi 400 with nothing the model
                        // can do about it
                        hiddenRequired.add(key);
                    }
                    continue;
                }
        ...
        // Both lists are checked against what the route actually accepts. A name that matches no
        // argument is the failure mode this exists to catch: it reads as a decision that was made
        // and is not in force - a still-exposed knob under `omit`, a still-hidden one under
        // `keep`.
        for (const [label, declared] of [
            ...
            for (const key of declared) {
                if (!argSources.has(key)) {
                    throw new Error(`MCP tool ${options.name}: ${label} argument ${key} is not an argument of ${route.method} ${route.path}`);
                }
            }
        }
        ...
        // argument of the route rather than against the tool's own, since a hidden key has left
        // the latter - which is also why `force` on a still-offered argument is refused. That pair
        // is the whole mechanism, and declaring one half of it would advertise an argument whose
        // value is then thrown away, which reads to the model as a control it does not have.
        ...
        for (const [key, value] of force) {
            ...
            if (!hidden.has(key)) {
                throw new Error(`MCP tool ${options.name}: forced argument ${key} must also be hidden, or the schema offers a choice that is not there`);
            }
            forcedSources.set(key, sourceName);
            forcedValues[key] = value;
            hiddenRequired.delete(key);
        }

        if (hiddenRequired.size) {
            throw new Error(
                `MCP tool ${options.name}: required argument${hiddenRequired.size > 1 ? 's' : ''} ${[...hiddenRequired].join(', ')} ` +
                    `${hiddenRequired.size > 1 ? 'are' : 'is'} hidden but not forced, so every call would fail validation`
            );
        }

        // Ceilings the MCP layer lowers on an argument the route keeps offering in full (see
        // MCP_MAX_PAGE_SIZE in lib/consts.js for the case this exists for). The advertised
        // maximum comes down AND the dispatch clamps, so the bound holds for a client that
        // ignores the schema too.
        //
        // Refused at build time unless it actually narrows an exposed numeric argument - a bound
        // that names a hidden key, or one the route has since raised past, is a bound that
        // silently stopped doing anything.
        const bounds = new Map();
        for (const [key, max] of Object.entries(options.bounds || {})) {
            if (!sources.has(key)) {
                throw new Error(`MCP tool ${options.name}: bounded argument ${key} is not an exposed argument of ${route.method} ${route.path}`);
            }
            if (typeof properties[key].maximum !== 'number' || properties[key].maximum <= max) {
                throw new Error(`MCP tool ${options.name}: bound on ${key} does not narrow the schema maximum (${properties[key].maximum})`);
            }
            properties[key] = Object.assign({}, properties[key], { maximum: max });
            bounds.set(key, max);
        }
        ...
        const annotations = {
            ...
            // Sending mail reaches arbitrary external recipients. A tool whose grant does not
            // imply that but whose purpose is reaching a host outside the instance - the
            // autodiscovery lookup, the connection tests - declares `openWorld` itself.
            openWorldHint: grant.action === ACTION.SEND || !!options.openWorld
        };
        ...
        byName.set(options.name, {
            ...
            grant,
            // The per-request scopes whose table covers that operation, in the order the scopes
            // are declared: the other half of what tools/list filters on, and what the admin
            // pages group and count by
            surfaces: [...PER_REQUEST_SURFACES].filter(scope => surfaceAdmits(scope, grant)),
    ...
    // The catalog reduced to what the admin pages count and group by, in tools/list order. Built
    // here with the registry rather than on each read: the registry is immutable for the life of
    // the worker and three admin pages plus the MCP door read this.
    const grants = tools.map(tool => {
        const entry = byName.get(tool.name);
        return Object.assign({ name: tool.name, accountScoped: entry.sources.has('account'), surfaces: entry.surfaces }, entry.grant);
    });

    return { tools, byName, grants };
}
...
function effectiveArguments(tool, args, boundAccount) {
    ...
    // Clamped rather than refused: the value came out of a model reading the advertised maximum,
    // and answering a too-large page with an error teaches it nothing the smaller page does not.
    // Number() because the wire is not obliged to send the JSON type the schema names.
    for (const [key, max] of tool.bounds) {
...
function toolVisibleTo(entry, { tokenData, boundAccount }) {
    ...
    // The scope bound first: a credential holding only the mail scope is not offered the
    // management tools however wide its permissions record, and the other way round
    if (!surfaceBoundAdmits(tokenData, entry.grant)) {
        return false;
    }
 ...
 *
 * `accountScoped` and `surfaces` are what the binding and scope halves of toolVisibleTo() read,
 * carried along because the admin pages have to apply the same filters in a browser, where
 * neither this module nor the permission checker can be called. test/mcp-tools-test.js asserts
 * that the model those pages use agrees with toolVisibleTo() over the whole catalog, so the
 * prediction cannot drift away from the rule it predicts.
 *
 * @param {Object} server - the Hapi server
 * @returns {Array<{name: String, action: String, group: String, accountScoped: Boolean, surfaces: Array<String>}>}
 */
function toolGrants(server) {
    return getToolRegistry(server).grants;
}

/**
 * The per-request scopes a catalog actually exercises: a scope is held when a tool only that
 * scope's table covers is in the catalog. The tools both tables cover (the account reads) say
 * nothing about either. What the instructions are written from, so a narrowed record that
 * empties a scope out is described accurately.
 *
 * @param {Object} server - the Hapi server
 * @param {Array} catalog - tool definitions, as listToolsFor() returns them
 * @returns {Array<String>} scopes, in declaration order
 */
function catalogSurfaces(server, catalog) {
    const names = new Set(catalog.map(tool => tool.name));
    const held = new Set();
    for (const entry of getToolRegistry(server).grants) {
        if (entry.surfaces.length === 1 && names.has(entry.name)) {
            held.add(entry.surfaces[0]);
        }
    }
    return [...PER_REQUEST_SURFACES].filter(scope => held.has(scope));
}
