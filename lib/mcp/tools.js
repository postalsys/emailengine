...
async function callTool({ server, tool, args, request }) {
    ...
    const missing = (tool.definition.inputSchema.required || []).filter(key => args[key] === undefined || args[key] === null);
    if (missing.length) {
 ...
 *
 * `accountScoped` is the second half of what tools/list filters on: a credential bound to one
 * account never sees the instance-wide tools, because they take no account argument to bind. A
 * consumer that predicts the catalog has to apply both halves or it overstates what the agent
 * will get - see the listTools filter in lib/api-routes/mcp-routes.js, which is the enforcement.
 *
 * @param {Object} server - the Hapi server
...
module.exports = { buildToolRegistry, getToolRegistry, toolGrants, callTool, MAX_TOOL_RESULT_BYTES, MAX_TOOL_BINARY_BYTES };
