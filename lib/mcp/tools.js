...
function toolGrants(server) {
    ...
    return tools.map(tool => {
        ...
        return Object.assign(
            {
                name: tool.name,
                accountScoped: entry.sources.has('account'),
                // The per-request scopes whose table covers this tool, in the order the scopes
                // are declared
                surfaces: [...PER_REQUEST_SURFACES].filter(scope => surfaceAdmits(scope, entry.grant))
            },
            entry.grant
        );
    });
