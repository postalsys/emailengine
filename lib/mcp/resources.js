...
function parseAccountUri(uri) {
    ...
    const account = decodeURIComponent(uri.slice(ACCOUNT_URI_PREFIX.length));
...
async function listResources({ server, request }) {
    ...
    return res.result.accounts.map(accountData => accountResource(accountData));
}
...
module.exports = { listResources, readResource, accountUri, parseAccountUri, ACCOUNT_URI_PREFIX };
