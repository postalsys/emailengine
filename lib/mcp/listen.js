...
// Accept-time authorization injects one API request per subscribed URI, so the list is bounded.
const MAX_RESOURCE_SUBSCRIPTIONS = 50;
...
async function acceptFilter({ server, request, filter }) {
    ...
    // a broad subscription would pay one full request pipeline per URI in connect latency
    ...
    const uris = admitted.filter(Boolean);
    if (uris.length) {
 ...
 */
function openListenStream({ h, subscriptionId, accepted }) {
...
module.exports = { acceptFilter, openListenStream, publishAccountChange };
