...
// What the instructions say about the mail tools, for a credential that holds them. The collapse
...
// What the instructions say about the management tools. Settings apply at once and several of the
// tools are one-way, so the confirmations are named rather than left to the tool descriptions
// alone, which a client may or may not surface.
const MANAGEMENT_INSTRUCTIONS = [
 ...
 * Which tool sets it holds: a management credential is told to start from the instance and
 * what to confirm, a mail credential is told how messages are shaped, and one holding both hears
 * both. And whether it is bound to one account: the generic opening tells the agent to list the
 * accounts and pass an id, which a bound credential can do neither of - the listing tools are not
 * advertised to it, and its tools carry no `account` argument because the endpoint already knows
 * which account they act on. Saying so up front is what stops the agent looking for a way to
 * supply one.
 *
 ...
 * @param {Boolean} [profile.manage] - the credential is offered the instance management tools
 * @param {Boolean} [profile.mail] - the credential is offered the mail tools
 * @returns {String} instructions
 */
function serverInstructions(profile) {
    const { account = null, manage = false, mail = false } = profile || {};
    ...
    return opening
        .concat(manage ? MANAGEMENT_INSTRUCTIONS : [])
        .concat(mail ? MAIL_INSTRUCTIONS : [])
        .join(' ');
}

// The fallback for a caller that supplies no per-credential instructions: everything
const SERVER_INSTRUCTIONS = serverInstructions({ manage: true, mail: true });
