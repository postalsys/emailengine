 ...
 * @param {Boolean} [profile.hasTools] - whether the advertised catalog has anything in it, for a
 *        credential that exercises no scope on its own: the tools both sets share say nothing
 *        about either, and a credential narrowed to only those still has something to call
 * @returns {String} instructions
 */
function serverInstructions(profile) {
    const { account = null, surfaces = [], hasTools = false } = profile || {};
    ...
    if (!manage && !mail) {
        // Told so rather than handed a workflow it cannot perform
        opening.push(
            hasTools
                ? 'This credential is offered only the tools both tool sets share; ask the operator to widen it for anything else.'
                : 'This credential is offered no tools; ask the operator to widen it.'
        );
    }
