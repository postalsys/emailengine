...
// admin consent page, and exchanges the resulting code for an EmailEngine access token carrying
// the MCP scopes the operator approved (`mcp-manage` for the instance, `mcp` for mail, or both).
// Every credential this issues is an ordinary access token: visible on the admin tokens page,
// revocable there, and bounded by the SURFACE_GRANTS table of each scope it holds like any other
// MCP-scoped token. There are no refresh tokens and no expiry by default - revocation is the
// lifecycle.
//
...
// The scopes this server issues, in the order the discovery documents advertise them and the
// order a token response lists them: management first, because that is what the consent page
// offers by default, and mail on request. A client that sends `scope=mcp` in its authorization
// request is asking for mail access; the consent page pre-ticks it, the person still decides.
const MCP_OAUTH_SCOPES = ['mcp-manage', 'mcp'];

/**
 * The subset of the issuable scopes a list names, in canonical order. Anything else is dropped:
 * the scope vocabulary is this server's to define, and an unknown value is not a narrower or
 * wider grant, only noise from a client guessing.
 *
 * @param {Array|String} scopes - a scope list, or a space-separated scope string from the wire
 * @returns {Array<String>}
 */
function normalizeScopes(scopes) {
    const requested = new Set(typeof scopes === 'string' ? scopes.split(/\s+/) : [].concat(scopes || []));
    return MCP_OAUTH_SCOPES.filter(scope => requested.has(scope));
}
 ...
 *        binding for the token), scopes (the MCP scopes the operator approved; `mcp` alone when
 *        absent, which is what the flow issued before the management scope existed),
 *        permissions, description (for the admin tokens listing)
 * @returns {Promise<String>} the code
 */
async function createAuthorizationCode(opts) {
    const scopes = opts.scopes === undefined ? ['mcp'] : normalizeScopes(opts.scopes);
    if (!scopes.length) {
        // A code that mints a token with no surface is a token that can do nothing; the consent
        // page refuses the combination before it gets here, so this is the belt to that brace
        throw oauthFailure('invalid_scope', 'No MCP scope was approved');
    }
    ...
    const record = {
        ...
        // means unnarrowed within the scopes, the same shape the admin generator posts.
...
async function redeemAuthorizationCode({ code, clientId, redirectUri, codeVerifier, resource, origin, ip }) {
    ...
    // A code minted before the scopes were recorded on it carries none; it was issued for the
    // one scope the flow had then
    const scopes = Array.isArray(record.scopes) && record.scopes.length ? record.scopes : ['mcp'];
    ...
    return {
        access_token: token,
        token_type: 'Bearer',
        // RFC 6749 3.3: space separated, and what was actually granted
        scope: scopes.join(' ')
    };
}
