...
class SchemaConverter {
    ...
    setRequiredOnParent(name, parent, description) {
        ...
        // NOT named `require`: shadowing it is legal and runs fine under Node, but pkg walks
        // this file's AST looking for module requires and dies on the argument-less calls
        // ("Cannot read properties of undefined (reading 'tokens')"), taking every binary
        // build down with it. Nothing in the test suite can catch that - only a pkg build can.
        const markRequired = () => {
            if (!parent.required) {
                parent.required = [];
            }
        ...
        // `then` only, and `otherwise` deliberately not. It looks like an omission - `imap.host` is
        // required unless `disabled` is set, and an unbound access token has to set `permissions`,
        // and both say so in `otherwise`, so reading only `then` publishes them as optional. Adding
        // the second branch was tried and reverted, because a flat `required` array cannot hold two
        // sides of an exclusive choice: `CreateOAuth2App` then required `clientId`, `clientSecret`
        // and `redirectUrl` alongside `serviceClient` and `serviceClientEmail`, which are the
        // gmailService half of the same `when` and forbid the other three. No request body satisfies
        // that, where before at least the gmailService one did. Response schemas made it worse -
        // `IMAPResponse` gained `host`/`port`, which a strict generated client refuses to
        // deserialize when the server legitimately omits them.
        //
        // So the rule is: flatten a condition only where doing so cannot describe an impossible
        // body. A requirement that lives in `otherwise` belongs in the field's description instead,
        // which is where POST /v1/tokens states it.
    ...
    parseObject(property, description, context) {
        // The same pair parseArray() publishes, under the object spelling. `.min(1)` on an object is
        // how "at least one of these axes" is said in joi, and a token narrowing that lists neither
        // is refused - so leaving it out documented `{}` as an acceptable body.
        property.minProperties = ruleArg(description.rules, 'min');
        property.maxProperties = ruleArg(description.rules, 'max');
