...
//   plugins.openapi.deprecated -> the route is left out of the document (compatibility aliases)
...
function buildOpenApiDocument(routes, options) {
    ...
    // Routes marked `plugins.openapi.deprecated` are compatibility aliases: they stay registered
    // (and keep the `api` tag, which also drives the CSRF skip list) but are left out of the
    // published document and the reference page, so nothing steers a new integration onto them
    for (const route of sortRoutes(routes.filter(route => (route.settings.tags || []).includes(API_ROUTE_TAG) && !pluginOptions(route).deprecated))) {
