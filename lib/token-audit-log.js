...
function record(entry) {
    ...
    isEnabled()
        .then(async enabled => {
            ...
            // The path is as far as this goes. It is enough to answer "which credential touched that
            // message", because a message id travels in the path - and it stops short of anything
            // that would put a subject or a body into a second place with its own retention.
            const stored = {
                time: Date.now(),
                ip: entry.ip || null,
...
/**
 * Removes a token's log. Called when the token itself is deleted - the trail is about a credential
 * that no longer exists, and leaving it behind would keep the record after the thing it describes.
 *
 * @param {String} tokenId - SHA-256 hash of the token
 */
function del(tokenId) {
    return redis.del(logKey(tokenId));
}

module.exports = { record, list, del, resetFlagCache, LOG_ENTRIES, LOG_AGE };
