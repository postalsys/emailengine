...
// The page size served when a request does not ask for one. Joi does not validate a schema's own
// default against that schema, so a default above the maximum is handed back for an absent value
// while the same value sent explicitly is refused - which is what an instance keeping fewer than 20
// entries used to do to the exact number /swagger.json advertises as the default.
const DEFAULT_LOG_PAGE_SIZE = Math.min(20, MAX_PAGE_SIZE);
...
async function list(tokenId, opts) {
    ...
    const pageSize = Math.min(Math.max(Number(rawPageSize) || DEFAULT_LOG_PAGE_SIZE, 1), MAX_PAGE_SIZE);
...
module.exports = { record, allowed, denied, list, logKey, resetFlagCache, LOG_ENTRIES, MAX_PAGE_SIZE, DEFAULT_LOG_PAGE_SIZE };
