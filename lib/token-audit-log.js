...
// cap is also the upper bound of the log route's `pageSize`, so a value below 1 left `min(1).max(0)`
// on the query schema and no request could satisfy it, and Redis DELETES a key given a non-positive
...
const LOG_ENTRIES = positiveInt(process.env.EENGINE_TOKEN_LOG_ENTRIES, 200);
