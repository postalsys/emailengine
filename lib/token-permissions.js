...
// instead of them.
//
// An absent `actions` axis means every action. An absent `groups` axis means IMPLICIT_GROUPS - the
// sections that existed when the two-axis form shipped - and NOT every grantable group: the groups
// the admin split made grantable have to be named, so a record written before they existed keeps
...
const { NEVER_GRANTABLE, ACTION_VALUES, GROUP_VALUES, IMPLICIT_GROUPS, grantKey } = require('./api-routes/permission-map');
 ...
 * @returns {{narrowed: Boolean, actions: (Set|null), groups: (Set|null), grants: (Object|null), malformed: Boolean}}
 *          for the two-axis form, `actions` is null when the axis is absent (every action) and
 *          `groups` is always resolved (IMPLICIT_GROUPS when absent)
 */
function parsePermissions(permissions) {
    if (permissions === null || typeof permissions === 'undefined') {
        return { narrowed: false, actions: null, groups: null, grants: null, malformed: false };
    }

    const malformed = { narrowed: true, actions: null, groups: null, grants: null, malformed: true };
...
function parseRecord(permissions, malformed) {
    ...
    if (Object.hasOwn(permissions, GRANTS_KEY)) {
        ...
        return { narrowed: true, actions: null, groups: null, grants, malformed: false };
    }

    const axes = { actions: null, groups: null };
    ...
    // Resolved once per record, here, so the two readers below share one meaning of an absent
    // axis: every action, and IMPLICIT_GROUPS. Sets, because check() runs per tool on every
    // tools/list.
    return {
        narrowed: true,
        actions: axes.actions && new Set(axes.actions),
        groups: new Set(axes.groups || IMPLICIT_GROUPS),
        grants: null,
        malformed: false
    };
}
...
function check({ tokenData, operation }) {
    ...
    if (parsed.actions && !parsed.actions.has(action)) {
        return { allowed: false, reason: REASON.ACTION, required };
    }

    if (!parsed.groups.has(group)) {
 ...
 *
 * The two-axis form expands to its cross product, an absent axis read as parseRecord() resolved
 * it. The pair form is returned as it is. The never-grantable groups are left out either way,
 * because check() refuses them before it reads the record, so a pair naming one is not something
 * the record allows.
 *
...
function effectiveGrants(permissions) {
    ...
    const grants = [];
    for (const action of parsed.actions || ACTION_VALUES) {
