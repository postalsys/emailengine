...
const { NEVER_GRANTABLE, ACTION_VALUES, GROUP_VALUES, GRANTABLE_GROUPS } = require('./api-routes/permission-map');
 ...
 * @returns {Array|null} the pairs, or null when the list is not one this version can read
 */
function parseGrants(value) {
    ...
    for (const entry of value) {
        ...
        if (keys.length !== GRANT_KEYS.length || !GRANT_KEYS.every(key => Object.hasOwn(entry, key))) {
            return null;
        }
        ...
        grants.push({ action: entry.action, group: entry.group });
    }
...
function check({ tokenData, operation }) {
    ...
    if (parsed.grants) {
        if (!parsed.grants.some(grant => grant.action === action && grant.group === group)) {
 ...
 * record by its pairs, and the pages that count MCP tools against a record apply the pairs in a
 * browser. Neither should hold its own reading of the two forms.
 *
 * @param {*} permissions - the record's `permissions` field
...
function effectiveGrants(permissions) {
    ...
    if (parsed.grants) {
        return parsed.grants.filter(grant => !NEVER_GRANTABLE.has(grant.group)).map(grant => ({ action: grant.action, group: grant.group }));
    }
