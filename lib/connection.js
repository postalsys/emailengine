...
class Connection {
    ...
    async getCurrentListing(options) {
        ...
        for (let mailbox of listing) {
            ...
            if (!existingMailbox) {
                ...
                mailbox.isNew = true;
                hasChanges = true;
            } else if (existingMailbox.delimiter !== mailbox.delimiter || existingMailbox.specialUseSource !== mailbox.specialUseSource) {
                hasChanges = true;
            }
        }
        ...
        if (hasChanges) {
            ...
            listing.forEach(entry => {
                ...
                Object.keys(entry).forEach(key => {
                    if (['path', 'specialUse', 'name', 'listed', 'subscribed', 'delimiter', 'specialUseSource'].includes(key)) {
    ...
    async syncMailboxes() {
        ...
        const serverInfo = Object.assign({}, this.imapClient.serverInfo || {}, { capabilities });
    ...
    async notify(mailbox, event, data, extraOpts) {
        ...
        if (!skipWebhook && addDocumentQueueJob) {
            ...
            await Webhooks.pushToQueue(event, notifyPayload, { routesOnly: true });
        } else {
            ...
            if (addDocumentQueueJob) {
                await this.documentsQueue.add(event, payload, defaultJobOptions);
            }
        }
    }
    ...
    async start() {
        ...
        try {
            ...
            if (accountData.oauth2 && accountData.oauth2.auth) {
                ...
                console.log('LOADING ACCOUNT DATA', accountData.oauth2);
            ...
            console.log('UPDATE STATE', initialState, this.state);
    ...
    async queueMessageEntry(data, meta) {
        ...
        if (serviceUrl && data.listId && data.to && data.to.length === 1 && data.to[0].address) {
            ...
            let serviceDomain;
            try {
                serviceDomain = (new URL(serviceUrl).hostname || '').toLowerCase().trim();
            } catch (err) {
                // ignore error
            }
            serviceDomain = serviceDomain || (os.hostname() || '').toLowerCase().trim() || 'localhost';

            if (serviceDomain) {
                let unsubscribeUrlObj = new URL('/unsubscribe', serviceUrl);

                const serviceSecret = await getServiceSecret();

                let fromDomain = ((data.from && data.from.address) || '').split('@').pop().trim().toLowerCase() || serviceDomain;
                try {
                ...
                data.headers['List-ID'] = `<${data.listId}.${serviceDomain}>`;
