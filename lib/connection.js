...
class Connection {
    ...
    async start() {
        ...
        // check if either user account is tracked or everybody is tracked
        let logging = await settings.accountLogging(this.account);
        imapConfig.emitLogs = logging.enabled;
        ...
        this.imapClient.on('log', entry => {
            ...
            this.redis
                .multi()
                .lpush(this.getLogKey(), logRow)
                .ltrim(this.getLogKey(), 0, this.maxLogLines - 1)
                .exec()
