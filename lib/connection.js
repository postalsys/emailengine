...
class Connection {
    ...
    async start() {
        ...
        if (accountData.imap.authServer) {
            try {
                accountData.imap.auth = await this.resolvePassword(accountData.imap.authServer, 'imap');
            } catch (err) {
                this.logger.error({
                    msg: 'Failed to resolve authentication for user',
                    authServer: accountData.imap.authServer,
                    account: this.account,
                    err
                });
                return setImmediate(() => {
    ...
    async resolvePassword(url, proto) {
        ...
        let authResponse = await fetch(url, { method: 'GET', headers });
        if (!authResponse.ok) {
        ...
        if (!authData.username) {
            throw new Error('Invalid response from authServer');
        }
