...
class Connection {
    ...
    async submitMessage(data) {
        ...
        let raw, hasBcc, envelope, messageId, sendAt, queueId;
        ...
        if (data.prepared) {
            ...
            let rawEmailData = await getRawEmail(data);

            raw = rawEmailData.raw;
            hasBcc = rawEmailData.hasBcc;
            envelope = rawEmailData.envelope;
            messageId = rawEmailData.messageId;
            sendAt = rawEmailData.sendAt;
            ...
            if (sendAt > now) {
                ...
                await this.submitQueue.add(
                    'delayed',
                    { account: this.account, qId, created: now.getTime() },
                    {
                        removeOnComplete: true,
                        removeOnFail: true,
                        attempts: 5,
