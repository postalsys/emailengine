...
const { AUTH_ERROR_NOTIFY, AUTH_SUCCESS_NOTIFY, CONNECT_ERROR_NOTIFY, EMAIL_SENT_NOTIFY, QUEUE_REMOVE_AFTER } = require('./consts');
...
class Connection {
    ...
    async getLocalAddress(protocol) {
        ...
        try {
            let addressData = await redis.hget('interfaces', localAddress);
