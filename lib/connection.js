...
const { AUTH_ERROR_NOTIFY, AUTH_SUCCESS_NOTIFY, CONNECT_ERROR_NOTIFY } = require('./consts');
    ...
    async start() {
        ...
        if (accountData.imap.useAuthServer) {
            try {
                ...
                this.notify(false, AUTH_ERROR_NOTIFY, {
        ...
        let imapConnectionConfig = JSON.parse(JSON.stringify(accountData.imap));
        if (!imapConnectionConfig.tls) {
            imapConnectionConfig.tls = {};
        }
        imapConnectionConfig.tls.localAddress = this.getLocalAddress();
        ...
        try {
            await this.connect();
            this.notify(false, AUTH_SUCCESS_NOTIFY, {
            ...
            if (err.authenticationFailed) {
                ...
                this.notify(false, CONNECT_ERROR_NOTIFY, {
