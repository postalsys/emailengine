...
class Connection {
    ...
    periodicListRefresh() {
        if (this.isClosing || this.isClosed || this.refreshingList) {
            return false;
        }
    ...
    async notify(mailbox, event, data, extraOpts) {
        ...
        if (this.documentsQueue && [MESSAGE_NEW_NOTIFY, MESSAGE_DELETED_NOTIFY, MESSAGE_UPDATED_NOTIFY].includes(event)) {
    ...
    async queueMessage(data, meta) {
        ...
        if (data.template) {
            let templateData = await templates.get(accountData.account, data.template);
            if (!templateData) {
            ...
            for (let key of Object.keys(templateData.content || {})) {
                data[key] = data[key] || templateData.content[key];
            }
        ...
        for (let bulkEntry of bulkList) {
            // poor man's structuredClone() (EmailEngine needs to support Node versions 16+)
            let messageCopy = JSON.parse(JSON.stringify(data));
            messageCopy.to = [bulkEntry.to];

            if (bulkEntry.messageId) {
                messageCopy.messageId = bulkEntry.messageId;
            }

            if (bulkEntry.render) {
                messageCopy.render = bulkEntry.render;
            }
    ...
    async queueMessageEntry(data, meta) {
        ...
        let context = {
            values: (data.render && data.render.values) || {},
            account: {
        ...
        for (let key of ['subject', 'html', 'text']) {
            if (typeof data[key] === 'object' && data[key].template) {
                data[key] = this.render(data[key], context, key);
            }
        }
        ...
        let { raw, hasBcc, envelope, subject, messageId, sendAt, deliveryAttempts, trackingEnabled } = await getRawEmail(data);
