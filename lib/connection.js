...
class Connection {
    ...
    async connect() {
        ...
        imapClient.on('close', async () => {
            ...
            // FIXME: remove console.log entries

            console.log('CONNECTION CLOSED TO', this.account, new Date().toISOString());

            try {
                for (let mailbox of this.mailboxes) {
                    if (mailbox.selected) {
                        // should be at most one though
                        console.log('CLOSING MAILBOX', mailbox.path, new Date().toISOString());
                        await mailbox.onClose();
                        console.log('CLOSED MAILBOX', new Date().toISOString());
                    }
                }

                if (!imapClient.disabled) {
                    console.log('TRY TO RECONNECT', this._connecting, this.isClosing, this.isClosed, new Date().toISOString());
