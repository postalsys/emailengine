...
const { normalizePath, resolveCredentials, emitChangeEvent, selectRendezvousAddress, convertDataUrisToAtachments } = require('./tools');
...
class Connection {
    ...
    async queueMessageEntry(data, meta) {
        ...
        let inlineOptions = {
            locale: data.locale || (await settings.get('locale')),
            tz: data.tz
        };
        delete data.locale;
        delete data.tz;
        ...
        if (data.reference && data.reference.message) {
            response.reference = {
                ...
                status: referencedMessage ? true : false
            };
