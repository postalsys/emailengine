...
class Connection {
    ...
    async getCurrentListing() {
        ...
        // ignore non-subscribed folders
        listing = listing.filter(mailbox => mailbox.subscribed && !mailbox.flags.has('\\Noselect'));
            ...
            this.getCurrentListing()
                .then(listing => {
                    ...
                    for (let entry of listing) {
                        // only care about subscribed folders and in case of gmail All mail folder + trash+junk that are not included in All
                        if (
                            entry.subscribed &&
                            // previously unseen
    ...
    async start() {
        ...
        this.imapClient.on('error', err => {
            this.logger.error({ msg: 'IMAP connection error', err });
            this.reconnect().catch(err => {
                this.logger.error({ msg: 'IMAP reconnection error', err });
            });
        });
