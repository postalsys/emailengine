...
class Connection {
    ...
    async queueMessageEntry(data, meta) {
        ...
        if (serviceUrl && data.listId && data.to && data.to.length === 1 && data.to[0].address) {
            ...
            let blocked = await redis.hexists(`${REDIS_PREFIX}lists:unsub:entries:${data.listId}`, data.to[0].address);
            if (blocked) {
                return {
                    responseValue: {
            ...
            let serviceDomain;
            try {
                serviceDomain = new URL(serviceUrl).hostname;
            } catch (err) {
                // ignore
            }
            ...
            if (serviceDomain) {
                ...
                if (!data.messageId) {
                    let fromDomain = ((data.from && data.from.address) || '').split('@').pop().trim().toLowerCase() || 'localhost';
                    try {
