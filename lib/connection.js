...
class Connection {
    ...
    async submitMessage(data) {
        ...
        if (accountData.oauth2 && accountData.oauth2.auth) {
            ...
            switch (accountData.oauth2.provider) {
                ...
                case 'gmailService':
                    ...
                    break;
                case 'outlook':
                    smtpConnectionConfig.host = 'smtp.office365.come';
        ...
        let { raw, hasBcc, envelope, messageId, qId: queueId, reference, job: jobData } = data;
        ...
        try {
            ...
            err.statusCode = Number(err.responseCode) || 502;
    ...
    async queueMessage(data, meta) {
        ...
        let qId = Buffer.concat([timeBuf.slice(2), crypto.randomBytes(4)])
            .toString('hex')
            .substr(1);
        ...
        await this.redis.hsetBuffer(`${REDIS_PREFIX}iaq:${this.account}`, qId, msgEntry);
