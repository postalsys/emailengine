...
class OutlookOauth {
    ...
    async getToken(code) {
        ...
        if (!res.ok) {
            ...
            err.tokenRequest = { grant: 'authorization_code', provider: 'outlook', status: res.status, clientId: this.clientId, scopes: this.scopes, code };
            try {
    ...
    async refreshToken(opts) {
        ...
        if (!res.ok) {
            let err = new Error('Token request failed');
            err.tokenRequest = { grant: 'refresh_token', provider: 'outlook', status: res.status, clientId: this.clientId, scopes: this.scopes };
            try {
                err.tokenRequest.response = await res.json();
            } catch (err) {
                // ignore
