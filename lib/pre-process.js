...
class PreProcessHandler {
    ...
    async getHandler() {
        ...
        try {
            ...
            logger.error({ msg: 'Failed to compile pre-processing script', component: 'pre-process', type: 'filter', err });
        ...
        if (preProcessData.compiledFn) {
            try {
                ...
                logger.error({ msg: 'Failed to compile pre-processing script', component: 'pre-process', type: 'map', err });
                preProcessData.compiledFn = null;
                preProcessData.compiledMap = null;
            }
        }
        ...
        if (preProcessData.compiledFn) {
            preProcessData.filterFn = async payload => {
                try {
                    ...
                    logger.error({ msg: 'Failed to execute pre-processing script', component: 'pre-process', type: 'filter', err });
                    return null;
                }
            };
        }
        ...
        if (preProcessData.compiledMap) {
            preProcessData.mapFn = async payload => {
                try {
                    ...
                    logger.error({ msg: 'Failed to execute pre-processing script', component: 'pre-process', type: 'map', err });
                    return null;
                }
            };
        }
    ...
    async storeLog(type, payload, error) {
        ...
        try {
            ...
            logger.error({ msg: 'Failed to insert error log entries', component: 'pre-process', err });
        }
    }
    ...
    async getErrorLog() {
        ...
        for (let line of logLines) {
            try {
                ...
                logger.error({ msg: 'Failed to decode log line', component: 'pre-process', entry: line && line.toString('base64'), err });
            }
        }
