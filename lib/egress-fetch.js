...
// Headers that authenticate a request to one specific origin, and so cannot travel to another one.
// A credential is issued against the host that asked for it; a redirect naming a different host is
// not a reason to hand it over. fetch() drops these on a cross-origin hop and so does this.
 ...
 * @returns {URL} Absolute URL of the next hop. Returned parsed, so the caller can compare origins
...
/**
 * Drops a set of headers from a plain header object (the only shape the callers pass).
 *
 * @param {Object} [headers] - Header name to value
 * @param {Set} names - Lowercased header names to remove
 * @returns {Object|undefined} Filtered copy
 */
function withoutHeaders(headers, names) {
    return headers && Object.fromEntries(Object.entries(headers).filter(([name]) => !names.has(name.toLowerCase())));
}
 ...
 * do so for anything but GET and HEAD, 307 and 308 replay the request unchanged, and a hop to
 * another origin drops the credential headers. A 3xx without a Location is returned as it is,
 * since it is not a redirect anyone can follow.
 *
...
async function fetchWithVettedRedirects(fetchImpl, url, options) {
    ...
    for (let hop = 0; ; hop++) {
        ...
        if (next.origin !== new URL(current).origin) {
            headers = withoutHeaders(headers, CREDENTIAL_HEADERS);
        }
