...
const { DEFAULT_MAX_LOG_LINES, PDKDF2_ITERATIONS, PDKDF2_SALT_SIZE, PDKDF2_DIGEST } = require('./consts');
...
function applyRoutes(server, call) {
    ...
    server.route({
        ...
        async handler(request, h) {
            console.log(request.app.licenseInfo);
            ...
            try {
                ...
                if (authData && authData.password) {
                    try {
                        ...
                        err.details = [{ path: 'password0', message: err.message }];
                        throw err;
                    }
                }
