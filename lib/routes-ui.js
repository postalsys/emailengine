        ...
        async handler(request, h) {
            ...
            let gmailWatch =
                accountData.watchResponse || accountData.watchFailure
                    ? {
                          lastCheckStr: accountData.lastWatch?.toISOString(),
                          expiresStr: accountData.watchResponse?.expiration ? new Date(Number(accountData.watchResponse?.expiration)).toISOString() : false
                      }
                    : false;
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            return h.view(
                'unsubscribe',
                {
                    pageTitleFull: gt.gettext('Subscription Management'),
