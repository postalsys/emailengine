...
const os = require('os');
const { ADDRESS_STRATEGIES, settingsSchema, templateSchemas, oauthCreateSchema, accountIdSchema, defaultAccountTypeSchema } = require('./schemas');
const fs = require('fs');
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                ...
                if (oauth2App?.pubsubUpdates?.pubSubSubscription) {
                ...
                return h.view(
                    'config/oauth/new',
                    {
                        ...
                        pubSubApps: pubSubApps?.apps?.map(app => {
