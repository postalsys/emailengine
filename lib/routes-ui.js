        ...
        options: {
            validate: {
                ...
                payload: Joi.object({
                    ...
                    params: Joi.object().description('Optional handlebars values')
                })
            }
        }
    });
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            if (!hostname) {
                return {
                    success: false,
                    error: 'Service URL must use a public hostname'
                };
            }

            let certificateData = await h.certs.getCertificate(hostname, true);
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                ...
                logger.info({ msg: 'Fetching certificate info', domain: hostname });

                let certificateData = await h.certs.getCertificate(hostname);
                if (!certificateData) {
                    return {
                        success: false,
                        error: `Failed to provision a ceritifcate for ${hostname}`
                    };
                }
