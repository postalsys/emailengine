...
-----END LICENSE-----`;
        ...
        async handler(request, h) {
            try {
                ...
                await passkeys.saveCredential({
                    ...
                    name: request.payload.name || 'Unnamed passkey',
                    user
                });
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            try {
                let user = (request.auth && request.auth.credentials && request.auth.credentials.user) || 'admin';
                await passkeys.deleteCredential(request.payload.credentialId, user);
        ...
        options: {
            validate: {
                ...
                payload: Joi.object({
                    credentialId: Joi.string().max(512).required().description('Credential ID to delete')
                })
            }
        }
    });
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                let rateLimit = await h.checkRateLimit('passkey:auth:options', 1, 10, 60);
                if (!rateLimit.success) {
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                let rateLimit = await h.checkRateLimit('passkey:auth:verify', 1, 10, 60);
                if (!rateLimit.success) {
                    return h.response({ error: 'Rate limited, please wait and try again' }).code(429);
                }
                ...
                // resolve user from credential and create session
                ...
                request.logger.error({ msg: 'Failed to verify passkey authentication', err });
                return h.response({ success: false, error: 'Authentication failed' }).code(400);
            }
        },
        options: {
