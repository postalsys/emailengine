...
return payload;`)
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            if (!webhook) {
                let error = Boom.boomify(new Error('Webhook Routing was not found.'), { statusCode: 404 });
                throw error;
            }
            ...
            validate: {
                ...
                params: Joi.object({
                    webhook: Joi.string()
                        ...
                        .example('AAAAAQAACnA')
                        .required()
                        .description('Webhook Routing ID')
                })
            }
        }
    });
    ...
    server.route({
        ...
        async handler(request, h) {
            let webhook = await webhooks.get(request.params.webhook);
            if (!webhook) {
                let error = Boom.boomify(new Error('Webhook Routing not found.'), { statusCode: 404 });
                throw error;
            }
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            try {
                ...
                await request.flash({ type: 'info', message: `Webhook Routing settings were updated` });
                return h.redirect(`/admin/webhooks/webhook/${request.payload.webhook}`);
            } catch (err) {
                await request.flash({ type: 'danger', message: `Failed to update Webhook Routing` });
                logger.error({ msg: 'Failed to update Webhook ROuting', err });
