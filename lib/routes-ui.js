...
function applyRoutes(server, call) {
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            let wh = await webhooks.getWebhookHandlers();
            console.log('WEBHOOK HANDLERS');
            console.log(wh);

            console.log('FILTER 1', wh[0].filterFn({ account: 'testaccount', path: 'INBOX', event: 'messageNew', data: {} }));
            console.log('FILTER 2', wh[0].filterFn({ account: 'testaccount2', path: 'INBOX', event: 'messageNew', data: {} }));
