...
const { failAction, verifyAccountInfo, getLogs, flattenObjectKeys, getStats } = require('./tools');
...
async function updatePublicInterfaces() {
    ...
    for (let iface of interfaces) {
        ...
        if (iface.defaultInterface) {
            await redis.hset('interfaces', `default:${iface.family}`, iface.localAddress);
        }

        let existingEntry = await redis.hget('interfaces', iface.localAddress);
        if (existingEntry) {
            try {
        ...
        await redis.hset('interfaces', iface.localAddress, JSON.stringify(iface));
    }
}
...
async function listPublicInterfaces(selectedAddresses) {
    ...
    let entries = await redis.hgetall('interfaces');
...
function applyRoutes(server, call) {
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            return h.view(
                'redirect',
                { httpRedirectUrl: data.redirectUrl ? data.redirectUrl : `/admin/accounts/${result.account}` },
                {
                    layout: 'public'
                }
            );
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                ...
                let updates = Object.assign(oldData, {
                    account: request.params.account,
                    name: request.payload.name || '',
                    email: request.payload.email,
    ...
    server.route({
        ...
        options: {
            validate: {
                ...
                payload: Joi.object({
                    imapStrategy: settingsSchema.imapStrategy,
                    smtpStrategy: settingsSchema.smtpStrategy,
                    localAddresses: settingsSchema.localAddresses,
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                ...
                await redis.hdel('interfaces', localAddress);
    ...
    server.route({
        ...
        async handler(request, h) {
            let values = {
                enabled: await settings.get('smtpServerEnabled'),
                password: await settings.get('smtpServerPassword'),
                authEnabled: await settings.get('smtpServerAuthEnabled'),
                port: await settings.get('smtpServerPort'),
                host: await settings.get('smtpServerHost'),
                proxy: await settings.get('smtpServerProxy')
            };
