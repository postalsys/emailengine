...
const os = require('os');
const { ADDRESS_STRATEGIES, settingsSchema, templateSchemas, oauthCreateSchema } = require('./schemas');
const fs = require('fs');
...
function applyRoutes(server, call) {
    server.route({
        ...
        async handler(request, h) {
            console.log('ADMIN PAGE', request.headers.cookies);
    ...
    server.route({
        ...
        async handler(request, h) {
            const values = {
                ...
                generateRiskAssessment: (await settings.get('generateRiskAssessment')) || false,
        ...
        async handler(request, h) {
            try {
                let data = {
                    ...
                    generateRiskAssessment: request.payload.generateRiskAssessment,
        ...
        options: {
            validate: {
                ...
                payload: Joi.object({
                    ...
                    generateRiskAssessment: settingsSchema.generateRiskAssessment.default(false),
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                ...
                logger.error({ msg: 'Failed to delete OAuth2 application', err, app: request.payload.app, remoteAddress: request.app.ip });
        ...
        options: {
            validate: {
                ...
                async failAction(request, h, err) {
                    await request.flash({ type: 'danger', message: `Failed to delete the OAuth2 application` });
                    logger.error({ msg: 'Failed to delete delete the OAuth2 application', err });
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                ...
                logger.error({ msg: 'Failed to register OAuth2 app', err });
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            try {
                ...
                logger.error({ msg: 'Failed to update OAuth2 app', app: request.payload.app, err });
            ...
            validate: {
                ...
                async failAction(request, h, err) {
                    ...
                    if (!appData) {
                        ...
                        logger.error({ msg: 'Application was not found.', app: request.payload.app });
                        return h.redirect('/admin').takeover();
                    }

                    await request.flash({ type: 'danger', message: `Failed to update OAuth2 app` });
                    logger.error({ msg: 'Failed to update OAuth2 app', err });
    ...
    server.route({
        ...
        options: {
            validate: {
                ...
                query: Joi.object({
                    account: Joi.string().empty('').trim().max(256).default(null).example('johnsmith').description('Account ID'),
                    page: Joi.number().min(1).max(1000000).default(1),
                    pageSize: Joi.number().min(1).max(250).default(DEFAULT_PAGE_SIZE)
                })
            }
...
-----END LICENSE-----`;
        ...
        async handler(request, h) {
            try {
                ...
                if (!rateLimit.success) {
                    logger.error({ msg: 'Rate limited', rateLimit });
                ...
                if (authData && authData.user && authData.user !== request.payload.username) {
                    logger.error({ msg: 'Invalid username', username: request.payload.username });
        ...
        async handler(request, h) {
            try {
                ...
                if (!reUseLimit.success) {
                    logger.error({ msg: 'TOTP code recently used', reUseLimit });
                ...
                logger.error({ msg: 'Failed to verify login', err });
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            try {
                ...
                logger.error({ msg: 'Failed to enable 2FA', err, remoteAddress: request.app.ip });
                return h.redirect(`/admin/account/security`);
            }
        },
        options: {
            validate: {
                ...
                async failAction(request, h, err) {
                    await request.flash({ type: 'danger', message: `Failed to enable 2FA` });
                    logger.error({ msg: 'Failed to enable 2FA', err });
    ...
    server.route({
        ...
        options: {
            validate: {
                ...
                async failAction(request, h, err) {
                    await request.flash({ type: 'danger', message: `Failed to disable 2FA` });
                    logger.error({ msg: 'Failed to disable 2FA', err });
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            try {
                ...
                logger.error({ msg: 'Failed to log out user sessions', err, remoteAddress: request.app.ip });
                return h.redirect(`/admin/account/security`);
            }
        },
        options: {
            validate: {
                ...
                async failAction(request, h, err) {
                    await request.flash({ type: 'danger', message: `Failed to log out user sessions` });
                    logger.error({ msg: 'Failed to log out user sessions', err });
    ...
    async function accountFormHandler(request, h) {
        ...
        let oauth2App = await oauth2Apps.get(request.payload.type);
    ...
    server.route({
        ...
        options: {
            validate: {
                ...
                params: Joi.object({
                    account: Joi.string().empty('').trim().max(256).required().example('johnsmith').description('Account ID').required()
                })
            }
        }
    });
        ...
        async handler(request, h) {
            ...
            return h.view(
                'config/document-store/index',
                {
                    ...
                    documentStorePreProcessingEnabled: (await settings.get(`documentStorePreProcessingEnabled`)) || false,
    ...
    server.route({
        ...
        async handler(request, h) {
            const errorLog = ((await preProcess.getErrorLog()) || []).map(entry => {
                if (entry.error && typeof entry.error === 'string') {
                    entry.error = entry.error
                        .replace(/\r?\n/g, '\n')
                        .replace(/^\s+at\s+.*$/gm, '')
...
return payload;`
    ...
    server.route({
        method: 'GET',
        path: '/admin/config/document-store/pre-processing/edit',
        async handler(request, h) {
            return h.view(
                'config/document-store/pre-processing/edit',
                {
                    menuConfig: true,
                    menuConfigDocumentStore: true,
                    documentStorePreProcessing: true,
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            try {
                ...
                logger.error({ msg: 'Failed to update Document Store pre-processing rules', err });
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                ...
                logger.error({ msg: 'Failed to create mapping', err });
...
<p>${now}</p>`,
        ...
        options: {
            validate: {
                ...
                payload: Joi.object({
                    account: Joi.string().empty('').trim().max(256).required().example('johnsmith').description('Account ID'),
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                ...
                logger.error({ msg: 'Failed to process subscription request', err });
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                ...
                logger.error({ msg: 'Failed to kill thread', err, thread: request.payload.thread, remoteAddress: request.app.ip });
                return h.redirect('/admin/internals');
            }
        },
        options: {
            validate: {
                ...
                async failAction(request, h, err) {
                    await request.flash({ type: 'danger', message: `Failed to kill thread` });
                    logger.error({ msg: 'Failed to kill thread', err });
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                ...
                logger.error({ msg: 'Failed to generate snapshot', err, thread: request.payload.thread, remoteAddress: request.app.ip });
                return h.redirect('/admin/internals');
            }
        },
        options: {
            validate: {
                ...
                async failAction(request, h, err) {
                    await request.flash({ type: 'danger', message: `Failed to generate snapshot` });
                    logger.error({ msg: 'Failed to generate snapshot', err });
