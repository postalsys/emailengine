    ...
    server.route({
        ...
        async handler(request) {
            ...
            try {
                ...
                if (testResponse) {
                    let mainSig = testResponse?.dkim?.results?.find(entry => entry.status?.result === 'pass' && entry.status?.aligned);

                    if (!mainSig) {
                        mainSig = testResponse?.dkim?.results?.find(entry => entry.status?.result === 'pass');
                    }

                    if (!mainSig) {
                        mainSig = testResponse?.dkim?.results[0];
                    }
                    ...
                    if (testResponse?.spf?.status?.comment) {
