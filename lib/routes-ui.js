...
const configOauthSchema = {
    ...
    oauth2Enabled: Joi.boolean().truthy('Y', 'true', '1', 'on').falsy('N', 'false', 0, '').default(false).description('Enable this app'),
...
function applyRoutes(server, call) {
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                ...
                appData.enabled = !!appData.oauth2Enabled;
                delete appData.oauth2Enabled;
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            let values = Object.assign({ oauth2Enabled: !!appData.enabled }, appData, {
                clientSecret: '',
                serviceKey: '',
