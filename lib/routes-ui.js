...
const { values } = require('./enum-message-flags');
...
const defaultMappingsList = Object.keys(defaultMappings)
    .map(key => ({
        key,
        type: defaultMappings[key].type || (defaultMappings[key].properties ? 'object' : 'text'),
...
return payload;`
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                // TODO: create mapping
                let error = new Error('Future feature');
                error.responseText = 'Future feature';
                throw error;
            ...
            validate: {
                ...
                payload: Joi.object({
                    ...
                    type: Joi.string()
                        ...
                        .default('text')
                        .label('Field type'),
                    indexed: Joi.boolean().truthy('Y', 'true', '1', 'on').falsy('N', 'false', 0, '').default(true)
                })
            }
        }
    });
