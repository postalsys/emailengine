...
function applyRoutes(server, call) {
    ...
    server.route({
        ...
        async handler(request) {
            try {
                ...
                logger.error({ msg: 'Failed to generate token', err, remoteAddress: request.ip, description: request.payload.description });
                return { success: false, error: err.message };
            }
        },
        options: {
    ...
    server.route({
        ...
        async handler(request, h) {
            try {
                let deleted = await tokens.delete(request.payload.token, { remoteAddress: request.ip });
                if (deleted) {
                ...
                logger.error({ msg: 'Failed to delete access token', err, token: request.payload.token, remoteAddress: request.ip });
                return h.redirect('/admin/tokens');
            }
        },
        options: {
