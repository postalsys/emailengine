        ...
        async handler(request, h) {
            ...
            return h.view(
                'gateways/index',
                {
                    ...
                    gateways: gateways.gateways.map(entry => Object.assign(entry, { timeStr: entry.lastUse ? entry.lastUse.toISOString() : null }))
                },
                {
                    layout: 'app'
                }
        ...
        async handler(request, h) {
            try {
                let gatewayData = {
                    ...
                    host: request.payload.smtp_host || null,
                    port: request.payload.smtp_port || null,
                    secure: request.payload.smtp_secure || null,
                    user: request.payload.smtp_user || null,
                    pass: request.payload.smtp_pass || null,
                    tls: {}
                };
        ...
        options: {
            validate: {
                ...
                payload: Joi.object({
                    ...
                    smtp_user: Joi.string().empty('').trim().max(1024).label('UserName'),
                    smtp_pass: Joi.string().empty('').max(1024).label('Password'),
                    smtp_host: Joi.string().hostname().example('smtp.gmail.com').description('Hostname to connect to').label('Hostname'),
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            let values = Object.assign({}, flattenObjectKeys(accountData), {
                imap: true,
                imap_disabled: (!accountData.imap && !accountData.oauth2) || accountData.imap.disabled,
