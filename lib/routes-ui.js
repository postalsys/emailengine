...
function applyRoutes(server, call) {
    ...
    server.route({
        ...
        options: {
            validate: {
                ...
                payload: Joi.object(oauthCreateSchema)
            }
        }
    });
    ...
    server.route({
        method: 'GET',
        path: '/admin/config/oauth-legacy/{provider?}',
        async handler(request, h) {
            let values = {
            ...
            switch (values.provider) {
                case 'gmail':
                    values.enabled = await settings.get('gmailEnabled');
                ...
                case 'gmailService':
                    values.enabled = await settings.get('gmailServiceEnabled');
                ...
                case 'outlook':
                    values.enabled = await settings.get('outlookEnabled');
                ...
                case 'mailRu':
                    values.enabled = await settings.get('mailRuEnabled');
    ...
    server.route({
        method: 'POST',
        path: '/admin/config/oauth-legacy',
        async handler(request, h) {
            try {
                const provider = request.payload.provider;
                const data = {};

                data[`${provider}Enabled`] = request.payload.enabled;
                ...
                return h.redirect(`/admin/config/oauth-legacy/${request.payload.provider}`);
            } catch (err) {
                await request.flash({ type: 'danger', message: `Failed to update configuration` });
                logger.error({ msg: 'Failed to update configuration', err });
...
-----END LICENSE-----`;
    ...
    async function accounFormHandler(request, h) {
        ...
        if (oauth2App && oauth2App.enabled && oauth2App.active) {
        ...
        options: {
            validate: {
                ...
                payload: Joi.object({
                    ...
                    account: Joi.string().empty('').max(256).example('johnsmith').description('Account ID').required(),
