...
const { createImapProxyAuthHandler, classifyCredentialFailure, isImapResponseError, toImapResponseError } = require('../imap-proxy-auth');
    ...
    server.onAuth = function (login, session, callback) {
        onAuth(login, session)
                ...
                createProxy({ imapConfig, id: session.id, logger: logger.child({ property: 'upstream', account }) })
                    ...
                    .catch(err => {
                        if (isImapResponseError(err)) {
                            ...
                            return callback(toImapResponseError(err));
                        } else {
            ...
            .catch(err => {
                ...
                if (isImapResponseError(err)) {
                    // a refused login or a temporary failure the client is told to retry, not a proxy fault
                    serverLogger.warn(entry);
                    return callback(toImapResponseError(err));
                }
                serverLogger.error(entry);
