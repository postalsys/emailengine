...
async function onAuth(auth, session) {
    ...
    // No transport configured, or the operator disabled IMAP. The `imap.disabled` half matters here
    // as much as on the sync path: it is also what setErrorState() sets when an account trips the
    // authentication-failure threshold, and proxying past it would keep reconnecting to the provider
    // with credentials that were just judged bad.
