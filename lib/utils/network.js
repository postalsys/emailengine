...
function parseComparableCidr(range) {
    ...
    if (parsed.kind() === 'ipv6' && parsed.isIPv4MappedAddress()) {
        // A prefix shorter than the 96-bit mapped block does not describe an IPv4 subnet: it spans
        // out of the mapped range entirely. Clamping the remainder to zero turned such an entry
        // into 0.0.0.0/0 - one mistyped digit in an allowlist and every IPv4 address matched, which
        // is the one direction an allowlist must never fail in. Refused instead, so the caller logs
        // it as the configuration error it is and the entry matches nothing.
        if (bits < 96) {
            throw new Error(`IPv4-mapped range ${range} has a prefix shorter than the 96-bit mapped block`);
        }
        return [parsed.toIPv4Address(), bits - 96];
    }
...
function matchIp(ip, addresses) {
    ...
    for (let addr of addresses) {
        try {
            ...
            logger.error({ msg: 'Failed to parse allowlist address', ip, addr, err });
        }
    }
...
async function getLocalAddress(redis, protocol, account, hint) {
    ...
    try {
        ...
        logger.warn({ msg: 'Failed to load address data', localAddress, err });
