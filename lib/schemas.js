...
// Same, for routes that return a raw stream or a binary body. hapi-swagger derives the 200
// entry from the route's response.schema, which those routes have none of - so without an
// explicit entry the generated spec documents ONLY their error cases, and every consumer
// (Swagger UI, the /admin/reference pages, Postman, code generators) sees an operation with
// no success response at all. hapi-swagger fills in `schema: {type: 'string'}` itself, so
// the description is all that is needed here.
function streamResponses(description, ...statusCodes) {
