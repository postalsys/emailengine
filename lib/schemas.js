...
const settingsSchema = {
    ...
    localAddresses: Joi.array()
        .items(Joi.string().ip({ version: ['ipv4', 'ipv6'], cidr: 'forbidden' }).label('LocalAddressEntry'))
        .single()
...
const lastErrorSchema = Joi.object({
    ...
    tokenRequest: Joi.object({
        grant: Joi.string().valid('refresh_token', 'authorization_code').example('refresh_token').description('OAuth2 grant type being requested').label('OAuthGrant'),
...
const oauthCreateSchema = {
    ...
    baseScopes: Joi.string().empty('').trim().valid('imap', 'api', 'pubsub').example('imap').description('Connection type (IMAP, API, or Pub/Sub)').label('OAuth2BaseScopes'),
...
const exportStatusSchema = Joi.object({
    ...
    status: Joi.string().valid('queued', 'processing', 'completed', 'failed', 'cancelled').example('processing').description('Export status').label('ExportStatusValue'),
...
const exportListEntrySchema = Joi.object({
    exportId: Joi.string().example('exp_abc123def456').description('Export job identifier'),
    status: Joi.string().valid('queued', 'processing', 'completed', 'failed', 'cancelled').example('completed').description('Export status').label('ExportListStatusValue'),
    created: Joi.date().iso().example('2024-01-15T10:30:00Z').description('When export was created'),
    expiresAt: Joi.date().iso().example('2024-01-16T10:30:00Z').description('When export file expires')
