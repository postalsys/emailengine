...
const bounceListSchema = Joi.array()
    ...
    .description('Bounce notifications recorded against this message when they arrived')
    .label('BounceList');
...
const messageDetailsSchema = Joi.object({
    ...
    isBounce: Joi.boolean()
        .example(false)
        .description(
            'Whether this message is a bounce (a delivery failure report). Checked for messages in the Inbox only; a delivery or delay notification is not one'
        ),
    relatedMessageId: Joi.string().example('<original@example.com>').description('Message-ID of the message that bounced. Set together with isBounce'),
