...
const settingsSchema = {
    ...
    openAiModel: Joi.string().allow('').valid('gpt-3.5-turbo', 'gpt-4').example('gpt-3.5-turbo').description('OpenAI API Model').label('OpenAiModel'),
    ...
    deliveryAttempts: Joi.number().empty('').min(0).description('How many times to retry an email sending before it is considered as failing'),
...
const tokenRestrictionsSchema = Joi.object({
    ...
    rateLimit: Joi.object({
        maxRequests: Joi.number().min(1).example(20).description('Allowed count of requests in the rate limit time window'),
        timeWindow: Joi.number().min(1).example(2).description('Rate limit time window in seconds')
    })
        .allow(false)
        .default(false)
