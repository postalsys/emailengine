...
const settingsSchema = {
    ...
    serviceUrl: Joi.string()
        ...
        .description('The base URL of the EmailEngine service')
        .label('ServiceURL'),
...
const oauth2AuthSchema = Joi.object({
    user: Joi.string().max(256).required().example('user@outlook.com').description('The username of the email account'),
    delegatedUser: Joi.string().max(256).optional().example('shared.mailbox@outlook.com').description('Username of the shared mailbox (MS365 only)')
})
    .when('authorize', {
        is: true,
        then: Joi.optional().valid(false, null),
...
const oauth2Schema = {
    ...
    redirectUrl: Joi.string()
        ...
        .description('URL to redirect the user after the OAuth2 consent screen (only valid if `authorize=true`)')
        .when('authorize', {
            is: true,
            then: Joi.optional(),
    ...
    provider: Joi.string().max(256).example('gmail').description('The OAuth provider to use'),

    auth: oauth2AuthSchema,

    useAuthServer: Joi.boolean().example(false).description('Set to true to use an external authentication server instead of managing access tokens'),
    ...
    authority: Joi.string()
        ...
        .max(1024)
        .example('consumers')
        .description("Outlook account type, such as 'consumers', 'organizations', 'common', or an organization ID")
...
const messageEntrySchema = Joi.object({
    id: Joi.string().example('AAAAAgAACrI').description('Unique identifier for the message assigned by the server').label('MessageEntryId'),
