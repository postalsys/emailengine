...
const settingsSchema = {
    ...
    tokenAuditLog: Joi.boolean()
        ...
        .example(false)
        .description(
            'Record which requests each access token makes, readable at GET /v1/token/{token}/log. Disabled by default, and entries only exist from the point it is enabled'
        ),
...
//
// `.min(1)` refuses `{}`, which is genuinely ambiguous between "no narrowing" and "grant nothing".
