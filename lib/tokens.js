...
module.exports = {
    ...
    async provision(opts) {
        opts = opts || {};
        const { account, restrictions, ip, remoteAddress, description, metadata, scopes, nolog, expires } = opts;
    ...
    async get(token, hashed, opts) {
        ...
        if (tokenData.expires && Date.now() >= tokenData.expires && !opts.allowExpired) {
            // Best-effort cleanup so short-lived tokens do not pile up. Done inline rather
            // than through delete() to avoid recursing back into this check.
            redis
                .multi()
                .hdel(`${REDIS_PREFIX}tokens`, hashedToken)
                .hdel(`${REDIS_PREFIX}tokens:access`, hashedToken)
                .srem(tokenData.account ? `${REDIS_PREFIX}iat:${tokenData.account}` : `${REDIS_PREFIX}iat`, hashedToken)
                .exec()
