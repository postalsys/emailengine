...
module.exports = {
    async getSessionToken(sessionId, ttl) {
        ...
        const results = await redis.multi().set(key, signature).expire(key, ttl).exec();
    ...
    async validateSessionToken(sessionId, token, ttl) {
        ...
        const [[errGet, storedSignature], [errExpire]] = results;
