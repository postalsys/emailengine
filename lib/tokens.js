...
module.exports = {
     ...
     * @param {String} [opts.scope] - keep only tokens whose scope list names this scope. Literal
     *   membership on purpose: a '*' token would also serve the surface, but the filter exists to
     *   answer "which tokens were minted for it", and those carry the scope by name.
     */
        ...
        for (let pos = 0; pos < list.length; pos += TOKEN_BATCH_SIZE) {
            ...
            for (let i = 0; i < batch.length; i++) {
                ...
                try {
                    ...
                    if (scope && !(Array.isArray(tokenData.scopes) && tokenData.scopes.includes(scope))) {
                        continue;
                    }
