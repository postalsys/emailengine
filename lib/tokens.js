...
module.exports = {
    ...
    async validateSessionToken(sessionId, token, expectedAccount, ttl) {
        ...
        // The HMAC key is the caller's session id, so a request with no `ee` cookie has nothing to
        // check against. createHmac() throws on an undefined key, which surfaced as a 500 where the
        // honest answer is "not authenticated" - a denial either way, but one that filled the error
        // log and masked real signals.
        ...
        // An expired or already-deleted token leaves no key, and GET answers null. Splitting that
        // threw, which is the same 500-instead-of-401 shape as the missing session id above - and
        // an expired session token is the ordinary case, not an exceptional one.
        if (typeof storedSignatureValue !== 'string') {
            return false;
        }
     ...
     * @param {String|Array<String>} [opts.scope] - keep only tokens whose scope list names one of
     *   these scopes. Literal membership on purpose: a '*' token would also serve the surface, but
     *   the filter exists to answer "which tokens were minted for it", and those carry the scope by
     *   name. A list because the MCP pages ask for both MCP scopes at once.
     */
    async list(account, page, pageSize, query, opts) {
        page = Math.max(Number(page) || 0, 0);
        pageSize = Math.max(Number(pageSize) || 20, 1);

        // Normalized here so that "is there a query" and "does this entry match" answer the same
        // question. matchesListQuery() trims before comparing and so matches everything for a
        // whitespace-only query, while the `!query` test below took it for a real one - which put a
        // single space in the search box on the read-everything path with no filter to show for it.
        query = (query || '').toString().trim();

        const scopes = [].concat((opts && opts.scope) || []).filter(Boolean);
        // Either filter forces the read-everything path: the fields they match on live on the
        // record and nowhere else
        const filtered = !!(query || scopes.length);
        ...
        // Without a filter only the visible page's records are read
        if (!filtered) {
        ...
        for (let pos = 0; pos < list.length; pos += TOKEN_BATCH_SIZE) {
            ...
            for (let i = 0; i < batch.length; i++) {
                ...
                try {
                    ...
                    // Before the entry is built: on a scope-filtered read of every record, the
                    // date coercion and the copy below would mostly construct throwaways
                    if (scopes.length && !(Array.isArray(tokenData.scopes) && scopes.some(scope => tokenData.scopes.includes(scope)))) {
                        continue;
                    }
        ...
        if (filtered) {
            ...
            // the same answer filterListPage() gave when it did the filtering here. `pages` follows
            // from the shared derivation below.
