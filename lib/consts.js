...
module.exports = {
    ...
    TRACK_OPEN_DESCRIPTION: '[experimental] Email open tracked – recipient opened an email',

    TRACK_CLICK_NOTIFY: 'trackClick',
    TRACK_CLICK_DESCRIPTION: '[experimental] Link click tracked – recipient clicked on a link',
    ...
    BLOCK_TLS_RENEW: 5 * 60 * 10000, // 8 * 3600 * 1000,
    // check if we need to renew the certificate once in an hour
    TLS_RENEW_CHECK_INTERVAL: 60 * 1000, //1 * 3600 * 1000,
    ...
    generateWebhookTable() {
        ...
        for (let entry of entries) {
            ...
            rows.push([`\`${entry.key}\``, entry.description.substr(0, splitter).trim(), entry.description.substr(splitter + 1).trim()]);
        }
