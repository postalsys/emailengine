...
module.exports = {
    ...
    // Account hash field marking an account parked after sustained authentication failures.
    // System-owned and transport-neutral - deliberately NOT `imap.disabled`, which is the
    // operator's "Disable IMAP" checkbox and does not exist on OAuth2 accounts at all. Read
    // through isConnectionBlocked() in lib/account/account-state.js rather than by name.
    ACCOUNT_AUTH_FAILURE_DISABLED_KEY: 'authFailureDisabled',

    // Account hash field holding when the OAuth2 token endpoint first started failing for this
    // account. Deliberately separate from `lastError:first`, which setErrorState() resets whenever
    // `serverResponseCode` changes - a provider alternating between, say, 503 and 400 would reset
    // that window on every flip and could never reach the auto-disable threshold. This one is only
    // set when unset and only cleared on a successful authentication, so it measures "how long has
    // the token endpoint been failing" regardless of how the failure is classified.
    ACCOUNT_TOKEN_ERROR_FIRST_KEY: 'tokenError:first',

    // serverResponseCode values this codebase attaches to a failure originating at an OAuth2 token
    // endpoint, as opposed to a mail server. Used to decide what counts toward the token-failure
    // window above. Every value here is one EmailEngine sets itself - see base-client.js
    // loadOAuth2LoginCredentials and the getToken() handlers in gmail-client.js/outlook-client.js.
    TOKEN_FAILURE_RESPONSE_CODES: new Set([
        'OauthRenewError', // credentials rejected while renewing
        'OauthRenewNetworkError', // transient failure while renewing
        'TokenGenerationError' // Gmail API / MS Graph token acquisition
    ]),
