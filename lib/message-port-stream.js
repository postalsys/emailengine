...
/**
 * Sends a finished download to the API worker over a transferred MessagePort. IMAP resolves a
 * download to a stream and Gmail and Graph to a Buffer; the Buffer is wrapped so both shapes take
 * the same path, leaving pipeToMessagePort() as the single place that has to get an aborted
 * consumer right.
 *
 * Call this only once there is content to send - handing the port over earlier leaks it, and its
 * message listener, whenever the lookup before it throws (a 404, say).
 *
 * @param {MessagePort} messagePort - Port transferred from the API worker
 * @param {Buffer|Readable} source - Payload to transfer
 * @param {Object} [logger] - Optional logger used to report transfer failures
 * @returns {MessagePortWritable} Destination bound to the port (used by tests; callers may ignore it)
 */
function sendToMessagePort(messagePort, source, logger) {
    const writable = new MessagePortWritable(messagePort);
    const stream = Buffer.isBuffer(source) ? Readable.from([source], { objectMode: false }) : source;

    // Constructing the writable starts the port, so a { cancel: true } already queued by a consumer
    // that gave up is delivered on the next turn of the event loop. Dispatching from the check
    // phase puts the guard in pipeToMessagePort() after that delivery rather than racing it.
