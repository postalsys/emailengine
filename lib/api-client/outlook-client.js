...
class OutlookClient extends BaseClient {
    ...
    async getRawMessage(emailId) {
        ...
        try {
            raw = await this.request(`${OUTLOOK_API_BASE}/me/messages/${emailId}/$value`, 'get', Buffer.alloc(0), { returnText: true });
        } catch (err) {
            switch (err.oauthRequest?.status) {
                case 404: {
    ...
    async deleteMessage(emailId, force) {
        ...
        if (force) {
            try {
                await this.request(`${OUTLOOK_API_BASE}/me/messages/${emailId}`, 'delete', Buffer.alloc(0), { returnText: true });
            } catch (err) {
                this.logger.error({
        ...
        let messageData;
        try {
            messageData = await this.request(`${OUTLOOK_API_BASE}/me/messages/${emailId}/move`, 'post', {
    ...
    async deleteMessages(path, search, force) {
        ...
        let submitBatch = async () => {
            let responseData;
            try {
                responseData = await this.request(`${OUTLOOK_API_BASE}/$batch`, 'post', {
                    requests: batch
                });
    ...
    async updateMessage(emailId, updates) {
        ...
        try {
            modifyResult = await this.request(`${OUTLOOK_API_BASE}/me/messages/${emailId}`, 'patch', flagUpdates);
        } catch (err) {
            this.logger.error({
    ...
    async getMessage(emailId, options) {
        ...
        try {
            messageData = await this.request(`${OUTLOOK_API_BASE}/me/messages/${emailId}`, 'get', {
    ...
    async ensureSubscription() {
        ...
        if (outlookSubscription.id) {
            ...
            if (existingExpirationDateTime && existingExpirationDateTime.getTime() < now) {
                ...
                let subscriptionRes;
                try {
                    subscriptionRes = await this.request(`${OUTLOOK_API_BASE}/subscriptions/${outlookSubscription?.id}`, 'patch', subscriptionPayload);
        ...
        if (!outlookSubscription.id) {
            ...
            let subscriptionRes;
            try {
                subscriptionRes = await this.request(`${OUTLOOK_API_BASE}/subscriptions`, 'post', subscriptionPayload);
                if (subscriptionRes?.expirationDateTime) {
                    outlookSubscription = {
    ...
    async getAttachmentContent(attachmentId, options) {
        ...
        try {
            attachmentResponse = await this.request(`${OUTLOOK_API_BASE}/me/messages/${emailId}/attachments/${id}`);
        } catch (err) {
            switch (err.oauthRequest?.status) {
                case 404: {
    ...
    async processUpdatedMessage(emailId) {
        ...
        try {
            messageData = await this.request(`${OUTLOOK_API_BASE}/me/messages/${emailId}`, 'get', {
                $select: ['id', 'isRead', 'isDraft', 'flag', 'conversationId'].join(',')
            });
        } catch (err) {
            this.logger.error({
                msg: 'Failed to fetch message data',
        ...
        // we do not know which flags were added or removed, so list all existing fields as added, and all missing as deleted

        const changes = { flags: { added: [], deleted: [] } };

        if (messageData.isRead) {
            changes.flags.added.push('\\Seen');
        } else {
            changes.flags.deleted.push('\\Seen');
        }

        if (messageData.isDraft) {
            changes.flags.added.push('\\Draft');
        } else {
            changes.flags.deleted.push('\\Draft');
        }

        if (messageData.flag?.flagStatus === 'flagged') {
            changes.flags.added.push('\\Flagged');
        } else {
            changes.flags.deleted.push('\\Flagged');
        }
