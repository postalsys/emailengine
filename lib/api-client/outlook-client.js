...
class OutlookClient extends BaseClient {
    ...
    async listMessages(query /*, options*/) {
        ...
        if (page * pageSize > totalMessages) {
            page = Math.floor(totalMessages / pageSize);
        }

        let pages = Math.floor(totalMessages / pageSize);
        ...
        try {
            ...
            console.log('MESSAGES', JSON.stringify(listing, false, 2));

            for (let messageData of listing?.value || []) {
                if (messageData.hasAttachments) {
                    try {
                        let attachments = await this.request(`${OUTLOOK_API_BASE}/me/messages/${messageData.id}/attachments`, 'get', {
                            $top: 1000,
                            $select: 'id,name,contentType,size,isInline,microsoft.graph.fileAttachment/contentId'
                        });
                        console.log('ATTACHMENTS', JSON.stringify(attachments, false, 2));
                    } catch (err) {
                        this.logger.error({
