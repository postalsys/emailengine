...
class OutlookClient extends BaseClient {
    ...
    async init() {
        ...
        let updates = {};
        if (accountData.name !== profileRes.displayName) {
            updates.name = profileRes.displayName;
        }

        if (accountData.email !== profileRes.mail) {
            updates.email = profileRes.mail;
        }

        if (accountData.auth?.user !== profileRes.userPrincipalName) {
            updates.oauth2 = { partial: true, auth: { user: profileRes.userPrincipalName } };
        }
    ...
    async listMailboxes(/*options*/) {
        ...
        for (let resolveResult of resolveResults) {
            ...
            if (resolveResult.status === 'fulfilled' && resolveResult.value) {
                console.log('FOUND SPECIAL USE', specialUseKey, specialTags.get(specialUseKey), resolveResult.value?.id);
                specialUseTagIds.set(resolveResult.value?.id, specialTags.get(specialUseKey));
            }
        }

        console.log('SPECIAL FOLDERS', require('util').inspect({ resolveResults, specialUseTagIds }, false, 22, true));

        let result = [];

        let traverse = async folderId => {
            let list = [];
            let done = false;
