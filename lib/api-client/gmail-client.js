...
class GmailClient extends BaseClient {
    ...
    async listMessages(query) {
        ...
        let nextPageCursor = listingResult.nextPageToken
            ? msgpack
                  .encode({
                      page: page + 1,
                      search: query.search || null,
                      pageToken: listingResult.nextPageToken,
                      messageCount: listingResult.resultSizeEstimate
                  })
                  .toString('base64url')
            : null;
...
if (/gmail-client\.js$/.test(process.argv[1])) {
    ...
    let main = async () => {
        ...
        let messages = await gmailClient.listMessages({ path: 'INBOX', search: { from: 'notify@chartmogul.com' } });
