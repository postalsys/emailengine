...
class GmailClient extends BaseClient {
    ...
    async init() {
        ...
        // TODO: run watch periodically (once a day or so)
        if (accountData._app?.pubSubApp && (!accountData.lastWatch || accountData.lastWatch < new Date(now - 24 * 3600 * 1000))) {
    ...
    async processHistoryEntry(historyEntry) {
        console.log('HISTORY ENTRY', require('util').inspect(historyEntry, false, 22, true));
    }

    async processHistory(currentHistoryId) {
        ...
        let getHistoryPage = async pageToken => {
            ...
            for (let entry of historyRes?.history || []) {
                try {
                    ...
                    if (historyEntryId && historyEntryId > currentHistoryId) {
