...
class GmailClient extends BaseClient {
    ...
    async listMailboxes(options) {
        ...
        console.log('LABELS LIST', util.inspect(labelsResult, false, 22, true));
    ...
    async updateMessage(messageId, updates) {
        ...
        const modifyResult = await this.request(`${GMAIL_API_BASE}/gmail/v1/users/me/messages/${messageId}/modify`, 'post', labelUpdates);
    ...
    async updateMessages(path, search, updates) {
        ...
        while (notDone) {
            ...
            console.log(123, messageListResult);
    ...
    async submitMessage(data) {
        ...
        console.log('SEND RESPONSE', submitInfo);
        /*
            SEND RESPONSE {
            id: '18f85d2eb6adb232',
        ...
        */
        ...
        if (submitInfo?.id) {
            ...
            console.log('MESSAGE ENTRY', util.inspect(messageEntry, false, 22, true));
    ...
    async renameMailbox(path, newPath) {
        ...
        try {
            ...
            console.log('LABEL RENAME ERROR', err?.oauthRequest?.response?.error);
            switch (err?.oauthRequest?.response?.error?.code) {
                case 409:
                case 400: {
                    // invalid name
        ...
        console.log('LABEL RENAME RESULT', util.inspect(label, false, 22, true));
    ...
    async deleteMailbox(path) {
        ...
        try {
            ...
            console.log('LABEL DELETE ERROR', err?.oauthRequest?.response?.error);
            switch (err?.oauthRequest?.response?.error?.code) {
                case 409:
                case 400: {
                    // invalid name
...
if (/gmail-client\.js$/.test(process.argv[1])) {
    ...
    let main = async () => {
        ...
        await gmailClient.createMailbox('Köki/Mööögel');

        await new Promise(r => setTimeout(r, 2000));

        let d = await gmailClient.deleteMailbox('Köki/Mööögel');
        console.log(d);
        process.exit();

        console.log('MODIFY MESSAGES');
        await gmailClient.updateMessages(false, { subject: 'proovikiri' }, { flags: { delete: ['\\Seen'], add: ['\\Flagged'] } });
        ...
        for (let msg of messages) {
            ...
            await gmailClient.updateMessage(msg.id, { flags: { delete: ['\\Seen'], add: ['\\Flagged'] } });
            console.log('HUH?', msg);
