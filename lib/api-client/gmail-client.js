...
class GmailClient extends BaseClient {
    ...
    prepareQuery(path, query) {
        ...
        if (path) {
            let key;
            let label;
            for (let sysPath of Object.keys(SYSTEM_NAMES)) {
                if (SYSTEM_NAMES[sysPath].toLowerCase().trim() === path.toLowerCase().trim()) {
                    // found match
                    if (/^CATEGORY_/.test(sysPath)) {
                        key = 'category';
                        label = sysPath.split('_').pop().toLowerCase().trim();
                    } else {
                        key = 'in';
                        label = sysPath.toLowerCase();
                    }
                    break;
                }
            }
            if (!key) {
                key = 'label';
                label = path.toLowerCase().trim();
            }
            queryParts.push(`${key}:${this.formatSearchTerm(label)}`);
        }

        if (typeof query.search.unseen === 'boolean') {
            queryParts.push(`${!query.search.flagged ? '-' : ''}is:starred`);
        }

        if (typeof query.search.unseen === 'boolean') {
            queryParts.push(`is:${query.search.unseen ? 'unread' : 'read'}`);
        }

        if (typeof query.search.seen === 'boolean') {
            queryParts.push(`is:${query.search.unseen ? 'read' : 'unread'}`);
        }

        for (let key of ['from', 'to', 'cc', 'bcc', 'subject']) {
            if (query.search[key]) {
                queryParts.push(`${key}:${this.formatSearchTerm(query.search[key])}`);
            }
        }

        for (let headerKey of Object.keys(query.search.header || {})) {
            switch (headerKey.toLowerCase().trim()) {
                case 'message-id':
                    queryParts.push(`rfc822msgid:${this.formatSearchTerm(query.search.header[headerKey])}`);
                    break;
            }
        }

        if (query.search.gmailRaw && typeof query.search.gmailRaw === 'string') {
            queryParts.push(query.search.gmailRaw);
        }

        if (query.search.body && typeof query.search.body === 'string') {
            queryParts.push(`${this.formatSearchTerm(query.search.body)}`);
        }
