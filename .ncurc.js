module.exports = {
    ...
    target: name => (['nanoid', 'ical.js', 'gettext-parser', 'xgettext-template', 'chai', 'undici', 'marked', 'ioredis'].includes(name) ? 'minor' : 'latest'),
    ...
    //   ioredis           - 6.x is a major we have not evaluated on its own merits yet. It used to be pinned BY
    //                       the bullmq cap (bullmq 5 carried ioredis 5.11.1 as a direct dependency); bullmq 6
    //                       demoted ioredis to an optional peer `>=5.0.0`, so that coupling is gone and the two
    //                       no longer have to move together. lib/db.js builds the client and passes it to bullmq
    //                       as `connection`, so a bump here changes what drives every queue - evaluate it as its
    //                       own piece of work rather than folding it into a refresh.
    //
