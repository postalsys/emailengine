...
The one exception is minting API tokens, and the reason is the asymmetry: admin access ENDS the moment a password is set, but a token issued before it keeps working afterwards, and nothing invalidates existing tokens at that transition. So a token minted during the unprotected window silently outlives it. Only that is gated, and only at the routes that can tell whether a credential was presented:
- `POST /admin/tokens/new` - unconditional, since the route has no credential to inspect
- `POST /v1/token` - only for the `preauth` caller that the `disableTokens` setting lets through with no credential at all. A caller presenting a real API token is trusted here, because minting tokens with no admin password set is a supported headless flow (`EENGINE_PREPARED_TOKEN`)

`tokens.provision()` itself is deliberately unguarded - gating it there refused the headless flow too. The shared predicate is `settings.isInstanceSecured()`.
