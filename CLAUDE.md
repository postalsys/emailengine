...
A renewal does **not** restart the SMTP or IMAP proxy worker. `reloadTlsCertificates()` sends `smtpReloadCertificates`/`imapProxyReloadCertificates`, and each worker swaps its secure context in place through `updateSecureContext()`; `reloadTlsServers()` is still there for changes to what the listener *is* (port, address, enabled). Restarting cut every submission and IMAP session in flight for a certificate with a month left on it.
